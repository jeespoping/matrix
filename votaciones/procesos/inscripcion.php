...
$westado_inscripcion = $array_conf_vot[$cod_vot]['Vothin'];  //Verifica si las inscripciones estan habilitadas.
$wfecha_inicio_ins = $array_conf_vot[$cod_vot]['Votins'];  //Fecha inicio inscripcion.
$whora_inicio_ins = $array_conf_vot[$cod_vot]['Vothii'];  //Hora inicio inscripcion.
...
$fechaHoraInicio = strtotime($wfecha_inicio_ins." ".$whora_inicio_ins);

// if($wfecha_inicio_ins > $wfecha_hoy){
if($fechaHoraActual < $fechaHoraInicio){
	
	encabezado("Votaciones",$wactualiz, $wlogoempresa);
		
		echo "<br>\n<br>\n".
        " <H1 align=center>No hay inscripciones abiertas en este momento.</H1>";
		echo "<center>
...
$cierre_inscripciones = cerrar_inscripciones($wemp_pmla, $wbasedato, $cod_vot, $aplicacion );

function cerrar_inscripciones($wemp_pmla, $wbasedato, $cod_vot, $aplicacion ){
	...
	$q =   " SELECT Votfmi as fecha_final_ins, Vothci as hora_final_ins 
			   FROM ".$wbasedato."_000001
			  WHERE Votcod = '".$cod_vot."'
				AND Votapl = '".$aplicacion."'";
	$res = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
	$row = mysql_fetch_array($res);
	$fecha_final = $row['fecha_final_ins'];
	$hora_final = $row['hora_final_ins'];
...
//-----------------------------------------------------------------------------------------------------------------------------------------            
// Actualizaciones
// 2018-02-28: Jessica Madrid Mejía 	- Se agrega a la validación de fecha la hora de inicio y cierre de inscripción
