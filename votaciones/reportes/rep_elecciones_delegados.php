...
$wactualiz="Marzo 8 de 2018";

//==========================================================================================================================================\\
//=========================================================================================================================================\\
//DESCRIPCION                                                                                                                              \\
//=========================================================================================================================================\\
//Se muestra el resultado de las votacion de delegados para la asamblea del fondo de empleados.                                      \\
//=========================================================================================================================================\\
//Marzo 8 de 2018 (Jonatan Lopez)
//- Se agrega los asociados inscritos que no tienen votos.
//- Se valida quien ha sido elegido en el campo Insele de la tabla votacion_0000003 (Inscritos) para que en el listado publico solo se muestren
// esos asociados.
//Marzo 6 de 2018 (Jonatan Lopez)
//Se repara la ultima posicion de los escanos para que se reinicie.
//=========================================================================================================================================\\
...
$(document).ready( function () {
			...
			$('.lightbox_suplente').click(function() {
			  ...
			  $( ".control_votos" ).each(function( i ) {
					var escanos = $("#emp_"+$(this).attr('id')).val();					
					$("."+$(this).val()).html(escanos);
...
foreach($wtalhumas as $key => $value){
	...
	$query_votados[] = " SELECT Insced, Insapr, Idegen, Ideced, Ideno1, Ideno2, Ideap1, Ideap2, Idecco, Ideccg, Ideuse, Inscsu, Insces, Insobs, Insemp, Inscco, Insesu, Insccs, Votced, COUNT(Votced) as Votos, Asoemp, Asoemr
				   ...
				   HAVING COUNT(Votced) > 0
						  UNION
					SELECT Insced, Insapr, Idegen, Ideced, Ideno1, Ideno2, Ideap1, Ideap2, Idecco, Ideccg, Ideuse, Inscsu, Insces, Insobs, Insemp, Inscco, Insesu, Insccs, Insced as Votced, '0' as Votos, Asoemp, Asoemr
						   FROM ".$wbasedato."_000003, ".$value['Detval'].", ".$wbasedato."_000006
						  WHERE Insced = Ideced
							AND Insapl = '".$aplicacion."'							
							AND Insvot = '".$wconsecutivo_votacion."'
							AND Insest = 'on'
							...
							AND Insapl = '".$aplicacion."'
							".$filtro_elegidos."
							AND Insced not in (SELECT Votced FROM ".$wbasedato."_000005 WHERE Votapl = '".$aplicacion."' AND Votvot = '".$wconsecutivo_votacion."' ) 							
						  UNION
	...
	$query_consultas = "SELECT * FROM (".$query_consultas.") as t";
	
	//Se agrega order by por fecha de registro y hora
	if ($wvotos=='on')
		 $query_consultas = $query_consultas." ORDER BY Votos*1 DESC";
	...
	$wempresas = empresas();
	$array_emp_posic_escanos = traer_escanos_empresa($conex,"NumeroEscanos"); //Arreglo que contiene la empresa con su posicion y el numero de escaños.
	...
	foreach ($array_emp_posic_escanos as $key => $array_posicion) {
			...
			if(array_key_exists($key,$array_votados_por_empresas)){
				...
				echo "<input id='emp_".$key."' value='".$array_posicion['escanos']."' type='hidden'>";
				...
				foreach ($array_votados as $key2 => $value) {				
					...
					if($j == $array_posicion['escanos']){
						$control_votos_finales = $value['Votos'];
					}
					//Controlo si se muestra la informacion del suplente.
					...
					if ($wvotos=='on')    //Maro 4 de 2014
				       {
						echo "<tr class='".$class." find'>
								<td nowrap align=center width=10px style='background-color: ".$class.";font-size: 20pt;' class='cajon_".$key."_".$value['Votos']."'>".$j."</td>
				...
				echo "<input type='hidden' class='control_votos' id='".$key."' value='cajon_".$key."_".$control_votos_finales."'>";
