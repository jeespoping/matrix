...
else
{
	...
	{
		echo "<form action='rep_hcsgl.php?empresa=$empresa' method=post>";
		...
		for ($y=0;$y<$nfechas;$y++)
		{
			...
			if($num>0)
			{
				...
				if ($row['Motivo_consulta_enfactual']!='.')
					echo "<tr><td colspan=4><font size=3 face='arial' ><B>MOTIVO CONSULTA Y ENF. ACTUAL Y ANTECEDENTES: </b>".$row['Motivo_consulta_enfactual']."</td></tr>";
				if ($row['Condicion_general']!='.')
					echo "<tr><td colspan=4><font size=3 face='arial' ><B>EXAMEN FISICO: </b>".$row['Condicion_general']."</td></tr>";
