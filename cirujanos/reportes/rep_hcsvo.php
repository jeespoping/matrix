...
include_once("conex.php");
  /***************************************************
	...
	*PARA EL MEDICOS SERGIO VANEGAS OSORIO  V.1.00	 *
	**************************************************/
...
else
{
	...
	{
		echo "<form action='rep_hcsvo.php?empresa=$empresa' method=post>";
	...
	else
	{
		...
		for ($y=0;$y<$nfechas;$y++)
		{
			...
			if($num>0)
			{
				...
				if ($row['Mc_ea']!='.')
					echo "<tr><td><font size=3 face='arial' ><B>MOTIVO CONSULTA Y ENFERMEDAD ACTUAL: </b>".$row['Mc_ea']."</td>"; 
				if ($row['Antecedentes_perquir']!='.')
					echo "<tr><td><font size=3 face='arial' ><B>ANTECEDENTES PERSONALES Y QUIRURGICOS: </b>".$row['Antecedentes_perquir']."</td>"; 
				if ($row['Antecedentes_familiares']!='.')
					echo "<tr><td><font size=3 face='arial' ><B>ANTECEDENTES FAMILIARES: </b>".$row['Antecedentes_familiares']."</td>"; 
				if ($row['Diagnostico']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>DIAGNOSTICO: </b>".$row['Diagnostico']."</td>"; 
				if ($row['Otros_diagnosticos']!='.')
					echo "<tr><td><font size=3 face='arial' ><B>OTROS DIAGNOSTICOS: </b>".$row['Otros_diagnosticos']."</td></table>"; 
				echo "<table align=center border=1 width=725 >";		
		...
		$querylaser="select * from	".$empresa."_000004 where Paciente='".$pac."' and Cirujano='".$medico."' order by Fecha";
		$err2 = mysql_query($querylaser,$conex);
		$num2 = mysql_num_rows($err2);
		if($num2>0)
		{
			for ($x=0;$x<$num2;$x++)
			{
				$laser = mysql_fetch_array($err2);
				echo "<table align=center border=1 width=725 >";
				echo "<tr><td align=left colspan='4' bgcolor='#cccccc' height='15'><font size='3'  face='arial'><b>LASER ".$laser[3]."</b></font></td></tr>";
				if ($laser['Area']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>AREA: </b>".$laser['Area']."</td>"; 
				if ($laser['Duracion_pulso']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>DURACION DE PULSO: </b>".$laser['Duracion_pulso']."</td>"; 
				if ($laser['Tamano_pulso']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>TAMANO PULSO: </b>".$laser['Tamano_pulso']."</td>"; 
				if ($laser['J_CM2']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>J/CM2: </b>".$laser['J_CM2']."</td>"; 
				if ($laser['Numero_pulsos']!='NO APLICA')
					echo "<tr><td><font size=3 face='arial' ><B>NUMERO DE PULSOS: </b>".$laser['Numero_pulsos']."</td>"; 
				if ($laser['Nota']!='.')
					echo "<tr><td><font size=3 face='arial' ><B>NOTA: </b>".$laser['Nota']."</td></table>";
			}
		}
