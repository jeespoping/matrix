...
else
{
	...
	{
		echo "<form action='rep_hcjpv.php?empresa=$empresa' method=post>";
	...
	else
	{
		...
		for ($y=0;$y<$nfechas;$y++)
		{
			...
			if($num>0)
			{
				...
				if ($row['Ant_dermatologico']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>EANTECEDENTES DERMATOLOGICOS:</b>".str_replace( "\n", "<br>", ($row['Ant_dermatologico'])  )."</td></tr>";
				if ($row['Terapias']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>TERAPIAS ACTUALES Y ANTERIORES:</b>".str_replace( "\n", "<br>", ($row['Terapias'])  )."</td></tr>";
				if ($row['Cirugias']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>CIRUGIAS:</b>".str_replace( "\n", "<br>", ($row['Cirugias'])  )."</td></tr>";
				if ($row['Examen_laboratorio']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>EXAMEN DE LABORATORIO:</b>".str_replace( "\n", "<br>", ($row['Examen_laboratorio'])  )."</td></tr>";
				...
				if ($row['Diagnostico']!='NO APLICA')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>DIAGNOSTICO CLINICO:</b>".str_replace( "\n", "<br>", ($row['Diagnostico'])  )."</td></tr>";
				if ($row['Otros_dx']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>OTROS DIAGNOSTICOS:</b>".str_replace( "\n", "<br>", ($row['Otros_dx'])  )."</td></tr>";
				if ($row['Conducta']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>CONDUCTA Y TRATAMIENTO:</b>".str_replace( "\n", "<br>", ($row['Conducta'])  )."</td></tr>";
				if ($row['Otros']!='.')
				...
				if($num1>0)
				{
					for ($x=0;$x<$num1;$x++)
					{
						...
						echo "<table align=center border=1 width=725 >";
							...
							if ($seg['Seguimiento']!='.')
								echo "<tr><td colspan='4'><font size=3  face='arial'><b>SEGUIMIENTO:</b> ".str_replace( "\n", "<br>", htmlentities( trim($seg['Seguimiento']) ) )."</td></tr>";
							if ($seg['Diagnostico_patologia']!='.')
								echo "<tr><td colspan='4'><font size=3  face='arial'><b>DIAGNOSTICO DE PATOLOGIA:</b> ".str_replace( "\n", "<br>", htmlentities( trim($seg['Diagnostico_patologia']) ) )."</td></tr></table>";
