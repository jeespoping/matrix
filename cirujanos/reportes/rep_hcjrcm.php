...
else
{
	...
	{
		echo "<form action='rep_hcjrcm.php?empresa=$empresa' method=post>";
	...
	else
	{
		...
		echo "<br>MEDICO OTORRINOLARINGOLOGO<BR>Reg.:</b>".$reg."</b></font>";
		echo "</tr></table>";
		...
		for ($y=0;$y<$nfechas;$y++)
		{
			...
			if($num>0)
			{
				...
				echo "<tr><td align=center colspan=4 bgcolor='#cccccc' height='15'><font size=3 face='arial' ><B>MOTIVO DE CONSULTA Y ENFERMEDAD ACTUAL</b></td></tr>";
				//MOTIVO CONSULTA
				if ($row['Mc']!='.')
				...
				if ($row['Ef']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>EF:</b>".str_replace( "\n", "<br>", ($row['Ef'])  )."</td></tr>";
				...
				if ($row['Conducta']!='.')
					echo "<tr><td colspan='4'><font size=3  face='arial'><B>Examenes:</b>".str_replace( "\n", "<br>", ($row['Conducta'])  )."</td></tr>";
