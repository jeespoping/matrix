...
else
{
	...
	{
		echo "<form action='000003_cx01.php' method=post>";
		...
		if(isset($medico) and isset($pac1))
		{
			...
			$query="select distinct Paciente from cirugia_000003 where Cirujano='".$medico."'and Paciente like '%".$pac1."%' order by Paciente";
		...
		if(isset($pac))
		{
			/*Si ya se tiene el paciente buscar las fechas de los seguimiento para este y
			 construir el drop down */	
			$query = "select Fecha  from cirugia_000003 where Paciente='".$pac."' and Cirujano='".$medico."' ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	...
	else
	{
		...
		$query="select Telefonos from cirugia_000001 where Nombre1='".$n1."' and Nombre2='".$n2."'  and ";
		...
		echo "</tr></table>";
		$query="select fecha from	cirugia_000003 where Paciente='".$pac."' and Cirujano='".$medico."' and ((Fecha > '".$fecha1."' and Fecha < '".$fecha2."') or Fecha = '".$fecha2."'or Fecha = '".$fecha1."') order by Fecha";
		$err = mysql_query($query,$conex);
		...
		for ($y=0;$y<$nfechas;$y++)
		{
			$querya="select * from	cirugia_000003 where Paciente='".$pac."' and Cirujano='".$medico."' and Fecha='".$fec[$y]."' order by Fecha";
			...
			if($num>0)
			{
				...
				echo "<tr><td colspan='4'><font size=3  face='arial'><b>MOTIVO CONSULTA:</b>".$row['Motivo_consulta']."</td></tr>";
			...
			//echo "y!= nfechas<br> *y=".$y."*  nfechas=".$nfechas."*  fec[y]=".$fec[$y]."*<br>";	
				...
				$queryseg="select * from	cirugia_000004 where Paciente='".$pac."' and Cirujano='".$medico."' and ".$pre." order by Fecha";
		//ECHO $queryseg;
