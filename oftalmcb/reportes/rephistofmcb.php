...
//								  Despliega la información básica contenida en la tabla oftalmcb_000001, todos los eventos de consulta
//			 					  en su orden cronologico, es decir todas las historias clínicas (oftalmcb_000003) y todos los 
//								  seguimientos (oftalmcb_000009) hechos a un paciente, inmediatamente despúes de cada evento 
//								  muestra todos los otros elementos asociados, contenidos el las tablas oftalmcb_000003 a la 
//								  oftalmcb_000008.
...
//		pues se cambian de la tabla oftalmcb_000003 a oftalmcb_000001.
//		Se borran los campos Pulso y Peso de Historia pues desaparecen de la tabla oftalmcb_000003.
//
//	2006-03-28
 ...
 * @param [][] 	 $row	Información aimprimir en pantalla, sacada del la tabla oftalmcb_000001
 */
...
/**
 * Despliega en pantalla toda la información de la historia clínica, sacada de la tabla oftalmcb_000003
 *
...
/**
 * Despliega en pantalla toda la información de un seguimiento, sacada de la tabla oftalmcb_000009
 *
...
/**
 * Busca en la BD la información de lentes recetados al paciente en la tabla oftalmcb_000004 segun el query
...
/**
 * Busca en la BD la información de los medicamentos recetados al paciente en la tabla oftalmcb_ 000005 segun el query
...
/**
 * Busca en la BD la información de los EXAMENES ORDENADOS al paciente en la tabla oftalmcb_ 000006 segun el query
...
/**
 * Busca en la BD la información de las CIRUGIAS ORDENADAS al paciente en la tabla oftalmcb_ 000008 segun el query
 ...
 * oftalmcb_ 000007 segun el query.
...
else
{
	...
	if(!isset($medico)  or !isset($pac) or !isset($fecha1) or !isset($year) )
	{
		...
		$query = "SELECT Subcodigo,Descripcion FROM `det_selecciones` WHERE medico = 'oftalmcb' AND codigo = '002'  order by Descripcion ";
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		if($num>0)
		{
		...
		if(isset($pac1))
		{
			...
			//$query="select DISTINCT Paciente from oftalmcb_000003 where Oftalmologo='".$medico."'and Paciente like '%".$pac1."%' order by Paciente";V1.03
			$query="select DISTINCT Paciente from oftalmcb_000003 where  Paciente like '%".$pac1."%' order by Paciente";
		...
		if(isset($pac))
		{
			...
			construir el drop down */
			$query = "select Fecha  from oftalmcb_000003 where Paciente='".$pac."' ";//and Oftalmologo='".$medico."' "; V 1.03
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	...
	APARTIR DE AQUI EMPIEZA LA IMPRESION DE LA HISTORIA		*/
	{
		...
		$querya="select * from oftalmcb_000001 where Nombre1='".$n1."' and Nombre2='".$n2."'  and ";
		...
		$query="select fecha from oftalmcb_000003 where Paciente='".$pac."'  and Fecha between '".$fecha1."' and '".$fecha2."' order by Fecha";
		$err = mysql_query($query,$conex);
		...
		for ($y=0;$y<$nfechas;$y++)	{
			...
			$query="select * from	oftalmcb_000003 where Paciente='".$pac."' and  Fecha='".$fec[$y]."' order by Fecha";
			$err = mysql_query($query,$conex);
			...
			if($numy>0)	{
				...
				$query="select Fecha from	oftalmcb_000009 where Paciente='".$pac."' and ".$pre." order by Fecha";
