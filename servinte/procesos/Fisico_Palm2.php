		...
		if($wsw == 1)   
		{
			...
			$hora = (string)date("H:i:s");			
			 /*$query = " INSERT ".$empresa."_000015(Medico,Fecha_data,Hora_data, Fisano, Fismes, Fiscco, Fisfec, Fisart, Fiscon, Fiscan, Fisest, Seguridad) values ('".$empresa."','".$fecha."','".$hora."','".date("Y")."','".date("m")."','".substr($wcco,0,strpos($wcco,"-"))."','".$wfec."','".$wart."',".$wcon.",".$wcan.", 'on'  , 'C-".$key."')";*/
              ...
              {
                //Verifico que no exista informacion para este articulo en este año,mes,servicio,articulo y tipo
                ...
                if (odbc_fetch_row($err))
                {
                 ...
                 Este mensaje ya no sale pues se quedo en que si ya tiene conteo acumule */
                 $wtot = odbc_result($err,6) + $wcan;
 			     $query = " UPDATE ivfis SET fiscaa = ".$wtot." Where fisano='".date("Y")."' and fismes='".date("m")."'";
				 $query = $query." and fisser='".substr($wcco,0,strpos($wcco,"-"))."' and fisart='".$wart."' and fistip='G'";     
                ...
                else
                {
                  ...
                  if (odbc_fetch_row($err))
			       ...
			       $query = " INSERT INTO ivfis(Fisano, Fismes, Fistaj, Fisser, Fisart, Fiscaa, fiscab, fiscan, Fisval, Fistip)";			  
			       $query = $query." VALUES ('".date("Y")."','".date("m")."','".$tj."','".substr($wcco,0,strpos($wcco,"-"))."','".$wart."',".$wcan.",0,0,0,'G')"; 
			  ...
			  else
			  {
                ...
                {                 
			   		if ($wcon == 2){   /* Conteo 2 ==> UPDATE	*/
			   		    /* Se quedo en que si ya tiene conteo acumule */
                        $wtot = odbc_result($err,7) + $wcan;

			    	 	$query = " UPDATE ivfis SET fiscab = ".$wtot." Where fisano='".date("Y")."' and fismes='".date("m")."'";
				 		$query = $query." and fisser='".substr($wcco,0,strpos($wcco,"-"))."' and fisart='".$wart."' and fistip='G'";
				 		
			   		}
			   		else{	 		/* Conteo 3 ==> UPDATE	*/
			   		    /* Se quedo en que si ya tiene conteo acumule */
                        $wtot = odbc_result($err,8) + $wcan;

						$query = " UPDATE ivfis SET fiscan = ".$wtot." Where fisano='".date("Y")."' and fismes='".date("m")."'";
						$query = $query." and fisser='".substr($wcco,0,strpos($wcco,"-"))."' and fisart='".$wart."' and fistip='G'";	
