...
function Listar($conex,$grupo,$codigo,$usera,&$w,&$DATA)
{
	...
	if ($num > 0)
	{
		...
		{
			$row = mysql_fetch_array($err);
			if(substr(strtolower($row[2]),0,31) == "f100.php?accion=w&grupo=americas&")
			{
				$w++;
				$DATA[$w][0]=$row[0];
				$DATA[$w][1]=$row[1];
...
if(!isset($accion))
{
	...
	echo "<form name='entrada' action='F100.php?accion=I' method=post>";
...
else
{
	...
	if($accion == "0")
	{
		...
		echo "<td id=tipo1><A HREF='F100.php?accion=0' target='_top'>YA EXISTE UNA SESION ABIERTA EN ESTE NAVEGADOR. CIERRE LA VENTANA</A></td></tr></table></body>";
	}
	elseif($accion == "I")
	{
		if(isset($_SESSION["IIPP"]) and isset($_SESSION["NS"]) and $_SESSION["NS"] > 1)
		{
			...
			echo "<tr><td id=tipo1 colspan=2 align=center><IMG SRC='/matrix/images/medical/root/GELA.png' BORDER=0></td></tr>";
			echo "<tr><td id=tipo1><IMG SRC='/matrix/images/medical/root/denegado.png' BORDER=0></td>";
			echo "<td id=tipo1><A HREF='F100.php?accion=0' target='_top'>YA EXISTE UNA SESION ABIERTA EN ESTE NAVEGADOR. HAGA CLICK AQUI!!</A></td></tr></table></body>";
		}
		else
		{
			@session_start();
			...
			if ($num > 0)
			{
				switch($grupo)
				{
					...
					case 'ayudaenl':
						...
						echo "<tr><td align=center><A HREF='F100.php?END=on' target='_top'>Haga Click Para Salir del Programa</A></td></tr></table>";
					break;
						
					default:
						echo "<frameset cols=20%,80% frameborder=0 framespacing=2>";
						echo "  <frame src='F100.php?accion=O&grupo=".$grupo."&amp;prioridad=".$prioridad."' name='options' marginwidth=0 marginheiht=0>";
						echo "  <frameset rows='81,*' frameborder=0 framespacing=0>";
						echo "    <frame src='F100.php?accion=T&grupo=".$grupo."' name='titulos' marginwidth=0 marginheiht=0>";
						echo "    <frame src='F100.php?accion=M&grupo=".$grupo."' name='main' marginwidth=0 marginheiht=0>";
						echo "  </frameset>";
						echo "</frameset>";
			...
			else
			{
				...
				echo "<tr><td id=tipo1><IMG SRC='/matrix/images/medical/root/denegado.png' BORDER=0></td>";
				@session_destroy();
				echo "<td id=tipo1><A HREF='f100.php?END=on'>USUARIO NO EXISTE O ESTA INACTIVO</a></td></tr></table></body>";
			}
		}	
	}
	else
	{
		if(!isset($_SESSION['user']))
		{
			...
			echo "<link rel='shortcut icon' href='favicon.png' type='image/png' />";
			echo "<A HREF='F100.php?END=on' target='_top'><IMG SRC='/matrix/images/medical/root/expire.png'>Su Sesion Ha Expirado. Por Favor Haga Click Aqui !!</A>";
			echo "</BODY>";
			echo "</html>";
		}
		elseif($accion != "W")
		{
			...
			switch($accion)
			{
				case "T":
					...
					echo"<tr><td align=center><IMG SRC='/matrix/images/medical/root/GELA1.png'  height='70%'></td><td align=right><A HREF='F100.php?END=on' target='_top' class='myButton1'>X</A></td></tr>";
					echo"<tr></table>";
				break;
				...
				case "O": 	
					...
					switch($grupo)
					{
						case 'AMERICAS':
							echo "<ol>";
							echo "<li onmouseover=".chr(34)."this.className='BlueThing';".chr(34)."  onmouseout=".chr(34)."this.className='GrayThing';".chr(34)."><A HREF='F100.php?accion=M&grupo=".$grupo."' target='main'>Inicio</A>";
							echo "<hr>";
							...
							if ($num > 0)
							{
								...
								{
									$row = mysql_fetch_array($err);
									echo "<li onmouseover=".chr(34)."this.className='BlueThing';".chr(34)."  onmouseout=".chr(34)."this.className='GrayThing';".chr(34)."><A HREF='f100.php?accion=W&grupo=".$grupo."&amp;codigo=".$row[0]."' target='main'>".strtoupper($row[1])."</A>";
									echo "<hr>";
								}
							}
							echo "<li onmouseover=".chr(34)."this.className='BlueThing';".chr(34)."  onmouseout=".chr(34)."this.className='GrayThing';".chr(34)."><A HREF='F100.php?END=on' target='_top' class='myButton'>Salida Segura</A>";
							echo "<hr>";
							echo "</ol>";
						break;
						case 'PASSWD':
							...
							echo "<li onmouseover=".chr(34)."this.className='BlueThing';".chr(34)."  onmouseout=".chr(34)."this.className='GrayThing';".chr(34)."><A HREF='F100.php?END=on' target='_top'>Salir del programa</A>";
							echo "</ol>";
						break;
						default:
							echo "<font size=2><ol>";
		...
		else
		{
			...
			if(!isset($_SESSION['user']))
			...
			else
				{
					...
					$key = substr($user,2,strlen($user));
					echo "<form name='inventario' action='F100.php' method=post>";
