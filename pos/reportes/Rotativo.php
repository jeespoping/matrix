<html>
<head>
  	<title>MATRIX Comparacion Fisico vs Teorico Iventario Rotativo</title>
</head>
<body BGCOLOR="FFFFFF">
<BODY TEXT="#000066">
<center>
<table border=0 align=center>
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Comparacion Fisico vs Teorico Iventario Rotativo</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b>Rotativo.php Ver. 2008-10-30</b></font></tr></td></table>
</center> 
<?php
...
function bi($d,$n,$k,$i)
{
	...
	if(isset($d[$li][$i]) and strtoupper($k) == strtoupper($d[$li][$i]))
		return $li;
	elseif(isset($d[$ls][$i]) and strtoupper($k) == strtoupper($d[$ls][$i]))
				return $ls;
			else
				return -1;
}
...
else
{
	...
	echo "<form name='Rotativo' action='Rotativo.php' method=post>";
	

	

	echo "<input type='HIDDEN' name= 'empresa' value='".$empresa."'>";
	if(!isset($wano) or !isset($wmes) or !isset($wcco) or !isset($wfec) or !isset($wlab))
	{
		...
		echo "</td></tr>";
		echo "<tr><td bgcolor=#cccccc align=center>Laboratorio</td><td bgcolor=#cccccc align=center>";
		$query = "select mid(Artcod,1,3) from ".$empresa."_000001 where mid(Artcod,1,1) = '1' group by 1 order by 1 ";
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		if ($num>0)
		{
			echo "<select name='wlab'>";
			for ($i=0;$i<$num;$i++)
			{
				$row = mysql_fetch_array($err);
	...
	else
	{
		...
		echo "<tr><td bgcolor=".$color." align=center><font face='tahoma'><b>LABORATORIO : </b>".$wlab."</td></tr>";
		$fisico=array();
			...
			for ($i=0;$i<$num;$i++)
			{
				...
				if($wtip == 1 or ($wtip == 2 and $pos != -1 and $fisico[$pos][1] != $fisico[$pos][2]))
				{
					...
					if($pos != -1)
					{
						...
						if($fisico[$pos][1] == -1 and $fisico[$pos][2] == -1 and $fisico[$pos][3] == -1)
							echo "<td bgcolor=#CC99FF align=center><font face='tahoma' size=2>PENDIENTE</font></td>";
						else
							if($dif < 0)
							{
