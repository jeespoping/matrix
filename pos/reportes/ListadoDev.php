...
/***********************************Consulto las devouciones sin nota credito********************/
      ...
      echo "<tr><td align=center colspan=2 bgcolor=".$wcf2."><font size=6 text color=#FFFFFF><b>LISTADO DE DEVOLUCIONES</b></font></td></tr>";
      echo "</tr>";  
      echo "</center></table >";
...
/***********************************Consulto las devouciones con nota credito********************/
      ...
      $table=date("Mdis").'1';
 		...
 		$q = " SELECT Menfac, Menfec, venffa, arttiv, artiva, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan), SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), tranum, vennfa"
	      ."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
          ."   WHERE Menfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND Menfac = vennum "
	  	...
	  	for ($i=0;$i<$num;$i++)
	   	{
	      		...
	      		echo "<td><font size=2>".$vector[$i][8]."</font></td>";
	      		echo "<td><font size=2>".$vector[$i][4]."</font></td>";
	      		echo "<td align=right><font size=2>".number_format($vector[$i][5],0,'.',',')."</font></td>";
	      		echo "<td align=right><font size=2>".number_format($vector[$i][6],0,'.',',')."</font></td>";
	     		echo "<td align=right><font size=2>".number_format($vector[$i][7],0,'.',',')."</font></td>";
