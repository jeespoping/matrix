...
else
{
	...
	echo "<form action='devolucionesPorTerceroIva.php' method=post>";
	...
	else
	{
		...
		$query= $query. "WHERE menfec BETWEEN '$wfecini' AND '$wfecfin' and mencon='801' and mencco like '".$wccoe[0]."' ";
		$query= $query. "and mendoc = mdedoc and  mdecon = mencon ";
		$query= $query. "and Mendoc not in (select tradev FROM " .$wbasedato."_000055 where tradev = Mendoc and traven = Menfac and tracco = Mencco) ";
			...
			while (($i<=$num) and ($wtipemp==$row[5]))
			{
				...
				while (($i<=$num) and ($wtipemp==$row[5]) and ($wempresa==$row[6]))
				{
					if ($wtipemp != "01-PARTICULAR")
					{
						...
						echo "<td ALIGN=RIGHT>".number_format($row[11],0,'.',',')."</td>";

						echo "</tr>";
					}

					$wtotemp = $wtotemp + $row[10];
					$wtoIemp = $wtoIemp + $row[11];
					$wtottem = $wtottem + $row[10];
					$wtoItem = $wtoIemp + $row[11];
					$wtotgen = $wtotgen + $row[10];
					$wtoIgen = $wtoIgen + $row[11];
		...
		$query= $query. "WHERE menfec BETWEEN '$wfecini' AND '$wfecfin' and mencon='801' and mencco like '".$wccoe[0]."' ";
		$query= $query. "and mendoc = mdedoc and  mdecon = mencon ";
		$query= $query. "and tradev=mendoc and traven = Menfac and  tracco=mencco and tratip <>'01-ANULACION' ";


		$err = mysql_query($query,$conex)or die (mysql_errno()." - ".mysql_error());


		$q = " SELECT menfac, menfec, mdeart, venffa, vennfa, ventcl, venfec,  vencod, empnit, empnom, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes) + SUM(ROUND((((vdecan*vdevun/(1+(vdepiv/100)))-vdedes)*(vdepiv/100)),0)), tranum, SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)) "
			...
			while (($i<=$num) and ($wtipemp==$row[5]))
			{
				...
				while (($i<=$num) and ($wtipemp==$row[5]) and ($wempresa==$row[6]))
				{
					if ($wtipemp != "01-PARTICULAR")
					{
						...
						echo "<td ALIGN=RIGHT>".number_format($row[10],0,'.',',')."</td>";
						echo "<td ALIGN=RIGHT>".number_format($row[12],0,'.',',')."</td>";
					...
					$wtottem = $wtottem + $row[10];
					$wtotgen = $wtotgen + $row[10];
					$wtoIemp = $wtoIemp + $row[12];
					$wtoItem = $wtoItem + $row[12];
					$wtoIgen = $wtoIgen + $row[12];
