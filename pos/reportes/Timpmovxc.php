...
else
{
	...
	echo "<form name='Timpmovxa' action='Timpmovxc.php' method=post>";
	...
	else
	{
		...
		echo "<tr><td align=center bgcolor=#999999><font size=6 face='tahoma'><b>MOVIMIENTO DE INVENTARIOS X CONCEPTO Ver. 2008-06-18</font></b></font></td></tr>";
		$color="#dddddd";
		...
		$query = "SELECT Menfec, Mendoc, Mencco, Menccd, Mendan, Menfac, Mennit, Mdepiv, sum(Mdevto)   from ".$empresa."_000010,".$empresa."_000011 ";
		...
		$query .= "     and Mencon=Mdecon ";
   		...
   		$query .= "     and Mdevto >= 0 ";
   		$query .= "     Group by  Menfec, Mendoc, Mencco, Menccd, Mendan, Menfac, Mennit, Mdepiv ";
   		$query .= " UNION ";
   		$query .= " SELECT Menfec, Mendoc, Mencco, Menccd, Mendan, Menfac, Mennit, Mdepiv, sum(Mdevto * (-1))   from ".$empresa."_000010,".$empresa."_000011 ";
		$query .= " where  Menfec between '".$wper1."' and '".$wper2."'";
		$query .= "     and Mencon='".substr($wcon,0,strpos($wcon,"-"))."'";
		$query .= "     and Mencon=Mdecon ";
   		$query .= "     and Mendoc=Mdedoc ";
		...
		for ($i=0;$i<$num;$i++)
		{
			...
			if($row[1] != $wdocant)
			{
				if($i > 0)
				{
					if($j % 2 == 0)
						...
						$color="#ffffff";
					$totg += $wtot;
					echo "<tr><td bgcolor=".$color."><font face='tahoma' size=2>".$wfec."</font></td>";	
					echo "<td bgcolor=".$color." align=right><font face='tahoma' size=2>".$wdoc."</font></td>";	
					echo "<td bgcolor=".$color." align=right><font face='tahoma' size=2>".number_format((double)$wtot,2,'.',',')."</font></td>";	
					echo "<td bgcolor=".$color." align=center><font face='tahoma' size=2>".$wcco."</font></td>";	
					echo "<td bgcolor=".$color." align=center><font face='tahoma' size=2>".$wccd."</font></td>";	
					echo "<td bgcolor=".$color." align=center><font face='tahoma' size=2>".$wdan."</font></td>";
					echo "<td bgcolor=".$color." align=center><font face='tahoma' size=2>".$wfac."</font></td>";
					echo "<td bgcolor=".$color."><font face='tahoma' size=2>".$wnit."</font></td>";
					echo "<td bgcolor=".$color."><font face='tahoma' size=2>".$wnni."</font></td>";
					$wtiva=0;
					for ($k=0;$k<$numi;$k++)
					{
						if($wtotiva[$k][0] != 0)
			...
			$wtot += $row[8];
			for ($k=0;$k<$numi;$k++)
				if($row[7] == $wtotiva[$k][0])
					if($row[7] == 0)
						$wtotiva[$k][1] += $row[8] ;
					else
						$wtotiva[$k][1] += ($row[7] *  $row[8] / 100);
		}
		if($j % 2 == 0)
			$color="#9999FF";
		else
		...
		echo "</tr>";
		$color="#999999";
		echo "<tr><td bgcolor=".$color." colspan=2><font face='tahoma' size=2><b>TOTAL GENERAL</b></font></td><td bgcolor=".$color." align=right><font face='tahoma' size=2><b>".number_format((double)$totg,2,'.',',')."</b></font></td><td bgcolor=".$color." colspan=10><font face='tahoma' size=2></font></td></tr>";	
		echo"</table><br><br>";
