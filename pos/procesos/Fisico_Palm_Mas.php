<html>
<head>
  	<title>MATRIX Programa Para Inventario Fisico x Palm (Adicion)</title>
...
<!--
	function enter()
	{
		document.forms.Fisico_Palm_Mas.submit();
	}
//-->
</script>
<BODY TEXT="#000066">
...
/**********************************************************************************************************************  
	   PROGRAMA : Fisico_Palm_mas.php
	   Fecha de Liberaci�n : 2008-08-27
	   Autor : Ing. Pedro Ortiz Tamayo
	   Version Actual : 2008-10-30
	   
	   OBJETIVO GENERAL : 
	   Este programa permite la grabacion de un inventario fisico x fecha y conteo para realizar ajustes en el teorico del Pos.
	   El programa Fisico_Palm_mas permite registrar varias cantidades de un mismo articulo en un mismo conteo.
	   Esta Opcion adicional no la tiene el programa original Fisico_Palm.php. Esta opcion se adiciono por solicitud de la 
       Unidad Visual Global de Coomeva.
	   
	   
	   REGISTRO DE MODIFICACIONES :
	   .2008-10-30
	  	 	Se modifico el programa para que la grabacion fuera mas rapida, posicionando el cursor en el campo de articulo
	  	 	y mostrando por defecto un valor de uno (1) en cantidad.
...
else
{
	...
	echo "<form name='Fisico_Palm_Mas' action='Fisico_Palm_Mas.php' method=post>";
	...
	if(isset($ok) and $wcan >= 0 and $wart != "" and $wdes != "")
	{
		...
		$fecha = date("Y-m-d");
			...
			switch (mysql_errno())
			{
				...
				break;
				case 1062:
					$query =  " update ".$empresa."_000015 set Fiscan = Fiscan + ".$wcan."  where Fisano='".date("Y")."' and Fismes='".date("m")."' and Fiscco='".substr($wcco,0,strpos($wcco,"-"))."' and Fisfec='".$wfec."' and Fisart='".$wart."' and Fiscon=".$wcon;
					$err1 = mysql_query($query,$conex);
					if(mysql_errno() != 0)
						echo "<font size=1><MARQUEE BEHAVIOR=SCROLL BGCOLOR=#FF0000 LOOP=-1>LOS DATOS NO SE PUDIERON ACTUALIZAR. POR FAVOR LLAME A SISTEMAS!!!!</MARQUEE></FONT>";
				break;
			}
		}
		else
	...
	if (!isset($wfec) or !isset($wcon) or $wcon < 1 or $wcon > 3)
	{
		...
		echo "<tr><td align=center bgcolor=#cccccc><font size=1><b>Ver. 2008-08-27</font></td></tr>";
	...
	else
	{
		...
		echo "<tr><td align=center bgcolor=#999999><font size=1><b>GRABACION DE INVENTARIO FISICO</font></td></tr>";
		echo "<tr><td align=center bgcolor=#cccccc colspan=1><font size=1><b>Ver. 2008-08-27</font></td></tr>";
		echo "<tr><td bgcolor=#dddddd align=center><font size=1>ARTICULO : </font></td></tr>";
		if(isset($wart) and $wart != "")
		{
			?>
			<script>
				function ira(){document.Fisico_Palm_Mas.wart.focus();}
			</script>
			<?php
			...
			else
			{
				...
				if ($num>0)
				{
					...
					{
						$row = mysql_fetch_array($err);
						echo "<tr><td bgcolor=#cccccc align=center><font size=1>DESCRIPCION : ".substr($row[0],strpos($row[0],"-")+1)."</font></td></tr>";
						$wart=substr($row[0],0,strpos($row[0],"-"));
						$wdes=substr($row[0],strpos($row[0],"-")+1);
		...
		if(!isset($wcan))
			echo "<tr><td bgcolor=#dddddd align=center><input type='TEXT' name='wcan' size=5 maxlength=10 value=1><font size=1> Unidad : ".$wuni."</font></td></tr>";
		else
