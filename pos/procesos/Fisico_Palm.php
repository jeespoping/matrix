...
/**********************************************************************************************************************  
	   ...
	   .2006-09-14
		...
		.2014-01-15 Jonatan Lopez Aguirre
			Se agrega la funcion trim a la variable $wart para que no permita el registro del articulo con espacios adelante y atras.
...
else
{
	...
	echo "<form align=center name='Fisico_Palm' action='Fisico_Palm.php' method=post>";
	...
	if(isset($ok) and $wcan >= 0 and $wart != "" and $wdes != "")
	{
		...
		$fecha = date("Y-m-d");
			...
			$query = " insert ".$empresa."_000015(Medico,Fecha_data,Hora_data, Fisano, Fismes, Fiscco, Fisfec, Fisart, Fiscon, Fiscan, Fisest, Seguridad) values ('".$empresa."','".$fecha."','".$hora."','".date("Y")."','".date("m")."','".substr($wcco,0,strpos($wcco,"-"))."','".$wfec."','".trim($wart)."',".$wcon.",".$wcan.", 'on'  , 'C-".$key."')";
	...
	if (!isset($wfec) or !isset($wcon) or $wcon < 1 or $wcon > 3)
	{
		...
		echo "<tr><td align=center bgcolor=#cccccc><font size=1><b>Ver. 2014-01-15</font></td></tr>";
	...
	else
	{
		...
		echo "<tr><td align=center bgcolor=#999999><font size=1><b>GRABACION DE INVENTARIO FISICO</font></td></tr>";
		echo "<tr><td align=center bgcolor=#cccccc colspan=1><font size=1><b>Ver. 2014-01-15</font></td></tr>";
