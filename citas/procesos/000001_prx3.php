...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Asignacion de Reservas de Turnos</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_prx3.php Ver. 1.01</b></font></tr></td></table>
</center>
<?php
...
else
{
	...
	echo "<form action='000001_prx3.php' method=post>";
	...
	else
	{
		...
		while($wfec1 <= $wfec2)
		{
			...
			$query = "select cod_med,cod_equ,cod_exa,fecha,hi,hf,nom_pac,nit_resp,telefono,edad,comentarios,usuario,activo from ".$empresa."_000001 where cod_equ = '".substr($wequip,0,3)."'";
			...
			else
			{
				...
				while ($phi < $hf)
				{
					...
					if ($phi >= $whi and $phf<=$whf)
					{
						...
						$query = "insert ".$empresa."_000001 (medico,fecha_data,hora_data,cod_med,cod_equ,cod_exa,fecha,hi,hf,nom_pac,nit_resp,telefono,edad,comentarios,usuario,activo,seguridad) values ('".$key."','".$fecha."','".$hora."','0','".substr($wequip,0,3)."','0','".$wfec1."','".$phi."','".$phf."','".ucwords($wreserva)."','0','0',0,'.','".substr($user,2,strlen($user))."','A','C-".$empresa."')";
						$err = mysql_query($query,$conex);
						 if ($err != 1)
						 {
							echo "<center><table border=0 aling=center>";
