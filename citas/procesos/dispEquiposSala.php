...
<?php
/*
Creacion: 2012-09-19  Este script se crea para mostrar la lista de equipos disponibles para las citas de caso 1 y caso 3(1 modificado) que son las de equipos, se muestra la lista de equipos de una fecha seleccionada en el calendarSalaio, al nombre del equipo se le puede dar clic y lleva a la agenda para la asignacion de citas correspondientes a dicho equipo, esta pagina se recarga cada 30 segundos para que se puedan visualizar los cambios en las citas asignadas. Viviana Rodas
Modificacion:
			...
			2012-11-27 Se agrega la verificacion de los dias festivos para dependiendo de ese mostrar la lista de equipo o no, tambien se agregan las 
					   variables month y year al link de retornar para que el calendarSalaio vuelva a mes del cual se llamo anteriormente. Viviana Rodas
			2012-10-31 Se cambia la funcion disponibilidadEquipos para evaluar cuando en un dia del calendarSalaio no hay atencion, para que muestre
...
function consultaMedicosEquipos()
{
	...
	{ 
		...
		echo "<th colspan='2' class='encabezadotabla'><font size='4'>No se asignan salas, dia Festivo</font></th>";
		echo "</table>";
	}
	else
	{
		...
		echo "<tr><td colspan='2' class='encabezadotabla' align='center'>Salas del Dia</font></td></tr>";
			...
			for( $i = 0; $rows = mysql_fetch_array( $err ); $i++ )
					   ...
					   while( $rows1 ){  //mientras sea true
						...
						if ($num2 > 0)
						{
								...
								if ($num4>0)  //tiene citas para asignar
								{
									...
									// {
										...
										// $rojo++;
										// echo "<tr $class>";
										// echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
									...
									// {
										...
										// echo " Citas Asignadas: ".$citasAsig." -";
										// echo " Citas Disponibles: ".$citasDisp."</font></td>";
										// echo "</tr>";
										// $j++;
									 // }	
								}
								...
								// {
									...
									// echo "<tr $class>";
									// echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
									// echo "<td><font size='2'> Todas las citas disponibles </font></td></tr>";
									// $j++;
								// }
						}
						// else
						...
						if( !$rows1 ){
							...
							if ($num4==0)  //todas las citas disponibles
							{
								...
								echo "<tr $class>";
								echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
								echo "<td><font size='2'> Todos los horarios disponibles </font></td></tr>";
								$j++;
							}
							elseif( $num2 == 0 ){
								$rojo++;
								echo "<tr $class>";
								echo "<td><font size='2'> ".$wequ."</font></td> ";
								echo "<td><font size='2'> No se asignan salas este dia</font></td>";
								echo "</tr>";
								$j++;
							}
							else{
								...
								{	
									...
									echo "<tr $class>";
									echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
									echo "<td><font size='2'> No tiene horario disponible</font></td>";
									echo "</tr>";
									$j++;
								...
								{	
									...
									echo "<tr $class>";
									echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' >".$wequ."</a></font></td> ";
									echo "<td><font size='2'> Total Horas sala: ".($totalCitas-$tiempoGastado)." -";
									echo " Horas Sala Asignadas: ".$citasAsig." -";
									echo " Horas Sala Disponibles: ".$citasDisp."</font></td>";
									echo "</tr>";
									$j++;
								 }
							}
				...
				if ($num1==0) /******no tiene excepciones******/
				{
					...
					if ($num2 > 0 )
					{
								...
								if ($num4>0)  //tiene citas para asignar
								{
									...
									if ($citasDisp==0)
									{
										...
										echo "<tr $class>";
										echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
										echo "<td><font size='2'> No tiene horas en sala disponibles</font></td>";
										echo "</tr>";
										$j++;
									...
									if ($num4==0)  //todas las citas disponibles
										{
											...
											echo "<tr $class>";
											echo "<td><font size='2'><a href='agendaEquiposSala.php?empresa=$wbasedato&wequ=$wequ&wsw=$wsw&colorDiaAnt=$colorDiaAnt&wfec=$wfec&caso=$caso&wemp_pmla=$wemp_pmla' > ".$wequ."</a></font></td> ";
											echo "<td><font size='2'> Todas las horas de sala disponibles </font></td></tr>";
											$j++;
										}
					}
					else
					{
						...
						echo "<tr $class>";
						echo "<td><font size='2'> ".$wequ."</font></td> ";
						echo "<td><font size='2'> No se asignan Horas Sala este dia</font></td>";
						echo "</tr>";
						$j++;
					}
	...
	$year=$fecha[0];
	$month=$fecha[1];
	echo "<meta content='30;URL=dispEquiposSala.php?wemp_pmla=$wemp_pmla&wbasedato=$wbasedato&consultaAjax=10&wfec=$wfec&wsw=$wsw&colorDiaAnt=$colorDiaAnt&caso=$caso&fest=$fest' http-equiv='REFRESH'> </meta>";
	
	echo "<br>";
	echo "<center><b><A HREF='calendarSala.php?empresa=".$wbasedato."&wemp_pmla=$wemp_pmla&caso=".$caso."&wsw=".@$wsw."&month=".$month."&year=".$year."&fest=".$fest."&consultaAjax='>Retornar</A></b><br></center>";
	echo"<br>";
