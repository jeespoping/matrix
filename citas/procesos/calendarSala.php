...
/****************************************************************************************************************************************
*Fecha de creacion: 2012-09-08 Este calendarSalaio permite agilizar la asignacion de citas en las unidades de la clinica como en 
* la clinica del sur, este muestra el calendarSalaio del mes actual, permitiendo navegar por los distintos meses del año o de cualquier año,
...
*                  2012-11-29 Se cambia la validacion donde se pregunta si el dia anterior es rojo y la fecha que se esta pintando en el calendarSalaio
				   ...
				   2012-11-26 Se hace la validacion para que cuando se haga la recarga del calendarSalaio tome el mes en el cual esta el calendarSalaio, 
							  sin importar que no sea el mes actual.
				   2012-10-31 Se cambia la funcion dispColores para evaluar cuando en un dia del calendarSalaio no hay atencion, para que muestre ese 	dia en color rojo y muestre un mensaje diciendo no hay atencion. Viviana Rodas
				   2012-09-10 Se agrega el tooltip para mostrar la lista de los medicos o equipos
				   2012-09-11 Se agregan nuevos estilos para el calendarSalaio ya que inicalmente era todo de color gris ademas se pone el 
				   calendarSalaio en castellano porque era en ingles.
...
{ 
					   ...
					   if ($exUniHora == 0)
					    ...
					    else
						{
							...
							$query = "select Cod_equ, Cod_exa, Fecha, Hi, Hf, Cedula, Nom_pac, Nit_res, Asistida, Activo, Atendido
									  ...
									  if ($num4>0)  //si tiene citas en esa fecha
										{
	...
	// if( $wfec == "2013-09-02" ){ return "rojo"; }
	//Se consulta la tabla de festivos 
	$sql="select Fecha from root_000063 where Fecha = '".$wfec."' ";
	$errs = mysql_query($sql,$conex)or die( mysql_errno()." - Error en el query $sql - ".mysql_error() );		
	$nums = mysql_num_rows($errs);
					...
					{						
						...
						if ($num6>0)
						{
							...
							*/
							...
							// else  //para el caso 1 que no tiene campo equipo en la tabla
							// {
   ...
   function AddDay($fday, $fmonth, $fyear)
   {
	 ...
	 if(strlen($fday) == 1)
		...
		// $wfec1=$fyear.'-'.$fmonth.'-'.$fday;  //fecha actual
      ...
      if (!isset($fday) || $fday == "")
      {
         echo '	<TD class="calendarSala" align="left" valign="top" width=90 height=70>
		&nbsp;
';
      }
      else
      {
         ...
         {													//faltan las otras que son caso 1 modificado (3)
			 if ($caso==3)
			 {
				...
				//coloresMSN
				//$wfec1=$fyear.'-'.$fmonth.'-'.$fday; //organizar la fecha para enviarla en la url
				...
				$schurl = 'dispEquiposSala.php?wemp_pmla='.$wemp_pmla.'&consultaAjax=10&wfec='.$wfec1.'&wsw='.@$wsw.'&colorDiaAnt='.$colorDiaAnt.'&wbasedato='.$solucionCitas.'&caso='.$caso.'&fest='.$fest.'';
...
';
         ...
         {                                            //faltan las otras citas
				 ...
				 //$wfec=$fyear.'-'.$fmonth.'-'.$fday;  //fecha seleccionada para todas las dias
				...
				if ($colorDiaAnt == "rojo" and $wfec>$wfec1)
				{
				 ...
				 else
				 {
					...
					$schurl = 'dispEquiposSala.php?wemp_pmla='.$wemp_pmla.'&consultaAjax=10&wfec='.$wfec.'&wsw='.@$wsw.'&colorDiaAnt='.$colorDiaAnt.'&wbasedato='.$solucionCitas.'&caso='.$caso.'&fest='.$fest.'';
		...
		';
		   ...
		   //para sacar el tooltip
		 if ($caso==3) 
		 {
			$dato = "<a class='tooltip preload {direction:ne;width:400px;}' href='../procesos/dispToolEquiposSala.php?wemp_pmla=".$wemp_pmla."&consultaAjax=10&wfec=".$wfec."&wbasedato=".$solucionCitas."&caso=".$caso."&fest=".$fest."' onclick='return false;' ><font size=7><b>".$fday."</b></font></a>";
		 }
		 else if ($caso==2) 
		 {
			$dato = "<a class='tooltip preload {direction:ne;width:700px;}' href='../procesos/dispTool.php?wemp_pmla=".$wemp_pmla."&consultaAjax=10&wfec=".$wfec."&nomdia=".$numDia."&wbasedato=".$solucionCitas."&caso=".$caso."&fest=".$fest."' onclick='return false;' ><font size=7><b>".$fday."</b></font></a>";
		 }
		 else if($caso==1) 
		 {
			$dato = "<a class='tooltip preload {direction:ne;width:400px;}' href='../procesos/dispToolEquiposSala.php?wemp_pmla=".$wemp_pmla."&consultaAjax=10&wfec=".$wfec."&wbasedato=".$solucionCitas."&caso=".$caso."&fest=".$fest."' onclick='return false;' ><font size=7><b>".$fday."</b></font></a>";
...
';
         if (isset($fvar) && $fvar != "")
         {
            echo '		<A class=\'calendarSala\' style="cursor: hand" onClick="javascript:window.open(\''.$schurl.'\', 
...
<HEAD>
   ...
   echo '<STYLE TYPE="text/css">
	<!--
		...
		A.calendarSala {color: #'.$calendarSala_link_color.'; font-size:10px}
		...
		TABLE.externa { margin: 0px; padding: 0px; border-width: none; font-size: 12px; text-indent: 0px;
			font-weight: normal; width: 50%; height: 50%; background-color: #'.$calendarSala_bg_color.';  color: #'.$plain_text_color.'}
		...
		TD.calendarSala {width: 90px; text-align: left; font-size: 10px; height: 70px; color: #000000;}
...
';
	...
	if ($wemp_pmla == 01)
	{
		encabezado1("ASIGNACIÓN DE SALAS", "2014-02-03", $wbasedato );
	}
	else
	{
		encabezado1("ASIGNACIÓN DE SALAS", "2014-02-03", "logo_".$wbasedato );
	}
	
	
	echo "<BODY TOPMARGIN=0 LEFTMARGIN=0 MARGINHEIGHT=0 MARGINWIDTH=0 onunload='window.opener.postCerrarVentanaCitas()'>
	<CENTER>";
	
	
	echo "<table border=0 width=1025 align=center>";  //tabla para ubicacion
	echo "<tr>";
	echo "<td width=700>";
	
	echo "<div style='width:100%;'>"; //externo
	...
	<TD class="form" align="center" valign="bottom" width="100%" COLSPAN=7>
		<FORM METHOD="post" ACTION="calendarSala.php?empresa='.$solucionCitas.'&wemp_pmla='.$wemp_pmla.'&fest='.$fest.'&consultaAjax=">
	...
	<TD align="center" valign="middle" height=60 COLSPAN=7>
		...
		if ($caso == 3 or $caso ==1)
		{
			echo '<TD class="ends" nowrap align="center" valign="bottom" >
				<b><A HREF="calendarSala.php?month='.$prevmonth.'&year='.$prevyear.'&empresa='.$solucionCitas.'&wemp_pmla='.$wemp_pmla.'&caso='.$caso.'&wsw='.$wsw.'&fest='.$fest.'&consultaAjax="><< '.$backward.'</a></b>
			</TD>';
		}
		else
		{
				echo '<TD class="ends" nowrap align="center" valign="bottom" >
				<b><A HREF="calendarSala.php?month='.$prevmonth.'&year='.$prevyear.'&empresa='.$solucionCitas.'&wemp_pmla='.$wemp_pmla.'&caso='.$caso.'&fest='.$fest.'&consultaAjax="><< '.$backward.'</a></b>
			</TD>';
		}
...
';
   ...
   echo '<br><div style="background:#2A5DB0">'.$current.'</div>
		...
		{
			echo '<TD class="ends" nowrap align="center" valign="bottom">
				<b><A HREF="calendarSala.php?month='.$nextmonth.'&year='.$nextyear.'&empresa='.$solucionCitas.'&wemp_pmla='.$wemp_pmla.'&caso='.$caso.'&wsw='.$wsw.'&fest='.$fest.'&consultaAjax=">'.$forward.' >></a></b>
			</TD>';
		}
		else
		{
			echo '<TD class="ends" nowrap align="center" valign="bottom">
				<b><A HREF="calendarSala.php?month='.$nextmonth.'&year='.$nextyear.'&empresa='.$solucionCitas.'&wemp_pmla='.$wemp_pmla.'&caso='.$caso.'&fest='.$fest.'&consultaAjax=">'.$forward.' >></a></b>
			</TD>';
		}
	echo'	</TR>
		</TABLE>
...
';
   ...
   {
/* Alternate beginning day of the week for calendarSala view was created by Marion Heider of clixworx.net. */
      echo '<TR>';
      for ($j=0;$j<7;$j++)
      {
         if ($j == 0)
   ...
   echo '	</TABLE>
	...
	echo "<tr>";
	echo "<td bgcolor='#FFFFCC'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>Hay Horarios en Sala disponibles</td>";
	echo "<tr>";
	echo "<tr>";
	echo "<td bgcolor='#C4E2FF'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>Todas los Horarios de Sala disponibles</td>";
	echo "<tr>";
	echo "<tr>";
	echo "<td bgcolor='#FFCCCC'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>No hay Horario en Sala disponible</td>";
	echo "<tr>";
	echo "</table>";
	echo "</div>";
	...
	echo "</table>";  //tabla para ubicacion
															//empresa=citascs&wemp_pmla=02&caso=1&wsw=011&consultaAjax=
	...
	$year1 = $current1[1];
	echo "<meta name='met' id='met' http-equiv='refresh' content='60;url=calendarSala.php?empresa=".$solucionCitas."&wemp_pmla=$wemp_pmla&caso=".$caso."&wsw=".@$wsw."&month=".$month2."&year=".$year1."&fest=".$fest."&consultaAjax='>";
