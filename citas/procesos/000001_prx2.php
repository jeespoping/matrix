...
echo "<tr><td align=center bgcolor='#cccccc'><font size=2> <b> 000001_prx2.php Ver. 1.05</b></font></tr></td></table>";
echo "</center>";
echo "<form action='000001_prx2.php' method=post>";
session_start();
...
{		
	...
	if (isset($wpar))
	{
		...
		if ($tiperr == 0)
		{
			...
			{				
				...
				$query = "update ".$empresa."_000001 set cod_exa='".substr($Codexa,0,strpos($Codexa,"-"))."', cod_med= '".substr($Codmed,0,5)."', nom_pac='".ucwords($Nompac)."', nit_resp='".$Nitres."', telefono='".$Tel."', edad=".$Edad.", comentarios='".$Coment."',activo='".substr($Estado,0,1)."' where  cod_equ='".$Codequ."' and fecha='".$Fecha."' and hi='".$Hi."' and hf='".$Hf."'";
				$err = mysql_query($query,$conex);
				 if ($err = 1)
				{
				...
				else
				{
					...
					if ($num == 0)
					{
						$fecha = date("Y-m-d");
						$hora = (string)date("H:i:s");
						$query = "insert ".$empresa."_000004 (medico,fecha_data,hora_data,fecha,equipo,uni_hor,hi,hf,seguridad) values ('".$key."','".$fecha."','".$hora."','".$Fecha."','".substr($Codequ,0,3)."',".$row[2].",'".$row[3]."','".$row[4]."','C-".$empresa."')";
						$err = mysql_query($query,$conex);	
						 if ($err != 1)
					 	{
						 	echo "<font size=3><MARQUEE BEHAVIOR=SCROLL BGCOLOR=#99CCFF LOOP=-1>LOS DATOS ESTAN OK!!!!</MARQUEE></FONT>";
	...
	$num = mysql_num_rows($err);
	echo "<table border=0 align=center>";
	echo "<li><A HREF='000001_prx1.php?wequ=".$pos2."&wtit=".$pos9."&wfec=".$pos4."&empresa=".$empresa."'>Retornar</A><br>";
	if ($num > 0)
