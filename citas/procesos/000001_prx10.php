...
echo "<tr><td align=center bgcolor='#cccccc'><font size=2> <b> 000001_prx10.php Ver. 2007-04-03</b></font></tr></td></table>";
echo "</center>";
echo "<form  name='Citas' action='000001_prx10.php' method=post>";
session_start();
...
{		
		...
		if ($tiperr == 0)
		{
			...
			{				
				...
				if(isset($Asistida))
					...
					$Asistida="off";
				$query = "update ".$empresa."_000001 set cod_exa='".substr($Codexa,0,strpos($Codexa,"-"))."', hf= '".$Hf."', Ndias=".$Ndias.", Cedula='".$Cedula."', cod_med= '".substr($Codmed,0,5)."', nom_pac='".ucwords($Nompac)."', nit_resp='".$Nitres."', telefono='".$Tel."', edad=".$Edad.", comentarios='".$Coment."', Asistida='".$Asistida."', activo='".substr($Estado,0,1)."' where  cod_equ='".$Codequ."' and fecha='".$Fecha."' and hi='".$Hi."'";
				$err = mysql_query($query,$conex);
				 if ($err = 1)
				{
				...
				if(isset($Asistida))
					...
					$Asistida="off";
				$query = "insert ".$empresa."_000001 (medico,fecha_data,hora_data,cod_med,cod_equ,cod_exa,fecha,hi,hf,Ndias,Cedula,nom_pac,nit_resp,telefono,edad,comentarios,Asistida,usuario,activo,seguridad) values ('".$empresa."','".$fecha."','".$hora."','".substr($Codmed,0,5)."','".substr($Codequ,0,3)."','".substr($Codexa,0,strpos($Codexa,"-"))."','".$Fecha."','".$Hi."','".$Hf."',".$Ndias.",'".$Cedula."','".ucwords($Nompac)."','".$Nitres."','".$Tel."',".$Edad.",'".$Coment."','".$Asistida."','".substr($user,2,strlen($user))."','".substr($Estado,0,1)."','C-".$empresa."')";
				$err = mysql_query($query,$conex);
				if ($err != 1)
				{
	...
	$query = "select cod_med,cod_equ,cod_exa,fecha,hi,hf,nom_pac,nit_resp,telefono,edad,comentarios,usuario,activo,Ndias,Cedula,Asistida from ".$empresa."_000001 where  cod_equ='".$pos2."' and fecha='".$pos4."' and hi='".$pos5."'";
	$err = mysql_query($query,$conex);
	$num = mysql_num_rows($err);
	echo "<table border=0 align=center>";
	echo "<li><A HREF='000001_prx9.php?wequ=".$pos2."-".$wtit."&wtit=".$wtit."&wfec=".$pos4."&empresa=".$empresa."&amp;wsw=".$wsw."'>Retornar</A><br>";
	if ($num > 0)
	...
	else
	{
		...
		else
		{
			...
			{
				$row[14]=$Cedula;
				$query = "select Nom_pac, Nit_resp, Descripcion, Telefono, Edad, ".$empresa."_000001.id from ".$empresa."_000001,".$empresa."_000002 where  Cedula='".$Cedula."' and Nit_resp=Nit order by 6 desc ";
	...
	if(substr($wsw,0,1) == "1")
	{
		...
		$query .= "  order by ".$empresa."_000007.codigo";
	}
	else
	...
	else
	{
		...
		echo "<td bgcolor=#cccccc>";
		echo "<select name='Codexa'>";
		$query = "select codigo,descripcion,preparacion,cod_equipo,activo,especial from ".$empresa."_000006 where cod_equipo='".$pos2."' and codigo != '0' order by codigo";
		$err1 = mysql_query($query,$conex);
		$num1 = mysql_num_rows($err1);
		for ($i=0;$i<$num1;$i++)
		{	
	...
	echo "<tr>";
	echo "<td bgcolor=#cccccc>Numero de Dias</td>";
	echo "<td bgcolor=#cccccc><input type='TEXT' name='Ndias' size=5 maxlength=5 value='".$row[13]."'></td>";			
	echo "</td>";
	echo "</tr>";
	echo "<tr>";
	echo "<td bgcolor=#cccccc>Cedula</td>";
	echo "<td bgcolor=#cccccc><input type='TEXT' name='Cedula' size=12 maxlength=15 value='".$row[14]."' onblur='enter()'></td>";			
	echo "</td>";
	echo "</tr>";
	echo "<tr>";
