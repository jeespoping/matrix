...
/**
 PROGRAMA                   : funciones_talhuma.php
 ...
 *  Junio 15 de 2017    :   *   Cambio direccionamiento ODBC a nuevo Software SQL.
    Arleyda Insignares C.       

 *  Abril 17 de 2013
    Edwar Jaramillo     :   *   Se actualiza la función pintarMenuSeleccion() para que en la sección de indicadores, solamente pinte los códigos
                                de indicadores o estadísticas que estas realmente incluidos en el arbol de menús y submenús a pintar y que pertenezcan
                                a un padre determinado.
                            *   Se modifica pintarMenuSeleccion() para que al momento de pintar el arbol de menú y opciones de indicadores o estadísticas,
                                se dibuje mejor la jerarquía de árbol cuando es de solo lectura, p.e. al momento de ver los permisos por usuarios en el gestor de perfiles.
 ...
 * @return unknown
 */
function pintarMenuSeleccion($conex, $arbol, $arbol_info, &$divs_include, $modoImprimir = 'checks', $codmenu = '', $sangria = '20', $padre = '', $propiedades = array(), $propiedad_ok = '', $es_indicador = '', $pro_especial = '', $prop_especifica = '', $tabla_propiedades = '',$arr_arbol_origen = array())
{
    if(count($arr_arbol_origen) == 0) { $arr_arbol_origen = $arbol; }
    $result = '';
    ...
    else
    {
        ...
        foreach($arbol as $key => $value)
        {
            if($modoImprimir == 'checks')
            {
                ...
                if($es_indicador != 'on')
                {
                        ...
                        while ($rowq = mysql_fetch_array($resp_q))
                        {
                            ...
                            if($prop_especifica == '') // Si no es opcion reorganizar
                            {
                                ...
                                $grupos_contenedor .= " <td class='noms_propiedad' style='font-size:8pt;color:gray;font-weight:bold;' nowrap='nowrap'>
                                                            ...
                                                            <input type='radio' ".$chk_r." id='radio_".$tbl."_".$id_padre.$key."' name='grupo_propiedad' value='".$tbl."' ".$fn_evento." />".$rowq['Prodes']."</td>
                                                            </div>";
                            }
                    ...
                    $result .= pintarMenuSeleccion($conex, $arbol[$key], $arbol_info, $divs_include, $modoImprimir, $key, $sangria+20, $id_padre.$key, $propiedades, $prop_uno,'','','','',$arr_arbol_origen);
                }
                else
                {
                    // $result .=  print_r($propiedades,true);
            ...
            elseif($modoImprimir == 'solo_arbol')
            {
                ...
                $result .= "<div style='color:#2A5DB0; font-weight:bold; font-size: 9pt; text-aling:justify; background-color: #F4F4F4; border-bottom: 1px #ffffff solid; margin-left: ".$sangria."px;'>
                            <table cellpadding='0' cellspacing='0' border='0'>
                                <tr>
                                    <td valign='top' style='color:#2A5DB0; font-weight:bold; font-size: 8pt; text-align:justify; background-color: #F4F4F4; border-bottom: 1px #ffffff solid;'>&bull;&nbsp;</td>
                                    <td align='justify' valign='' style='color:#2A5DB0; font-weight:bold; font-size: 8pt; text-align:justify; background-color: #F4F4F4; border-bottom: 1px #ffffff solid;'>".$arbol_info[$key]['info']['nombre_subtab']."</td>
                                </tr>
                            </table>
                            </div>";
        ...
        elseif($modoImprimir == 'checks' && count($propiedades) > 0 && $propiedad_ok != '')
        {
            ...
            foreach($propiedades[$propiedad_ok] as $id_propiedad => $indicadores_list)
            {
                ...
                {
                    if(array_key_exists($id_indicador, $arr_arbol_origen[$id_men_padre[1]]))
                    {
                        $result_aux .= $html_chk;
                    }
                }
            ...
            case "costosyp_000005":
                    $query = "  SELECT  tb1.Ccocod AS codigo, tb1.Cconom AS nombre
                                FROM    costosyp_000005 AS tb1
                                        INNER JOIN
                                        ".$wbasedato."_000013 AS tal ON (tb1.Ccocod = tal.Idecco)
                                WHERE   Ccoemp = '".$wemp_pmla."' AND tb1.Ccocod IN ('".trim($ids_in)."')
                                GROUP BY    tb1.Ccocod
                                ORDER BY    tb1.Cconom";
                    break;
            case "uvglobal_000003":
    ...
    if($row = mysql_fetch_array($res))
    {
        ...
        switch ($tabla_CCO)
        {
            ...
            case "farstore_000003":
                    ...
                    break;
            case "costosyp_000005":
                    $qcc="select Ccocod AS codigo, Cconom AS nombre from costosyp_000005 where Ccoemp = '".$wemp_pmla."' ORDER BY Cconom ";
                    break;
            case "uvglobal_000003":
...
function buscarEnUnix($conex,$wemp_pmla,$wbasedato,$user_session,$wcodigo,$wced,$wnombre1,$wnombre2,$wapellido1,$wapellido2,$wccostos,$wccargo,$wsexo,$westado,$wfechaingreso,$wfecharetiro,$wingresoretiro)
{
    ...
    $conexunix = odbc_connect('queryx7','','') or die("No se ralizo Conexion con Sql");
