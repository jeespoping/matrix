...
*/
                                                       // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
$wactualiz="2017-04-18";                               // Aca se coloca la última fecha de actualización de este programa //
                                                       // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
...
{
    //return ((strtotime($fechafin)-strtotime($fechainicio))/86400); --> no contar el primer dia
    //echo "en la funcion $fechainicio, $fechafin ".(((strtotime($fechafin)-strtotime($fechainicio))/86400)+1)."<br>";
...
function fila_periodo_disponible($wclass, $fecha_inicio, $fecha_fin, $dias_disponibles, $consecutivo, $codigo_empleado, $wcentro_costo_empleado, $arr_solicitudesEnviadas, $div_acordeon, $es_un_coordinador, $WMAXIMO_DIAS, $WMINIMO_DIAS, $diasLicencia, $detalleDias, $diasDisponiblesReales, $wfechaFinRealPeriodo='', $diasDisfrutadosHoy = 0)
{
    ...
    $html_tr .= '
            <tr align="center" class="'.$wclass.' '.$class_solicitado.' tr_solicitudes_personales solicitudesPendietes '.$claseDisfrutada.'" id="tr_'.$idx_diferenciador_html.'" fechaInicioPendiente="'.$fecha_inicio.'" fechaFinalPendiente="'.$fecha_fin.'" disabled="disabled" vencida="'.$solicitud_vencida.'">
                ...
                <td align="center">'.$diasLicencia.'</td>
        ...
        while ($row = mysql_fetch_array($result))
        {
            ...
            if( strtotime( date('Y-m-d') ) - strtotime( $row['fecha_fin_pendiente'] ) <= 0 ){
                ...
                foreach ($aux as $keyTipo => $dias ) {
                    ...
                    //echo "<br>".$datosDiasLicencia['diasNoContados'][$keyTipo]."   -*--> $keyTipo ----> $diasLicenciaPeriodo";
...
function validarUsuarioCoordinador($conex, $wemp_pmla, $wbasedato, $wtalhuma, $wusuario)
{
    ...
    if(mysql_num_rows($result) > 0)
    {
        ...
        while($row = mysql_fetch_array($result))
        {
            ...
            /*if(!array_key_exists($codigo_usumatrix, $usuario_coordinador["lista_solicitantes"]))
            {
                $usuario_coordinador["lista_solicitantes"][$codigo_usumatrix] = array();
            }*/
...
function diasLicenciaNoRemunerada( $wfechaInicio, $wfechaFinal, $codigo ){
    ...
    while ( odbc_fetch_row($rs) ){
        ...
        /*echo "<br> limite inferior: ".odbc_result($rs,2)." --> ".$wfechaInicio;
        echo "<br> limite superior: ".odbc_result($rs,3)." --> ".$wfechaFinal;
        echo "<br>---------------------------------------------------------->";*/
        ...
        //echo "<br> LIMITES -->  {$fecLimiteInferior},  {$fecLimiteSuperior}";
        $tuvoLicencia  = true;
...
function validarRangoFechas( $conex, $wemp_pmla, $wbasedato, $arr_parametros, $user_solicitud, $idSolicitud = '' ){
    ...
    //$wusuario               = empresaEmpleado($wemp_pmla, $conex, $wbasedato, $user_solicitud);
    $fecha_inicio_solicitud = $arr_parametros['fecha_inicio_solicitud'];
    $fecha_fin_solicitud    = $arr_parametros['fecha_fin_solicitud'];
...
function buscarFechaRegreso( $wusuario_solicitud, $wfec_i_periodo, $wfec_i, $wfec_f_periodo, $objSolicitud ,$diasDisfrutar, $diasDisfrutadosHoy=0 ){
    ...
    if( $porAprobar != "on"){
        if( $objSolicitud == "fecha" ){
            //echo "entro por 1 ----> {$wfec_f_periodo}   ----> {$wfec_i} ---> ".date("Y-m-d")."\n";
...
if( $consultaAjax == "si" ){
    if( $peticion == "buscarFechaRegreso" ){
        ...
        //$diasDisfrutar = $diasDisfrutar*1 - $diasDisfrutadosHoy*1;
...
//=======================================================================================================//
   ...
   ***********************************************/
    ...
    //-> se consulta si el usuario tiene restricción de acceso al software por casos especiales de riesgo 5
    $query = " SELECT count(*)
                 FROM {$wbasedato}_000013
                WHERE Rieusu = '{$wusuario_solicitud}'
                  AND Rieest = 'on'
    ...
    $rs     = mysql_query( $query, $conex );
    $rowRsv = mysql_fetch_array( $rs );
    if( $rowRsv[0] > 0 ){
        echo '  <br /><br /><br /><br />
            <div style="color: #676767;font-family: verdana;background-color: #E4E4E4; text-align:center;" >
                <br>
                <img src="../../images/medical/root/Advertencia.png"/>
                [?]Para solicitar sus vacaciones debe acudir directamente al área de nómina o comunicarse a la extensión 1285.
                <br>
            </div>';
        return;
    }
    ...
    while(odbc_fetch_row($res))
    {
        ...
        //--> 2015-09-10
        $aux               = diasDisponiblesPeriodo( odbc_result($res,1), odbc_result($res,2), $cedulaUsuario, $arrayPeriodosRiesgo );//dias a los que se tiene derecho en el último periodo disfrutado
        //--> 2015-09-22
        $datosDiasLicencia = diasLicenciaNoRemunerada( odbc_result($res,1), odbc_result($res,2), $wusuario_solicitud_unix );//mas uno porque se incluye el dia final
        $diasLicencia      = $datosDiasLicencia['diasLicencia'];//mas uno porque se incluye el dia final

        //-->2015-11-25
        if( $liquidaciones > 0 ){
            if( $fecha_anterior != odbc_result($res,1) ){//--> si hubo cambio de fecha de inicio desde la iteración anterior a esta.
        ...
        $liquidaciones++;
        $fecha_anterior       = odbc_result($res,1);
        $diasLicenciaAnterior = $diasLicencia;

        $diasDisponibles   = 0;
        if( !isset( $diasDisfrutadosPeriodo[odbc_result($res,1)."->".odbc_result($res,2)] ) ){
            $diasDisfrutadosPeriodo[odbc_result($res,1)."->".odbc_result($res,2)] = 0;
        }
        foreach ($aux as $keyTipo => $dias ) {
        ...
        $fecini_cumplido = substr(odbc_result($res,1),0,10);
        $fecfin_cumplido = substr(odbc_result($res,2),0,10);
        $fecini_disfruta = substr(odbc_result($res,4),0,10);
        $fecfin_disfruta = substr(odbc_result($res,5),0,10);
        ...
        $diasDisfrutadosPeriodo[odbc_result($res,1)."->".odbc_result($res,2)] += $dias_total;
        ...
        if( $diasPendientesDisfrutar < 0 ){
            if( count( $pila_dias_pendientes ) > 0 ){
                ...
                while( $diasPendientesDisfrutar < 0 and count( $pila_dias_pendientes ) > 0 ){
                    ...
                    $diasDisfrutadosPeriodo[odbc_result($res,1)."->".odbc_result($res,2)] = $diasDisfrutadosEnPeriodo;

                    array_pop($pila_dias_pendientes);
                }

                if( $diasPendientesDisfrutar < 0 ){
                    $aux = array( 'periodo'=>odbc_result($res,1)."  -->  ".odbc_result($res,2), "diasDisfrutados"=>$diasDisfrutadosEnPeriodo, "diasFaltantesPorDisfrutar"=>$diasFaltantesPorDisfrutar );
                    array_push( $pila_dias_pendientes, $aux );
                }

            }else{

                $aux = array( 'periodo'=>odbc_result($res,1)."  -->  ".odbc_result($res,2), "diasDisfrutados"=>$dias_total );
                array_push( $pila_dias_pendientes, $aux );
            }
    ...
    if( $haDisfrutadoVacaciones ){
        ...
        //echo "<br>".$wfec_i."  -->  ".$wfec_f." --> dias ultimo periodo: $wdiasUltimoPeriodo<br>";
        ...
        //echo "<br> $wfec_f + {$diasLicencia} day <br>".$wfec_i."  -->  ".$wfec_f."<br>";
        ...
        if($wtotdias < $diasDisponibles && $wtotdias > 0)
        {
            $datosDiasLicencia        = diasLicenciaNoRemunerada( $wfec_i, $wfec_f, $wusuario_solicitud_unix );//mas uno porque se incluye el dia final
            $diasLicencia             = $datosDiasLicencia['diasLicencia'];//mas uno porque se incluye el dia final
            //--> se quitó porque ya se sumaba arriba
            //$wfec_f                   = date("Y-m-d", strtotime($wfec_f."+ {$diasLicencia} day")); //se mueve  la Fecha Final próximo período la cantidad de dias que se han dado de licencia
    ...
    //echo "<br>EDB--><BR>dias: $wdias ---> periodos: $wperiodos<br>";
    ...
    if($usuario_coordinador['codigo_coordinador'] != '')
    {
        ...
        foreach ($usuario_coordinador['lista_solicitantes'] as $wusuario_solicitud_aprobar => $arr_empleado)
        {
            ...
            var_dump($arr_solicitudesEnviadasSubEmpleado);
  ...
  <title>VACACIONES</title>
    ...
    <script type="text/javascript">
        ...
        function url_ajax()
        {
            ...
            return "vacaciones.php?wemp_pmla="+wemp_pmla+"&wbasedato="+wbasedato+"&wtalhuma="+wtalhuma;
        }
...
<body>
    ...
    <!-- //FORMA ================================================================ -->
    <form name='vacaciones' action='vacaciones.php' method=post>
        ...
        <table align="center">
            <tr class="fila1"><td>Codigo: <input name='wusuario_solicitud' id='wusuario_solicitud'></td></tr>
            ...
            <div align=center id="disfrutadas" style='display:'>
                <table>
                    <tr align="center" class="encabezadoTabla">
                        ...
                        <td align="center">Días Causados disfrutados</td>
                        <td colspan="3" align="center">Fecha disfrutado</td>
                        <td align="center">Días Calendario<br>Disfrutados</td>
                        <?=$html_disfrutadas?>
                    </tr>
                </table>
            </div>
        ...
        <div width='' id='accordion_pendientes' style="text-align:left;" class="div_alinear">
            ...
            <div align="center" id="pendientes" style='display:'>
                ...
                <table>
                    ...
                    <tr align="center" class="encabezadoTabla">
                        <td colspan="3" align="center">Período Cumplido</td>
                        <td align="center">Días<br>de Licencia</td>
                        <td align="center">Días pendientes<br> Causados </td>
