<html>
<head>
  <title>REPORTE DE CARGOS DE PACIENTES AMBULATORIOS</title>
<script type="text/javascript">

	//Redirecciona a la pagina inicial
	function inicioReporte(wfecini,wfecfin,wcco,wemp_pmla,bandera){
	 	document.location.href='rep_cargospacamb.php?wfecini='+wfecini+'&wfecfin='+wfecfin+'&wccocod='+wcco+'&wemp_pmla='+wemp_pmla+'&bandera='+bandera;
	}
...
<?php
  ...
  /******************************************************************************
   *     REPORTE DE CARGOS DE PACIENTES AMBULATORIOS                 		    *
   ******************************************************************************/
   
	/*--------------------------------------------------------------------------
	| DEDSCRIPCIÓN: Reporte de cargos de pacientes Unidades ambulatorias        |
...
//RESULTADO DE CONSULTA DEL REPORTE
else
  {
	...
	$wfecfin1= strtotime($wfecfin);
	$wfecini1= strtotime($wfecini);
	$dias = ($wfecfin1 - $wfecini1)/86400 ;//hay que dividirlo por esto porque la resta da en segundos y asi me da los segundos de 1 dia son 86400
	//echo ' dias '.$dias.'<br>';
	if ($dias <= 7)
    {
			...
			echo "<tr>";
			echo "<td height='37' colspan='2' class='titulo'><p align='left'><strong> &nbsp; Cargos Pacientes Unidades Ambulatorias&nbsp;  &nbsp; </strong></p></td>";
			echo "</tr>";
			echo "<tr>";
			...
			$num = mysql_num_rows($err);
				  ...
				  // Abro el archivo
				  $archivo = fopen("rep_cargospacamb.txt","w"); 
				  //Coloco los titulos
		...
		$color3="#aaaaaa";
			...
			for ($i=1;$i<=$num;$i++) 
				...
				echo "</table>";
				fclose($archivo);
				echo "<li><A href='rep_cargospacamb.txt'>Presione Clic Derecho Para Bajar El Archivo ...</A>";
				echo "<br>";
	...
	{
		echo '<li>Dias generados '.$dias.'<br>';
		echo "<li>Este reporte no debe tener un rango mayor a 7 dias !!!";
	}
  }
}
?>
