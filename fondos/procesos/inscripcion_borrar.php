...
$wbasedato = consultarAliasPorAplicacion($conex, $wemp_pmla, "fondos" );
...
function validar_clave_ppal($wusuario, $wclave_ppal, $wempresa){
	...
	//On
	echo "PAsoooooooooooooooooooooo<br>";
...
function inscribirse($wemp_pmla, $wusuario, $wcedula, $wcentro_costos, $wtipo_inscripcion, $wclave_del_ppal, $wclave_del_suplente, $wdatos_suplente, $wnombre_suplente, $wempresa_asociado, $wempresa_real)
    {
	...
	//Separo lo datos del control de votaciones
	$array_datos_inscripcion = explode("-",$wdatos_inscripcion);	
	$wano_votacion = $array_datos_inscripcion[0]; 			//Ano de votacion.
	$wconsecutivo_votacion = $array_datos_inscripcion[1];	//Consecutivo de votacion.  
	...
	if($validar_clave_ppal == ''){
		...
		if($validar_clave_suplente == ''){
			...
			if( $num1 > 0 ){
				...
				$q = " INSERT INTO ".$wbasedato."_000003 (   Medico  ,    Fecha_data,      Hora_data,        Insano         ,        Insvot               ,    Inscod    ,    Insced     ,      Insemp     ,          Inscco      ,  Insapr,          Instip         ,         Inscsu        ,        Insces         ,         Insesu         ,           Insccs             ,  Insest,     Seguridad     ) "
						  ."                     VALUES ('".$wbasedato."','".$wfecha."','".$whora."','".$wano_votacion."' , '".$wconsecutivo_votacion ."','".$wusuario."', '".$wcedula."', '".$wempresa_asociado."', '".$wcentro_costos."',  'off' , '".$wtipo_inscripcion."','".$wcodigo_suplente."','".$wcedula_suplente."','".$wempresa_suplente."','".$wcentro_costos_suplente."',    'on', 'C-".$wusuario."')";
...
if(isset($consultaAjax))
	...
	switch($consultaAjax){
		...
		case 'inscribirse':  
					{
					echo inscribirse($wemp_pmla, $wusuario, $wcedula, $wcentro_costos, $wtipo_inscripcion, $wclave_del_ppal, $wclave_del_suplente, $wdatos_suplente, $wnombre_suplente, $wempresa_asociado, $wempresa_real);
					}					
		break;
...
function inscribirse(wempresa_asociado, wusuario, wcedula, wtipo_inscripcion, wcentro_costos, wempresa_real)
{
...
$array_datos_inscripcion = explode("-",$wdatos_inscripcion);
$wano_inscripcion = $array_datos_inscripcion[0];
$wconsecutivo_inscripcion = $array_datos_inscripcion[1];
$westado_inscripcion = $array_datos_inscripcion[2];
...
if($num_asoc == 0)
	{
	echo "<br>\n<br>\n".
        " <H1 align=center>No se encuentra asociado al fondo de empleados.<FONT COLOR='RED'>" .
        " </FONT></H1>\n</CENTER>";
	return;
	}
...
$query_ppales_y_delegado[] = " SELECT Insced, Insapr, Idegen, Ideced, Ideno1, Ideno2, Ideap1, Ideap2, Idecco, Ideccg, Ideuse, Insces
							    ...
							    WHERE Insced = Ideced
							      AND Insano = '".$wano_inscripcion."'
								  AND Insvot = '".$wconsecutivo_inscripcion."'
...
$query_asociados[] = " SELECT Idegen, Ideced, Ideno1, Ideno2, Ideap1, Ideap2, Idecco, Ideccg, Ideuse
...
if($num_inscrito > 0){
	...
	if($wfecha_max_insc_asoc >= $wfecha_inscripcion){
...
echo "<div id='form_contrasenas' style='display:none;' title='Ingresar contraseñas'>		
		<table border='0' >
		  <tbody>
			...
			<tr>
			  <td align=center colspan='3' rowspan='1'><input type='button' id='inscribirse_ppal' value='Inscribirse' onclick='inscribirse(\"$wempresa_asociado\", \"$key\", \"$wcedula\",\"01\", \"$wcentro_costos\", \"$wempresa_real\")' /></td>
			</tr>
		  </tbody>
		</table>					
	  </div>";
