<title>INGRESO MATERIAL UNIX V3.00</title>
</head>
<body BGCOLOR="">
<BODY TEXT="#000066">
<?php
...
else
{
	...
	/*fUNCION PARA BUSCAR EL ESTADO DEL REGISTRO*/
	FUNCTION Estado($regNum,$fecha,$hora)
	{
		$conex_o = odbc_connect('inventarios','','');
		$ini1=strpos($regNum,"-");
		$lin=substr($regNum,$ini1+1);
		$num=substr($regNum,0,$ini1);
		$query="select droest from itdro where dronum='".$num."' and drolin='".$lin."'and drofad='".$fecha." ".$hora."'";
		//echo $query."<br>";
		$err_o = odbc_do($conex_o,$query);
		if (odbc_fetch_row($err_o))
		{
			
			$est=odbc_result($err_o,1);
		//	echo "entro por aqui $regNum = $est <br>";
			switch ($est)
			{
				case 'S': $est="NO INGRESADO A FACT.";
					$color="#00AADD";
					return array($est,$color);
				break;
				case 'I': 
					$est="";
					$query="select droincdes from itdroinc where droincnum='".$num."' and droinclin='".$lin."'";
					$err_o = odbc_do($conex_o,$query);
					while (odbc_fetch_row($err_o))
					{
						//Encontro inconsistencias
						$est=$est.odbc_result($err_o,1);
					}
					$color="#ffff00";
					return array($est,$color);
				break;
	...
	if(!isset($year1)  or !isset($year2) )
	{
		...
		echo "<td bgcolor=#cccccc><select name='hora1'>";
			...
			echo "<option>10:00:00</option>";
			echo "<option>13:00:00</option>";
			echo "<option>16:00:00</option>";
			echo "<option>19:00:00</option>";
			echo "<option>22:00:00</option>";
			echo "<option>01:00:00</option>";
			echo "<option>04:00:00</option>";
		echo "</select></td></tr>";
		...
		echo "<td bgcolor=#cccccc><select name='hora2'>";
			...
			echo "<option>09:59:59</option>";
			echo "<option>12:59:59</option>";
			echo "<option>15:59:59</option>";
			echo "<option>18:59:59</option>";
			echo "<option>21:59:59</option>";
			echo "<option>24:59:59</option>";
			echo "<option>03:59:59</option>";
		echo "</select></td></tr>";
		...
		if($cc != "")
		{
			...
			if($num>0)
			{
				for ($j=0;$j<$num;$j++)
				{
					...
					list($est,$color)=Estado($row[2],$row[0],$row[1]);
					/*Si el numero del registro es diferente al que habia almacenado*/
					if(substr($row[2],0,$ini1) != $nums and $est == "P")
					{
		...
		else
		{
			...
			if($num>0)
			{
				for ($j=0;$j<$num;$j++)
				{
					...
					list($est,$color)=Estado($row[3],$row[1],$row[2]);
					/*Si el numero del registro es diferente al que habia almacenado*/
					if(substr($row[3],0,$ini1) != $nums and $est == "P")
					{
