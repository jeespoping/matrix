...
IF($conex_o != 0)
{
	...
	$query="select * from farmpda_000001 where fuente='1' ";
	...
	if($num_ppal>0)
	{
		...
		for($ppal=0;$ppal<$num_ppal;$ppal++)
		{
				...
				IF ($ok == "ok") {
					...
					if($ok == "ok") {
						...
						if(odbc_num_rows($err_o) > 0)
						{
							...
							$query1="update farmpda_000001 set fuente='".$fuente."', paciente='".$pacnom."', Cod_articulo='".$artcod."', Descripcion_art='".$artgen."-".$art1."', Cantidad='".$cant."' where Reg_num='".$row["Reg_num"]."' and id='".$row["id"]."' ";
						...
						{
							//echo "NO FUE POSIBLE INGRESAR EL REGISTRO ".$row["Reg_num"]." A ITDRO";
							$ok ="NO PASO A ITDRO";
							$color="#ff0000";
						}
			...
			{
				/*YA FUE INGRESADO A ITDRO*/
				/*HACER EL CAMBIO PARA QUE INGRESE*/
				$queryo="select Max(drolin) from itdro where dronum='".$num."' ";
				$err_o=odbc_do($conex_o,$queryo);
				if(odbc_fetch_row($err_o))
				{
					$nu=odbc_result($err_o,1)+1;
					$query="update farmpda_000001 set Reg_num='".$num."-$nu' ";
					$query= $query."where Fuente='1' and Reg_num='".$row["Reg_num"]."' and id='".$row["id"]."' ";
					//		echo "<br>$query<br>";
					$err=mysql_query($query,$conex);
					IF(mysql_affected_rows()>0)
					{
						$ok="NO PASO, YA ESTABA EN EN ITDRO, SE SOLUCIONO";
						$color="#0000FF";
					}
					ELSE
					{
						$ok="NO PASO, YA ESTABA EN EN ITDRO";
						$color="#ff0000";
					}
				}
			}
	...
	*********************************************************************************/
	$query="select * from farmpda_000003 where fuente='2'";
	$err_ppal=mysql_query($query,$conex);
	$num_ppal = mysql_num_rows($err_ppal);
	if($num_ppal>0)
	{
		...
		for($ppal=0;$ppal<$num_ppal;$ppal++)
		{
			...
			if(!odbc_fetch_row($err_o))
			{
				...
				IF ($ok == "ok") {
					...
					if($ok== "ok")										/*INGRESO DE LA INFORMACION A ITDRO*/
					{
						...
						if($ok == "ok") {
							...
							if($cant != 0) {
								...
								$query1="update farmpda_000003 set fuente='".$fuente."',Cod_articulo='".$artcod."',Descripcion_art='".$artgen."' where Reg_num='".$row["Reg_num"]."'  and id='".$row["id"]."' ";
								$err1=mysql_query($query1,$conex);
								/*No ingreso a invetras*/
								if($err1 != "")	{
									$codInt="1007";
			...
			else
			{
				...
				{
					$nu=odbc_result($err_o,1)+1;
					$query="update farmpda_000003 set Reg_num='".$num."-$nu' ";
					$query= $query."where Fuente='2' and Reg_num='".$row["Reg_num"]."' and id='".$row["id"]."' ";
					//echo "<br>$query<br>";
					$err=mysql_query($query,$conex);
					IF(mysql_affected_rows()>0)	{
						$ok="NO PASO, YA ESTABA EN EN ITDRO, SE SOLUCIONO";
