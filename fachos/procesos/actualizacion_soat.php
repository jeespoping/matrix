...
//=====================================================================================================================================================================
	...
	function obtener_querys()
	{
		$array_querys = array();
		//----------------------------------------------
		// --> Detcon = 14 (Direccion de residencia)
		//----------------------------------------------
		// --> Query inpac
			$q = "select ' '
				   from inpac
		...
		// --> Query inpac
			$q = "select ' '
				 ...
				 select munnom
				  ...
				  where pachis=HIS
					and pacnum=ING
					and paclug is not null
					and paclug=muncod
				";
			$array_querys['16']['inpac'] = $q;
		// --> Query inpaci
			$q = "select ' '
				   from inpaci
				  where pachis=HIS
				 ...
				 select munnom
				  ...
				  where pachis=HIS
					...
					and paclug=muncod
				";
			$array_querys['16']['inpaci'] = $q;
		//----------------------------------------------
		// --> Detcon = 17 (Departamento)
		//----------------------------------------------
		// --> Query inpac
			$q = " select ' '
				     from inpac
		...
		// --> Query inpaci
			$q = "   select 0.0 ,accacc
					...
					order by 2 desc
				";
			$array_querys['24']['inpaci'] = $q;
		//----------------------------------------------------------------
		// --> Detcon = 95 (Tipo de consulta (P=Primera vez, C=Control))
		//----------------------------------------------------------------
		// --> Query inpac
			$q = "   select ' ' ,accacc
					   from inpac,inacc
		...
		//----------------------------------------------------------------------------------
		// --> Detcon = 99 (Direccion del lugar del accidente)
		//----------------------------------------------------------------------------------
		// --> Query inpac
			$q = "   select accdetlug
...
if(isset($accion))
{
	...
	switch($accion)
	{
		...
		case 'realizar_actualizacion':
		{
			...
			{
				$q_update = "UPDATE ".$wbasedato."_000137
								SET movdat = '".$valor."'
							  WHERE id = '".$id."'
							";
...
else
{
	...
	<link type="text/css" href="../../../include/root/monthpicker.css" rel="stylesheet" charset="utf-8" />
