...
else
{
	...
	{
		echo "<form action='000004_of01_new.php' method=post>";
		...
		if(isset($pac1))
		{
			...
			$query="select Paciente from ".$empresa."_000004 where Oftalmologo='".$medico."' and Paciente like '%".$pac1."%' order by Paciente";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
		...
		if(isset($pac))
		{
			/*Si ya se tiene el paciente buscar las fechas de los seguimiento para este y
			 construir el drop down */	
			$query = "select Fecha  from ".$empresa."_000004 where Paciente='".$pac."' and Oftalmologo='".$medico."' ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	  ...
	  ********************************/
	{
		...
		$query="select * from ".$empresa."_000004 where Paciente='".$pac."' and Oftalmologo='".$medico."' and Fecha='".$fecha."' ";
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		if($num>0)
		$row = mysql_fetch_array($err);
		...
		//echo "<tr><td><br></td></tr>";
		//echo "<tr><td ><b>FECHA: </b>".$row[1]."</td></tr><tr><td >".$pacn."</td></tr><tr><td><br><br></td></tr>";
		...
		echo "<tr><td>&nbsp&nbsp&nbsp&nbsp<table  border=1 width=400><td width=80>.</td><td width=80 align=center><b>ESFERA</b></td>";
		...
		echo "<tr><td >&nbsp&nbsp".$row[14]."</td></tr>";
		echo"</table>";
	}
}
?>
