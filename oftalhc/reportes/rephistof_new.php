...
function Encabezado($nomed,$reg,$pacn,$id,$nrohist,$row){
	...
	echo "<td align=center colspan='4' ><font size=4 color='#000080' face='arial'><B>LAS AMÉRICAS</b></font></td>";
	...
	echo "<tr><td><font size=3  face='arial'><b>FECHA NACIMIENTO:</b>".$row[10]." </td>";
	echo "<td><font size=3  face='arial'><b>SEXO:</b> ".STRTOLOWER(SUBSTR($row[11],3))." </td>";
	echo "<td><font size=3  face='arial'><b>OCUPACION:</b> ".$row[12]." </td></tr>";
	echo "<tr><td ><font size=3  face='arial'><b>DIRECCION:</b>".$row[15]." </td>";
	echo "<td><font size=3  face='arial'><b>CIUDAD:</b> ".$row[14]." </td>";
	echo "<td><font size=3  face='arial'><b>TELEFONOS:</b>".$row[13]." </td>";
...
function Historia ($row){
	...
	echo "<tr><td align=left  colspan='3' bgcolor='#AADDFF'><font size='3' face='arial'><b>MOTIVO CONSULTA Y ENFERMEDAD ACTUAL</b>";
	echo "<fieldset style='background-color: #FFFFFF'>".$row['Motivo_consulta_y_enf']."</fieldset>";
	echo "<td align=left  colspan='3' bgcolor='#AADDFF'><font size='3' face='arial'><b>ANTECEDENTES PERSONALES</b>";
	echo "<fieldset style='background-color: #FFFFFF; '>".$row['Antec_personales']."</fieldset></TR>";
	echo "<td align=left  colspan='6' bgcolor='#AADDFF'><font size='3' face='arial'><b>ANTECEDENTES FAMILIARES</b>";
	echo "<fieldset style='background-color: #FFFFFF; '>".$row['Antec_familiar']."</fieldset></TR>";
	...
	echo "<TR><td colspan=6 width='756'><font size=3  face='arial'><b>ADICION:</b> ".$row['Adicion']."</td></TR>";
...
function Imagenes (&$A,&$p,$query,$conex) {
	...
	if($num>0)
	{
		for ($j=0;$j<$num;$j++)
		{
			...
			$A[$p][1]="IMAGENES".$row['Descripcion']."<br><b><A HREF='/matrix/graficas.php?Graph=".$row['Imagen']."&amp;usuario=1-oftalhc' target = '_blank'>Ver imagen";
			$A[$p][0]=$row['Fecha'];
			$p++;
		}
	}
...
else
{
	...
	if(!isset($medico)  or !isset($pac) or !isset($fecha1) or !isset($year) )
	{
		...
		echo "<form action='rephistof_new.php?empresa=$empresa' method=post>";
		...
		if(isset($pac1))
		{
			...
			$query="select DISTINCT Paciente from ".$empresa."_000003 where  Paciente like '%".$pac1."%' order by Paciente";
		...
		if(isset($pac))
		{
			...
			construir el drop down */
			$query = "select Fecha  from ".$empresa."_000003 where Paciente='".$pac."' ";//and Oftalmologo='".$medico."' "; V 1.03
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	...
	APARTIR DE AQUI EMPIEZA LA IMPRESION DE LA HISTORIA		*/
	{
		...
		$querya="select * from ".$empresa."_000001 where Nombre1='".$n1."' and Nombre2='".$n2."'  and ";
		...
		$query="select fecha from ".$empresa."_000003 where Paciente='".$pac."'  and Fecha between '".$fecha1."' and '".$fecha2."' order by Fecha";
		$err = mysql_query($query,$conex);
		...
		for ($y=0;$y<$nfechas;$y++)	{
			...
			$query="select * from	".$empresa."_000003 where Paciente='".$pac."' and  Fecha='".$fec[$y]."' order by Fecha";
			$err = mysql_query($query,$conex);
			...
			if($numy>0)	{
				...
				$query="select Fecha from	".$empresa."_000009 where Paciente='".$pac."' and ".$pre." order by Fecha";
				...
				//Lentes
				Lentes($A,$p,str_replace('tabla',$empresa.'_000004',$query),$conex);

				/*MEDICAMENTOS*/
				Medicamentos($A,$p,str_replace('tabla',$empresa.'_000005',$query),$conex);

				//EXAMENES
				Examenes($A,$p,str_replace('tabla',$empresa.'_000006',$query),$conex);


				// ORDEN DE CIRUGIA
				Cirugia ($A,$p,str_replace('tabla',$empresa.'_000008',str_replace('Fecha','Fecha_actual',$query)),$conex);

				//IMAGENES
				Imagenes ($A,$p,str_replace('tabla',$empresa.'_000007',$query),$conex);
				...
				if($nseg>0)	{
					for ($x=0;$x<$nseg;$x++) {
						$query="select * from	".$empresa."_000009 where Paciente='".$pac."' and Fecha='".$fecSeg[$x]."'";
						$err2 = mysql_query($query,$conex);
		...
		$querynota="select * from	".$empresa."_000011 where Paciente='".$pac."' and ".$pre." order by Fecha";
		//ECHO $queryseg;
				$err1 = mysql_query($querynota,$conex);
				$num1 = mysql_num_rows($err1);
				if($num1>0)
				{
					for ($x=0;$x<$num1;$x++)
					{
						$nota = mysql_fetch_array($err1);
						echo "<table align=center border=1 width=725 >";
						echo "<tr><td align=left colspan='4' bgcolor='#cccccc' height='15'><font size='3'  face='arial'><b> ".$nota[3]."</b></font></td></tr>";
						echo "<td  align=left colspan='4'  bgcolor='#AADDFF'><font size='3' face='arial'><b>NOTA ACLARATORIA</b>";
						echo "<fieldset style='background-color: #FFFFFF; '>".$nota[6]."</fieldset></TR></table>";		
					}
				}
		echo "<table align=center border=1 width=725 >";
