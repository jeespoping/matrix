...
else
{
	...
	if(!isset($medico)  or !isset($pac) or !isset($fecha) ) 	{
		...
		echo "<tr><td align=center bgcolor='#cccccc'><font size='2'> <b> ordenesof_new.php Ver.2006-04-01</b></font></tr></td></table><br><br>";
		echo "</center>";
		...
		if(isset($pac1))
		{
			...
			$query="select Paciente  from ".$empresa."_000006 where Oftalmologo='".$medico."'and Paciente like '%".$pac1."%' order by Paciente";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
		...
		if(isset($pac))
		{
			/*Si ya se tiene el paciente buscar las fechas de los seguimiento para este y
			 construir el drop down */	
			$query = "select Fecha  from ".$empresa."_000006 where Paciente='".$pac."' and Oftalmologo='".$medico."' ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	  ...
	  ********************************/
	{
		...
		$query="select Examenes from ".$empresa."_000006 where Paciente='".$pac."' and Oftalmologo='".$medico."' and Fecha='".$fecha."' ";
		$err = mysql_query($query,$conex);
		if($err)
		{
			$row = mysql_fetch_array($err);
			...
			echo "</td><tr><td >&nbsp&nbsp&nbsp&nbsp<b>EXAMENES:</b></td></tr>";
			echo "<tr><td >&nbsp&nbsp<br>".$examenes."</td></tr>";
			echo"</table>";
		}	
		else
