...
else
{
	...
	
	{
		...
		echo "<tr><td align=center colspan=2><font size=5><img src='/matrix/images/medical/calidad/logo_promo.GIF' WIDTH=300 HEIGHT=80></font></td></tr>";
		echo "<tr><td><br></td></tr>";
	...
	else
	{
	     ...
	     $query= " SELECT Concon,Connum,Contip,Conpa1,Conpa2,Connit,Conobj,Convai,Convac,Confin,Conffi,Confpr,Conres,Conpro,Conter
	      		 	  FROM ".$wbasedato."_000002
		         	 WHERE Confpr between '".$fecha1."' and '".$fecha2."'
			      	   AND (Conpro = 'on' OR Confpr > '".$fecha."')
					   ".$todos."			      	 
			      	 ORDER BY 1 ";
	    $err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		...
		//echo $num;
		 ...
		 for ($i=1;$i<=$num;$i++) 
	     {
		    ...
		    if ($row[10]=='off')// para cuando sea booleano poner si o no --> VALOR INDETERMINADO
		    {
		    	$ind='NO';
		    }
		    else
		    ...
		    if ($row[16]=='off')// para cuando sea booleano poner si o no --> PRORROGADO
		    {
		    	$pro='NO';
		    }
		    else
		    ...
		    if ($row[17]=='off')// para cuando sea booleano poner si o no --> TERMINADO
		    {
		    	$ter='NO';
		    }
		    else
		    {
		  	...
		  	echo "<tr bgcolor=".$wcf." border=1><td align=left>".$row[3]."</td><td align=left>".$row[4]."</td><td align=left>".$row[5]."</td><td align=left>".$row[6]."</td><td align=left>".$row[7]."</td>";
		  	echo "<td align=left>".$row[8]."</td><td align=left>".$row[9]."</td><td align=right>".$ind."</td><td align=right>".number_format($row[11],0,'.',',')."</td><td align=right>".$row[12]."</td><td align=right>".$row[13]."</td><td align=left>".$row[14]."</td>";
		  	echo "<td align=left>".$row[15]."</td>";
		  	// para el hipervinculo para editar el registro
		  	$hyper="<A HREF='/matrix/det_registro.php?id=".$row[19]."&pos1=talento&pos2=2007-06-04&pos3=11:18:24&pos4=000002&pos5=0&pos6=talento&tipo=P&Valor=&Form=000002-talento-C-CONVENIOS%20Y%20CONTRATOS&call=0&change=0&key=talento&Pagina=1' target='new' >Editar</a>";
