...
else
{
	...
	else
	{
		...
		if($num > 0)
		{
			...
			// $wnor=str_pad($wnor, 9, "0", STR_PAD_LEFT);
			$wnor = $wnor.$witem;
			$longC=strlen($wnor);
			...
			if($longC < 10)
			{
				...
				$paquete=$paquete."?".chr(13).chr(10);
				$paquete=$paquete.$wnor.chr(13).chr(10);
				...
				else 
				{
					...
					$query = "insert agfa_000010(medico, fecha_data, hora_data, Numero, Tipo, Clase, Hising, Mensaje, Contador, Estado, Estado_servinte, seguridad)";
					$query .= " values ('AGFA','".$fecha."','".$hora."',".$ProcessingID.",'ADT_A01','O','".$ID."','".$texto."',0,'off','off','C-AGFA')";
