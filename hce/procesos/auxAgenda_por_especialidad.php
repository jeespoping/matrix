...
function altaPacienteUrgencias($basedatos,$basedatoshce,$codcco,$paciente,$ingreso,$wemp_pmla,$seguridad,$bandera)
{
	global $conex; 
	global $conexUnix; 
			...
			else
			{
				...
				else
				{
					...
					$q = "	INSERT INTO log_agenda 
									   ...
									   VALUES
											  ('".$fechaLog."', '".$horaLog."', '".$paciente."', '".$ingreso."', 'Actualizacion tabla 18', '".$seguridad."', '".$bandera."')";
					$res1 = mysql_query($q, $conex) or die ("Error: " . mysql_errno() . " - en el query: " . $q . " - " . mysql_error());
					*/
		...
		elseif($tipo_alta == "borrado")
		{
			...
			else
			{
				...
				$q = "	INSERT INTO log_agenda 
										  (Fecha, Hora, Historia, Ingreso, Accion, Seguridad, Bandera) 
								   VALUES
										  ('".$fechaLog."', '".$horaLog."', '".$paciente."', '".$ingreso."', 'Borrado tabla 16', '".$seguridad."', '".$bandera."')";
				...
				$q = "	INSERT INTO log_agenda 
										  (Fecha, Hora, Historia, Ingreso, Accion, Seguridad, Bandera) 
								   VALUES
										  ('".$fechaLog."', '".$horaLog."', '".$paciente."', '".$ingreso."', 'Borrado tabla 18', '".$seguridad."', '".$bandera."')";
...
function reasignarPacienteUrgencias($basedatos,$basedatoshce,$codcco,$paciente,$ingreso,$wemp_pmla,$seguridad)
{
	...
	if($numtra==0) 
	{
		...
		$q = "	INSERT INTO log_agenda 
								  (Fecha, Hora, Historia, Ingreso, Accion, Seguridad, Bandera) 
						   VALUES
								  ('".$fechaLog."', '".$horaLog."', '".$paciente."', '".$ingreso."', 'Borrado tabla 16', '".$seguridad."', 'Reasignacion')";
		$resl1 = mysql_query($q, $conex) or die ("Error: " . mysql_errno() . " - en el query: " . $q . " - " . mysql_error());
		...
		$q = "	INSERT INTO log_agenda 
								  (Fecha, Hora, Historia, Ingreso, Accion, Seguridad, Bandera) 
						   VALUES
								  ('".$fechaLog."', '".$horaLog."', '".$paciente."', '".$ingreso."', 'Borrado tabla 18', '".$seguridad."', 'Reasignacion')";
		$resl2 = mysql_query($q, $conex) or die ("Error: " . mysql_errno() . " - en el query: " . $q . " - " . mysql_error());
...
function actualizarAltaPacientesUnix($basedatos,$basedatoshce,$ccoUrgencias,$wemp_pmla,$seguridad,$listados)
{
	...
	foreach ($altas_unix as $j => $value)
	{
		if(isset($altas_unix[$j]) && $altas_unix[$j]!="")
		{
			...
			altaPacienteUrgencias($basedatos,$basedatoshce,$ccoUrgencias,$historia_paciente,$ingreso_paciente,$wemp_pmla,"Movhos","auto");
			$conting++;
		}
	}	
}
			...
			if($pacienteUnix)
			{
				if($pacienteMatrix->ingresoHistoriaClinica != $pacienteUnix->ingresoHistoriaClinica)
				{
					if(isset($pacienteMatrix->historiaClinica) && !empty($pacienteMatrix->historiaClinica))
					{
						...
						@altaPacienteUrgencias($basedatos,$basedatoshce,$ccoUrgencias,$pacienteMatrix->historiaClinica,$pacienteMatrix->ingresoHistoriaClinica,$wemp_pmla,"Movhos","Ingreso");
...
if(isset($consultaAjax))
{
	switch($consultaAjax)
	{
		...
		break;
		case 11:
			echo altaPacienteUrgencias($basedatos,$basedatoshce,$codcco,$paciente,$ingreso,$wemp_pmla,$seguridad,"Manual");
		break;
		case 12:
