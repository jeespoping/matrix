...
[DOC]
	   ...
	   .2004-04-20
		...
		.2021-12-27 Ruben Dario Holguin Castro 
			Se modifica la visualización de los enlaces de la pantalla de inicio de sesión 
...
if(!isset($accion))
{
	...
	?>
		...
		<style type='text/css'>
			...
			.item-lista-sin-estilo {
				list-style-type: none;
			}			
			.enlaces-inicio {
				font-family: Century Gothic, CenturyGhotic,sans-serif;
				font-size: 14px;
				margin-top: 5px;
				margin-bottom: 5px;	
				color: #FFFFFF;			
	...
	//Se construye el titulo del sistema por medio de html y css para que sea mas facil de editar a futuro
	/**echo "<tr><td align=center><IMG SRC='/matrix/images/medical/root/GELA.png'></td></tr>";*/
	echo "<tr>
			<td align=center>
				<div class='titulo-inicio'>
					<h4><b>Sistema de Informaci&oacute;n AUNA</b></h4>
				</div>
			</td>
		 </tr>";
	...
	if ($num > 0)
	{
		...
		{			
			...
			{
				/**
				 * Si el campo enlace está vacío no se imprime en la etiqueta <a> el HREF para que no 
				 * redirija a ningún sitio.
				 */
				$tieneEnlace = $row[2] == '' ? false:true;	
				$enlace = $tieneEnlace ? "HREF='".$row[2]."'" : '';
				if($tieneEnlace)
				{
					$campo = "	<A class='enlaces-inicio' ".$enlace." target='_blank'>".$row[0]."</A><br>";				
				} 
				else
				{
					$campo = "	<p class='enlaces-inicio' target='_blank'><b>".$row[0]."</b></p><br>";				
				}
				echo "<div class='boton-link'>";
				echo $campo;
				echo "</div>";
				echo "<div class='degradado'></div>";
			} 						
		}
	}
	echo "			</div>";
