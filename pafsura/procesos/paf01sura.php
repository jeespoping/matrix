<html> 
<head>
<title>Actualizacion Autorizaciones Programa PAF SURA</title>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf01sura.submit();   // Ojo para la funcion paf01sura <> paf01sura  (sencible a mayusculas)
	}

	function vaciarCampos()
	{document.forms.paf01sura.wfau.value = 'yyyy-mm-dd';
	 document.forms.paf01sura.wced.value = '';
     document.forms.paf01sura.wap1.value = '';
	 document.forms.paf01sura.wap2.value = '';
     document.forms.paf01sura.wnom.value = '';
     document.forms.paf01sura.wtel.value = '';
     document.forms.paf01sura.wran.value = '';	
     document.forms.paf01sura.wtip.value = '';	
     document.forms.paf01sura.wdia.value = '';	
     document.forms.paf01sura.wexa.value = '';	 
	 document.forms.paf01sura.wcco.value = '';
	 document.forms.paf01sura.west.value = '';
	 document.forms.paf01sura.word.value = '';
	 document.forms.paf01sura.wser.value = '';
	 document.forms.paf01sura.wran.value = '';	
     document.forms.paf01sura.wcr1.value = '';    
     document.forms.paf01sura.wcr2.value = '';     
    }
...
//==========================================================================================================================================
//PROGRAMA				      :Actualizacion Autorizaciones Programa PAF SURA.                                                                   
//AUTOR				          :Jair  Saldarriaga Orozco.                                                                        
//FECHA CREACION			  :Junio 12 de 2012
//FECHA ULTIMA ACTUALIZACION  :Junio 12 de 2012.                                                                             

$wactualiz="PROGRAMA: paf01sura.php Ver. 2012-07-12   JairS";
...
{	    
   ...
   */
   ...
   if (empty($cco))
   {
    ...
    // Chequeo el nro de la Orden por Sura 
 	if (empty($ord))
    {
      $todok = false; 
      $msgerr=$msgerr." El Nro de la Orden por Sura no puede ser nula.";   
    }	

	
    // Chequeo la fecha de emision de la orden  en SURA 
	if (empty($fem) or ($fem == '0000-00-00') )
    {
      $todok = false; 
      $msgerr=$msgerr." La Fecha de emision de la orden por Sura no puede ser nula.";   
    }
    else	
     if ($fem > $fec) 	
     {
      $todok = false;     
      $msgerr=$msgerr." Fecha de emision de Orden en Sura no puede ser mayor a la fecha de Vigencia. ";   
     }	
...
echo "<form name='paf01sura' action='paf01sura.php' method=post>";  
...
echo "<tr><td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FFFFFF><b>Actualizacion Autorizaciones Programa Cardiovascular SURA</b></font>";
...
{
  $query = "SELECT Max(id) FROM pafsura_000001";
  $resultado = mysql_query($query);
  $registro = mysql_fetch_row($resultado);  	
  $nrorden=$registro[0];	
 ...
 if ($wproceso == "Modificar" )  	
  {
   ...
   $query = "SELECT Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla"
           ." FROM pafsura_000001 Where id = '".$wid."'";
  ...
  { 
    $query = "SELECT Docaf,Apeaf1,Apeaf2,Nomaf,Sexo,Fec_nacimiento,Tip_afiliado,Telaf,Codips FROM pafsura_000098 Where  Docaf = '".$wid."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
    if ($nroreg > 0)      //  Encontro 
    {
...
/***********************************************************************************************************************************/	
	...
	echo "<td align=center bgcolor=#C0C0C0 colspan=1><font text color=#003366 size=3>Nro de orden SURA: </font>";
    if (isset($word))
	...
	{	
   	...
   	if ($wproceso == "Modificar" )  	
      echo "<A HREF='paf09sura.php?wid=".$wid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
...
if ( $conf == "on" and isset($wap1) and $wap1<>'' and isset($wexa) and $wexa<>'' and isset($wcco) and $wcco<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   // Busco la fecha del ultimo examen igual al solicitado si es menor a 90 dias mensaje informativo
     $query = "SELECT paffau,Id FROM pafsura_000001 Where pafced = '".$wced."' And pafexa='".$c4[0]."' Order by paffau DESC";
     ...
     $query = "SELECT * FROM pafsura_000001 Where paffau = '".$wfau."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
       ...
       $query = "Update pafsura_000001 SET pafap1='".$wap1."',pafap2='".$wap2."',pafnom='".$wnom."',paftel='".$wtel."',pafran='".$c1[0]."'"
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO pafsura_000001 (Medico,Fecha_data,Hora_data,Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,"
	  ...
	  if ($resultado)
	   {
			...
			$query = "SELECT Id FROM pafsura_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
			// $resultadoB = mysql_query($query,$conex);
			$nroreg = mysql_num_rows($resultadoB);
			if ( $nroreg > 0 )                   //Encontro
			{
