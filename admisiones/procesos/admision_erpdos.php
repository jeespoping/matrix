...
2017-10-23  camilo zapata: A la hora de guardar los responsables se verifica el saldo del soat, en caso de ser necesario, para que cuando el saldo sea de cero
						   el segundo responsable se mantenga como principal en las tablas correspondientes 101, 16, etc pero manteniendo el orden de responsabilidad
...
if (isset($accion) and $accion == 'guardarDatos'){
	...
	if( !empty( $historia ) && !empty( $ingreso ) )
	{
	   ...
	   /**ingreso**/
		...
		$sql = "select Inghis,Ingnin,id, Ingunx, ingvre
				from ".$wbasedato."_000101
				where Inghis = '".utf8_decode($historia)."'
				and Ingnin = '".utf8_decode($ingreso)."'
				";
		...
		if( $res )
		{
			...
			{
				//2017-10-23 seguir esta variable $cambioTemporalSaldoCero para revisar el cambio de revision de saldo cero en soat de ser necesario.
				$cambioTemporalSaldoCero = false;
				$responsables205         = $responsables1;
				/*echo " edb--> se presume ingreso por actualización <br>";
				echo " edb--> <pre>".print_r( $responsables1, true )."</pre>";*/
				$queryV = " SELECT *
							  FROM {$wbasedato}_000204
							 WHERE tophis = '{$historia}'
							   AND toping = '{$ingreso}'
							   AND topres = '{$responsables1[0]['ing_cemhidCodAse']}'";
				//echo " edb--> <pre>".print_r( $query, true )."</pre>";
				$rsV    = mysql_query( $queryV, $conex );
				$rowV   = mysql_fetch_assoc( $rsV );
				if( $rowV['Topres'] == $responsables1[0]['ing_cemhidCodAse'] and $rowV['Topsal']*1 == 0 ){
					//---> esto quiere decir que aunque el principal es soat, ya el saldo llego a 0 y el principal debe pasar a ser
					//     el segundo responsable en todas menos en la 205
					/* intentar manipulacion del arreglo $responsables1*/
		...
		foreach( $responsablesNuevos as $codigoNuevoResponsable=> $valueNresponsable ){
			...
			if( $valueNresponsable != $responsableAnterior and $valueNresponsable != "" and $responsableAnterior != ""){//--> si hubo cambio de responsable se debe realizar el análisis y la correspondiente regrabación.
				...
				if( $cargosAregrabar > 0 ){
					guardarPendientesRegrabacion( $historia, $ingreso, $responsableAnterior, $valueNresponsable, "admision_erpdos.php");
				}
			}else{
		...
		if( $data[ "error" ] == 0 )
		{
			...
			if( $res4 )
			{
				...
				if ($num4>0)
				{
					...
					$res5 = mysql_query( $sql5, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error consultando la tabla unica de pacientes ".mysql_errno()." - Error en el query $sql5 - ".mysql_error() ) );
					...
					if ($res5)
					{
						...
						if ($num5>0)
						{
							...
							$res6 = mysql_query( $sql6, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error actualizando la tabla unica de pacientes ".mysql_errno()." - Error en el query $sql6 - ".mysql_error() ) );
							if (!$res6){
								$data[ 'error' ] = 1; //sale el mensaje de error
							}
						}
						else //se inserta el registro tabla de pacientes
						{
							...
							$res7 = mysql_query( $sql7, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error insertando en la tabla unica de pacientes ".mysql_errno()." - Error en el query $sql7 - ".mysql_error() ) );
							if (!$res7){
								$data[ 'error' ] = 1; //sale el mensaje de error
							}
						}
	...
	if( ( ( $campo_ccocir == "on" or $campo_ccoadm =="on" ) and $campo_ccoayu == "off") or ( $campo_ccoayu == "on" and $habilitarPreanestesiaAD == "on" ) ){//---> validar admision en cirugía o admisiones para manejo de preanestesia 2017-05-16
	...
	if($hay_unix && $tieneConexionUnix == 'on' && $ping_unix == true )
	{
		...
		*/
		...
		if( $modoConsulta == "false" || ( $modoConsulta == "true" && $estadoPaciente == true ) ){
			//$ping_unix = ping_unix();
			while( $reintentar && $intentos*1 <= $intentosIngreso*1 ){
...
if(isset($accion) and $accion == 'anularAdmision')
{
	...
	if( $num == 1 )
	{
		...
		$sqlPac = "DELETE FROM
				...
				";
		//Inactivo al paciente
		$sqlRoot = "DELETE root_000037 b FROM
						root_000037 b
					WHERE
						orihis = '{$historia}'
			...
			if( $resIng )
			{
				if( mysql_affected_rows() > 0 )
				{
					...
					if( !empty( $aplMovhos ) )
					{
						...
						$sqlMov = "DELETE ".$aplMovhos."_000016 b, ".$aplMovhos."_000018 a
									 FROM ".$aplMovhos."_000016 b, ".$aplMovhos."_000018 a
									WHERE ubihis = '".$historia."'
									  AND ubiing = '".$ingreso."'
									  AND inghis = ubihis
									  AND inging = ubiing";
					...
					if( !empty( $aplHce ) )
					{
						...
						//Borro los datos de movimiento hospitalario
						$sqlHce = "DELETE ".$aplHce."_000022 a
									 FROM ".$aplHce."_000022 a
								    WHERE mtrhis = '".$historia."'
									  AND mtring = '".$ingreso."'";
					...
					//Borro el último ingreso
					$sqlEv = "DELETE ".$wbasedato."_000149 a, ".$wbasedato."_000150 b FROM
								".$wbasedato."_000149 a, ".$wbasedato."_000150 b
							WHERE Evnhis = '".$historia."'
								AND Evning = '".$ingreso."'
								AND Devcod 	= Evncod
								AND Evnest  = 'on'
...
if (isset($accion) and $accion == 'adicionar_fila')
{
	...
	$data['html'].= "<th style='background-color:#6694E3; font-size:10pt;' colspan='6'>".htmlentities("Datos Autorización")."</th>";
	$data['html'].= "<tr>";
	...
	$data['html'].= "<td class='fila2'><input type='text' name='ing_fhatxtFecAut' id='ing_fhatxtFecAut".$id_fila."' fecha value='".$fechaAct."' ></td>";
...
if (isset($accion) and $accion == 'verTurnosCancelados'){
	...
	<table id='tablaListaTurnos' style='' align='center' id='tablaPacTurnos'>
		<tr align='center'>
			...
			<td colspan='2' class='encabezadoTabla'>Asignación turno</td>
			<td colspan='4' class='encabezadoTabla'>Log Acción</td>
...
if (isset($accion) and $accion == 'cambiarOpcionesSelect'){
	...
	$wbasedatoCliame 	= consultarAplicacion2($conex,$wemp_pmla,"cliame");
...
function consultarSiPreanestesia(){
	...
	console.log("consultando preanestesia");
	$.post("admision_erpdos.php",
	{
		wbasedato: wbasedato,
		consultaAjax:   '',
		...
		if (data.error == 1){
			...
			$("#div_preanestesia").dialog({
			    ...
			    open: function(event, ui) {
			        ...
			        console.log(" y entonces que pasa ps si cambio el focus ");
			    },
...
function buscarPaises()
{
	...
	$( "#pac_pantxtPaiNac, #pac_pahtxtPaiRes" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarPais", {
		cacheLength:1,
		delay:300,
		max: 100,
		scroll: false,
...
function buscarDepartamentos()
{
	...
	$( "#pac_deptxtDepNac,#pac_dehtxtDepRes,#pac_dretxtDepResp,#AccConductordp,#AccDepPropietario,#Catdep,#Accdep" ).autocomplete( "admision_erpdos.php?consultaAjax=&accion=consultarDepartamento",
	{
		extraParams: {
...
function buscarMunicipios()
{
	...
	$( "#pac_ciutxtMunNac,#pac_muhtxtMunRes,#pac_mretxtMunResp,#AccMunPropietario,#AccConductorMun,#Catmun,#Accmun" ).autocomplete( "admision_erpdos.php?consultaAjax=&accion=consultarMunicipio",
	{
		extraParams: {
			dep: function( campo ){
...
function buscarBarrios()
{
	...
	$( "#pac_bartxtBarRes" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarBarrio",
	{
		extraParams: {
			mun: function( campo ){
...
function buscarOcupaciones()
{
	...
	$( "#pac_ofitxtocu" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarOcupacion",
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
function buscarAseguradoras(tabla_referencia)
{
	...
	$( codEsp ).autocomplete("admision_erpdos.php?consultaAjax=&wemp_pmla="+$("#wemp_pmla").val()+"&accion=consultarAseguradora&wbasedato="+wbasedato+"&origenConsulta=autoCompletar",
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
function buscarCUPS( contenedor, indice )
{
	...
	//Asigno autocompletar para la busqueda de paises
	$( "[name=ing_cactxtcups]:eq("+indice+")", $("#"+contenedor )).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarCUPS",
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
function buscarImpresionDiagnostica()
{
	...
	$( "#ing_digtxtImpDia" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarImpresionDiagnostica",
	{
		extraParams: {
			edad: function( campo ){
...
function buscarTarifaParticular()
{
	...
	//Asigno autocompletar para la busqueda de impresiones diagnosticas
	$( "#ing_tartxt" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=buscarTarifaParticular&wbasedato="+$("#wbasedato").val(),
	{
		cacheLength:0,
		delay:300,
		max: 100,
...
function enviarDatos( automatico = '')
{
	...
	*/
	     // 1.                   2.
	//console.log( modoConsulta +" && "+ informacionIngresos.ultimoIngreso[$( "#ing_histxtNumHis" ).val()] +" == "+ $( "#ing_nintxtNumIng" ).val() +" && "+ informacionIngresos.infoing[ informacionIngresos.posAct ].pac_act +" == "+ 'on' );
...
function buscarIpsRemite()
{
	...
	$( "#pac_iretxtIpsRem" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarIpsRemite&wbasedato="+wbasedato,
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
{
	console.log( "entro por aca: mostrarDatosDemograficos " );
	//No haga el llamado si el documento está vacio
	if( $.trim($( "#pac_doctxtNumDoc" ).val()) == '' )
		return;
				...
				else
				{
					...
					if( data.infoing )
					{
						if( true )
						{
							...
							// ultimaPreadmisionCargada = $("#pac_doctxtNumDoc").val();
...
function buscarAseguradorasVehiculo()
{
	var wbasedato = $("#wbasedato").val();
	//Asigno autocompletar para la busqueda de aseguradoras
	$( "[name=_ux_accasn]" ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarAseguradoraVehiculo&wbasedato="+wbasedato,
	{
		cacheLength:1,
		delay:300,
		max: 100,
		...
		function(event, item ){
			...
			$( "input[type=hidden]", this.parentNode ).val( datos[0].valor.cod );
			buscarPrimerResp(); //se busca para llenar codigo responsable y tarifa
			// llenarPlan(datos[0].valor.cod, "ing_plaselPlan" )
		}
	).on({
		change: function(){
		...
		//Pregunto si la pareja es diferente
			// if( ( this._lastValue && this._lastValue != this.value && this._lastCodigo ) || ( this._lastCodigo != $( "input[type=hidden]", this.parentNode ).val() ) ){
				// alert( "Digite una aseguradora valida" )
				// $( "input[type=hidden]", this.parentNode ).val( '' );
				// this.value = '';
			// }
			// if( this._lastValue ){
...
function buscarCcoTopes(tabla_referencia)
{
	...
	//Asigno autocompletar para la busqueda de paises
	$("[id^=top_ccotxtCcoTop]",$("#div_cont_tabla_topes"+idCodResp) ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarCcoTopes&wbasedato="+wbasedato+"&wemp_pmla="+wemp_pmla,
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
function buscarClasificacionConceptosFac(tabla_referencia)
{
	...
	//Asigno autocompletar para la busqueda de paises
	$( "[id^=top_clatxtClaTop]",$("#div_cont_tabla_topes"+idCodResp) ).autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarClasificacinConceptos&tipo=02&wbasedato="+wbasedato+"&wemp_pmla="+wemp_pmla,
	{
		extraParams: {
			tipo: function( campo ){
...
function buscarMedicos(){
	...
	$("#ing_meitxtMedIng").autocomplete("admision_erpdos.php?consultaAjax=&accion=consultarMedico&wbasedato="+wbasedato+"&filtraEspecialidadClinica="+filtraEspecialidadClinica+"&wemp_pmla="+wemp_pmla,
	{
		cacheLength:1,
		delay:300,
		max: 100,
...
function verTurnosCancelados()
{
	$.post("admision_erpdos.php",
	{
		...
		$("#divTurnosCancelados").html(respuesta).dialog({
			modal	: true,
			width	: 'auto',
			//title	: "<div align='left'>Fecha ingreso: <input type='text' id='fechaTurnosCancel' style='width:110px' />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Turnos cancelados y sin finalizar admisión.</div>",
			title	: "<div align='center'>Turnos cancelados y sin finalizar admisión.</div>",
			show	: { effect: "slide", duration: 600 },
			hide	: { effect: "fold", duration: 600 },
			close: function( event, ui ) {
				listarPacientesConTurno();
...
function consultarHistoriaPendienteDeAlta()
{
	...
	if(historia_alta.replace(/ /gi, "") != '')
	{
        ...
        $.post("admision_erpdos.php", obJson,
            function(data){
                if(data.error == 1)
                {
                    fnModalLoading_Cerrar();
...
function abrirTableroDigitalizacion(historia, ingreso,  empresa,fecha)
{
	var w = $(window).width();
	var h = $(window).height();
	...
	sleep(500).then(() => { 
			var html =  '<iframe id="iframeModalTablero" src="../../ips/procesos/tableroDigitalizacion.php?AbiertoDesdeAdmision=si&DesdeAdmisionHistoria='+historia+'&DesdeAdmisionIngreso='+ingreso+'&DesdeAdmisionResponsable='+empresa+'&DesdeAdmisionFecha='+fecha+'" width="100%" height="100%" frameborder="0" allowtransparency="true"></iframe>';  
	...
	/*$('#iframeModalTablero').css('overflow', 'hidden'); //this line does the actual hiding
	$('#modaliframe').css('overflow', 'hidden'); //this line does the actual hiding
...
function cambioCobertura(e)
{
	...
	// debo limitar el select de tipo de afiliciones segun la tabla 105 en sus campos 
	//alert("uno");
	
	$.post("admision_erpdos.php",
	{
		consultaAjax:   		'',
		accion:         		'cambiarOpcionesSelect',
		wemp_pmla:        		$('#wemp_pmla').val(),
		valor:					$("#"+idselect).val()
		...
		$("#pac_tafselTipAfi option").each(function (){
			...
			{
				if($.inArray( $(this).val() ,contenido) != -1)
				{
					$(this).show();
...
//-- mirar
function crearSelectHTMLAccEspecial( $res, $id, $name, $style = "", $opcionDefecto = "" , $cambio){

       echo "<SELECT id='$id' name='$name' $style  onchange='".$cambio."'>";
       echo "<option value='' >Seleccione...</option>";
	...
	if( $res )
	{
		...
		if( $num > 0 )
		{
			...
			for( $i = 0; $rows = mysql_fetch_array( $res ); $i++ )
			{
				...
				//--------------
				$sqlresponsable = "  select Ingtdo,Ingdoc,a.id ,Ingcem , Empnom
	...
	$data= array('error'=>0,'mensaje'=>'','html'=>'');
			...
			$data['html'].="</tr>";
				...
				$data['html'].="</table>";
					...
					//tabla de datos totales topes
					/*$data['html'].="<table style='width:100%' border='0'>";
					$data['html'].="<tr class='fila2' id='_tr_tabla_topes'>";
					$data['html'].="<td align='right' class='' style='width:52%;'><b>Total&nbsp;&nbsp;&nbsp;&nbsp;</b>";
					$data['html'].="<input type='hidden' name='top_tcoselTipCon' id='top_tcoselTipConTotal' class='' ux='' value='*'>
					<input type='hidden' name='top_clahidClaTop' id='top_clahidClaTopTotal' class='' ux='' value='*'>
					<input type='hidden' name='top_ccohidCcoTop' id='top_ccohidCcoTopTotal' class='' ux='' value='*'>
					<input type='hidden' id='top_reshidTopResTotal' name='top_reshidTopRes' value='' >
					</td>";
					$data['html'].="<td align='center' class='' style='width:12%;'>";
					$data['html'].="<input type='text' name='top_toptxtValTop' id='top_toptxtValTopTotal' class='' ux='' placeholder='Total Tope'></td>";
					$data['html'].="<td align='center' class='' style='width:12%;'>";
					$data['html'].="<input type='text' name='top_rectxtValRec' id='top_rectxtValRecTotal' class='' ux='' value='100' placeholder='(%)Total Reconocido porcentaje 0 a 100'></td>";
					$data['html'].="<td align='center' class=''style='width:20%;'>&nbsp;&nbsp;&nbsp;&nbsp;";
					$data['html'].="<input type='hidden' name='top_diachkValDia' id='top_diachkValDiaTotal' class='' ux='' value='off'></td>";
					$data['html'].="</tr>";
					$data['html'].="</table>";*/
...
echo "<td class='fila1espacio'><input type='text' msgcampo='Detalle direccion de Residencia' name='pac_dedtxtDetDir' id='pac_dedtxtDetDir' class='reset' ux='_ux_pacdir_ux_middir' msgError='Digite detalle de la Direccion' depend='pac_dirtxtDirRes'></td>";
...
crearSelectHTMLAccEspecial($res1,'pac_tusselCobSal','pac_tusselCobSal',$param, '', 'cambioCobertura(this)');
echo "</td>";
...
crearSelectHTMLAccEspecial($res1,'pac_tafselTipAfi','pac_tafselTipAfi',$param, '', 'cambioTipoAfiliacion(this)');
echo "</td>";
...
echo "</table>";
echo "</td>";
echo "<td class='fila2'><input msgcampo='IPS que remite' type='text' name='pac_iretxtIpsRem' id='pac_iretxtIpsRem' class='reset' disabled msgError='Digite la Ips' ux='_ux_murent'>";
echo "<input type='hidden' name='pac_irehidIpsRem' id='pac_irehidIpsRem' ux='_ux_pacrem'>";
...
echo "<td class='fila2'><input msgcampo='Valor Remitido SOAT' type='text' name='ing_vretxtValRem' id='ing_vretxtValRem' class='reset' msgaqua='' numerico></td>";
echo "<td style='width:45;'>&nbsp;</td>";
echo "</tr>";
echo "</table></center>";
...
//primer responsable no accidente o evento catastrofico y tercer responsable en accidente o evento
	...
	echo "<td class='fila2'><input type='text' name='ing_fhatxtFecAut' id='ing_fhatxtFecAut".$id_fila."' fecha value='".$fechaAct."' ></td>";
...
echo "<div id='div_doc_change' class='fila2' style='display:none;'><br>
 ...
 echo " &nbsp;&nbsp; <b>DOCUMENTO:</b> <input id='select_doc_new' value=''  style='border-radius: 4px;border:1px solid #AFAFAF;width:200px;'></div><center><br>
	 ...
	 echo "<div id='div_preanestesia' align='center' style='display:none;'>
				...
				<p> Este paciente tiene una preanestesia asociada al turno de cirugía número: <span  class='subtituloPagina2' id='turno_preanestesia'>  </span> </p>
