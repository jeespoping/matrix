...
if (isset($accion) and $accion == 'validarFacturacionUnix') {
	...
	if ($res1) {
		...
		$data['error']   = 1;
		$data['mensaje'] = "No se ejecuto la consulta " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex) . "";
	}
...
if (isset($accion) and $accion == 'consultarConsecutivo') {
	...
	$sql3 = "select Carcon
			 ...
			 where Carfue='01'";
	$res3 = mysql_query($sql3, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el consecutivo de historia " . mysqli_errno($conex) . " - Error en el query $sql3 - " . mysqli_error($conex)));
	if ($res3) {
...
if (isset($accion) and $accion == 'llenarSelectPlan') {
	...
	$res = mysql_query($sql, $conex) or die(mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));

	$data['html'] .= "<option value=''>Seleccione..</option>";
	for ($i = 0; $rows = mysqli_fetch_array($res); $i++) {
...
if (isset($accion) and $accion == 'traertopespreadmision') {
	...
	while ($rowspro = mysqli_fetch_array($respro)) {
...
if (isset($accion) and $accion == 'guardarDatos') {
	...
	$resPre = mysql_query( $sql, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error consultando el documento ".mysqli_errno($conex)." - Error en el query $sql - ".mysqli_error($conex) ) );

	if( $resPre ){
		$numPre = mysqli_num_rows( $resPre );
		if( $numPre > 0 )
		{
			if( $rowsPre = mysqli_fetch_array( $resPre ) )
			{
	...
	$sqlpro = "SELECT Accres, Accrei
			   ...
			   WHERE Acchis = '{$historia}'
				 AND Accing = '{$ingreso}'";
	$res2 = mysql_query($sqlpro, $conex) or ($data['mensaje'] = utf8_encode("Error consultando datos de accidente " . mysqli_errno($conex) . " - Error en el query $sql2 - " . mysqli_error($conex)));
	...
	$sql = "SELECT ingcai
			 ...
			 WHERE inghis = '{$historia}'
			   AND ingnin = '{$ingreso}'";
	$rs  = mysql_query($sql, $conex) or die(mysqli_error($conex));
	$rows2 = mysqli_fetch_array($rs);
	...
	/*if( !empty( $historia ) && !empty( $ingreso ) && $ing_caiselOriAte != '02' )
	{
		...
		$res2 = mysql_query( $sql2, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error consultando el ingreso " .mysqli_errno($conex)." - Error en el query $sql2 - ".mysqli_error($conex) ) );
		if( $res2 )
		{
			$num2=mysqli_num_rows($res2);
			if ($num2>0)
				...
				if( $rows2[0] == '02' ){ //Si el origen de la atencion es accidente de transito
					...
					$sqlpro = "SELECT COUNT(*) cantidad
								...
								WHERE Acchis = '{$historia}'
								  AND Accing != '{$ingreso}'
								  AND Accrei = '{$ingreso}'";
					$res3 = mysql_query( $sqlpro, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error consultando el ingreso " .mysqli_errno($conex)." - Error en el query $sql2 - ".mysqli_error($conex) ) );
					if( $res3 )
					{
						$num3=mysqli_fetch_array( $res3 );
						$num3 = $num3['cantidad'];
						if ($num3>0)
						{
							$data[ "error" ] = 1;
							$data[ "mensaje" ] = utf8_encode("No se puede cambiar el origen de la atencion, tiene ingresos posteriores asociados a este accidente.");
							echo json_encode($data);
							return;
						}
					}
	...
	if (!empty($documento)) //se consulta si ese documento existe
	{
		...
		if ($historia == "") {
			...
			$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el documento " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
			...
			} else //no tiene registros con ese documento
			{
				...
				if (isset($historiaUx) and $historiaUx != "") { //-->2017-05-23
					...
					$resquery = mysql_query($query, $conex) or ($data['mensaje'] = utf8_encode("Error bloqueando la tabla de consecutivo de historia " . mysqli_errno($conex) . " - Error en el query $query - " . mysqli_error($conex)));
					$sql3 = "select Carcon
							  from " . $wbasedato . "_000040
							 where Carfue='01'";
					$res3 = mysql_query($sql3, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el consecutivo de historia " . mysqli_errno($conex) . " - Error en el query $sql3 - " . mysqli_error($conex)));
					if ($res3) {
						...
						if ($num3 > 0) {
							...
							$sql4 =  " UPDATE " . $wbasedato . "_000040
								...
								where Carfue='01'
								";
							$res4 = mysql_query($sql4, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando el consecutivo de historia " . mysqli_errno($conex) . " - Error en el query $sql4 - " . mysqli_error($conex)));
							//}
	...
	if ($ingreso * 1 > 1) {
		...
		$rsAD    = mysql_query($queryAD, $conex) or die(mysqli_error($conex) . "  -->  " . $queryAD);
		$row     = mysqli_fetch_array($rsAD);
	...
	if (!empty($historia) && !empty($ingreso)) {
		...
		if ($res) {
			...
			if ($ing_tpaselTipRes == 'P') //mirar si le pongo con el nombre del txt si toma la ultima
			{
				...
				$rest = mysql_query($sqlt, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlt - " . mysqli_error($conex)));

				if ($rest) {
					$numt = mysqli_num_rows($rest);
					if ($numt > 0) {
						$rowst = mysqli_fetch_array($rest);
			...
			} else //hace la actualizacion
			{
				...
				$res1 = mysql_query($sqlUpdate, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query de ingreso $sqlUpdate - " . mysqli_error($conex)));
				...
				if ($res1) {
					...
					$data["error"] = 1;
					$data["mensaje"] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUpdate - " . mysqli_error($conex));
				}
			}
		}
		/**fin ingreso**/
		...
		if ($data["error"] == 0) {
			...
			$res1 = mysql_query($sql1, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
			...
			if ($res1) {
				...
				} else //hace la actualizacion
				{
					...
					$rowaud = mysqli_fetch_assoc($rsaud);
					...
					$res2 = mysql_query($sqlUpdate, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUpdate - " . mysqli_error($conex)));
		...
		if ($data["error"] == 0) {
			...
			$sqlpro = "SELECT Resnit as codigo, Resord as orden, Toptco as tipo, Toptop as tope, Topsal as saldo
						...
						WHERE Reshis = '" . $historia . "'
						  ...
						  AND Resdes = 'off'";
			//echo $sqlpro;
			$respro = mysql_query($sqlpro, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000205 " . mysqli_errno($conex) . " - Error en el query $sqlpro - " . mysqli_error($conex)));
			if ($respro) {
				$numpro = mysqli_num_rows($respro);
				if ($numpro > 0) {
					while ($rowspro = mysqli_fetch_assoc($respro)) {
		...
		if ($data["error"] == 0) {
			...
			if (!empty($ing_caiselOriAte) && $ing_caiselOriAte == '06') {
				...
				$resUptEvc = mysql_query($sqlUptEvc, $conex) or die($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUptEvc - " . mysqli_error($conex)));
			}
		}
		...
		if ($data["error"] == 0) {
			...
			if (true) {
				...
				if (!empty($responsablesReal) && count($responsablesReal) > 0) {
					...
					while ($rowra = mysqli_fetch_array($rsra)) {
						if ($rowra['resdes'] == "off") {
					...
					foreach ($responsablesReal as $keRes => $valueRes) {
						...
						if (array_key_exists($valueRes['ing_cemhidCodAse'], $responsablesActualesDescartados)) { //--> si estaba descartado y se vuelve a activar.
							...
							$resDel = mysql_query($sqlDel, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlDel - " . mysqli_error($conex)));
						...
						$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000205 - " . mysqli_error($conex)));
						...
						if (!$resEnc) {
							...
							if (isset($valueRes['cups'])) {
								foreach ($valueRes['cups'] as $valueCup) {
									...
									$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000209 - " . mysqli_error($conex)));
								}
							}
						}
		...
		if ($data["error"] == 0) {
			if (!empty($ing_caiselOriAte) && $ing_caiselOriAte == '02') {
				...
				$resTopeSoat = mysql_query($sqlTopeSoat, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error consultando en la tabla " . $wbasedato . "_000194 topes soat- " . mysqli_error($conex)));

				if (!$resTopeSoat) {
					$data['error'] = 1;
				} else {
					$numTopeSoat = mysqli_num_rows($resTopeSoat);
					if ($numTopeSoat > 0) {
						$rowsTopeSoat = mysqli_fetch_array($resTopeSoat);
				...
				if ($res3) {
					...
					if ($num3 > 0) {
						...
						$res39 = mysql_query($sqlpro, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el ingreso " . mysqli_errno($conex) . " - Error en el query $sql2 - " . mysqli_error($conex)));
						if ($res39) {
							$num39 = mysqli_num_rows($res39);
							if ($num39 > 0) {
								$rowto9 = mysqli_fetch_array($res39);
				...
				/*$resDel = mysql_query( $sqlDel, $conex ) or ( $data[ 'error' ] = utf8_encode( mysqli_errno($conex)." - Error en el query $sqlDel - ".mysqli_error($conex) ) );
				if (!$resDel){
					$data['error'] = 1;
				}*/
				...
				$resEncSOAT = mysql_query($sqlInsertSOAT, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000204 - " . mysqli_error($conex)));
		...
		if (isset($topesPreadmision) && $topesPreadmision) {
			...
			if ($data["error"] == 0) {
				...
				while ($rowemptop = mysqli_fetch_array($rsemptop)) {
					...
					foreach ($topes as $keRes => $valueRes) {
						...
						if (!($valueRes['top_tcoselTipCon'] == '*' && $valueRes['top_clahidClaTop'] == '*' && $valueRes['top_ccohidCcoTop'] == '*'
							...
							if (isset($valueRes['top_id']) && $valueRes['top_id'] != "") {
								...
								$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000204 - " . mysqli_error($conex)));
							} else {
								$sqlInsert = crearStringInsert($wbasedato . "_000204", $datosEnc);
								$resEnc    = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000204 - " . mysqli_error($conex)));
								$idResultante = mysqli_insert_id($conex);
								if (!isset($responsablesTopes[$valueRes['top_reshidTopRes']])) {
									$responsablesTopes[$valueRes['top_reshidTopRes']] = array();
								}
			...
			if ($valueNresponsable != $responsableAnterior and $valueNresponsable != "" and $responsableAnterior != "") { //--> si hubo cambio de responsable se debe realizar el análisis y la correspondiente regrabación.
				...
				$sqlCargos = "	UPDATE {$wbasedato}_000106
								 ...
								 WHERE Tcarhis = '{$historia}'
								   ...
								   AND Tcarfre = 0
								   AND Tcarest = 'on'";
				$resCargos = mysql_query($sqlCargos, $conex) or die("<b>ERROR EN QUERY MATRIX:</b><br>" . mysqli_error($conex));
				$cargosAregrabar = mysqli_affected_rows($conex);
	...
	if ($data["error"] == 0) {
		...
		$res4 = mysql_query($sql4, $conex) or ($data['mensaje'] = utf8_encode("Error consultando las empresas " . mysqli_errno($conex) . " - Error en el query $sql4 - " . mysqli_error($conex)));
		...
		if ($res4) {
			...
			if ($num4 > 0) {
				...
				$res5 = mysql_query($sql5, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla unica de pacientes " . mysqli_errno($conex) . " - Error en el query $sql5 - " . mysqli_error($conex)));
				...
				if ($res5) {
					...
					if ($num5 > 0) {
						...
						$res6 = mysql_query($sql6, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando la tabla unica de pacientes " . mysqli_errno($conex) . " - Error en el query $sql6 - " . mysqli_error($conex)));
						if (!$res6) {
							$data['error'] = 1; //sale el mensaje de error
						}
					} else //se inserta el registro tabla de pacientes
					{
						...
						$res7 = mysql_query($sql7, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla unica de pacientes " . mysqli_errno($conex) . " - Error en el query $sql7 - " . mysqli_error($conex)));
						if (!$res7) {
							$data['error'] = 1; //sale el mensaje de error
						}
					}
				...
				$res8 = mysql_query($sql8, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla origen historia del paciente " . mysqli_errno($conex) . " - Error en el query $sql8 - " . mysqli_error($conex)));
				if ($res8) {
					...
					if ($num8 > 0) {
						...
						$res9 = mysql_query($sql9, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando la tabla de origen historia del paciente " . mysqli_errno($conex) . " - Error en el query $sql9 - " . mysqli_error($conex)));
						if (!$res9) {
							$data['error'] = 1; //sale el mensaje de error
						}
					} else //si es registro nuevo en la tabla origen historia del paciente
					{
						...
						$res10 = mysql_query($sql10, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla de origen historia del paciente " . mysqli_errno($conex) . " - Error en el query $sql10 - " . mysqli_error($conex)));
						if (!$res10) {
							$data['error'] = 1; //sale el mensaje de error
						}
					}
	...
	if ($data["error"] == 0) {
		...
		// $res = mysql_query( $sql, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error insertando en la tabla de log admisiones ".$wbasedato." 164 ".mysqli_errno($conex)." - Error en el query $sql - ".mysqli_error($conex) ) );
		// if (!$res)
		// {
		// $data[ 'error' ] = 1; //sale el mensaje de error
		// }
	...
	if ($data["error"] == 0 and $aplicacion != "") {
		...
		if ($ing_tpaselTipRes == 'P') {
			...
			$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "_000024 para guardar en movhos_000016 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));

			if ($res) {
				$fecha = date("Y-m-d");
				$hora = (string)date("H:i:s");
				$num5a = mysqli_num_rows($res);
				if ($num5a > 0) {
					$rows5 = mysqli_fetch_array($res);
		...
		$res5 = mysql_query($sql5, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla movhos_000016 " . mysqli_errno($conex) . " - Error en el query $sql5 - " . mysqli_error($conex)));
		...
		if ($res5) {
			...
			if ($num5 > 0) {
				...
				$res6 = mysql_query($sql6, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando la tabla movhos_000016 " . mysqli_errno($conex) . " - Error en el query $sql6 - " . mysqli_error($conex)));
				if (!$res6) {
					$data['error'] = 1; //sale el mensaje de error
				}
			} else //se inserta el registro tabla movhos_000016
			{
				...
				$res7 = mysql_query($sql7, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla movhos_000016 " . mysqli_errno($conex) . " - Error en el query $sql7 - " . mysqli_error($conex)));
				if (!$res7) {
					$data['error'] = 1; //sale el mensaje de error
				}
			}
		...
		$res11 = mysql_query($sql11, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $aplicacion . "_000011 " . mysqli_errno($conex) . " - Error en el query $sql11 - " . mysqli_error($conex)));
		if ($res11) {
		...
		$res8 = mysql_query($sql8, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $aplicacion . "_000018 " . mysqli_errno($conex) . " - Error en el query $sql8 - " . mysqli_error($conex)));
		if ($res8) {
			$num8 = mysqli_num_rows($res8);
			...
			} else //si es registro nuevo en la tabla movhos_000018
			{
				...
				$res10 = mysql_query($sql10, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla movhos_000018 " . mysqli_errno($conex) . " - Error en el query $sql10 - " . mysqli_error($conex)));
				if (!$res10) {
					$data['error'] = 1; //sale el mensaje de error
				}
			}
	...
	if ($campo_ccourg == "on") {
		...
		$rowTrUrg         = mysqli_fetch_assoc($rsTrUrg);
		...
		if ($historiaTemporal != "") {
			...
			$regAfectados36 = mysqli_affected_rows($conex);
			...
			$regAfectados152 = mysqli_affected_rows($conex);
	...
	while ($rowPreAn = mysqli_fetch_assoc($rsPreAn)) {
	...
	if ($historiaTemporal != "" and (($habilitarPorCirugia) or ($habilitarPorAyuda))) {
		$queryUp36 =  "UPDATE {$wbasedatoHce}_000036
						...
						WHERE Firhis = '$historiaTemporal'
						  ...
						  AND Firpro = '000075'";
		$rsup36    = mysql_query($queryUp36, $conex);
		$regAfectados3675 = mysqli_affected_rows($conex);
		...
		$regAfectados75 = mysqli_affected_rows($conex);
	...
	if ($data["error"] == 0 and $aplicacionHce != "") {
		...
		$res8 = mysql_query($sql8, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla hce_000022 " . mysqli_errno($conex) . " - Error en el query $sql8 - " . mysqli_error($conex)));
		if ($res8) {
			...
			if ($num8 > 0) {
				...
				$res9 = mysql_query($sql9, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando la tabla hce_000022 " . mysqli_errno($conex) . " - Error en el query $sql9 - " . mysqli_error($conex)));
				if (!$res9) {
					$data['error'] = 1; //sale el mensaje de error
				}
			} else //si es registro nuevo en la tabla movhos_000018
			{
				...
				SELECT Hctcch, Hctcom
				 ...
				 WHERE Hctcon = '" . $campoConducta . "'
				   AND Hctpin = 'on'
				   AND Hctest = 'on'
				";
				$resHomoCon = mysql_query($sqlHomoCon, $conex) or ($data['mensaje'] = utf8_encode("Error en el query sqlHomoCon:$sqlHomoCon - " . mysqli_error($conex)));
				while ($rowHomoCon = mysqli_fetch_array($resHomoCon)) {
				...
				$res10 = mysql_query($sql10, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla hce_000022 " . mysqli_errno($conex) . " - Error en el query $sql10 - " . mysqli_error($conex)));
				if (!$res10) {
					$data['error'] = 1; //sale el mensaje de error
				} elseif (trim($turno) != '') {
					...
					UPDATE " . $wbasedatoMov . "_000178
					 ...
					 WHERE Atutur = '" . $turno . "'
					";
					$resActTurnoAdm = mysql_query($sqlActTurnoAdm, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando el turno - Error en el query sqlActTurnoAdm:$sqlActTurnoAdm - " . mysqli_error($conex)));
					if (!$resActTurnoAdm)
						$data['error'] = 1;
					...
					";
					$resRegMov = mysql_query($sqlRegMov, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando el turno - Error en el query sqlRegMov:$sqlRegMov - " . mysqli_error($conex)));
					if (!$resRegMov)
						$data['error'] = 1;
				}
			}
	...
	if (isset($solucionCitas) && $solucionCitas != "") {
		...
		$rowVal     = mysqli_fetch_assoc($resSqlVal);
	...
	if ($data["error"] == 0) {
		...
		$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error borrando el registro de la tabla " . $wbasedato . "000163 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
		//$data[ 'mensaje' ] = $sql;
	}
	...
	if ($ccoIngresoAyuda && $modoConsulta != "true") { //--> 2016-12-27 inserts de alta automática para ayudas diagnósticas.
		...
		$err32 = mysql_query($q32, $conex) or die(mysqli_errno($conex) . $q32 . " - " . mysqli_error($conex));
		$row32 = mysqli_fetch_array($err32);
		...
		if ($wingser == 1) {
			...
			$err = mysql_query($q32, $conex) or die(mysqli_errno($conex) . $q32 . " - " . mysqli_error($conex));
		}
		...
		$q33 = " SELECT *
				   ...
				   WHERE Historia_clinica = '{$historia}'
					 ...
					 AND Servicio = '{$servicioIng}'";
		$res33 = mysql_query($q33, $conex) or die("Error: " . mysqli_errno($conex) . " - en el query: " . $q33 . " - " . mysqli_error($conex));
	...
	if ($hay_unix && $tieneConexionUnix == 'on' && $ping_unix == true) {
		...
		if ((!empty($responsables1) && count($responsables1) > 0)) {
			...
			if ($res2xy) {
				$num2xy = mysqli_num_rows($res2xy);
				if ($num2xy > 0) {
					$rows2xy = mysqli_fetch_array($res2xy);
					$rows2xy  = mysqli_fetch_array($res2xy);
	...
	if ($data["error"] == 0) {
		...
		if (in_array($tipodoc, $tipoDocumentoPacienteInternacional)) {
			...
			if (($estadoAdmision == "Nueva") || ($estadoAdmision == "Modificada" && $cambiaTipoDocumento)) {
				...
				$resCco = mysql_query($sqlCco, $conex) or ($data['mensaje'] = utf8_encode("Error consultando descripcion de centro de costos " . mysqli_errno($conex) . " - Error en el query $sqlCco - " . mysqli_error($conex)));
				$numCco = mysqli_num_rows($resCco);

				$descCcoIngreso = $servicioIng;
				if ($numCco > 0) {
					$rowCco     = mysqli_fetch_assoc($resCco);
...
if (isset($accion) and $accion == 'mostrarDatosAlmacenados') {
	...
	$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000100 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
	if ($res) {
		$num = mysqli_num_rows($res);
		...
		if ($num > 0) {
			...
			consulta de la 101
			*/
			for ($i = 0, $j = 0; $rows = mysqli_fetch_array($res, MYSQL_ASSOC); $j++) { //solo se puede buscar por el nombre del campo
				...
				if (!empty($rows['Pachis']) && !empty($_POST['ingreso']) && ($rows['Pacact'] == "on")) {
					...
					//$rowAct   = mysqli_fetch_array( $rsAct );

					if ($rowAct   = mysqli_fetch_array($rsAct)) {
						if ($rowAct[0] > 0) {
				...
				$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000101 " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
				if ($res1) {
					$num1 = mysqli_num_rows($res1);
					$data['numRegistrosIng'][$rows['Pachis']] = $num1;
					$data['ultimoIngreso'][$rows['Pachis']] = 0;
					if ($num1 > 0) {
						for ($i; $rows1 = mysqli_fetch_array($res1, MYSQL_ASSOC); $i++)  //solo se puede buscar por el nombre del campo
						{
							...
							if ($aplicacion != '') {
								...
								if ($rowsIngSei = mysqli_fetch_array($resIngSei)) {
										...
										while ($rowspro = mysqli_fetch_assoc($respro)) {
											...
											if ($esSoat == false) {
												...
												if (!empty($rowspro['Resnit'])) {
													...
													$rescup = mysql_query($sqlcupc, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000209 " . mysqli_errno($conex) . " - Error en el query $sqlcupc - " . mysqli_error($conex)));
													if ($rescup) {
														$numpro = mysqli_num_rows($rescup);
														if ($numpro > 0) {
															$data['infoing'][$i]['responsables'][$k]['cups'] = array();
															while ($rowcup = mysqli_fetch_assoc($rescup)) {
																$cupdato = array();
																$cupdato['nombre'] = "";
							...
							if ($data['error'] == 0 && $hayResponsables == true) {
								...
								$respro1 = mysql_query($sqlpro1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000204 " . mysqli_errno($conex) . " - Error en el query $sqlpro1 - " . mysqli_error($conex)));
								if ($respro1) {
									$numpro1 = mysqli_num_rows($respro1);
									if ($numpro1 > 0) {
										for ($k = 0; $rowspro1 = mysqli_fetch_array($respro1); $k++) {
...
if (isset($accion) and $accion == 'consultarResponsable') {
	...
	$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000100 con responsable de usuario " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
	if ($res) {
		$num = mysqli_num_rows($res);
		if ($num == 0) //no encontro como responsable
		{
			...
			$sql1 = "select Pactdo,Pacdoc,Pacap1,Pacap2,Pacno1,Pacno2,Pacdir,Pactel,Pacdeh,Pacmuh,Pacmov,Paccor,Pacded
				...
				";
			$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000100 con datos paciente " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
			if ($res1) {
				$num1 = mysqli_num_rows($res1);
				if ($num1 == 0) {
					$data['mensaje'] = "No se encontraron datos de responsable del usuario como paciente";
				} else {  //resultados como paciente
					...
					$num = mysqli_num_rows($res);
					if ($rowdp = mysqli_fetch_array($res)) {
...
if (isset($accion) and $accion == 'enviarnivelsiguiente') {
	...
	/*{
			$registroAfectado = mysqli_affected_rows($conex);
			$accion 	    = "UPDATE";
...
function insertLog($wemp_pmla, $user_session, $accion, $tabla, $err, $descripcion, $identificacion, $sql_error = "",  $plan = "", $servicio = "", $wsoporte)
{
	...
	$res = mysql_query($insert, $conex) or die("Error: " . mysqli_errno($conex) . " - en el query (Insertar En Log): " . $insert . " - " . mysqli_error($conex));
...
if (isset($accion) and $accion == 'llamarModalNoFlujo') {
	...
	$vecresp = array();
	while ($rowres = mysqli_fetch_array($resp)) {
...
if (isset($accion) and $accion == 'crearListaAutomatica') {
	...
	$res = mysql_query($select, $conex);
	$responsableactual = '';
	if ($rowres = mysqli_fetch_assoc($res)) {
	...
	if ($wflujo != '' and $wplan != '') {
		...
		if ($responsableactual != '' and $creoautomatico == 'si') {
			...
			while ($rowres = mysqli_fetch_assoc($res)) {
				...
				while ($rowresflujo = mysqli_fetch_assoc($resflujo)) {
	...
	if ($hayPlanesEmpresa) {
		...
		/*$hora1 = str_replace( ":", "_", $hora);
			...
			$resdr = mysql_query($qaux,$conex) or die (mysqli_errno($conex).":".mysqli_error($conex));
			$query = "CREATE TEMPORARY TABLE IF NOT EXISTS {$tmp}
					  ...
					  SELECT a.id numLista, delsop soporte, delemp empPlan, delfor formato, delobs observacion, delest estado, delres responsable, delser servicio
					   ...
					   WHERE lenhis = '{$whis}'
						 ...
						 AND deling = lening
						 AND lenest = 'on'";
			$resdr = mysql_query($query,$conex) or die (mysqli_errno($conex).":".mysqli_error($conex));
...
if (isset($accion) and $accion == 'llenarTablaDatosLog') {
	...
	$sql1 = "select *
			...
			where Logusu='" . utf8_decode($key) . "'
				";
	$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000163 para el usuario " . $key . " " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	if ($res1) {
		$num1 = mysqli_num_rows($res1);
		if ($num1 > 0) {
			...
			where Logusu ='" . $key . "'
			";
			$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando la tabla " . $wbasedato . "000163 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
			if (!$res) {
				$data['error'] = 1; //sale el mensaje de error
			}
		} else {
			...
			$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error insertando la tabla " . $wbasedato . "000163 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
			if (!$res) {
				$data['error'] = 1;
			}
		}
...
if (isset($accion) and $accion == 'traerTablaDatosLog') {
	...
	$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000163 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
	if ($res) {
		$num = mysqli_num_rows($res);
		if ($num > 0) {
			$rows = mysqli_fetch_array($res);
		...
		if ($num == 0) {
			...
			if ($res) {
				...
				} else //hace la actualizacion
				{
					...
					$res1 = mysql_query($sqlUpdate, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUpdate - " . mysqli_error($conex)));
			...
			if ($data["error"] == 0) {
				if (!empty($responsables1) && count($responsables1) > 0) {
					...
					foreach ($responsables1 as $keRes => $valueRes) {
						...
						$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000216 - " . mysqli_error($conex)));
						...
						if (!$resEnc) {
							...
							if (isset($valueRes['cups'])) {
								foreach ($valueRes['cups'] as $valueCup) {
									...
									$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000217 - " . mysqli_error($conex)));
								}
							}
						}
						$ordenRes++;
				...
				if (!empty($topes) && count($topes) > 0) {
					...
					foreach ($topes as $keRes => $valueResT) {
						...
						if (!($valueResT['top_tcoselTipCon'] == '*' && $valueResT['top_clahidClaTop'] == '*' && $valueResT['top_ccohidCcoTop'] == '*'
							...
							$resEnc = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error grabando en la tabla " . $wbasedato . "_000215 - " . mysqli_error($conex)));
			...
			if ($data["error"] == 0) {
				...
				if (!empty($ing_caiselOriAte) && $ing_caiselOriAte == '02') {
					...
					$res1 = mysql_query($sqlInsert, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlInsert - " . mysqli_error($conex)));
...
if (isset($accion) and $accion == 'mostrarDatosAlmacenadosPreadmision') {
	...
	$res = mysql_query($sql, $conex) or ($data['error'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000100 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
	if ($res) {
		$num = mysqli_num_rows($res);
		$data['numRegistrosPac'] = $num;
		if ($num > 0) {
			...
			consulta de la 101
			*/
			for ($i = 0, $j = 0; $rows = mysqli_fetch_assoc($res); $j++) { //solo se puede buscar por el nombre del campo
				//posicion de historia
				...
				if ($res1) {
					...
					if ($num1 > 0) {
						for ($i; $rows1 = mysqli_fetch_assoc($res1); $i++)  //solo se puede buscar por el nombre del campo
						{
							...
							if ($rows1['Ingcai'] == '02') {
								...
								$res = mysql_query($sql, $conex) or ($data['error'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000227 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));

								if ($res) {
									$num = mysqli_num_rows($res);

									if ($num > 0) {
										if ($rowAcc = mysqli_fetch_array($res, MYSQL_ASSOC)) {
											...
											$num = mysqli_num_rows($res);
											if ($rowAs = mysqli_fetch_array($res)) {
												$ase = $rowAs['Asedes'];
											} else {
												$ase = '';
											}
							...
							if ($data['error'] == 0) {
								...
								$respro = mysql_query($sqlpro, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000216 " . mysqli_errno($conex) . " - Error en el query $sqlpro - " . mysqli_error($conex)));
								if ($respro) {
									$numpro = mysqli_num_rows($respro);
									if ($numpro > 0) {
										$hayResponsables = true;
...
if (isset($accion) and $accion == 'cancelarPreadmision') {
	...
	$res = mysql_query($sql, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));
...
if (isset($accion) and $accion == 'anularAdmision') {
	...
	if (!empty($aplMovhos)) {
		$query = " SELECT ingsei
					...
					WHERE inghis = '{$historia}'
					  AND ingnin = '{$ingreso}'";
		$resMov = mysql_query($query, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlMov - " . mysqli_error($conex));
		$row = mysqli_fetch_array($resMov);
		...
		if ($ccoAyuda) {
			$query = "DELETE
					   ...
					   WHERE Historia_clinica = '{$historia}'
					     AND Num_ingreso = '{$ingreso}'
					     AND servicio = '{$row[0]}'";
			$res = mysql_query($query, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlMov - " . mysqli_error($conex));
		}
		...
		$resMov = mysql_query($sqlMov, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlMov - " . mysqli_error($conex));

		if ($resMov) {
			$num = mysqli_num_rows($resMov);
			if ($num > 0) {
		...
		$resMov2 = mysql_query($sqlMov2, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlMov2 - " . mysqli_error($conex));

		if ($resMov2) {
			$num2 = mysqli_num_rows($resMov2);
			if ($num2 > 0) {
	...
	$resInfPac = mysql_query($sql, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));
	...
	$rowhab = mysqli_fetch_array($rshab);
	...
	$resPac = mysql_query($sqlPac, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlPac - " . mysqli_error($conex));
	...
	if ($resPac) {
		if (mysqli_affected_rows($conex) > 0) {
			...
			$resIng = mysql_query($sql, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));
			...
			if ($resIng) {
				if (mysqli_affected_rows($conex) > 0) {
					...
					$resPread = mysql_query($sqlPre, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlPre - " . mysqli_error($conex));
					...
					$resRoot = mysql_query($sqlRoot, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlRoot - " . mysqli_error($conex));
					...
					if (!empty($aplMovhos)) {
						...
						$rshab = mysql_query($sqlhab, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlhab - " . mysqli_error($conex));
					...
					if (!empty($aplHce)) {
						...
						$rowtur = mysqli_fetch_array($rstur);
						$turno  = $rowtur[0];
						if ($turno != "") {
						...
						$resHce = mysql_query($sqlHce, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlHce - " . mysqli_error($conex));
						...
						if ($resHce) {
							if (mysqli_affected_rows($conex) > 0) {
								...
								$resl1 = mysql_query($q, $conex) or die("Error: " . mysqli_errno($conex) . " - en el query: " . $q . " - " . mysqli_error($conex));
							}
						} else {
							$data1['error'] = 1;
						}
					...
					$resIng = mysql_query($sqlAcc, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlAcc - " . mysqli_error($conex));
						...
						if ($numpro > 0) {
							while ($rowspro = mysqli_fetch_assoc($respro)) {
								...
								$sqlRes = "DELETE FROM
											...
											WHERE Cprhis = '" . $historia . "'
											  AND Cpring = '" . $ingreso . "'";
								$rescup = mysql_query($sqlRes, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlRes - " . mysqli_error($conex));
							}
					...
					$resIng = mysql_query($sqlRes, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlRes - " . mysqli_error($conex));
					...
					$resEv = mysql_query($sqlEv, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlEv - " . mysqli_error($conex));
		...
		*/
		if ($num == 0) {
			...
			$sql5 = " SELECT Oriori
					  ...
					  AND Oriced='" . utf8_decode($datosEnc['pacdoc']) . "' Group by Oriori";
			...
			if ($res5) {
				...
				if ($num5 > 0) {
					...
					$wbasedato = consultarAliasPorAplicacion(
						$conex,
					...
					$sql = "select Pactdo,Pacdoc,Pactat,Pacap1,Pacap2,Pacno1,Pacno2,Pacfna,Pacsex,Pacest,Pacdir,Pactel,
					  Paciu,Pacbar,Pacdep,Paczon,Pactus,Pacofi,Paccea,Pacnoa,Pactea,Pacdia,Pacpaa,Pacact,Paccru,Pacnru,
					  Pactru,Pacdru,Pacpru,Paccor,Pactam,Pacpan,Pacpet,Pacded,Pactrh,Pacpah,Pacdeh,Pacmuh,Pacmov,Pacned,
					  Pacemp,Pactem,Paceem,Pactaf,Pacrem,Pacire,Paccac,Pactda,Pacddr,Pacdre,Pacmre,Pacmor,Paccre,a.Fecha_data, Pacdle, Pactle, Pacaud, Paczog from " . $wbasedato . "_000100 a
					  where Pachis !='0'
					  ";
					$sql .= $where;
					$sql .= " Group by  Pachis  ";
...
// if(isset($accion) and $accion == 'buscarBaseDatos'){

// 		$sql5 = " SELECT Orihis,Oriori,Oriing
// 				  FROM root_000037
// 				  WHERE Oritid='".utf8_decode($datosEnc['pactdo'])."'
...
// 				  $sql = "select * from ".$wbasedato."_000100 a
// 				  where Pachis !='0'
// 				  ";
// 				  $sql .= $where;
// 				  $sql .=" Group by  Pachis  ";
...
if (isset($accion) and $accion == 'mostrarDetalleEvento') {
	...
	$res = mysql_query($sql, $conex) or ($data['error'] = utf8_encode("Error consultando la tabla " . $wbasedato . "000149 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
	...
	if ($res) {
		...
		if ($num > 0) {
			if ($rows = mysqli_fetch_array($res, MYSQL_ASSOC)) {
				foreach ($rows as $key => $value) {
...
if (isset($accion) and $accion == 'adicionar_fila') {
	...
	$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '09' and Selest='on'", '', 'Selcod', '2');
	...
	$data['html'] .= "<option value=''>Seleccione...</option>";
	$num = mysqli_num_rows($res1res);
	if ($num > 0) {
		while ($rows = mysqli_fetch_array($res1res, MYSQL_ASSOC)) {
			$value        = $rows['Codigo'];
...
if (isset($accion) and $accion == 'consultarSiPreanestesia') {
	...
	if ($row['Ccoayu'] == "on") {
		...
		$qprean = " SELECT ahthte historiaTemporal, ahttur turnoCirugia, id
					 ...
					 LIMIT 1";
		$rsPreAn          = mysql_query($qprean, $conex);
		$rowPreAn         = mysqli_fetch_assoc($rsPreAn);
		$historiaTemporal = $rowPreAn['historiaTemporal'];
		$turnoCir204      = $rowPreAn['turnoCirugia'];
		$turnoCirugia     = explode("_", $turnoCir204);
		$turnoCirugia     = $turnoCirugia[1];
...
if (isset($accion) and $accion == 'llamarPacienteAtencion') {
	...
	";
	$resValLla = mysql_query($sqlValLla, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlValLla):</b><br>" . mysqli_error($conex));
	if ($rowValLla = mysqli_fetch_array($resValLla)) {
		...
		";
		$resNomUsu = mysql_query($sqlNomUsu, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlNomUsu):</b><br>" . mysqli_error($conex));
		if ($rowNomUsu = mysqli_fetch_array($resNomUsu))
		...
		UPDATE " . $wbasedatoMov . "_000178
		 ...
		 WHERE Atutur = '" . $turno . "'
		";
		mysql_query($sqlLlamar, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlLlamar):</b><br>" . mysqli_error($conex));
		...
		";
		mysql_query($sqlRegLLamado, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlRegLLamado):</b><br>" . mysqli_error($conex));
	}
...
if (isset($accion) and $accion == 'cancelarLlamarPacienteAtencion') {
	...
	UPDATE " . $wbasedatoMov . "_000178
	 ...
	 WHERE Atutur = '" . $turno . "'
	";
	mysql_query($sqlCancelar, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlCancelar):</b><br>" . mysqli_error($conex));
...
if (isset($accion) and $accion == 'cancelarTurno') {
	...
	if ($rowValLla = mysqli_fetch_array($resValLla)) {
		...
		UPDATE " . $wbasedatoMov . "_000178
		 ...
		 WHERE Atutur = '" . $turno . "'
		";
		mysql_query($sqlTur, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlTur):</b><br>" . mysqli_error($conex));
		...
		";
		mysql_query($sqlCancelarTurno, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlCancelarTurno):</b><br>" . mysqli_error($conex));
	}
...
if (isset($accion) and $accion == 'verTurnosCancelados') {
	...
	";
	$resTurCancelados = mysql_query($sqlTurCancelados, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlTurCancelados):</b><br>" . mysqli_error($conex));

	if (mysqli_num_rows($resTurCancelados) == 0)
		echo "<tr class='fila1'><td colspan='10' align='center'>No se encontraron registros.</td></tr>";

	$coloFila = 'fila1';
	while ($rowTurCancelados = mysqli_fetch_array($resTurCancelados)) {
...
if (isset($accion) and $accion == 'habilitarTurno') {
	...
	UPDATE " . $wbasedatoMov . "_000178
	 ...
	 WHERE Atutur = '" . $turno . "'
	";
	$sqlHabTurno = mysql_query($sqlHabTurno, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlHabTurno):</b><br>" . mysqli_error($conex));
	...
	if ($sqlHabTurno) {
		...
		";
		mysql_query($sqlLogHabTurno, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlLogHabTurno):</b><br>" . mysqli_error($conex));
	}
...
if (isset($accion) and $accion == 'apagarAlertaDeLlamado') {
	...
	UPDATE " . $wbasedatoMov . "_000178
	 ...
	 WHERE Atutur = '" . $turno . "'
	";
	mysql_query($sqlApagAlerta, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlApagAlerta):</b><br>" . mysqli_error($conex));
...
if (isset($accion) and $accion == 'cambiarOpcionesSelect') {
	...
	$res = mysql_query($query, $conex) or die("<b>ERROR EN QUERY MATRIX(query):</b><br>" . mysqli_error($conex));
	$htmlresp = '';
	while ($row = mysqli_fetch_array($res)) {
		$htmlresp .= ',' . $row['Selcod'];
	}
...
if (isset($accion) and $accion == 'cambiarPuestoTrabajo') {
	...
	SELECT Descripcion
	 ...
	 WHERE Puecod = '" . $puestoTrabajo . "'
	   ...
	   AND Pueusu = Codigo
	";
	$resValPuesTra = mysql_query($sqlValPuesTra, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlValPuesTra):</b><br>" . mysqli_error($conex));
	if ($respetarOcupacion == 'true' && $rowValPuesTra = mysqli_fetch_array($resValPuesTra)) {
		$respuesta["Error"] 	= TRUE;
		...
		";
		mysql_query($sqlUpdatePues, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlUpdatePues):</b><br>" . mysqli_error($conex));
...
if (isset($accion) and $accion == 'solicitarCambioDocumento') {
	...
	$query = " INSERT INTO {$wbasedato}_000288 ( Medico,Fecha_data,Hora_data,Scdhis,Scding,Scdtda,Scddoa,Scdtdn,Scddon,Scdjus,Scdest,Seguridad)
				    VALUES ( '{$wbasedato}',
				    	     ...
				    	     'C-{$user2}')";
	$rs = mysql_query($query, $conex);
	$num = mysqli_affected_rows($conex);
	if ($num >= '0') {
...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#div_datosAdmiPreadmi").css({
				display: "none"
			});
			$("#div_ext_agendaPreadmision").css({
				display: ""
			});
	  ...
	  */
		...
		function mostarOcultarDatosPreadmisiones(ocultar) {
			if (ocultar) {
				...
				$("#div_int_datos_personales,[id!=div_datosIng_Per_Aco][acordeon]", $("#div_admisiones")).css({
					display: "none"
				});
				$("#div_ext_agendaPreadmision").css({
					display: ""
				...
				$("#datos_ingreso center").last().css({
					display: "none"
				});
				$("div[name='div_botones']").css({
					display: "none"
				});
				/************************************************************************************************/
			} else {
		...
		function iniciar() {
			...
			$("#bot_navegacion1,#bot_navegacion").css({
				display: "none"
			});
		...
		// function buscarDatosPacientes(objJson = null){

		// 	objJson.wemp_pmla_anterior = objJson.wemp_pmla;
		...
		function guardarRelacionHistoriaEvento() {
			...
			$("#div_accidente_evento").css({
				display: ""
			}); //muestro el div que tiene los botones para abrir los formularios
			$("td", $("#div_accidente_evento")).eq(0).css({
				display: "none"
			}); //oculto el boton de accidentes de transito
			$("td", $("#div_accidente_evento")).eq(1).css({
		...
		*/
		...
		function verTurnosCancelados() {
			$.post("admision_erp.php", {
				...
				$("#fechaTurnosCancel").next().css({
					"cursor": "pointer"
				}).attr("title", "Seleccione");
				...
				if (r) {
					...
					$.post("../../hce/procesos/agenda_urgencias_por_especialidad.php", obJson,
						function(data) {
							...
							$("#popup_container").find(":input[type=button]").css({
								"width": "100px"
							});
							return data;
						}).done(function(data) {
...
==================================================================================================================================================*/
	...
	function consultarPermisosUsuario($wcodigoUsuario)
	{
		...
		$rsper = mysql_query($query, $conex) or die(mysqli_error($conex) . " - " . $query);
		$row   = mysqli_fetch_assoc($rsper);
		return ($row);
	}
	...
	function consultaMaestros($tabla, $campos, $where, $group, $order, $cant = 1)
	{
		...
		$res1 = mysql_query($q, $conex) or die(" Error: " . mysqli_errno($conex) . " - en el query: " . $q . " - " . mysqli_error($conex));
		// $num1 = mysqli_num_rows($res1);
	...
	function consultarBarrios($bar, $mun)
	{
		...
		if ($res) {
			$rowGeneral = mysqli_fetch_assoc($res);
	...
	function consultarTarifasParticular($wtar)
	{
		...
		$query = "SELECT Tarcod Codigo, Tardes Descripcion
			   ...
			   WHERE (Tarcod LIKE '%" . utf8_decode($wtar) . "%' or Tardes LIKE '%" . utf8_decode($wtar) . "%')
			     AND Tarest='on'
			   ORDER BY Tardes";
		$res = mysql_query($query, $conex) or die(mysqli_errno($conex) . "error en linea 2663" . mysqli_error($conex) . " --------> " . $query);
		$num = mysqli_num_rows($res);
		if ($num > 0) {
			for ($i = 0; $rows = mysqli_fetch_array($res); $i++) {
	...
	function consultaNombrePais($codPais)
	{
		...
		and Paiest='on'";
		$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el pais de nacimiento " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	...
	function consultaNombreDepartamento($codDep)
	{
		...
		$sql1 = "select *
				...
				where Codigo = '" . $codDep . "'
				";
		$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando el departamento de nacimiento " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	...
	function consultaNombreTarifa($codTarifa)
	{
		...
		$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando NOMBRE DE TARIFA " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	...
	function guardarAccidentes($his, $ing)
	{
		...
		if (!empty($his) && !empty($ing)) {
			...
			$res = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query es este el que genera error $sql  - " . mysqli_error($conex)));
				...
				if ($num == 0) {
					...
					if (isset($accidente_previo) && $accidente_previo != "") { //2016-27-12
						...
						if ($numacc > 0) { //hay un reingreso mayor al que se está registrando
							...
							$rsaccp2 = mysql_query($qaccp2, $conex) or die(mysqli_error($conex));
						} else {
					...
					$res1 = mysql_query($sqlInsert, $conex) or die($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlInsert - " . mysqli_error($conex)));
					...
					while ($accPrevioReal != "") {
						...
						$datosTabla[ 'Accing' ] = $accPrevioReal;*/
							...
							$res1 = mysql_query($sqlUpdate, $conex) or ($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));
							$accPrevioReal          = $rowacc['accrei'];
						} else {
							$accPrevioReal = "";
						}
	...
	function guardarEventos($his, $ing)
	{
		...
		if (!empty($his) && !empty($ing)) {
			...
			$res = mysql_query($sql, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
			...
			if ($res) {
				...
				if ($num == 0) {
					...
					if (empty($cod)) {
						...
						if ($resCon) {
							$numCon = mysqli_num_rows($resCon);
							if ($numCon > 0) {
								$rowsCon = mysqli_fetch_array($resCon);
					...
					$resEnc = mysql_query($sqlInsert, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlInsert - " . mysqli_error($conex)));

					if (mysqli_affected_rows($conex) > 0) {	//si inserto el encabezado
						...
						$res1 = mysql_query($sqlInsert, $conex) or ($data['error'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlInsert - " . mysqli_error($conex)));
						...
						if ($res1) {
							if (mysqli_affected_rows($conex) > 0) {
								...
								// $resUptEnc = mysql_query( $sqlUptEnc, $conex ) or ( $data[ 'error' ] = utf8_encode( mysqli_errno($conex)." - Error en el query $sqlUptEnc - ".mysqli_error($conex) ) );
							}
						}
					} else {
						$data["error"] = 1;
					...
					$resDet = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
					...
					if ($resDet) {
						...
						$res1 = mysql_query($sqlUpdate, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUpdate - " . mysqli_error($conex)));
						...
						if ($res1) {
							if (mysqli_affected_rows($conex) > 0) {
								...
								$resUptEnc = mysql_query($sqlUptEnc, $conex) or ($data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sqlUptEnc - " . mysqli_error($conex)));
								if (!$resUptEnc) {
									$data['error'] = 1;
								}
							}
	...
	function consultarAplicacion2($conexion, $codigoInstitucion, $nombreAplicacion)
	{
		...
		//	echo $q;
		$res = mysql_query($q, $conexion) or die("Error: " . mysqli_errno($conex) . " - en el query: " . $q . " - " . mysqli_error($conex));
		$num = mysqli_num_rows($res);
	...
	function consultarCC($alias, $where, $usuario = "")
	{
		...
		if ($usuario != "") {
			$query = " SELECT Percca, Perccd
					...
					WHERE Perfue = '01'
					  AND Perusu ='{$usuario}'";
			$rsAux = mysql_query($query, $conex);
			$numRs = mysqli_num_rows($rsAux);
			while ($rowRs = mysqli_fetch_assoc($rsAux)) {
		...
		$res = mysql_query($q, $conex) or die("Error: " . mysqli_errno($conex) . " - en el query: " . $q . " - " . mysqli_error($conex));
	...
	function agendaAdmisiones($fecha, $incremento = 0)
	{
		...
		$res = mysql_query($sql, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql -" . mysqli_error($conex));
		...
		if ($res) {
			...
			if ($num > 0) {
				...
				$sqlempresascondigitalizacion = "SELECT Empcod ,Empccd
											   FROM " . $wbasedato . "_000024
											  WHERE Empdso ='on'";
				$resempresascondigitalizacion = mysql_query($sqlempresascondigitalizacion, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlCar -" . mysqli_error($conex));
				$array_datosempresadigitalizacion = array();
				while ($rowsempresascondigitalizacion = mysqli_fetch_array($resempresascondigitalizacion)) {
				...
				for ($i = 0; $rows = mysqli_fetch_array($res); $i++) {
					...
					$sqlresponsable = "  select Ingtdo,Ingdoc,a.id ,Ingcem , Empnom , Ingsei
										...
										and Pacidi = a.id
										and Ingcem = Empcod ";
					$resresponsable = mysql_query($sqlresponsable, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sql -" . mysqli_error($conex));
					$nombreresponsable = "";
					if ($rowresponsable  = mysqli_fetch_array($resresponsable)) {
	...
	function agendaAdmitidos($fecha, $incremento = 0)
	{
		...
		$sqlCcoUrg = "SELECT Ccocod
				   ...
				   WHERE Ccocod = '" . $cco_usuario . "'
				     AND Ccourg = 'on'
	";
		$resCcoUrg = mysql_query($sqlCcoUrg, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlCcoUrg):</b><br>" . mysqli_error($conex));
		...
		while ($rowRs = mysqli_fetch_assoc($rsAux)) {
			...
			if ($ccoPermitidos != "") {
				...
				while ($rowcco = mysqli_fetch_assoc($rscco)) {
		...
		if ($res) {
			...
			if ($num > 0) {
				...
				for ($i = 0; $rows = mysqli_fetch_array($res); $i++) {
					...
					$resCar = mysql_query($sqlCar, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlCar -" . mysqli_error($conex));

					$disabledAnulacion = '';
					$numCar = mysqli_num_rows($resCar);
					if ($numCar > 0) {
				...
				";
					$resObtTurno = mysql_query($sqlObtTurno, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlObtTurno):</b><br>" . mysqli_error($conex));
					if (@$rowObtTurno = mysqli_fetch_array($resObtTurno))
					...
					$resnombreempresa = mysql_query($sqlnombre, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlCar -" . mysqli_error($conex));
					$nombreempresa = '';
					if ($rownombreempresa = mysqli_fetch_array($resnombreempresa)) {
	...
	function consultaNombreAseguradoraVehiculo($codAse)
	{
		...
		$sql1 = "select Asecod, Asedes
				...
				where Asecod = '" . $codAse . "'
				";
		$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando las aseguradoras de vehiculos " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	...
	function guardarRelacion($historia, $ingreso, $hidcodEvento)
	{
		...
		if (!empty($historia) && !empty($ingreso) && !empty($hidcodEvento)) {
			...
			if ($res) {
				...
				$data['error'] = 1;
				$data['mensaje'] = utf8_encode(mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex));
			}
		}
		return $data;
	}
	...
	function listaEventos()
	{
		...
		if ($resEv) {
			...
			$data['html'] .= "</tr>";
			for ($i = 0; $rows = mysqli_fetch_array($resEv, MYSQL_ASSOC); $i++) //MYSQL_ASSOC se coloca para que devuelva solo los nombres de los campos
			{
			...
			$data['mensaje'] = "No se ejecuto la consulta a la tabla " . $wbasedato . "-000149 $sqlEv" . mysqli_error($conex) . "";
		}
		//echo json_encode($data);
	...
	function mostrarFormTopes($responsable, $historia, $ingreso, $documento, $tipodocumento, $esadmision)
	{
		...
		if ($esadmision == 'si') {
			...
			$restopes = mysql_query($selecttopes, $conex)  or die(mysqli_errno($conex) . " - Error en el query $select - " . mysqli_error($conex));
			$vectortopesgrabados = array();
			$vectortopesgrabados = array();
			while ($rowstopes = mysqli_fetch_array($restopes)) {
		...
		$select  = "SELECT Ccfcod ,Cpgcod, Cpgnom
				 ...
				 WHERE Ccfcod = Cpgccf
				   AND Cpgest = 'on'
				   AND Cpgtda != 'off'";
		$res = mysql_query($select, $conex)  or die(mysqli_errno($conex) . " - Error en el query $select - " . mysqli_error($conex));
		...
		$selectcco = "SELECT Ccocod ,	Cconom
				...
				WHERE Ccoest ='on'
				ORDER BY Cconom";
		$res = mysql_query($selectcco, $conex)  or die(mysqli_errno($conex) . " - Error en el query $selectcco - " . mysqli_error($conex));
		$arrayccos = array();
		...
		if (isset($vectortopesgrabados['*']['*']['cco'])) {
			...
			$rescco = mysql_query($selectcco, $conex)  or die(mysqli_errno($conex) . " - Error en el query $selectcco - " . mysqli_error($conex));
			$selectcco4 = "<option value='*'>Todos</option>";
			while ($rowscco = mysqli_fetch_array($rescco)) {
	...
	*/
	...
	function registrarLogTopes($whistoria, $wingreso, $responsable, $cadenaTope, $idTope, $accionLog, $usuario)
	{
		...
		if ($accionLog == "activo") {
			...
			$resInsertLog = mysql_query($insertLog, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla de log de topes " . $wbasedato . "_000315 " . mysqli_errno($conex) . " - Error en el query " . $insertLog . " - " . mysqli_error($conex)));
			...
			$resUpdateDetalle = mysql_query($updateLog, $conex) or ($data['mensaje'] = utf8_encode("Error actualizando en la tabla de log de topes " . $wbasedato . "_000315 " . mysqli_errno($conex) . " - Error en el query " . $updateLog . " - " . mysqli_error($conex)));
			$numInactivarTope = mysqli_affected_rows($conex);
	...
	function grabartoperesponsable($whistoria, $wingreso, $responsable, $insertar, $activo, $esadmision, $documento, $tipodocumento)
	{
		...
		if ($esadmision == 'si') {
			...
			$resTopesActivos = mysql_query($queryTopesActivos, $conex) or die("Error: " . mysqli_errno($conex) . " - en el query: " . $queryTopesActivos . " - " . mysqli_error($conex));
			$numTopesActivos = mysqli_num_rows($resTopesActivos);


			if ($numTopesActivos > 0) {
				while ($rowTopesActivos = mysqli_fetch_array($resTopesActivos)) {
		...
		for ($i = 1; $i < count($vectortexto); $i++) {
			...
			if ($esadmision == 'si') {
				...
				$res = mysql_query($resultado, $conex) or ($data['mensaje'] = utf8_encode("Error insertando en la tabla de topes " . $wbasedato . " 204 " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));

				$numInsertTope = mysqli_affected_rows($conex);
	...
	/*function mostrarFormTopes()
{
	...
	function consultaNombreConcepto($codCon)
	{
		...
		$sql1 = "select Cpgcod,Cpgnom
				...
				where Cpgcod = '" . $codCon . "'
				and Cpgest = 'on'";
		$res1 = mysql_query($sql1, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla de clasificacion de procedimientos " . mysqli_errno($conex) . " - Error en el query $sql1 - " . mysqli_error($conex)));
	...
	function consultaNombreCco($codCco)
	{
		...
		$res1 = mysql_query($sql, $conex) or ($data['mensaje'] = utf8_encode("Error consultando la tabla de centros de costo " . mysqli_errno($conex) . " - Error en el query $sql - " . mysqli_error($conex)));
		...
		if (in_array($user2, $exentos_cco) == false) {
			...
			$sqlusu = "SELECT perccd as cco,Ccoing as ing,Ccoayu as ayu,Ccourg as urg,Ccocir as cir
					...
					WHERE Perusu = '" . $user2 . "'
					  ...
					  AND ccoest='on'";
			$resusu = mysql_query($sqlusu, $conex) or die($data['mensaje'] = mysqli_errno($conex) . " - Error en el query $sqlusu -" . mysqli_error($conex));

			$disabledAnulacion = '';
			$numusu = mysqli_num_rows($resusu);
			if ($numusu > 0) {
				$rowusu = mysqli_fetch_assoc($resusu);
	...
	";
		$resForTri = mysql_query($sqlForTri, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlForTri):</b><br>" . mysqli_error($conex));
		if ($rowForTri = mysqli_fetch_array($resForTri)) {
		...
		SELECT movcon, movdat
		 ...
		 WHERE Fecha_data = '" . $rowForTri['Fecha_data'] . "'
		   ...
		   AND moving 		= '" . $ingreso . "'
		";
			$resDatosHce = mysql_query($sqlDatosHce, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlDatosHce):" . $sqlDatosHce . "</b><br>" . mysqli_error($conex));
			while ($rowDatosHce = mysqli_fetch_array($resDatosHce))
	...
	SELECT Puecod, Puenom, Pueusu
	 ...
	 WHERE Puetve = 'on'
	   AND Pueest = 'on'
	";
		$resVentanillas = mysql_query($sqlVentanillas, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlVentanillas):</b><br>" . mysqli_error($conex));
		while ($rowVentanillas = mysqli_fetch_array($resVentanillas)) {
	...
	<div style='height:280px;overflow:auto;background:none repeat scroll 0 0;'>
		...
		";
		$resTurnos 	= mysql_query($sqlTurnos, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlTurnos):</b><br>" . mysqli_error($conex));
		$coloFila	= 'fila2';
		$turnoConLlamadoEnVentanilla = '';
		while ($rowTurnos = mysqli_fetch_array($resTurnos)) {
			$coloFila 			= (($coloFila == 'fila2') ? 'fila1' : 'fila2');
			...
			if (trim($datosHce) != '' || trim($datosHce2) != '') {
				...
				if ($arrHomoConductas[$campoConducta][$datosHce]['permiteIngreso'] != 'on' || $arrHomoConductas[$campoPlanManejo][$datosHce2]['permiteIngreso'] != 'on') {
					...
					";
					mysql_query($sqlALtRed, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlALtRed):" . $sqlALtRed . "</b><br>" . mysqli_error($conex));
					...
					";
					$resNomEsp 	= mysql_query($sqlNomEsp, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlNomEsp):" . $sqlNomEsp . "</b><br>" . mysqli_error($conex));
					if ($rowNomEsp = mysqli_fetch_array($resNomEsp))
		...
		if (mysqli_num_rows($resTurnos) == 0) {
	...
	";
		$resNomPac = mysql_query($sqlNomPac, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlNomPac):</b><br>" . mysqli_error($conex));
		if ($rowNomPac = mysqli_fetch_array($resNomPac))
...
echo "</tr>";*/
	...
	";
			...
			$sqlCcoAyu =  "SELECT Ccocod
				   ...
				   WHERE Ccocod = '{$cco_usuario}'
				     AND Ccoayu = 'on'";
			$resCcoAyu = mysql_query($sqlCcoAyu, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlCcoUrg):</b><br>" . mysqli_error($conex));
			if (mysqli_fetch_array($resCcoAyu)) {
				$filtrarCcoAyuda = "on";
			}
		}
	...
	<td align='center'>
		...
		if ($aplicacion == "") {
			...
			echo "<td class='fila2'>";
			$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '20' and Selest='on'", '', '', '2');
			...
			echo "<td class='fila'>";
			$res1 = consultarCC($aplicacion, $where, $userCcos);
		...
		echo "<td class='fila2'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '21' and Selest='on'", '', 'Seldes', '2'); //msgError='Numero de Historia' msgError='Numero de Ingreso'
		...
		$param = "class='reset'";
		echo "<td class='fila2' colspan='1'>";
		$res1 = consultaMaestros('root_000099', 'Clacod,Clades', $where = "Claest='on'", '', 'Clades');
		crearSelectHTMLAcc($res1, 'ing_claselClausu', 'ing_claselClausu', $param, $consultarDefecto);
		echo "</td>";

		$param = "class='reset' {$pedirTipoServicio}";
		echo "<td class='fila2espacio' colspan='1' id='td_tipoServicioSelect' {$pedirTipoServicio}>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '14' and Selest='on'", '', 'Seldes', '2');
		...
		echo "<td class='fila1espacio' colspan='2'>";
		$res1 = consultaMaestros('root_000007', 'Codigo,Descripcion,alfanumerico, docigualhis', $where = "Estado='on'", '', '');
		...
		echo "<td class='fila2espacio' colspan='2'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '25' and Selest='on'", '', '', '2');
		...
		echo "<td class='fila1espacio' colspan='2'>";
		$res1 = consultaMaestros('root_000098', 'Petcod,Petdes', $where = "Petest = 'on'", '', 'Petcod');
		...
		echo "<td class='fila1espacio'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '05' and Selest='on'", '', '', '2');
		crearSelectHTMLAcc($res1, 'pac_zonselZonRes', 'pac_zonselZonRes', $param);
		echo "</td>";
		$param = "class='reset' msgcampo='Zona geografica'";
		if (INCLUIR_ZONA_GEOGRAFICA == 'on') {
			echo "<td class='fila1espacio'>";
			$res1 = consultaMaestros('000256', 'Zogcod,Zogdes', $where = "Zogest = 'on'", '', '', '2');
		...
		echo "<td class='fila1espacio' colspan='2'>";
		$res1 = consultaMaestros('root_000066', 'Scocod,Scodes', $where = "Scoest='on' and Scoley='off'", '', '');
...
echo "</td>";*/
	  ...
	  <input type='hidden' name='pac_prpidPaiRes' id='pac_prpidPaiRes' value='".CODIGO_COLOMBIA."'>
		...
		echo "<td class='fila2espacio' colspan='2'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '06' and Selest='on'", '', 'Selcod', '2');
		...
		echo "<td class='fila2espacio'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '16' and Selest='on'", '', 'Selcod', '2');
		...
		echo "<td class='fila2'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '12' and Selest='on'", '', 'Selcod', '2');
	...
	echo"</td>";*/
		...
		echo "<td class='fila1espacio' width='17%'>";
		$res1res = consultaMaestros('root_000007', 'Codigo,Descripcion,alfanumerico, docigualhis', $where = "Estado='on'", '', '');
		...
		echo "<td class='fila2' width='20%'>";
		$res1 = consultaMaestros('000105', 'Selcod,Seldes', $where = "Seltip = '17' and Selest='on'", '', 'Selcod', '2');
