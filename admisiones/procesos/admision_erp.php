			...
			if ($num>0) // si tiene registros se consulta el numero de ingreso
			{
				...
				if( $res2 )
				{
					...
					if ($num2>0)
					{
						...
						//Si no es ambulatorio sigue su curso normal
						if( !$numAmbulatoria ){
	...
	if( $data[ "error" ] == 0 and $aplicacion != "")
	{
		...
		if ($res8)
		{
			...
			else //si es registro nuevo en la tabla movhos_000018
			{
				...
				//Query para verificar la admision ambulatoria
				$esAmbulatoria = "SELECT * FROM root_000051 WHERE Detemp = '".$wemp_pmla."' AND Detapl = 'admisionAmbulatoria' AND Detval = 'on'";
				$esAmbulatoria = mysql_query($esAmbulatoria, $conex) or ( $data[ 'mensaje' ] = utf8_encode( "Error consultando la tabla root_000051 ".mysql_errno()." - Error en el query $esAmbulatoria - ".mysql_error() ) );
				$numAmbulatoria = mysql_num_rows($esAmbulatoria);
				if($numAmbulatoria > 0){
					...
					$sql10 = "insert into ".$aplicacion."_000018 (       medico    ,    fecha_data           ,hora_data               , Ubihis                     , Ubiing                    , Ubisac                         , Ubialp, Ubiald, Ubiptr			  ,     seguridad     )
														values ('".$aplicacion."','".utf8_decode($fecha)."','".utf8_decode($hora)."','".utf8_decode($historia)."','".utf8_decode($ingreso)."','".utf8_decode($servicioIng2)."', 'on' , 'on' , '".$campo_ccocir."' ,'C-".$aplicacion."')";
					...
					if (!$res10)
					{
						...
						//Script para actualizar el ingreso 1 de la historia
						$sqlUpdateIng1 = "update ".$aplicacion."_000018 set Ubiald = 'off' where Ubihis = '".utf8_decode($historia)."' and Ubiing='1'";						
						$resUpIng1 = mysql_query( $sqlUpdateIng1, $conex ) or ( $data[ 'mensaje' ] = utf8_encode( "Error actualizando alta definita en la tabla movhos_000018 para ingreso 1".mysql_errno()." - Error en el query $sqlUpdateIng1 - ".mysql_error() ) );
						if (!$resUpIng1)
						{
							$data[ 'error' ] = 1; //sale el mensaje de error
						}
		...
		if( $num == 0 ){
			...
			if ($res5)
			{
				...
				if ($num5 > 0)
				{
					  ...
					  $wbasedato = consultarAliasPorAplicacion($conex,

					  $wemp_pmla, "facturacion" );
				...
				)
				{
					...
					if( $("[name='dat_Acccas_ux_acccas']").val() != $("[name='dat_Acccas_ux_acccas']").attr("codigoOriginal") ){
						if( $.trim($("[name='dat_Acccas_ux_acccas']").attr("codigoOriginal")) != "" ){//--> si original = vacio, no verifica saldo unix
									/* 
										iSap_retiro_odbc_admision_erp_01
										1. Se agrega condicion para validar si la integracion este encendido.
										2. La consulta de validar responsable no se podra consultar mas en servinte.
										Autor: Didier Orozco Carmona.
										Fecha: 2021-05-03
									*/
							if ($InactivarCodigo == 0 ){
...
//$tieneConexionUnix = "off";
	/* 
		iSap_retiro_odbc_admision_erp_02
		1. Se agrega condicion para validar si la integracion este encendido.
		2. La funcion de consulta si esta activo servinte esta encendido no se ejecutara.
		Autor: Didier Orozco Carmona.
		Fecha: 2021-05-03
	*/
if($InactivarCodigo == 0 ){ 
