  ...
  if (!isset($wfecini) or !isset($wfecfin) or !isset($wcco) or !isset($wtar))
   {
    ...
    echo "</tr>";
  ...
  else 
     {
	  ...
	  $q = "  SELECT fenffa, fenfac, fenfec, arttiv, artiva, SUM((vdecan*vdevun/(1+(vdepiv/100)))), SUM(round(((vdecan*vdevun/(1+(vdepiv/100)))*(vdepiv/100)),2)), SUM((vdecan*vdevun))"
	      ."    FROM farstore_000018, farstore_000019, farstore_000001, farstore_000016, farstore_000017 "
	  ...
	  for ($i=0;$i<$num;$i++)
	   ...
	   echo "<td align=right><font size=2>".number_format($wtotvenbru,2,'.',',')."</font></td>";
	   echo "<td align=right><font size=2>".number_format($wtotveniva,2,'.',',')."</font></td>";
	   echo "<td align=right><font size=2>".number_format($wtotventot,2,'.',',')."</font></td>";
	   echo "<tr>";
	  ...
	  $q = "  SELECT fenffa, fenfac, fenfec, fenval, sum(mdevto)  "
	      ."    FROM farstore_000018, farstore_000019, farstore_000016, farstore_000011 "
          ."   WHERE fenfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND fenffa = fdeffa "
          ."     AND fenfac = fdefac "
	  ...
	  for ($i=0;$i<$num;$i++)
	   ...
	   echo "<td align=right><font size=2>".number_format($wtotventot,2,'.',',')."</font></td>";
	   echo "<td align=right><font size=2>".number_format($wtotcostot,2,'.',',')."</font></td>";
	   echo "<tr>";
	  ...
	  $q = "  SELECT grucod, grudes, sum(mdevto)  "
	      ."    FROM farstore_000018, farstore_000019, farstore_000016, farstore_000011, farstore_000001, farstore_000004 "
          ."   WHERE fenfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND fenffa = fdeffa "
          ."     AND fenfac = fdefac "
	  ...
	  for ($i=0;$i<$num;$i++)
	     {
	      ...
	      echo "<td><font size=2>".$row[0]."</font></td>";
	      echo "<td><font size=2>".$row[1]."</font></td>";
	      echo "<td align=right><font size=2>".number_format($row[2],2,'.',',')."</font></td>";
	      echo "</tr>"; 
	  ...
	  $q = "  SELECT fenffa, fenfac, grucod, grudes, arttiv, artiva, SUM((vdecan*vdevun/(1+(vdepiv/100)))), SUM(round(((vdecan*vdevun/(1+(vdepiv/100)))*(vdepiv/100)),2)), SUM((vdecan*vdevun))"
	      ."    FROM farstore_000018, farstore_000019, farstore_000001, farstore_000016, farstore_000017, farstore_000004 "
          ."   WHERE fenfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND fenffa = fdeffa "
          ."     AND fenfac = fdefac "
	  ...
	  $q = "  SELECT grucod, grudes, arttiv, artiva, SUM((vdecan*vdevun/(1+(vdepiv/100)))), SUM(round(((vdecan*vdevun/(1+(vdepiv/100)))*(vdepiv/100)),2)), SUM((vdecan*vdevun))"
	      ."    FROM farstore_000018, farstore_000019, farstore_000001, farstore_000016, farstore_000017, farstore_000004 "
          ."   WHERE fenfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND fenffa = fdeffa "
	  ...
	  $q = "  SELECT fenffa, fenfac, vdepiv, sum(vdevun*vdecan), sum((vdevun*vdecan)*(vdepiv/100)) "
	      ."    FROM farstore_000018, farstore_000019, farstore_000016, farstore_000017 "
          ."   WHERE fenfec between '".$wfecini."'"
          ."     AND '".$wfecfin."'"
          ."     AND fenffa = fdeffa "
          ."     AND fenfac = fdefac "
	  ...
	  for ($i=0;$i<$num;$i++)
	     {
	      ...
	      echo "<td align=right><font size=2>".number_format($row[3],2,'.',',')."</font></td>";
	      echo "<td align=right><font size=2>".number_format($row[4],2,'.',',')."</font></td>";
	      echo "<td align=right><font size=2>".number_format(($row[3]+$row[4]),2,'.',',')."</font></td>";
	      echo "</tr>"; 
	      
	      $wtotbrutot = $wtotbrutot + $row[3];
	      $wtotivatot = $wtotivatot + $row[4];
	      $wtottottot = $wtotbrutot + $wtotivatot;
	      
	     }
	   echo "<tr>";  
	   echo "<td colspan=3>TOTAL</td>";
	   echo "<td align=right><font size=2>".number_format($wtotbrutot,2,'.',',')."</font></td>";
	   echo "<td align=right><font size=2>".number_format($wtotivatot,2,'.',',')."</font></td>";
	   echo "<td align=right><font size=2>".number_format($wtottottot,2,'.',',')."</font></td>";
	   echo "<tr>";
