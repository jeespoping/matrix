...
function pintarTitulo($pintar)
{
	...
	if ($pintar==0)
	{
		...
		echo "<td class='texto6' width='15%'><a style='text-decoration:none;color:white' href='pospru.php?wbasedato=pospru.php&tipo=A'>VENTA EXTERNA</a></td></TR>";
...
function pintarBusqueda($consultas, $forcon, $pintar)
{
	...
	echo "<form name='producto2' action='pospru.php' method=post>";
...
{
	echo "<form name='producto3' action='pospru.php' method=post>";
	echo "<tr><td colspan=3 class='titulo3' align='center'><INPUT TYPE='submit' NAME='NUEVO' VALUE='Nuevo' class='texto5' ></td></tr>";
	echo "<input type='hidden' name='pintar' value='".$pintar."'></td>";
	echo "</table></form>";

	echo "<form name='producto' action='pospru.php' method=post>";
	echo "<table border=0 ALIGN=CENTER width=90%>";
		...
		{			
			if ($insumos[0]['lot']=='on')
			{
				...
				else
				{
					...
					//Busco el codigo anterior
					$expca = explode('-',$insumo);
					$codant = $expca[0]; 
					
					$lotes=consultarLotes($insumos[0]['cod'], $cco, $lote, &$cantidad);
					
					//En caso de que sean diferentes el codigo a buscar
					//Se borra la primera posicion de $lotes
					$index = 0;
					if( $codant != $insumos[0]['cod'] ){
						$index = 1;
						if( count($lotes) < $lote )
							unset( $lotes[0] );
					}

					if( $lote != '' && ($lote > $lotes[ $index ]) ){
						$lote = $lotes[ $index ];
					}
