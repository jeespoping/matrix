...
function condicionesKE( &$pac, $art ){
	...
	$sql = "SELECT 
				(kadcdi-kaddis) as sal, kadart, kadcpx
			FROM 
				{$bd}_000054
			WHERE	
				kadhis = '{$pac['his']}' 
	...
	echo "tiempoDispensacion: $tiempoDispensacion <br>tmpDispensacion: $tmpDispensacion";
	/****************************************************************************************************
	 * Agosto 28 de 2012
	 ****************************************************************************************************/
	$disCco = consultarHoraDispensacionPorCco( $conex, $bd, $servicio );
	...
	/************************************************************************************************************/
	consultarInfoTipoArticulos( $conex, $bd );
	echo "<br>tmpDispensacion: $tmpDispensacion <br>tempRonda: $tempRonda";
	$tempRonda = ( intval( ( date( "H" )+intval( $tempRonda/3600 ) )/2 )*2 )  + ($tmpDispensacion - ($tempRonda/3600) );			

	echo "<br>tempRonda: $tempRonda";
	...
	echo "<br>tempRonda y rondaActual: $tempRonda";
...
{
    echo "<form name='producto3' action='cargocpx_2.php' method=post>";
...
{
	global $pac;
    echo "<form name='producto' action='cargocpx_2.php' method=post>";
...
else
{
    ...
    include_once( "cenpro/cargos.inc_2.php" );	//incluye las funciones para ciclos de produccion
    include_once( "conex.php" );
			                                    			...
			                                    			if( !isset( $hiReemplazar ) || ( isset( $hiReemplazar ) && isset($rdArticulo) && !empty($rdArticulo)  )
						                                        ...
						                                        if( isset( $hiReemplazar ) && isset($rdArticulo) && !empty($rdArticulo) && $val ){
						                                        	...
						                                        	if( reemplazarMedicamentoKardex( $rdArticulo, $cod ) ){
						                                        		...
						                                        		pintarAlerta('NO SE REALIZO EL REEMPLAZO DEL MEDICAMENTO<br>EL MEDICAMENTO YA EXISTE CON LA MISMA FECHA Y HORA DE INICIO<br>Y SIN SALDO');
		                                    							pintarBoton();
						                                        		exit;
						                                        	}
						                                        }
