...
function reemplazarMedicamentoKardex( &$id, $art ){
    ...
    if( $numRepetidos <= 0 ){
    	...
    	if( $rowsRepetidos['Kadcdi'] > $rowsRepetidos['Kaddis'] ){
...
{
	global $pac;
    echo "<form name='producto' action='cargo.php' method=post>";
    ...
    for ($i = 0; $i < count($inslis); $i++)
    {
        ...
        for ($j = 0;$j < $tam;$j++)
        {
            if ($tam == 1)
            {
                $faltante = (integer) $inslis[$i]['can'] - (integer) $presen[$i][$j]['caj'];
...
function actualizarAjuste($presentacion, $ajuste, $cantidad, $historia, $cco, $usuario, $ingreso, $cnv, $total)
{
    ...
    if ($cantidad > 0)
    {
        $saldo = (integer) $cantidad * (integer) $cnv + (integer) $ajuste - (integer) $total;
								...
								else
								{
									...
									for ($i = 0; $i < count($inslis); $i++)
									{
										...
										for ($j = 0; $j < count($presen[$i]); $j++)
										{
											...
											$cantidad = (integer) $cantidad + (integer) $presen[$i][$j]['can'] + (integer) $presen[$i][$j]['caj'];
										} 
