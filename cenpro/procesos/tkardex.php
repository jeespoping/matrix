...
else
{
	...
	else
	{
		...
		for ($i=0;$i<count($dsan);$i++)
		{
			...
			$query = "insert ".$empresa."_000011 (medico,fecha_data,hora_data, Kxmcon, Kxmano, Kxmmes,     Kxmcco,             Kxmcod,             Kxmdes,           Kxmuni,           Kxmgru,      Kxmcsi, Kxmvsi,                 Kxmcen,                   Kxmven,                Kxmcsa,                   Kxmvsa,               Kxmcsf,           Kxmvsf,         Kxmcdi, Kxmvdi, Kxmvro, Kxmdro, Kxmind, Kxmins, seguridad) ";
			$query .= "values ('".$empresa."','".$fecha."','".$hora."',".$contador.",".$wano.",".$wmes.",'".$dsac[$i][0]."','".$dsac[$i][1]."','".$dsac[$i][4]."','".$dsac[$i][5]."',   '',".$dsan[$i][2].",".($dsan[$i][2]*$dsan[$i][3]/$dsan[$i][6]).",".$dsac[$i]['entradas'].",".(($dsac[$i]['entradas']*$dsac[$i][3]/$dsan[$i][6])+$dsac[$i]['reve']).",".$dsac[$i]['salidas'].",".(($dsac[$i]['salidas']*$dsac[$i][3]/$dsan[$i][6])+$dsac[$i]['revs']).",".$dsac[$i][2].",".($dsac[$i][2]*$dsac[$i][3]/$dsan[$i][6]).",".$dif.",".$difv.",'','','on', '".$dsac[$i][7]."','C-".$empresa."')";
