...
else
{
	...
	{
		echo "<form action='000008_of01vlaser.php' method=post>";
		...
		if(isset($pac1))
		{
			...
			$query="select Paciente from vlaser_000008 where Oftalmologo='".$medico."' and Paciente like '%".$pac1."%' order by Paciente ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
		...
		if(isset($pac))
		{
			/*Si ya se tiene el paciente buscar las fechas de los seguimiento para este y
			 construir el drop down */	
			$query = "select Fecha_actual  from vlaser_000008 where Paciente='".$pac."' and Oftalmologo='".$medico."' ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	  ...
	  ********************************/
	{
		...
		$query="select Diagnostico,tratamiento from vlaser_000008 where Paciente='".$pac."' and Oftalmologo='".$medico."' and Fecha_actual='".$fecha."' ";
		$err = mysql_query($query,$conex);
		if($err)
			$row = mysql_fetch_array($err);
		else
