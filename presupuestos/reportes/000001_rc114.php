...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Ejecucion Presupuestal de Cuentas</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc114.php Ver. 1.00</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc114.php' method=post>";
		...
		if(!isset($wanop) or !isset($wper1)  or !isset($wper2) or ((!isset($wcco1)  or !isset($wcco2)) and !isset($wccof)) or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>EJECUCION PRESUPUESTAL DE CUENTAS</td></tr>";
		...
		else
		{
			...
			{
				$query = "SELECT Rvpcpr,mganom,sum(Rvpvre) as wmonto1,sum(Rvpvpr) as wmonto2 from ".$empresa."_000044,".$empresa."_000028 ";
				...
				echo "<tr><td colspan=6 align=center>EJECUCION PRESUPUESTAL DE CUENTAS</td></tr>";
				echo "<tr><td colspan=6 align=center>PERIODO INICIAL : ".$wper1." PERIODO FINAL : ".$wper2. " A�O : ".$wanop."</td></tr>";
				echo "<tr><td colspan=6 align=center>UNIDAD INICIAL : ".$wcco1. " UNIDAD FINAL : ".$wcco2."</td></tr>";
				echo "<tr><td bgcolor=#dddddd><b>CODIGO</b></td><td bgcolor=#dddddd><b>RUBRO</b></td><td align=right bgcolor=#dddddd><b>REAL</b></td><td align=right bgcolor=#dddddd><b>PRESUPUESTADO</b></td><td align=right bgcolor=#dddddd><b>% CUMPLIMIENTO</b></td><td align=right bgcolor=#dddddd><b>DIFERENCIA</b></td></tr>";
				$wtotal=array();
				$ita=0;
				for ($i=0;$i<$num;$i++)
				{
					...
					$wpor=0;
					if($row[3] != 0)
						$wpor=$row[2]/$row[3]*100;
					$wdif=$row[2] - $row[3];
					echo"<tr><td>".$row[0]."</td><td>".$row[1]."</td><td align=right>".number_format((double)$row[2],0,'.',',')."</td><td align=right>".number_format((double)$row[3],0,'.',',')."</td><td align=right>".number_format((double)$wpor,2,'.',',')."</td><td align=right>".number_format((double)$wdif,0,'.',',')."</td></tr>";
	    		}
			}
	         else
			{
