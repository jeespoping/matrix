...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Evaluacion de Tarifas Tipo P Costos Totales</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc102.php Ver. 2017-08-23</b></font></tr></td></table>
</center>
...
else
{
	...
	echo "<form action='000001_rc102.php' method=post>";
	echo "<center><input type='HIDDEN' name= 'empresa' value='".$empresa."'>";
	if(!isset($wano) or !isset($wemp1) or $wemp1 == "Seleccione" or !isset($wmes) or !isset($wemp) or !isset($wcco))
	{
		echo "<center><table border=0>";
		echo "<tr><td align=center colspan=3><b>PROMOTORA MEDICA LAS AMERICAS S.A.<b></td></tr>";
		echo "<tr><td align=center colspan=3>EVALUACION DE TARIFAS TIPO P COSTOS TOTALES</td></tr>";
		...
		echo "<tr><td bgcolor=#cccccc align=center colspan=2>Empresa de  Proceso</td><td bgcolor=#cccccc align=center>";
		...
		if(isset($wemp1) and $wemp1 != "Seleccione")
		{
			...
			$query = "select Empgru,Empseg,Empcin,Empdes from ".$empresa."_000061 where Empemp='".substr($wemp1,0,2)."' and Empeva='S' group by 1,2,3 order by 1,2,3 ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if ($num>0)
			{
	...
	else
	{
		...
		//$query = "SELECT Ano, Mes from ".$empresa."_000048 where Cierre_costos='on' order by 1 desc,2 desc";
		//$err = mysql_query($query,$conex);
		//$num = mysql_num_rows($err);
		//if ($num>0)
		//{
		...
		$COSVAR=array();
		$query  = "select cvacco,LPAD(cvacod,10,'0'),LPAD(cvagru,4,'0'),cvacon,cvapro from ".$empresa."_000082 ";
		$query .= " where cvaano = ".$wano;  
		$query .= "   and cvaemp = '".$wemp1."'"; 
		$query .= "   and cvames = ".$wmes; 
		if(!isset($wccoT))
			$query .= "    and cvacco IN ".$wcc;  
		...
		$query  = "select Empgru,Empseg,Empcin,Pcacco,Cconom,Pcacod,Pcacon,Pcapor,Pcapro,tarcta,Tarmon,Mprnom,ntades,cfades,Empdes,mprnom,pcagru ";
		$query .= " from ".$empresa."_000105,".$empresa."_000097,".$empresa."_000060,".$empresa."_000095,".$empresa."_000005,".$empresa."_000140,".$empresa."_000061,".$empresa."_000150 ";
		$query .= "  where tarcco=pcacco ";
		$query .= "    and ".$empresa."_000105.taremp = '".$wemp1."'"; 
		$query .= "    and ".$empresa."_000105.taremp = pcaemp "; 
		$query .= "    and tarcod=pcacod "; 
		$query .= "    and tarcon=pcacon ";
		$query .= "    and pcaano=".$wano; 
		$query .= "    and pcames=".$wmes;  
		$query .= "    and pcatip IN ('P','O') ";
		$query .= "    and pcaemp=cfaemp ";
		$query .= "    and pcacon=cfacod ";
		$query .= "    and pcagru between '0000' and 'zzzz' ";
		$query .= "    and pcaemp=mpremp ";
		$query .= "    and pcacco=mprcco ";
		$query .= "    and pcacod=mprpro "; 
		$query .= "    and pcacon=mprcon ";
		$query .= "    and pcaemp=ccoemp ";
		$query .= "    and pcacco=ccocod ";
		if(!isset($wccoT))
		...
		$query .= "    and ".$empresa."_000140.taremp = empemp "; 
		$query .= "    and tarent=empcin ";
		if(!isset($wempT))
			$query .= "    and empcin IN ".$wcin;
		$query .= "    and Empeva='S' "; 
		$query .= "    and ".$empresa."_000140.taremp = ntaemp ";  
		...
		echo "<tr><td colspan=18 align=center>EVALUACION DE TARIFAS TIPO P COSTOS TOTALES</td></tr>";
		echo "<tr><td colspan=18 align=center>EMPRESA : ".$wemp1t."</td></tr>";
		echo "<tr><td colspan=18 align=center>A&Ntilde;O : ".$wano." MES : ".$wmes."</td></tr>";
