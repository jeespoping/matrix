...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Facturacion Comparativa por Cliente</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc182.php Ver. 2017-07-27</b></font></tr></td></table>
</center>
<?php
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form name='r130' action='000001_rc182.php' method=post>";
		...
		if(!isset($wanoi) or !isset($wemp) or $wemp == "Seleccione" or !isset($wtipi) or (isset($wtipi) and strtoupper($wtipi) != "P" and strtoupper($wtipi) != "T" and strtoupper($wtipi) != "G") or !isset($wanof) or !isset($wmesi) or !isset($wmesf) or !isset($wcco1)  or !isset($wcco2) or (isset($wmesi) and ($wmesi < 1 or $wmesi > 12)) or (isset($wmesf) and ($wmesf < 1 or $wmesf > 12)))
		{
			...
			echo "<tr><td align=center colspan=2>FACTURACION COMPARATIVA POR CLIENTE</td></tr>";
		...
		else
		{
			...
			//                   0       1       2      3        4            5   
			$query  = "select Empcin, Empdes, Mioano, Miomes, Empseg, sum(".$wval.") from ".$empresa."_000063,".$empresa."_000061 ";
			...
			if($num > 0)
			{
				...
				echo "<tr><td colspan=".$ncol." align=center><font size=2>FACTURACION COMPARATIVA POR CLIENTE</font></td></tr>";
				...
				echo "<tr><td bgcolor=#cccccc><font size=2><b>CODIGO<BR>CLIENTE</b></font></td><td bgcolor=#cccccc><font size=2><b>CLIENTE</b></font></td><td bgcolor=#cccccc><font size=2><b>SEGMENTO</b></font></td>";
				$inc=periodo($wmesi,$wfactor);
				$inf=periodo($wmesf,$wfactor);
				$iter=12 / $wfactor;
				for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
				...
				for ($w=0;$w<=$s;$w++)
				{
					...
					if($ncolor % 2 == 0)
						...
						$color="#99CCFF";
					echo "<tr><td bgcolor=".$color."><font size=2>".$data[$w][$fil+1][0]."</font></td><td bgcolor=".$color."><font size=2>".$data[$w][$fil+1][1]."</font></td><td bgcolor=".$color."><font size=2>".$data[$w][$fil+1][2]."</font></td>";
					$inc=periodo($wmesi,$wfactor);
					$inf=periodo($wmesf,$wfactor);
					for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
					{
				...
				echo "</tr>";
				$color="#CCCCCC";
				echo "<tr><td bgcolor=".$color." colspan=3><font size=2>TOTALES</font></td>";
				$inc=periodo($wmesi,$wfactor);
				$inf=periodo($wmesf,$wfactor);
				for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
				{
