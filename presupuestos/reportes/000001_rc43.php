...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Informe Ejecucion Presupuestal de Costos Operacionales</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc43.php Ver. 1.01</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc43.php' method=post>";
		...
		if(!isset($wanop) or !isset($wtip) or (strtoupper ($wtip) != "T" and strtoupper ($wtip) != "A") or !isset($wper1)  or !isset($wper2) or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>INFORME EJECUCION PRESUPUESTAL DE COSTOS OPERACIONALES</td></tr>";
		...
		else
		{
			...
			$query = "select Rescco,Cconom,Ccouni,sum(Resmon) from ".$empresa."_000043,".$empresa."_000028,".$empresa."_000005 ";
			$query = $query."  where resano = ".$wanop;
			$query = $query."    and resper between ".$wper1." and ".$wper2;
			$query = $query."    and rescpr between '200' and '299' ";
			...
			echo "<tr><td colspan=5 align=center>INFORME EJECUCION PRESUPUESTAL DE COSTOS OPERACIONALES</td></tr>";
			echo "<tr><td colspan=5 align=center> A�O : ".$wanop." PERIODO INICIAL : ".$wper1." PERIODO FINAL : ".$wper2. "</td></tr>";
			...
			for ($i=0;$i<=$num;$i++)
			{
				...
				if($wdata[$i][4] != 0)
					$wpor=($wdata[$i][3])/$wdata[$i][4]*100;
				else
					$wpor=0;
				echo"<tr><td>".$wdata[$i][1]."</td><td align=right>".number_format((double)$wdata[$i][3],0,'.',',')."</td><td align=right>".number_format((double)$wdata[$i][4],0,'.',',')."</td><td align=right>".number_format((double)$wdif,0,'.',',')."</td><td align=right>".number_format((double)$wpor,2,'.',',')."</td></tr>";
			}
			$wdif=$wtotal2-$wtotal1;
			if($wtotal1 != 0)
				$wpor=($wtotal2)/$wtotal1 *100;
			else
				$wpor=0;
