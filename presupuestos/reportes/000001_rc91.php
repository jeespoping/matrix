...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Movimiento Comparativo de Un Criterio de Indirectos</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc91.php Ver. 2017-03-02</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc91.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1)  or !isset($wper2)   or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>MOVIMIENTO COMPARATIVO DE UN CRITERIO DE INDIRECTOS</td></tr>";
			...
			echo "<tr><td bgcolor=#cccccc align=center>Codigo Criterio</td>";
			echo "<td bgcolor=#cccccc align=center>";
			$query = "SELECT Cricod, Crides from ".$empresa."_000051 order by Cricod";
		...
		else
		{
			...
			$query = "select    Mcrcco,cconom,Mcrmes,sum( Mcrpor)    from ".$empresa."_000053,".$empresa."_000005 ";
			$query = $query."  where Mcrano  = ".$wanop;
			$query = $query."    and Mcremp = '".$wemp."'";
			$query = $query."    and Mcrmes  between ".$wper1." and ".$wper2;
			$query = $query."    and Mcrcri = '".$wseg."'";
			$query = $query."    and Mcremp = ccoemp ";
			$query = $query."    and Mcrcco = ccocod ";
			$query = $query."   group by Mcrcco,cconom,Mcrmes  ";
			$query = $query."   order by  Mcrcco,Mcrmes ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			echo "<table border=1>";
			...
			echo "<tr><td colspan=16 align=center>MOVIMIENTO COMPARATIVO DE UN CRITERIO DE INDIRECTOS</td></tr>";
			echo "<tr><td colspan=16 align=center><b>EMPRESA : ".$wempt."</b></td></tr>";
			echo "<tr><td colspan=16 align=center>CIRTERIO : ".$wseg." - ".$wsegm."</td></tr>";
			...
			for ($i=0;$i<$num;$i++)
			{
				...
				$wdat[$seg][$row[2]+1]+=$row[3] * 100;
				//$wdat[$seg][13]+=$row[3] * 100;
				$wdatt[$row[2]+1]+=$row[3] * 100;
				//$wdatt[13]+=$row[3] * 100;
			}
