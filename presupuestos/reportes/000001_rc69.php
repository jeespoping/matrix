...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Indirecto Comparativo Entre Meses</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc69.php Ver. 2016-05-27</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc69.php' method=post>";
		...
		if(!isset($wanop)  or !isset($wemp) or $wemp == "Seleccione" or !isset($wres) or (strtoupper ($wres) != "R" and strtoupper ($wres) != "P")  or !isset($wper1)  or !isset($wper2)   or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>INDIRECTO COMPARATIVO ENTRE MESES</td></tr>";
		...
		else
		{
			...
			{
			$query = "select mdicco,cconom,mdimes,sum(mdimon) as monto from ".$empresa."_000054,".$empresa."_000005 ";
			...
			$query = $query."   group by mdicco,cconom,mdimes  ";
			$query = $query."   order by mdicco,mdimes";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			echo "<table border=1>";
			...
			echo "<tr><td colspan=16 align=center>INDIRECTO COMPARATIVO ENTRE MESES</td></tr>";
