...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Horas x Cargo y Unidad (nomina)</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc132.php Ver. 2016-03-04</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc132.php' method=post>";
		...
		if(!isset($wanoi) or !isset($wemp) or $wemp == "Seleccione" or !isset($wanof) or !isset($wmesi) or !isset($wmesf) or !isset($wcco1)  or !isset($wcco2) or (isset($wmesi) and ($wmesi < 1 or $wmesi > 12)) or (isset($wmesf) and ($wmesf < 1 or $wmesf > 12)))
		{
			...
			echo "<tr><td align=center colspan=2>HORAS X CARGO Y UNIDAD (NOMINA)</td></tr>";
			echo "<tr><td align=center colspan=2>000001_rc132.php Ver. 2008-05-20</td></tr>";
		...
		else
		{
			...
			//                   0       1       2      3       4         5          6       7 
			$query  = "select Norcco, Carnom, Connom, Norano, Norper, sum(Norhor), Norcod, Norcar  from ".$empresa."_000036,".$empresa."_000008, ".$empresa."_000004 ";
			...
			$query .= "	group by Norcco, Carnom, Connom, Norano, Norper, Norcod, Norcar ";
			$query .= "	order by Norcco, Norcar, Connom  ";
			...
			if($num > 0)
			{
				...
				echo "<tr><td colspan=".$ncol." align=center><font size=2>HORAS X CARGO Y UNIDAD (NOMINA)</font></td></tr>";
				...
				echo "<tr><td bgcolor=#cccccc><font size=2><b>CENTRO<BR>DE COSTOS</b></font></td><td bgcolor=#cccccc><font size=2><b>CARGO</b></font></td><td bgcolor=#cccccc><font size=2><b>CONCEPTO</b></font></td>";
				$inc=periodo($wmesi,$wfactor);
				$inf=periodo($wmesf,$wfactor);
				$iter=12 / $wfactor;
				for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
					...
					if($wclaa != $wcla)
					{
						if($j != 0)
						{
							...
							if($ncolor % 2 == 0)
								...
								$color="#99CCFF";
							$path="/matrix/presupuestos/reportes/000001_rc133.php?wcarg=".$wcarn."&wcargn=".$wcar."&wcco=".$wcco."&wanoi=".$wanoi."&wmesi=".$wmesi."&wanof=".$wanof."&wmesf=".$wmesf."&wcco1=".$wcco1."&wcco2=".$wcco2."&wfactor=".$wfactor."&empresa=".$empresa."&wemp=".$wemp;
							echo "<tr onclick='ejecutar(".chr(34).$path.chr(34).")'><td bgcolor=".$color."><font size=2>".$wcco."</font></td><td bgcolor=".$color."><font size=2>".$wcarn."-".$wcar."</font></td><td bgcolor=".$color."><font size=2>".$wcon."</font></td>";
							$inc=periodo($wmesi,$wfactor);
							$inf=periodo($wmesf,$wfactor);
							$iter=12 / $wfactor;
							for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
							...
							if($wcarn != trim($row[7]))
							{
								$color="#FFCC66";
								$path="/matrix/presupuestos/reportes/000001_rc133.php?wcarg=".$wcarn."&wcargn=".$wcar."&wcco=".$wcco."&wanoi=".$wanoi."&wmesi=".$wmesi."&wanof=".$wanof."&wmesf=".$wmesf."&wcco1=".$wcco1."&wcco2=".$wcco2."&wfactor=".$wfactor."&empresa=".$empresa."&wemp=".$wemp;
								echo "<tr onclick='ejecutar(".chr(34).$path.chr(34).")'><td bgcolor=".$color."><font size=2>".$wcco."</font></td><td bgcolor=".$color."><font size=2>TOTAL : ".$wcarn."-".$wcar."</font></td><td bgcolor=".$color."><font size=2>".$wcon."</font></td>";
								$inc=periodo($wmesi,$wfactor);
								$inf=periodo($wmesf,$wfactor);
								$iter=12 / $wfactor;
								for ($i=($wanoi-$wanoi);$i<=($wanof-$wanoi);$i++)
