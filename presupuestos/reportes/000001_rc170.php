...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Movimiento Comparativo Costo x Actividad x Unidad x Mes (CP)</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc170.php Ver. 2018-07-05</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc170.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1)  or !isset($wper2)  or ((!isset($wcco1) or !isset($wcco2)) and !isset($wccof)) or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>MOVIMIENTO COMPARATIVO COSTO X ACTIVIDAD X UNIDAD X MES(CP)</td></tr>";
		...
		else
		{
			...
			for ($wcco=$wcco1;$wcco<=$wcco2;$wcco++)
			{
				...
				if($row[0] > 0)
				{
					...
					echo "<td bgcolor=#cccccc><b>PROMEDIO</b></td>";
					echo "</tr>";
					$query = "select Mdpsub,Subdes,Mdpmes,sum(Mdpcan) as monto,Dicdri,Dicndr from ".$empresa."_000157,".$empresa."_000104,".$empresa."_000018 ";
					$query = $query."  where Mdpano  = ".$wanop;
					$query = $query."    and Mdpemp = '".$wemp."'"; 
					$query = $query."    and Mdpmes  between ".$wper1." and ".$wper2;
					$query = $query."    and Mdpcco = '".$wcco."' ";
					$query = $query."    and Mdpsub = subcod ";
					$query = $query."    and Mdpemp = Dicemp ";
					$query = $query."    and Mdpcco = Diccco ";
					$query = $query."    and Mdpcco = Diccco ";
					$query = $query."    and Mdpsub = Dicsub ";
					$query = $query."   group by  Mdpsub,Subdes,Mdpmes  ";
					$query = $query."   order by Mdpsub,Mdpmes";
					$err = mysql_query($query,$conex);
					$num = mysql_num_rows($err);
					$seg=-1;
					$segn="";
					...
					echo "</tr>";	
					//                 0      1      2      3         4
					$query = "select Cxpsub,Subdes,Cxpmes,Cxpest,sum(Cxpcos)  as monto from ".$empresa."_000154,".$empresa."_000104 ";
					$query = $query."  where Cxpano  = ".$wanop;
					$query = $query."    and Cxpemp = '".$wemp."'"; 
					$query = $query."    and Cxpmes  between ".$wper1." and ".$wper2;
					$query = $query."    and Cxpcco = '".$wcco."' ";
					$query = $query."    and Cxpsub = subcod ";
					$query = $query."   group by  Cxpsub,Subdes,Cxpmes,Cxpest  ";
					$query = $query."   order by Cxpsub,Cxpmes ";
					$err = mysql_query($query,$conex);
					$num = mysql_num_rows($err);
					$seg=-1;
					$segn="";
