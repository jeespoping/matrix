...
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc98.php Ver. 2017-08-24</b></font></td></tr></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc98.php' method=post>";
		echo "<center><input type='HIDDEN' name= 'empresa' value='".$empresa."'>";
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wpv) or !isset($wtco) or (strtoupper ($wtco) != "P" and strtoupper ($wtco) != "M") or !isset($wco) or !isset($wpro1) or !isset($wpro2)  or (strtoupper ($wco) != "N" and strtoupper ($wco) != "S") or !isset($wcco1) or !isset($wcco2) or !isset($wper1)  or !isset($wper2)   or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
		...
		else
		{
			...
			$query = $query."select Mpremp,Mprcco,Mprpro, Mprnom, Mprgru, Mprpor, Mprtip, Mprcon from ".$empresa."_000095 ";
			$query = $query." where Mpremp = '".$wemp."' ";
			$query = $query."   and Mprcco  between '".$wcco1."' and '".$wcco2."'";
			...
			$query = "select  Mprcco, cconom, Mprgru, grudes, Mprpro, Mprnom ,  Mprpor,  Pcames, Pcactp, Pcapro, Mprtip, Mprcon from ".$empresa."_000097,".$wtable.",".$empresa."_000005,".$empresa."_000088  ";
			$query = $query." where Pcaemp = '".$wemp."' ";
			$query = $query."   and Pcaano  = ".$wanop;
			$query = $query."   and Pcames between ".$wper1." and ".$wper2;
			$query = $query."   and Pcacco between '".$wcco1."' and '".$wcco2."'";
			$query = $query."   and Pcaemp = Mpremp ";
			$query = $query."   and Pcacco = Mprcco ";
			$query = $query."   and Pcacod = Mprpro ";
			$query = $query."   and Pcagru = Mprgru ";
			$query = $query."   and Pcacon = Mprcon ";
			$query = $query."   and Pcacco = ccocod ";
			$query = $query."   and Pcaemp = ccoemp ";
			$query = $query."   and Mpremp = Gruemp ";
			$query = $query."   and Mprgru = Grucod ";
			$query = $query."  order by Mprcco, Mprgru, Mprpro, Mprcon, Pcames ";
					...
					for ($j=$wper1;$j<=$wper2;$j++)
						if($j==$wper1 or $wdat[$i][$j+6-1] != 0)
						{
							if( $j > $wper1 and abs((($wdat[$i][$j+6] / $wdat[$i][$j+6-1]) - 1) * 100) > $wpv)
							{
								if($wdat[$i][21] == "P")
								{
									$path="/matrix/presupuestos/reportes/000001_rc89.php?wanop=".$wanop."&wper1=".$j."&wcco1=".$wcco."&wpro=".$wdat[$i][4]."&wgru=".$wlin."&wserv=S&wcon=".$wdat[$i][22]."&empresa=".$empresa."&wemp=".$wempt;
									if(strtoupper($wco) == "S")
								...
								{
									$path="/matrix/presupuestos/reportes/000001_rc176.php?wanop=".$wanop."&wper1=".$j."&wcco1=".$wcco."&wpro=".$wdat[$i][4]."&wgru=".$wlin."&wcon=".$wdat[$i][22]."&wserv=S&wtco=".$wtco."&empresa=".$empresa."&wemp=".$wempt;
									if(strtoupper($wco) == "S")
										echo "<td align=right bgcolor=#FF9900 onclick='ejecutar(".chr(34).$path.chr(34).")'><img src='../../images/medical/presupuestos/P.PNG' alt='".$wmeses[$j]."' />".number_format((double)$wdat[$i][$j+6],0,'.',',')."</td>";
									else
										echo "<td align=right bgcolor=#FF9900 onclick='ejecutar(".chr(34).$path.chr(34).")'>".number_format((double)$wdat[$i][$j+6],0,'.',',')."</td>";
