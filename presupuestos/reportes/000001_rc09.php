...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Reporte de Control de Totales X Unidad (Nomina)</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc09.php Ver. 2016-06-02</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc09.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1)  or !isset($wper2) or ((!isset($wcco1)  or !isset($wcco2)) and !isset($wccof))  or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>REPORTE DE CONTROL DE TOTALES X UNIDAD (NOMINA)</td></tr>";
		...
		else
		{
			...
			{
			$query = "SELECT norcco,cconom,sum(normon*(norpre+1))  from ".$empresa."_000036,".$empresa."_000005,".$empresa."_000008 ";
			$query = $query."  where norano = ".$wanop;
			$query = $query."    and noremp = '".$wemp."'";
			$query = $query."    and norcco between '".$wcco1."' and '".$wcco2."'";
			$query = $query."    and norper between ".$wper1." and ".$wper2;
			$query = $query."    and noremp = ccoemp ";
			$query = $query."    and norcco = ccocod ";
			$query = $query."    and noremp = conemp ";
			$query = $query."    and norcod = concod ";
			$query = $query."    and (contip = 'B'";
			$query = $query."    or contip = 'R')";
			$query = $query." group by norcco ,cconom";
			$query = $query." order by norcco ";
			...
			echo "<tr><td colspan=6 align=center>REPORTE DE CONTROL DE TOTALES X UNIDAD (NOMINA)</td></tr>";
			...
			echo "<tr><td><b>CODIGO</b></td><td><b>CENTRO DE COSTOS</b></td><td align=right><b>PRESUPUESTADO</b></td><td align=right><b>NOMINA</b></td><td align=right><b>DIFERENCIA</b></td></tr>";
			$wdata=array();
			$k1=0;
			$k2=0;
			$num=-1;
			...
			for ($i=0;$i<=$num;$i++)
			{
				echo"<tr><td>".$wdata[$i][0]."</td><td>".$wdata[$i][1]."</td><td align=right>".number_format((double)$wdata[$i][2],0,'.',',')."</td><td align=right>".number_format((double)$wdata[$i][3],0,'.',',')."</td><td align=right>".number_format((double)$wdata[$i][4],0,'.',',')."</td></tr>";
    		}
		}
		else
		{
