...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Facturacion Comparativa Para Un Grupo X A&tilde;os</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc48.php Ver. 2016-02-26</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc48.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wgru) or !isset($wper1)  or !isset($wper2) or !isset($wcco1)  or !isset($wcco2) or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>FACTURACION COMPARATIVA PARA UN GRUPO X A&Ntilde;OS</td></tr>";
		...
		else
		{
			...
			$Graph="/matrix/images/medical/presupuestos/rc48_".$wanop."_".$wper2."_".$wgru.".jpg";
			$d=array();
			$d[0]=31;
			$d[1]=28;
			$d[2]=31;
			...
			echo "<tr><td colspan=9 align=center>FACTURACION COMPARATIVA  PARA UN GRUPO X A&Ntilde;OS</td></tr>";
			...
			echo "<tr><td colspan=9 align=center>PERIODO INICIAL : ".$wper1." PERIODO FINAL : ".$wper2. " A&Ntilde;O : ".$wanop."<B> ACTUALIZADO A : ".$fecha_cierre."</B></td></tr>";
			$wdat=array();
			$wdatt=array();
			$wanos=array();
