...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Validacion de Activos Sin Depreciacion</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc81.php Ver. 2016-03-10</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_rc81.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1)  or !isset($wcco1) or !isset($wcco2)  or $wper1 < 1 or $wper1 > 12)
		{
			...
			echo "<tr><td align=center colspan=2>VALIDACION DE ACTIVOS SIN DEPRECIACION</td></tr>";
		...
		else
		{
			...
			$query = "SELECT   Depcco, Depcod, Acfdes, sum(Depvde)  from ".$empresa."_000076,".$empresa."_000005,".$empresa."_000075";
			...
			$query = "SELECT Daccco, Daccod, Acfdes, sum(Dacpor)  from ".$empresa."_000084,".$empresa."_000075 ";
			$query = $query."   where Daccco between '".$wcco1."' and '".$wcco2."'";
			...
			echo "<tr><td colspan=8 align=center>VALIDACION DE ACTIVOS SIN DEPRECIACION</td></tr>";
			...
			echo "<tr><td><b>C.C.</b></td><td><b>ACTIVO</b></td><td><b>DESCRIPCION</b></td><td align=right><b>PORCENTAJE</b></td></tr>";
			$wdata=array();
			$k1=0;
			$k2=0;
			$num=-1;
			...
			for ($i=0;$i<=$num;$i++)
			{
				echo"<tr><td>".$wdata[$i][0]."</td><td>".$wdata[$i][1]."</td><td>".$wdata[$i][2]."</td><td align=right>".number_format((double)$wdata[$i][3],2,'.',',')."</td></tr>";
    		}
		}
	}
?>
