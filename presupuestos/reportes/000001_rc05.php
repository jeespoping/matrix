...
<!--
	function enter()
	{
		document.forms.rc05.submit();
	}
//-->
</script>
<center>
<table border=0 align=center>
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Estado de Resultados Comparativo Mes (Real)</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc05.php Ver. 2016-09-19</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form name= 'rc05' action='000001_rc05.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione"  or !isset($wper1)  or !isset($wper2) or ((!isset($wcco1)  or !isset($wcco2)) and !isset($wccof)) or !isset($wserv) or (strtoupper ($wserv) != "S" and strtoupper ($wserv) != "N") or !isset($wres)  or (strtoupper ($wres) != "R" and strtoupper ($wres) != "D") or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
		{
			...
			echo "<tr><td align=center colspan=2>ESTADO DE RESULTADOS COMPARATIVO MES (REAL)</td></tr>";
		...
		else
		{
			...
			{
			$query = "SELECT meccpr,mecmes,mganom,sum(mecval) as wmonto from ".$empresa."_000026,".$empresa."_000028 ";
			...
			$query = $query."   group by meccpr,mecmes,mganom";
			$query = $query."   order by meccpr,mecmes";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			echo "<table border=1>";
			...
			echo "<tr><td colspan=14 align=center>ESTADO DE RESULTADOS COMPARATIVO MES (REAL)</td></tr>";
