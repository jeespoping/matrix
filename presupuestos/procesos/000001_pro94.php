...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Calculo de Ingresos Promedio Laboratorio x CC (T17)</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro94.php Ver. 2015-09-25</b></font></tr></td></table>
</center>
<?php
...
else
{
	...
	if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wanob) or !isset($wper1) or !isset($wper2) or $wper1 < 1 or $wper1 > 12 or $wper2 < 1 or $wper2 > 12 or $wper1 > $wper2)
	{
		...
		echo "<tr><td align=center colspan=2>CALCULO DE INGRESOS PROMEDIO LABORATORIO X CC (T17)</td></tr>";
		echo "<tr>";
	...
	else
	{
		...
		if($num > 0 and $row[0] == "on")
		{
			...
			$query = $query." where Labano = ".$wanob;
			$query = $query."   and Labemp = '".$wemp."' ";
			$err = mysql_query($query,$conex) or die("ERROR EN BORRADO T17");
			
			$IPR=array();
			$query  = "SELECT igicco, sum(Igiinp) from ".$empresa."_000149 where Igiano = ".$wanob." and Igimes between ".$wper1." and ".$wper2." and Igiccd = '3081' ";
			$query .= "   Group by 1 ";
			$query .= "   order by 1 ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			...
			if ($num>0)
			{
				...
				for ($i=0;$i<$num;$i++)
				{
					...
					$query = "insert ".$empresa."_000017 (medico,fecha_data,hora_data,Labemp, Labano, Labcco, Labinp, Labter, Seguridad) values (";
					$query .=  "'".$empresa."','";
					$query .=  $fecha."','";
					$query .=  $hora."','";
					$query .=  $wemp."',";
