...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Seleccion Centros de Servicios Para Costos</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro61.php Ver. 2015-12-18</b></font></tr></td></table>
</center>
<?php
...
else
{
	...
	echo "<form action='000001_pro61.php' method=post>";
	...
	if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1) or $wper1 < 1 or $wper1 > 12)
	{
		...
		echo "<tr><td align=center colspan=2>SELECCION CENTROS DE SERVICIOS PARA COSTOS</td></tr>";
	...
	else
	{
		...
		$query = $query."    and Mgatip =  'SERVICIOS' ";
		$err = mysql_query($query,$conex);
		$query = "SELECT  Proccd, Procco, sum(Promon) from ".$empresa."_000073,".$empresa."_000005  ";
		$query = $query."  where Proano =  ".$wanop;
		$query = $query."    and Promes =  ".$wper1;
		...
		$query = $query."  group by Proccd, Procco " ;
		$query = $query."  order by  Proccd, Procco  " ;
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		$count=0;
		for ($i=0;$i<$num;$i++)
		{	
			...
			$query = "insert ".$empresa."_000092 (medico,fecha_data,hora_data,Mgaemp, Mgaano, Mgaper, Mgacco, Mgagas, Mgasga, Mgaval, Mgatip , seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$wper1.",'".$row[0]."','".$row[1]."','".$row[1]."',".$row[2].",'SERVICIOS','C-".$empresa."')";
