...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Replicacion de Porcentajes Distribucion Nomina</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro55.php Ver. 2018-03-28</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_pro55.php' method=post>";
		...
		if(!isset($wanop1) or !isset($wemp) or $wemp == "Seleccione" or !isset($wcco1) or !isset($wcco2) or !isset($wper1) or !isset($wanop2) or !isset($wper2))
		{
			...
			echo "<tr><td align=center colspan=2>REPLICACION DE PORCENTAJES DISTRIBUCION NOMINA</td></tr>";
		...
		else
		{
			...
			{
				$count=0;
				$query = "delete from ".$empresa."_000098 ";
				$query = $query."  where Pdiano = ".$wanop2;
				$query = $query."    and Pdiemp = '".$wemp."'";
				$query = $query."    and Pdimes = ".$wper2;
				$query = $query."    and Pdicco between '".$wcco1."' and '".$wcco2."' ";
				$err = mysql_query($query,$conex);
				$query = "SELECT Pdiano, Pdimes, Pdicco, Pdisub, Pdiofi, Pdipor, Pditip  from ".$empresa."_000098 ";
				$query = $query."  where Pdiano = ".$wanop1;
				$query = $query."    and Pdiemp = '".$wemp."'";
				$query = $query."    and Pdimes = ".$wper1;
				$query = $query."    and Pdicco = '".$wcco1."' and '".$wcco2."' ";
				$err = mysql_query($query,$conex);
				$num = mysql_num_rows($err);
				for ($i=0;$i<$num;$i++)
				{
					...
					$query = "insert ".$empresa."_000098 (medico,fecha_data,hora_data,Pdiemp, Pdiano, Pdimes, Pdicco, Pdisub, Pdiofi, Pdipor, Pditip, seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop2.",".$wper2.",'".$row[2]."','".$row[3]."','".$row[4]."',".$row[5].",'".$row[6]."','C-".$empresa."')";
