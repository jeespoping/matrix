...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Proyeccion Ingresos de Unidades Con Calculo Ingreso Promedio Forma Manual (T33)</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro110.php Ver. 2015-11-06</b></font></tr></td></table>
</center>
<?php
...
else
{
		...
		echo "<form action='000001_pro110.php' method=post>";
		...
		if(!isset($wanop) or !isset($wper1) or $wper1 < 1 or $wper1 > 12  or !isset($wemp) or $wemp == "Seleccione")
		{
			...
			echo "<tr><td align=center colspan=2>PROYECCION INGRESOS DE UNIDADES CON CALCULO INGRESO PROMEDIO FORMA MANUAL (T33)</td></tr>";
			echo "<tr>";
		...
		else
		{
			...
			if($num > 0 and $row[0] == "on")
			{
				...
				$err = mysql_query($query,$conex);
				//                 0      1      2               3                        4
				$query =" select Mopano,Mopmes,Mopcco,sum(Ipminp * Mopcan),sum(Ipminp * Mopcan * (1 - Ipmpte/100)) ";
				$query = $query." from ".$empresa."_000041,".$empresa."_000031  ";
				$query = $query." where Ipmano = ".$wanopa; 
				$query = $query."   and Ipmemp = '".$wemp."' ";
				$query = $query."   and Ipmemp = Mopemp  ";
				$query = $query."   and Ipmcco = Mopcco  ";
				$query = $query."   and Ipmgru = Mopcod  ";
				$query = $query."   and Mopano = ".$wanop;
				$query = $query." group by Mopano,Mopmes,Mopcco ";
				...
				if($num > 0)
				{
					for ($i=0;$i<$num;$i++)
					{
						...
						$query = "insert ".$empresa."_000033 (medico,fecha_data,hora_data,Dipemp, Dipano, Dipmes, Dipcco, Dipip1, Dipit1, Dipip2, Dipit2, Dipip3, Dipit3, Diptip, seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$row[1].",'".$row[2]."',".round($wip1, 0).",".round($wit1, 0).",".round($wip2, 0).",".round($wit2, 0).",".round($wip3, 0).",".round($wit3, 0).",'MA','C-".$empresa."')";
