...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Calculo Costo x Actividad (CP)</font></a></tr></td>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro185.php Ver. 2015-11-20</b></font></tr></td></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		echo "<form action='000001_pro185.php' method=post>";
		...
		if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1) or !isset($wcco1) or !isset($wcco2) or $wper1 < 1 or $wper1 > 12)
		{
			...
			echo "<tr><td align=center colspan=2>CALCULO COSTO X ACTIVIDAD (CP)</td></tr>";
		...
		else
		{
			...
			{
				$DATA=array();
				$query = "SELECT Cxpano, Cxpmes, Cxpcco, Cxpsub, Cxpest from ".$empresa."_000154 ";
				$query = $query."  where Cxpano = ".$wanop;
				$query = $query."    and Cxpemp = '".$wemp."'";
				$query = $query."    and Cxpmes = ".$wper1;
				$query = $query."    and Cxpcco between '".$wcco1."' and '".$wcco2."'";
				$err = mysql_query($query,$conex);
				$num = mysql_num_rows($err);
				if($num > 0)
				{
				...
				$ext=$num;
				$query = "delete from ".$empresa."_000154 ";
				$query = $query."  where Cxpano = ".$wanop;
				$query = $query."    and Cxpemp = '".$wemp."'";
				$query = $query."    and Cxpmes = ".$wper1;
				$query = $query."    and Cxpcco between '".$wcco1."' and '".$wcco2."'";
				...
				echo "<tr><td colspan=2 align=center>ERRORES EN CALCULO COSTO X ACTIVIDAD (CP)</td></tr>";
				...
				$REG=array();
				$query = "SELECT Mdpcco, Mdpsub  from ".$empresa."_000157 ";
				$query = $query."  where Mdpano = ".$wanop;
				$query = $query."    and Mdpemp = '".$wemp."'";
				$query = $query."    and Mdpmes = ".$wper1;
				$query = $query."    and Mdpcco between '".$wcco1."' and '".$wcco2."'";
				$query = $query."   group by Mdpcco, Mdpsub  ";
				$query = $query."   order by Mdpcco, Mdpsub  ";
				$err = mysql_query($query,$conex);
				$num = mysql_num_rows($err);
				$numreg=$num;
				for ($i=0;$i<$num;$i++)
				...
				{
					$row = mysql_fetch_array($err);
					actualizarreg(&$REG,$row[0],$row[1],$numreg);
					$query = "SELECT Mdpcan from ".$empresa."_000157 ";
					$query = $query."  where Mdpano = ".$wanop;
					$query = $query."    and Mdpemp = '".$wemp."'";
					$query = $query."    and Mdpmes = ".$wper1;
					$query = $query."    and Mdpcco = '".$row[0]."'";
					$query = $query."   and Mdpsub = '".$row[1]."'";
					...
					if($num2 > 0)
					{
						for ($j=0;$j<$num2;$j++)
						{
							...
							if($row2[0] != 0)
							{
								...
								$query = "insert ".$empresa."_000154 (medico,fecha_data,hora_data,Cxpemp, Cxpano, Cxpmes, Cxpcco, Cxpsub, Cxpcos, Cxpest, Cxppro, seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$wper1.",'".$row[0]."','".$row[1]."',".$wpor.",'".$pos."',0,'C-".$empresa."')";
				...
				echo "<br><br>REGISTROS SIN COSTOS EN TABLA 154<br>";
				for ($i=0;$i<$numreg;$i++)
				{
					...
					if($REG[$i][2] == 0)
					{
						...
						$query = "insert ".$empresa."_000154 (medico,fecha_data,hora_data,Cxpemp, Cxpano, Cxpmes, Cxpcco, Cxpsub, Cxpcos, Cxpest, Cxppro, seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$wper1.",'".$REG[$i][0]."','".$REG[$i][1]."',0,'".$pos."',0,'C-".$empresa."')";
						$err1 = mysql_query($query,$conex);
						if ($err1 != 1)
							echo mysql_errno().":".mysql_error()."<br>";
						$count++;
