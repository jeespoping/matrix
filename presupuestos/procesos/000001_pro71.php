...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5>Consolidacion de Protocolos</font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro71.php Ver. 2016-05-19</b></font></td></tr></table>
</center>
...
if(!isset($_SESSION['user']))
...
else
	{
		...
		else
		{
			...
			//                  0      1      2      3     4      5      6      7
			$query = "select Mprcco,Mprpro,Mprgru,Mprpor,Conccn,Conpro,pcactp,Mprcon ";
			$query = $query." from ".$empresa."_000097,".$wtable;
			$query = $query." where Pcaano = ".$wanop;
			$query = $query."   and Pcaemp = '".$wemp."'";
			$query = $query."   and Pcames = ".$wper1;
			$query = $query."   and Pcacco = Conccn ";
			$query = $query."   and Pcacod = Conpro ";
			$query = $query."   and Pcagru = Congrp ";
			$query = $query."   and Pcacon = Concoa ";
			...
			for ($i=0;$i<$num;$i++)
			{
				...
				if($row[0].$row[1].$row[2].$row[7] != $key1)
				{
					if($i > 0)
					{
						...
						$query = "insert ".$empresa."_000097 (medico,fecha_data,hora_data,Pcaemp, Pcaano, Pcames, Pcacco, Pcacod, Pcagru, Pcacon, Pcapor, Pcactp, Pcatmn, Pcapro,Pcatip,seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$wper1.",'".$kcco."','".$kpro."','".$kgru."','".$kcon."',".$wpor.",".$wtotal.",".$wtmn.",0,'O','C-".$empresa."')";
