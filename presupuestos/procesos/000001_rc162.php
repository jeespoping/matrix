...
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_rc162.php Ver. 2013-08-22</b></font></tr></td></table>
</center>
<?php
...
else
{
		...
		if(!isset($wanop) or !isset($wper1) or $wper1 < 1 or $wper1 > 12 or !isset($wper2) or $wper2 < 1 or $wper2 > 12 or !isset($ok) or $wnoway == 0 or $wnoway == 1)
		{
			...
			echo "<tr><td align=center colspan=2>EVALUACION DE CONVENIOS</td></tr>";
			echo "<tr>";
			...
			$items[0][1]="SELECT Empgru from costosyp_000061 where Empgru like '%";
			$items[0][2]="%' group by 1 order by 1";
			$items[0][3]="SELECT Empgru from costosyp_000061 group by 1 order by 1";
			$items[0][4]=1;
			$items[1][0]="Segmentos";
			$items[1][1]="SELECT Segcod from costosyp_000045 where Segcod like '%";
			$items[1][2]="%' group by 1 order by 1";
			$items[1][3]="SELECT Segcod from costosyp_000045 group by 1 order by 1";
			$items[1][4]=1;
			$items[2][0]="Empresas";
			$items[2][1]="SELECT Empcin,Empdes from costosyp_000061 where Empdes like '%";
			$items[2][2]="%' group by 1 order by 2";
			$items[2][3]="SELECT Empcin,Empdes from costosyp_000061 group by 1 order by 2";
			$items[2][4]=2;
			$items[3][0]="Lineas";
			$items[3][1]="SELECT Lincod,Lindes from costosyp_000107 where Lindes like '%";
			$items[3][2]="%' group by 1 order by cast(Lincod as UNSIGNED)";
			$items[3][3]="SELECT Lincod,Lindes from costosyp_000107 group by 1 order by cast(Lincod as UNSIGNED)";
			$items[3][4]=2;
			$items[4][0]="Centros de Costos";
			$items[4][1]="SELECT Ccocod, Cconom from costosyp_000005 where Ccoclas='PR' and Cconom like '%";
			$items[4][2]="%' group by 1 order by 1";
			$items[4][3]="SELECT Ccocod, Cconom from costosyp_000005 where Ccoclas='PR' and Ccoest='on' group by 1 order by 1";
			$items[4][4]=2;
			$items[5][0]="Concepto de Facturacion";
			$items[5][1]="SELECT Cfacod,Cfades from costosyp_000060 where Cfades like '%";
			$items[5][2]="%' group by 1 order by 1";
			$items[5][3]="SELECT Cfacod,Cfades from costosyp_000060 group by 1 order by 1";
			$items[5][4]=2;
		...
		{
			$evaco1=array();
			$query = "SELECT Segcod, Segdes from costosyp_000045 group by 1 order by 1";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			$tot1=$num;
			for ($i=0;$i<$num;$i++)
			...
			$evaco2=array();
			$query = "SELECT Empcin,Empdes from costosyp_000061 group by 1 order by 1";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			$tot2=$num;
			for ($i=0;$i<$num;$i++)
			...
			$evaco3=array();
			$query = "SELECT Lincod,Lindes from costosyp_000107 group by 1 order by 1";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			$tot3=$num;
			for ($i=0;$i<$num;$i++)
			...
			$evaco4=array();
			$query = "SELECT Ccocod, Cconom from costosyp_000005 where Ccoclas='PR' and Ccoest='on' group by 1 order by 1";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			$tot4=$num;
			for ($i=0;$i<$num;$i++)
			...
			$evaco5=array();
			$query = "SELECT Cfacod,Cfades from costosyp_000060 group by 1 order by 1";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			$tot5=$num;
			for ($i=0;$i<$num;$i++)
			...
			$query .= " select Empcin,Empseg,Empgru from costosyp_000061 group by 1,2,3 ";
			$err = mysql_query($query,$conex) or die(mysql_errno().":".mysql_error());
			...
			if(strpos($query,"Emp") !== false)
				$query .= ",sum(Mosipr),sum(Mosite),sum(Mosctt),sum(Mosutt),sum(Mosctv),sum(Mosutv) FROM costosyp_000108,tbis WHERE Mosano=".$wanop." and Mosmes between ".$wper1." and ".$wper2." and Mosent = Empcin AND Mosest='on' ";
			else
				$query .= ",sum(Mosipr),sum(Mosite),sum(Mosctt),sum(Mosutt),sum(Mosctv),sum(Mosutv) FROM costosyp_000108 WHERE Mosano=".$wanop." and Mosmes between ".$wper1." and ".$wper2." and Mosest='on' ";
			for ($i=0;$i<count($orden);$i++)
			{
				if($orden[$i][3] != "Todos")
				{
			...
			if($num1 > 0)
			{
				...
				for ($j=0;$j<$num1;$j++)
				{
					...
					$margenT = $row1[$z+3] / $row1[$z];
					$margenC = $row1[$z+5] / $row1[$z];
					...
					echo "<td align=right>".number_format((double)$margenD,1,'.',',')."</td>";
					echo "</tr>";
				}
			}
		}
