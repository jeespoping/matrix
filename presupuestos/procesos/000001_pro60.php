...
<tr><td align=center bgcolor="#cccccc"><A NAME="Arriba"><font size=5><b>Seleccion Gastos Indirectos Para Costos</b></font></a></td></tr>
<tr><td align=center bgcolor="#cccccc"><font size=2> <b> 000001_pro60.php Ver. 2015-12-18</b></font></tr></td></table>
</center>
<?php
...
else
{
	...
	echo "<form action='000001_pro60.php' method=post>";
	...
	if(!isset($wanop) or !isset($wemp) or $wemp == "Seleccione" or !isset($wper1) or $wper1 < 1 or $wper1 > 12)
	{
		...
		echo "<tr><td align=center colspan=2>SELECCION GASTOS INDIRECTOS PARA COSTOS</td></tr>";
	...
	else
	{
		...
		$query = $query."  and Mgatip =  'INDIRECTOS' ";
		$err = mysql_query($query,$conex);
		$query = "SELECT  Mdicco, Mdiind, sum(Mdimon) from ".$empresa."_000054,".$empresa."_000005  ";
		...
		$query = $query."  group by   Mdicco, Mdiind " ;
		$query = $query."  order by   Mdicco, Mdiind  " ;
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		$count=0;
		for ($i=0;$i<$num;$i++)
		{	
			...
			$query = "insert ".$empresa."_000092 (medico,fecha_data,hora_data,Mgaemp, Mgaano, Mgaper, Mgacco, Mgagas, Mgasga, Mgaval, Mgatip , seguridad) values ('".$empresa."','".$fecha."','".$hora."','".$wemp."',".$wanop.",".$wper1.",'".$row[0]."','".$row[1]."','".$row[1]."',".$row[2].",'INDIRECTOS','C-".$empresa."')";
