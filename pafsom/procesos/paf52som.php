...
echo "<tr><td align=center bgcolor=#DDDDDD colspan=1><b><font text color=#003366 size=3><i>COOMEVA - CLINICA LAS AMERICAS</font></b><br>";

     $query="SELECT atefec,atehor,ateced,atenom,atehis,atecar,ateord,atecit,atecon,ateex1,ateex2,atemed,atefpr,atehpr,ateobs From pafsom_000003 "
     ...
     if ($nroreg > 0)      //  Encontro 
     {
      ...
      // TOMO EL NOMBRE DEL MEDICO
        $query = "SELECT Descripcion FROM citasom_000010 WHERE codigo='".$registro[11]."' Group By Descripcion"; 
        $resultado=mysql_query($query); 
        $nroreg = mysql_num_rows($resultado);
        if ($nroreg > 0)      //  Encontro 
        {
