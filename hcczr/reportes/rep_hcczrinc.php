...
else
{
	...
	{
		echo "<form action='rep_hcczrinc.php?empresa=$empresa' method=post>";
		...
		if(isset($medico) and isset($pac1))
		{
			...
			$query="select distinct Paciente from ".$empresa."_000006 where Cirujano='".$medico."'and Paciente like '%".$pac1."%' order by Paciente";
		...
		if(isset($pac))
		{
			...
			$query = "select Fecha_inicio  from ".$empresa."_000006 where Paciente='".$pac."' and Cirujano='".$medico."' ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
	...
	{	
		...
		echo "<table align=left border=0 width=725 ><tr><td rowspan=4 align='center' colspan='0'>";
		...
		echo "</tr><tr><td  align=center colspan='4'><font size=2 color='#000080' face='arial'><B>".$nomed."</font>";
		echo "<BR>Reg.:</b>".$reg."</b></font></tr>";
		echo "<br><tr><td colspan=4><HR></td></tr>";
		//               0          1              2            3          4          5
		$query="select fecha,Tipo_incapacidad,Tipo_riesgo,Fecha_inicio,Numero_dias,Prorroga	 from	".$empresa."_000006 where Paciente='".$pac."' and Cirujano='".$medico."' and Fecha_inicio = '".$fecha1."' order by Fecha";
		$err = mysql_query($query,$conex);
		if($err)
		{
			$row = mysql_fetch_array($err);
			
			echo "<tr><td colspan=4 ><b>Nombre: </b>".$pacn."</td></tr>";
			echo "<tr><td colspan=4 ><b>Identificacion: </b>".$td."&nbsp&nbsp".$doc."</td></tr>";
			echo "<tr><td colspan=4><b>Historia Clinica: </b>".$nrohist."</td></tr>";
			echo "<tr><td colspan=4><b>Diagnostico: </b>".$row1[0]."</td></tr>";
			$tincap=explode("-",$row[1]);
			echo "<tr><td colspan=4><b>Tipo Incapacidad: </b>".$tincap[1]."</td>";
			$triesgo=explode("-",$row[2]);
			echo "<tr><td colspan=4><b>Tipo riesgo: </b>".$triesgo[1]."</td>";
			echo "<tr><td colspan=4><b>Fecha inicio: </b>".$row[3]."</td>";
			echo "<tr><td colspan=4><b>Numero dias: </b>".$row[4]."</td>";
			$dias=$row[4] - 1;
			$fecfinal= date("Y-m-d", strtotime("$row[3] + $dias days")); 
			echo "<tr><td colspan=4><b>Fecha Final: </b>".$fecfinal."</td>";
			$prorroga=explode("-",$row[5]);
			echo "<tr><td colspan=4><b>Prorroga: </b>".$prorroga[1]."</td>";
			echo "<br><tr><td colspan=4><HR></td></tr>";
			
		}
				
		//echo "<br><table align=left border=0 width=725 >";
			echo "&nbsp&nbsp<br>";
			echo "&nbsp&nbsp<br>";
