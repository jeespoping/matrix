	...
	switch($accion)
	{
			...
			if ($modeloconolgura=='no')
			{
				...
				else
				{
						...
						$insert =	"INSERT  ".$wbasedatoimpuesto."_000012 
									  ...
									  VALUES	 ('impuesto' , '".$wfecha."' , 	'".$whora."',   '".$wconsecutivo_grabar."'  	, '".$wfechaprocesoaux[2]."-".$wfechaprocesoaux[1]."-".$wfechaprocesoaux[0]."' 	,  '".$widentificacion."'  	, '".$wnombre."' , '".$wdireccion."'	, '".$wciudad."'	, '".$wtelefono."'	,  '".$wconcepto."'	,	'".$wvalor."'	,	'".$wtarifa."'	,	'".$wtope."'		,	'".$wimpuesto."' 	, 	'".$wtotal."'	,	'".$wobservaciones."'	,	'C-".$wusuario."'	)";
		...
		case "traer_datos" :
		{
			...
			if ($num>0)
			{
				...
				$respuesta['observaciones']=$row[16];
 ...
 <body>
		<!-- LO QUE SE MUESTRA AL INGRESAR POR PRIMERA VEZ -->
			<?php
					...
					while($row = mysql_fetch_array($res))
					{
						$select_tope .="<option value='".$row['subcodigo']."-".$row['descripcion']."'>".$row['subcodigo']."-".$row['descripcion']."</option>";	
					}
					$select_tope .="</select>";
					//------------------------
