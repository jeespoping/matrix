...
include_once("conex.php");
	...
	function consultarRequerimiento($req, $cco, &$tipreq, &$clareq, &$temreq, &$resreq, &$desreq, &$fecap, &$horap, &$porcen, &$fecen, &$horen, &$estreq, &$prireq, &$codigo, &$obsreq, &$fecreq, &$recreq, &$ccoreq, &$acureq, &$horreq, $id, &$wcodigo_caso)
	{
		global $conex;
		global $wbasedato, $id_req;

		/*$q= " SELECT Reqcco, Reqnum, Reqtip, Reqfec, Requso, Requrc, Reqdes, Reqpurs, Reqpri, Reqest, Reqcla, Reqfae, Reqhae, Reqcum, Hora_data, Reqfen, Reqobe, Reqtde, Reqhen, Reqtpn, Reqsat "
		."       FROM ".$wbasedato."_000040 "
		."    WHERE Reqnum = '".$req."' "
		."       AND Reqcco = '".$cco."' "
		."       AND Reqtip = '".$id."' ";*/
	...
	function almacenarSeguimiento($codigo, $ccoreq, $acureq, $req, $seg, $env, $segnum, $segest, $tipreq, $id, $wusuario_creacaso, $wcodigo_caso)
	{
		global $conex;
		global $wbasedato;
		...
		if(trim($seg) != '')
		{
			...
			$q= " INSERT INTO ".$wbasedato."_000045 (          Medico,            Fecha_data,                   Hora_data,              Segfec,                                         Segcco ,     Segreq,
																  Segtpn,                                         Segtip,         Segnum,        Segpcu,    Segtxt,     Segenv,        Segusu,        Segest,               Segmcr,                 Segmen,   Seguridad) "
			."                               VALUES ('".$wbasedato."',   '".date('Y-m-d')."', '".(string)date("H:i:s")."', '".date('Y-m-d')."', mid('".$ccoreq."',1,instr('".$ccoreq."','-')-1), '".$req."',
													 '".$wcodigo_caso."', mid('".$tipreq."',1,instr('".$tipreq."','-')-1), '".$segnum."', '".$acureq."', '".$seg."','".$env."', '".$codigo."', '".$segest."', '".$esmsj_creador."', '".$esmsj_encargado."', 'C-".$codigo."') ";
	...
	function actualizarRequerimiento(&$req, $ccoreq, $tipreq, $clareq, $temreq, $recreq, $resreq, $prireq, $acureq, &$estreq, $fecen, $horen, $obsreq, $fecap, $horap, &$id, $wusuario_creacaso, $identificador_unico_req)
	{
		global $conex;
		global $wbasedato;
		...
		if($id!=$exp[0])
		{
			...
			// $err = mysql_query($q,$conex)or die (mysql_errno()." -NO SE HAN PODIDO ACTUALIZAR LOS SEGUIMIENTOS CON SU NUMERACION".mysql_error());
	...
	function consultarSeguimientos($req, $cco, $usucco, $tipreq, $id, $wcodigo_caso)
	{
		...
		if ($exp[0]==$cco)
		{
			$q= " SELECT Segfec, Segnum, Segpcu, Segtxt, Segusu, Segest, Descripcion, Hora_data, seg.id AS id_seg "
	...
	function pintarRequerimiento($usucco, $req, $cco, $tipos, $clases, $tiempos, $responsables, &$desreq, $fecap, $horap, &$porcen, &$fecen, &$horen, $estados, $prioridades, &$codigo, &$obsreq, &$fecreq, $recreq, &$ccoreq, $porcentaje, $wusuario, $receptores, $seguimientos, $especiales, $horreq, $cerrado, $id, $wcodigo_caso, $wusuario_creacaso, &$estado_cerrado, $satisfaccion,$resreq)
	{
		//echo '=> usucco:'.$usucco. '=> req:'.$req. '=> cco:'.$cco. '=> tipos:'.$tipos. '=> clases:'.$clases. '=> tiempos:'.$tiempos. '=> responsables:'.$responsables. '=> desreq:'.$desreq. '=> fecap:'.$fecap. '=> horap:'.$horap. '=> porcen:'.$porcen. '=> fecen:'.$fecen. '=> horen:'.$horen. '=> estados:'.$estados. '=> prioridades:'.$prioridades. '=> codigo:'.$codigo. '=> obsreq:'.$obsreq. '=> fecreq:'.$fecreq. '=> recreq:'.$recreq. '=> ccoreq:'.$ccoreq. '=> porcentaje:'.$porcentaje. '=> wusuario:'.$wusuario. '=> receptores:'.$receptores. '=> seguimientos:'.$seguimientos. '=> especiales:'.$especiales. '=> horreq:'.$horreq. '=> cerrado:'.$cerrado. '=> id:'.$id. '=> wcodigo_caso:'.$wcodigo_caso;

		global $conex;
		global $id_req;
		global $wbasedato;
		global $ids_segs_pte;
		global $msj_para_creador, $id_req, $cco_auditoria_corporativa_clinica;
		...
		echo "<tr class='fila2'><td class=''  align='center'>Centro de costos: ".$ccoreq."</td>";
		...
		{
			echo "<td class='' align='center'>Tipo de requerimiento:";
		...
		{
			echo "<td class='' align='center'>Tipo de requerimiento: ".$tipos[0];
	...
	function pintarBoton($cco, $req, $ccoreq, $id,$estado_cerrado)
	{
		...
		if($numEstados > 0){
			// if($estado_cerrado=="off" || $wusuario_creacaso == $wusuario)
	...
	else
	{
		...
		if ((!isset($enviar) and (!isset($grabar) or $grabar==0))) //primera vez que se ingresa al programa o tiene el valor de ser con botones de ingreso de datos
		{
			...
			if (!isset($ccoreq))
			{
				consultarRequerimiento($req, $cco, $tipreq, $clareq, $temreq, $resreq, $desreq, $fecap, $horap, $porcen, $fecen, $horen, $estreq, $prireq, $codigo, $obsreq, $fecreq, $recreq, $ccoreq, $acureq, $horreq, $id, $wcodigo_caso);
			}
			...
			pintarRequerimiento($usucco, $req, $cco, $tipos, $clases, $tiempos, $responsables, $desreq, $fecap, $horap, $porcen, $fecen, $horen, $estados, $prioridades, $codigo, $obsreq, $fecreq, $recreq, $ccoreq,  $porcentaje, $wusuario, $receptores, $seguimientos, $especiales, $horreq, $cerrado, $id, $wcodigo_caso, $wusuario_creacaso, $estado_cerrado, $satisfaccion,$resreq);
			...
			// if ($cerrado != 'on' && $wusuario == $wusuario_creacaso) //$usucco==$ccoreq and
			// {
				// global $usucco, $ccoreq;
				// pintarBoton($cco, $req, $ccoreq, $id,$estado_cerrado);
			// }
		...
		// }
			...
			actualizarRequerimiento($req, $ccoreq, $tipreq, $clareq, $temreq, $recreq, $resreq, $prireq, $porcen, $estreq, $fecen, $horen, $obsreq, $fecap, $horap, $id, $wusuario_creacaso, $wcodigo_caso);
			almacenarSeguimiento($wusuario, $ccoreq, $porcen, $req, $seg, $enviar, $segnum, $estreq, $tipreq, $id, $wusuario_creacaso, $wcodigo_caso);
