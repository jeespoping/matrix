...
<head>
  ...
  <style type="text/css">
   ...
   <script type="text/javascript">
	...
	//	$.each($('.AlinearInputs'),function() { if ($(this).val().length == 0) { alert('empty'); } }); 
...
// 		$.each($('.AlinearInputs'),function() { 
// 			if ($(this).val().length == 0) 
// 			{ 
// 				alert('No ha ingreseado ninguna cantidad');
// 				return false;
// 			}
// 			return true; 
// 		});
//    }
   ...
   function validarMsj()
   {
        ...
        //var textoseg = document.informatica.desreq.value;
   		//if(textoseg.replace(/\s/g,'') == "")
   		//{
   			//alert("[!] Escriba el texto de la descripcion.");
			//return false;
   		//}
   		return true;
   }
...
function consultarUsuarioReq($codigo, $cco)
{
	...
	if ($cco=='')
	{
		$q= " SELECT Empresa, Ccostos, Usuext, Usuema, Usucar, Ususup, Descripcion "
	...
	if ($num>0)
	{
		for ($i=1;$i<=$num;$i++)
		{
			...
			//{
				...
				$q= " SELECT Usuext, Usuema, Usucar, Ususup, Usucod, Descripcion "
				."       FROM ".$wbasedato."_000039, usuarios "
...
function almacenarUsuario2($codigo, $cco, $car, $ext, $email)
{
    global $conex;
    global $wbasedato;
    ...
    {
       echo $q= " update ".$wbasedato."_000039 set Usuext='".$ext."', Usuema='".$email."', Usucar='".$car."' "
    ...
    else
    {
        ...
        {
            echo$q= " INSERT INTO ".$wbasedato."_000039 (   Medico       ,   Fecha_data,                  Hora_data,              Usucod,      Usucco ,     Usuext   ,    Usuema   ,  Usucar   ,   Ususup ,         Usuest    , Seguridad        ) "
            ."                               VALUES ('".$wbasedato."',   '".date('Y-m-d')."', '".(string)date("H:i:s")."','".$codigo."', '".$cco."', '".$ext."' , '".$email."' , '".$car."', 'off'  , 'on' ,  'C-".$codigo."') ";
...
function almacenarUsuario($codigo, $cco, $car, $ext, $email)
{
    global $conex;
    global $wbasedato;
    ...
    if(mysql_num_rows($res1) > 0)
    {
        $q= " update ".$wbasedato."_000039 set Usuext='".$ext."', Usuema='".$email."', Usucar='".$car."'"
    ...
    else
    {
        ...
        if(mysql_num_rows($res2) == 0)
        {
            $q= " INSERT INTO ".$wbasedato."_000039 (   Medico       ,   Fecha_data,                  Hora_data,              Usucod,      Usucco ,     Usuext   ,    Usuema   ,  Usucar   ,   Ususup ,         Usuest    , Seguridad        ) "
            ."                               VALUES ('".$wbasedato."',   '".date('Y-m-d')."', '".(string)date("H:i:s")."','".$codigo."', '".$cco."', '".$ext."' , '".$email."' , '".$car."', 'off'  , 'on' ,  'C-".$codigo."') ";
            $err = mysql_query($q,$conex) or die (mysql_errno()." -NO SE HAN PODIDO GUARDAR SUS DATOS EN EL MÓDULO DE REQUERIMIENTOS ".mysql_error());
        }
        else
...
function almacenarRequerimiento($cco, $usuNombre, $usucod, $ccoreq, $tipreq, $clareq, $temreq, $resreq, $desreq, $fecap, $horap, $porcen, $fecen, $horen, $estreq, $prireq, $codigo, $obsreq, $email)
{
	global $conex;
	global $wbasedato;
    ...
    // $numeracion_unica = ($cod_tipo_req_exp[0].$reqnum) * 1;
		...
		$q= " INSERT INTO ".$wbasedato."_000040 (          Medico,            Fecha_data,                   Hora_data,                                          Reqcco,       Reqnum ,                                          Reqtip,                  Reqtpn,              Reqfec,        Requso,                                          Requrc,        Reqdes,                                     Reqpurs,                                              Reqpri,       Reqfae,        Reqhae,                                           Reqest,        Reqcum,       Reqfen,   Reqhen,      Reqobe,                                           Reqtde,                                             Reqcla, Reqccs,      Seguridad) "
		."                               VALUES ('".$wbasedato."',   '".date('Y-m-d')."', '".(string)date("H:i:s")."', mid('".$ccoreq."',1,instr('".$ccoreq."','-')-1), '".$reqnum."', mid('".$tipreq."',1,instr('".$tipreq."','-')-1), '".$numeracion_unica."', '".date('Y-m-d')."', '".$usucod."', mid('".$recreq."',1,instr('".$recreq."','-')-1), '".$desreq."', mid('".$resreq."',1,instr('".$resreq."','-')-1), mid('".$prireq."',1,instr('".$prireq."','-')-1), '".$fecap."',  '".$horap."', mid('".$estreq."',1,instr('".$estreq."','-')-1) , '".$porcen."', '".$fecen."', '".$horen."', '".$obsreq."',  mid('".$temreq."',1,instr('".$temreq."','-')-1),   mid('".$clareq."',1,instr('".$clareq."','-')-1) ,'".substr($cco,0,8)."', 'C-".$codigo."') ";

		if($err = mysql_query($q,$conex))
		{
	        //       "Medico;Fecha_data;Hora_data;Reqcco;Reqnum ;Reqtip;Reqfec;Requso;Requrc;Reqdes;Reqpurs;Reqpri;Reqfae;Reqhae;Reqest;Reqcum;Reqfen;Reqhen;Reqobe;Reqtde;Reqcla;Seguridad";
            // ."".$wbasedato.";".date(Y-m-d).";".(string)date("H:i:s").";mid(".$ccoreq.";1;instr(".$ccoreq.";-)-1);".$reqnum.";mid(".$tipreq.";1;instr(".$tipreq.";-)-1);".date(Y-m-d).";".$usucod.";mid(".$recreq.";1;instr(".$recreq.";-)-1);".$desreq."; mid(".$resreq.";1;instr(".$resreq.";-)-1);mid(".$prireq.";1;instr(".$prireq.";-)-1);".$fecap.";".$horap.";mid(".$estreq.";1;instr(".$estreq.";-)-1);".$porcen.";".$fecen.";".$horen.";".$obsreq.";mid(".$temreq.";1;instr(".$temreq.";-)-1);mid(".$clareq.";1;instr(".$clareq.";-)-1);C-".$codigo.")";
...
*/
function pintarUsuario($usuario, $sup, $centros, $personas)
{
...
function pintarRequerimiento($usuario ,$costos,  $tipos, $clases, $responsables, $prioridades, $fecap, $horap, $estados, $tiempos, $porcentaje, $fecen, $horen, $especiales, $desreq, $perf, $clareq, $cco)
{
	...
	echo "<table border=0 ALIGN=CENTER width=90%>";
	// echo "<tr><td colspan=2 class='encabezadoTabla' align='center'><b>Ingreso de Nuevo Requerimiento</b></td></tr>";
	...
	// echo "<tr class='fila2'><td style='font-weight:bold; ' class='' align='center' colspan='2'>Centro de costos:";
	...
	if ($clases!='')
	{
		...
		else
		{
			...
			if($perf)
			{
				...
				// echo "<td style='font-weight:bold; ' class='' align='center' colspan='".$colspan."'>Asignado a:";
...
else
{
  ...
  //
	...
	if (!isset($enviar) or !isset($cco) or $cco=='' or !isset($ccoreq) or $ccoreq=='' or !isset($tipreq) or $tipreq=='' or !isset($desreq) or $desreq=='' || !isset($ok)) //primera vez que se ingresa al programa o tiene el valor de ser con botones de ingreso de datos
	{
		...
		if ($usuario['sup']=='on')
		{
			...
			if (!isset($perbus))
			{
				...
				{
					pintarUsuario($usuario, 'on', $centros, $personas);
				}
			}
			else
			{
				if ($perbus!='')
				{
					...
					if (!$persona)
					{
						...
						{
							//pintarAlert2('EL USUARIO NO ESTA RELACIONADO CON NINGUN CENTRO DE COSTOS, POR FAVOR COMUNICARSE CON SISTEMAS');
							pintarUsuario($usuario, 'on', '', $personas);
						}
						else
						{
							pintarUsuario($usuario, 'on', $centros, $personas);
					...
					{
						$centros=consultarCentros($persona['cod'], $persona['cco']);
						if(!$centros)
						{
							//pintarAlert2('EL USUARIO NO ESTA RELACIONADO CON NINGUN CENTRO DE COSTOS, POR FAVOR COMUNICARSE CON SISTEMAS');
							pintarUsuario($persona, 'on', '', $personas);
						}
						else
						{
							pintarUsuario($persona, 'on', $centros, $personas);
						}
					}
				}
				else
		...
		else
		{
			...
			{
				pintarUsuario($usuario, 'off', $centros, '');
			}
		}
		...
		pintarRequerimiento(0, $costos, $tipos, $clases, $responsables, $prioridades, date('Y-m-d'), date('H:i:s'), $estados, $tiempos, $porcentaje, date('Y-m-d'), date('H:i:s'), $especiales, $desreq, $perf, $clareq, $usuario['cco']);
	...
	else //se almacena todo el requerimiento y se vuelve a pintar sin opcion de cambio
	{
		...
		if (!isset($per) or $per=='')
		{
			almacenarUsuario($usuario['cod'], $cco, $car, $ext, $email);
			$usuCod=$usuario['cod'];
		...
		{
			$exp=explode('-', $per);
			almacenarUsuario($exp[0], $cco, $car, $ext, $email);
			$usuCod=$exp[0];
            $usuNombre = $exp[1];
		}
        ...
        if(is_array($resreq))
        {
            ...
            {
                $resreq = $value;
                $reqnum=almacenarRequerimiento($cco, $usuNombre, $usuCod, $ccoreq, $tipreq, $clareq, $temreq, $resreq, $desreq, $fecap, $horap, $porcen, $fecen, $horen, $estreq, $prireq, $usuario['cod'] , $obsreq, $email);
                if (isset($especiales))
                {
                    if($perf)
