...
class RealAjaxUploader
{
	...
	private function uploadAjax()
	{
		...
		$this->file_name = utf8_decode($this->file_name);
		...
		$this->file_name = str_replace( $caracteres, $caracteres2, $this->file_name );		
		...
		//Consulto si el archivo ya fue publicado par el grupo.
		$query_pro = " SELECT id
						 FROM root_000102
						WHERE Actgru = '".$grupo."'
						  AND Actnom = '".$this->file_name."'";
		$res_pro = mysql_query( $query_pro, $conex) or die( mysql_errno()." - Error en el query $query_pro - ".mysql_error() );
		$num_pro = mysql_num_rows($res_pro);
		
		if($num_pro == 0){
		
		//Hacer el registro de subida de archivos
		$q = " INSERT INTO root_000102 (    Medico ,Fecha_data   , Hora_data  ,       Actnom          ,    Actgru    , Actest  ,    Seguridad     ) "
					."          VALUES (    'root' ,'".$wfecha."','".$whora."','".$this->file_name."' , '".$grupo."' ,  'on'   , 'C-".$wusuario."')";
		...
		else
		{
			...
			//Si el archivo existe lo elimino y luego subo el nuevo, para que la respuesta sea correcta, doy a $ret el valor de 1.
			if(!$ret)
			{			
			...
			if($ret)
			{
				...
				$this->message(1, 'File uploaded'.$ret.'', $extra_info);				
			}
			else
			{
				$this->message(1, 'File move error'.$this->upload_path.$this->file_name.'', $extra_info);
			}
		}
	}
