  ...
  else
  {
    ...
    */ 
    ...
    if ($n > 0)
    {
       ...
       $propuso = ($diasuso/$diasestancia);  // proporción de uso
       sort($diassonda);
       $posiper25 = (round((25*$n)/100)-1)<0?0:round((25*$n)/100)-1;        // cálculo para la posición del percentil 25
       $percentil25 = $diassonda[$posiper25][6];                            // percentil 25
       $posiper50 = (round((50*$n)/100)-1)<0?0:round((50*$n)/100)-1;        // cálculo para la posición del percentil 50
       $percentil50 = $diassonda[$posiper50][6];                            // percentil 50
       $posiper75 = (round((75*$n)/100)-1)<0?0:round((75*$n)/100)-1;        // cálculo para la posición del percentil 75
    ...
    echo "<center><table border=1 width=500>";// este es el encabezado del resultado
	  echo "<tr><td align=center colspan=1 rowspan=3><font size=3><img src='/matrix/images/medical/invecla/INVECLA.jpg' WIDTH=100 HEIGHT=80></font></td>";
		echo "<td align=center colspan=5 rowspan=1><font size=2><b>REPORTE UTILIZACIÓN DE SONDAS VESICALES</b></font></td></tr>";
    ...
    echo "<tr><td colspan=5 rowspan=1 width=380><font size=3>Número de sondas vesicales</font></td>";
    echo "<td align=center><b>".number_format($n,0,"",".")."</b>";
    echo "<tr><td colspan=5 rowspan=1 width=380><font size=3>Días de uso</font></td>";
    echo "<td align=center><b>".number_format($diasuso,0,"",".")."</b></tr>";
    echo "<tr><td colspan=5 rowspan=1 width=380><font size=3>Días estancia</font></td>";
    echo "<td align=center><b>".number_format($diasestancia,0,"",".")."</b></tr>";
    echo "<tr><td colspan=5 rowspan=1 width=380><font size=3>Promedio días de uso  </font><font size=1> (Días uso / Número de sondas)</font></td>";
    echo "<td  align=center><b>".number_format($promuso,1,"",".")."</b></tr>";
    echo "<tr><td colspan=5 rowspan=1 width=380><font size=3>Tasa de uso   </font><font size=1> (Días uso / Días estancia)</font></td>";
