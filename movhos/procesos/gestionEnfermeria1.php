...
function volver(wemp_pmla, waux, wsp)
{
	...
	location.href = 'gestionEnfermeria1.php?wemp_pmla='+wemp_pmla+extra;
}
...
function trasladoayuda( historia, ingreso, nombre, centro_costo, hab_actual, basedatos )
    {
            ...
            $.post("gestionEnfermeria1.php",  //whabitacion$wid
            {
                consultaAjax:       '',
...
function interconsultas($whistoria, $wingreso){
	...
	$q_inter = "  SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, Codcups as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle, Detpri
			FROM
			...
			UNION
			SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, c.Codigo as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle, Detpri
			FROM
...
function examenes_procedim_pend($whistoria, $wingreso){
	...
	$q = "  SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, Codcups as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle, Detpri, Detlog, Detpen
			FROM
				{$whce}_000027 LEFT JOIN ".$wbasedato."_000011 ON Ccocod = Ordtor, {$whce}_000028, {$whce}_000047 c, {$whce}_000015 d,".$wbasedato."_000045
			WHERE
				Ordhis = '$whistoria'
			...
			UNION
			SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, c.Codigo as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle, Detpri, Detlog, Detpen
			FROM
				{$whce}_000027 LEFT JOIN ".$wbasedato."_000011 ON Ccocod = Ordtor, {$whce}_000028, {$whce}_000017 c,  {$whce}_000015 d, ".$wbasedato."_000045
			WHERE
				Ordhis = '$whistoria'
	...
	if( count($datos) > 0 ){
		...
		foreach( $datos as $keyDatos => $valueDatos ){
			...
			if($wmuerte == 'on'){
					...
					$mensaje_muerte = "<font size=1 color=red> Fallecido </font>";					
				}
			...
			if( $i == 0 ){
				...
				$control_contingencia = control_contingencia($conex, $wemp_pmla, $wbasedato, $valueDatos->cco);
				...
				$path_contingencia = "Contingencia_Kardex_de_Enfermeria.php?wemp_pmla=".$wemp_pmla."&wcco=".$valueDatos->cco."&wfec=".$fecha;
								
				echo "<td colspan=3 id='contingencia_".$valueDatos->cco."' style='cursor: pointer' class='".$alerta_contingencia."' title='".$title_alerta_contingencia."' align='center' onclick='ejecutar(".chr(34).$path_contingencia.chr(34)."); registro_log_contingencia(\"".$wemp_pmla."\", \"".$wbasedato."\", \"".$valueDatos->cco."\", \"".$wuser."\", \"".date('Y-m-d')."\", \"".date('H')."\", \"".date('i')."\", \"".date('s')."\", \"".$url."\", \"".$control_contingencia."\")'><font size=1><b>Contingencia</b></td>";
				...
				if ($wnum > 0)
				   {
					...
					echo "<td colspan=6 align='center' id=td_cco><font size=4><b style='cursor: pointer' onclick='ejecutar(".chr(34).$path.chr(34).")'>".$datos_cco->codigo." - ".$datos_cco->nombre." ddsdsd ====>  </b></font></td>";
			...
			elseif($wccohos=='on'){
				...
				//Si el paciente tiene muerte activa no se puede quitar el alta en proceso y el cajon de muerte estara inactivo, cuando el paciente tenga
				//saldo en cero el paciente no saldra en la lista.
				if($wmuerte == 'on'){
						$checked_muerte = "disabled";
						$checked_proc_tras = "disabled";
						$mensaje_muerte = "<font size=1 color=red> Fallecido </font>";					
			...
			if($wir_a_ordenes == 'on'){
				...
				echo "<INPUT TYPE='button' value='Cerrar' onClick='grabar_leido_ordenes(\"".$wbasedato."\",\"".$whce."\",\"".$wuser."\",\"".$valueDatos->historia."\",\"".$valueDatos->ingreso."\");' style='width:100'><br><br>";
				...
				echo "<br><INPUT TYPE='button' value='Cerrar' onClick='grabar_leido_ordenes(\"".$wbasedato."\",\"".$whce."\",\"".$wuser."\",\"".$valueDatos->historia."\",\"".$valueDatos->ingreso."\");' style='width:100'>";
...
if(!$_SESSION["user"]){
	...
	if( $mostrar == "off"  ){
		...
		$q_pac_con_saldo_insum = "     SELECT Ubisac as Habcco,Ubihan as Habcod,Ubihis as Habhis, Ubiing as Habing, CONCAT(pacno1,' ',pacno2,' ',pacap1,' ',pacap2) as Nombre, 
										Cconom, Ubiptr, Ubialp, Ubifap, Ubihap, pacced, pactid, Ingres, Ingnre, tabla18.id as id_tabla18, Ubisan, '2000' as Habord, Ubimue
			...
			default:
					...
					$sql = "SELECT * FROM (
								SELECT
									...
									Cconom, Ubiptr, Ubialp, Ubifap, Ubihap, pacced, pactid, Ingres, Ingnre, {$wbasedato}_000018.id as id_tabla18, Ubisan, Habord, Ubimue
								FROM
