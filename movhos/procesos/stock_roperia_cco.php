...
function valida_existencia(i)
  {
    ...
    /*
    // Se Inhabilitará por solicitud de los usuarios
    // En reiteradas ocasiones se excede el stock por necesidad del servicio
    stock = parseInt(document.getElementById('wstock'+i).value);
...
include_once("conex.php");
   ...
   **************************************************************/
     ...
     ************************************************************
	 * 2016-11-18 - Jessica Madrid Mejía
	 * Se comenta el llamado a la funcion validar_browser() para evitar la validación de Ipods.
	 ************************************************************
	 * 2013-10-11 - Jonatan
	 * Habia una diferencia en el servidor que no estaba versionado, el codigo de la prenda ya habia sido puesta en la interfaz.
	 ************************************************************
	 * 2013-02-26 - Se cambió $cco = explode("-",$wcco); por $cco = explode(" - ",$wcco); ya que el primero estaba dejando 
	 * un espacio en blanco al final del código del centro de costo y esto hacia que se guardara con este espacio en la tabla
	 * 105 de movimiento hospitalario - Mario Cadavid															
     ************************************************************
	 * 2012-05-23 - Se incluyó la actualización (update) en la taba movhos_000105 cuando ya existen registros para la prenda
	 * en ese centro de costos y en esa ronda. También se adicionó la asignación de valor inicial par la variable $wcantidadant
     ************************************************************
     */
...
else    // Si el usuario está registrado inicia el programa
{
            ...
            if($whora>$row['Ronhin'] && $whora<$row['Ronhfi'])
              {
                ...
                //$orden_ronda_select = $row['Ronord']; // Solo para usar en el select de recorridos ya que la variable anterior cambia y en el select se necesita el orden según la hora que da esta función
                $jornada = $row['Ronjor'];
              }
        }
  ...
  // Esta actualización se hace el 11-Abr-2012
