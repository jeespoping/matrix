 ...
 **************************************************************************************************************/
					  /*						MODIFICACIONES				*/
 /*
 Octubre 28 de 2019 (Jessica)	: - Se agrega validación adicional para evitar que en movhos_000133 queden registros sin diagnóstico.
									Si $diagnostico es vacio  se consulta el último diagnóstico diligenciado en movhos_000133 para el paciente.
 Mayo 02 de 2018 (Edwin)		: - Se agrega buscador por fechas para el reporte
								  - Se optimiza queries para que el reporte sea más rápido
 Abril 12 de 2018 (Edwin)		: - Se hacen cambios para que siempre se genere orden de control aunque no se dispensa para el día en curso
 ...
 * 												FUNCIONES
 ****************************************************************************************************************/
function consultarDiagnosticoFormatoControl($conex, $wbasedato, $historia, $ingreso)
{
	$queryDiagnostico = " SELECT Ctrdia 
	...
	if($numDiagnostico > 0)
	{
		$rowDiagnostico = mysql_fetch_array($resDiagnostico);
		$diagnosticos = $rowDiagnostico['Ctrdia'];
	}
 ...
 ****************************************************************************************************************/
function consultarUbicacionPacientePorHistoriaIngreso( $conex, $wbasedato, $historia, $ingreso ){
...
function dimensionesImagen($idemed)
{
	...
	list($width, $height) = getimagesize('../../images/medical/hce/Firmas/'.$idemed.'.png');
	
	$altoimagen = '70';
	
	$anchoimagen = (70 * $width) / $height;
	
	if($anchoimagen<136 || $anchoimagen>180)
		$anchoimagen = 136;
}
 /****************************************************************************************************
 ...
 ****************************************************************************************************************/
function insertarArticulos( $conex, $wbasedato, $historia, $ingreso, $articulo, $frecuencia, $cantidad, $idOriginal, $codMedico, $dosisMaxima, $diagnostico, $consecutivo, $servcioActual, $habitacionActual ){
	...
	$sql = "INSERT INTO 
				{$wbasedato}_000133(     Medico  , Fecha_data , Hora_data ,    Ctrhis  ,   Ctring  ,   Ctrart   ,    Ctrper    ,    Ctrcan  ,    Ctrido    ,   Ctrmed    , Ctrfge  , Ctrhge ,    Ctrfim   ,   Ctrhim  , Ctruim , Ctrimp , Ctrest ,    Ctrdma     ,     Ctrdia    ,     Ctrcon    ,     Ctrsac      ,     Ctrhac         ,    Seguridad    )
							VALUES ( '$wbasedato', '$fecha'   ,  '$hora'  , '$historia', '$ingreso', '$articulo', '$frecuencia', '$cantidad', '$idOriginal', '$codMedico', '$fecha', '$hora', '0000-00-00', '00:00:00',    ''  ,  'off' ,  'on'  , '$dosisMaxima', '$diagnostico', '$consecutivo', '$servcioActual', '$habitacionActual','C-$wbasedato' )";
...
function generarArticulosAImprimir( $conex, $wbasedato, $wcenmez, $whce, $wcliame, $historia, $ingreso, $fechaKardex, $codMedico, $dx = '' ){
	...
	// si el diagnostico es vacio consulto el último diagnostico registrado en movhos_000133
	if($diagnostico=="")
	{
		$diagnostico = consultarDiagnosticoFormatoControl($conex, $wbasedato, $historia, $ingreso);
	}
	...
	$ubicacionPaciente 	= consultarUbicacionPacientePorHistoriaIngreso( $conex, $wbasedato, $historia, $ingreso );
		...
		if( $cantidad > 0 ){
			...
			if( $insertar ){
				...
				if( $conEncabezado ){
					...
					// insertarArticulos( $conex, $wbasedato, $historia, $ingreso, $rowsArticulos[ 'Kadart' ], $rowsArticulos[ 'Kadper' ], $cantidad, $rowsArticulos[ 'Kadido' ], $rowsArticulos[ 'Kadusu' ], $rowsArticulos[ 'Kaddma' ], $diagnostico, $consecutivo );
					insertarArticulos( $conex, $wbasedato, $historia, $ingreso, $rowsArticulos[ 'Kadart' ], $rowsArticulos[ 'Kadper' ], $cantidad, $rowsArticulos[ 'Kadido' ], $rowsArticulos[ 'Kadusu' ], $rowsArticulos[ 'Kaddma' ], $diagnostico, $consecutivo, $servcioActual, $habitacionActual );
				}
			}
		}
	}
...
if( $consultaAjax ){	//si hay ajax
	...
	if( !isset($imprimir) ){
		...
		$wactualiz = "Octubre 28 de 2019";
		
		encabezado("IMPRESION MEDICAMENTOS DE CONTROL",$wactualiz, "clinica");
		
		if( empty( $fechaGeneracion ) )
			$fechaGeneracion = date( "Y-m-d" );
		
		$fechaAnterior 	= date( "Y-m-d", strtotime( $fechaGeneracion." 00:00:00" ) - 3600*24 );
		$fechaSiguiente = date( "Y-m-d", strtotime( $fechaGeneracion." 00:00:00" ) + 3600*24 );
		
		$ocultarSiguiente = "";
		if( date( "Y-m-d" ) == $fechaGeneracion )
			$ocultarSiguiente = "display:none;";
		
		echo "<center>";
				echo "<table>";
				echo "<tr>";
					echo "<td class=encabezadotabla><font size=5><b>MEDICAMENTOS DE CONTROL SIN IMPRIMIR</b></font></td>";
				echo "</tr>";
			echo "</table>";
		echo "</center>";
		...
		echo "<td colspan=3 style='text-align:center;'>Fecha de generaci&oacute;n</td>";
		echo "</tr>";
		echo "<tr>";
		echo "<td style='text-align:center;width:60px;'>";
		echo "<button value='Atras' style='text-align:center;width:50px;border: 0;background: transparent;' onclick=cambiarFecha('".$fechaAnterior."')><img src='../../images/medical/sgc/atras.png'></button>";
		echo "</td>";
		echo "<td style='text-align:center;width:110px;'>";
		echo "<input type=text name='fechaGeneracion' id='fechaGeneracion' value='".$fechaGeneracion."' onchange='cambiarFecha( this.value );' style='text-align:center;width:90px;'>";
		echo "</td>";
		echo "<td style='text-align:center;width:60px;'>";
		echo "<button value='Adelante' style='text-align:center;width:50px;border: 0;background: transparent;$ocultarSiguiente' onclick=cambiarFecha('".$fechaSiguiente."')><img src='../../images/medical/sgc/adelante.png'></button>";
		echo "</td>";
		echo "</tr>";
		echo "</table>";
		echo "</div>";
		...
		$sql = "(SELECT
					Artcod, Artgen, Artcom, Artfar, Ubihac, Ubisac, Cconom, Pacno1, Pacno2, Pacap1, Pacap2, Pacnac, Pacced, Pactid, a.*, a.id as ctcid
				FROM
					{$wbasedato}_000133 a, {$wbasedato}_000026 b, {$wbasedato}_000018 d, {$wbasedato}_000011 e, root_000036 f, root_000037 g
				WHERE ctrimp = 'off'
					AND ctrest = 'on'
					AND artcod = ctrart
					AND ubihis = ctrhis
					...
					if( true ){
						...
						foreach( $pacientes as $keyPacientes => $hisPacientes ){
							...
							echo "<table>";
									...
									echo $value;
									echo "</td>";									
									echo "<td align='center'><a href=#null onclick=\"abrirVentana( 'impresionMedicamentosControl.php?wemp_pmla=$wemp_pmla&imprimir=on&historia={$hisPacientes[ 'his' ]}&id_registro={$key}&editable=$editable' )\">Imprimir</a></td>";								
									echo "</tr>";									
									$j++;
									}								
								echo "</table>";
		...
		$sql_imp = "(SELECT
					Artcod, Artgen, Artcom, Artfar, Ubihac, Ubisac, Cconom, Pacno1, Pacno2, Pacap1, Pacap2, Pacnac, Pacced, Pactid, a.*, a.id as idctc
				FROM
					{$wbasedato}_000133 a, {$wbasedato}_000026 b, {$wbasedato}_000018 d, {$wbasedato}_000011 e, root_000036 f, root_000037 g
				WHERE ctrimp = 'on'
					AND ctrest = 'on'
					...
					if( true ){
						...
						foreach( $pacientes_imp as $keyPacientes_imp => $hisPacientes_imp ){
							...
							echo "<table>";
									...
									echo "<td align='center'><a href=#null onclick=\"abrirVentana( 'impresionMedicamentosControl.php?wemp_pmla=$wemp_pmla&imprimir=on&reimprimir=on&historia={$hisPacientes_imp[ 'his' ]}&id_registro={$key}&editable=$editable' );\">Imprimir</a></td>";
									echo "</tr>";									
									$j++;
									}								
								echo "</table>";
