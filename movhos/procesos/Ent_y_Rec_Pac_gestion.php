...
*/
// =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
//$wactualiz = " Julio 25 de 2016 ";      // Aca se coloca la ultima fecha de actualizacion de este programa //
//=========================================================================================================================================================================================
// Agosto 11 de 2020 (Edwin MG) Se cambia el calculo de dias de estancia desde la fecha e ingreso del paciente a la fecha y hora actual, antes era desde la fecha de ingreso del servicio
//								a media noche hasta la fecha y hora actual
//=========================================================================================================================================================================================
// Mayo 04 de 2020 (Edwin MG) Al recibir un paciente se envia mensaje hl7 a laboratorio con la nueva ubicación del paciente
//=========================================================================================================================================================================================
// Marzo 16 de 2020 (Edwin MG) Al recibir un paciente que pasa de urgencias a piso, se desaprueba la aprobación del regente (kadare) en la tabla temporal del kardex (movhos_000060) ya que
//							   en ocasiones el kardex se encontraba abierto y la desaprobación del regente no se llevaba acabo en la tabla definitica del kardex (movhos_000054)
//=========================================================================================================================================================================================
// Julio 20 de 2019 (Edwin MG) Siempre que se hace un trasaldo a un destino hospitalario el campo pachos de la tabla inpac de unix siempre queda en H
//=========================================================================================================================================================================================
// Julio 25 de 2016 (Jonatan Lopez) Desde esta fecha se separa este programa que permite la entrega y el recibo de pacientes e interactua con el programa de gestion de enfermeria.
//=========================================================================================================================================================================================
//===============================================  ACTUALIZACIONES ANTIGUAS ASOCIADAS AL PROGRAMA Ent_y_Rec_Pac.php =================================================================================================================================
//2018-08-23 camilo zapata: adicion de la función registroTrasladoUnix la cual cambiará el tipo de estancia del paciente cuando este haya entrado por urgencias y se dirija a ser hospitalizado
...
// if (!function_exists("esUrgencias")){
...
function cambioDeUbicacionHL7Lab( $conex, $wemp_pmla, $wbasedato, $whis, $wing, $cco ){
	
	$mensaje_tm =   $encHl7.$mensaje_tm;
		
	$ch = curl_init();
	
	$data = array( 
			'consultaAjax'	=> '',
			'accion' 		=> 'cambioUbicacion',
			'wemp_pmla' 	=> $wemp_pmla, 
			'historia' 		=> $whis, 
			'ingreso' 		=> $wing, 
			'cco' 			=> $cco,
	...
	$options = array(
				CURLOPT_URL 			=> 'localhost/matrix/interoperabilidad/procesos/IoLaboratorio.php',
...
function CambiodeFlujo($historia, $ingreso, $ccoOrigen, $ccoDestino)
{
	global $wbasedato;
	...
	$infoCco   = array();
	// --> Obtener informacion relacionada a los dos centros de costos al origen y al destino.
	$sqlInfCco = "SELECT Ccocod, Ccotin, Ccourg, Ccohos
				    FROM ".$wbasedatoMovhos."_000011
				   WHERE Ccocod IN ('".$ccoOrigen."', '".$ccoDestino."')
				     AND Ccoest = 'on'
				   ";
	seguimientoLog($sqlInfCco);
	$resInfCco = mysql_query($sqlInfCco, $conex ) or die("<b>ERROR EN QUERY MATRIX:</b><br>".mysql_error());
	while($rowInfCco = mysql_fetch_array($resInfCco))
	{
		$infoCco[$rowInfCco['Ccocod']]['tipoIngRelacionado'] 	= $rowInfCco['Ccotin'];
		$infoCco[$rowInfCco['Ccocod']]['esUrgencias'] 			= $rowInfCco['Ccourg'];
		$infoCco[$rowInfCco['Ccocod']]['esHospitalario'] 		= $rowInfCco['Ccohos'];
	}

	// -->  Esta funcionalidad solo aplica si el origen del paciente es desde urgencias
	// 		Y si el cco destino es hospitalario, y es diferente de urgencias
	//		Y si el cco origen es diferente al cco destino
	if($infoCco[$ccoOrigen]['esUrgencias'] == 'on' && $infoCco[$ccoDestino]['esHospitalario'] == 'on' && $infoCco[$ccoDestino]['esUrgencias'] != 'on' && $ccoOrigen != $ccoDestino)
	{
		...
		$sqlflujo ="SELECT  lenpro , lenemp FROM 	 ".$wfachos."_000011 WHERE lenhis ='".$historia."' AND  lening='".$ingreso."' ";
		$res = mysql_query($sqlflujo, $conex ) or die("<b>ERROR EN QUERY MATRIX:</b><br>".mysql_error());

		if($row = mysql_fetch_array($res))
		{
			 $flujo = $row['lenpro'];
			 $empresaplan = $row['lenemp'];
		}
		// --> Obtener el responsable del paciente
		$sqlRes = " SELECT Ingcem, Ingcla, Emptts, Empceh, Emptem, Empnom, Emptar,Ingtpa
					  FROM ".$wcliame."_000101, ".$wcliame."_000024
					 WHERE Inghis = '".$historia."'
					   AND Ingnin = '".$ingreso."'
					   AND Ingcem = Empcod";
		$resRes = mysql_query($sqlRes, $conex ) or die("<b>ERROR EN QUERY MATRIX:</b><br>".mysql_error());
		seguimientoLog($sqlRes);
		if($rowRes = mysql_fetch_array($resRes))
		{
			$entidad 					= $rowRes['Ingcem'];
			$tipoPaciente 				= $rowRes['Ingcla'];
			$paciente					= $rowRes['Ingtpa'];
			$codigoEntidadHospi 		= $rowRes['Empceh'];
			$tipoRespAnterior 			= $rowRes['Emptem'];
			$antNomResponsable 			= $rowRes['Empnom'];
			$tarifaAnterior 			= $rowRes['Emptar'];
		}
		else
			return;

		if($paciente=='P' OR $paciente=='p')
		{
		  // no se hace nada porque es particular
		}
		else
		{


			$select = "SELECT Flucod , Flufsc
						 FROM ".$wfachos."_000026
						WHERE  Flutem = '".$tipoRespAnterior."'
						  AND  Fluemp ='".$entidad."'
						  AND  Flucod ='".$flujo."'"  ;
			$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 7:</b><br>".mysql_error());
			$flujosiguiente='';
			$row = mysql_fetch_array($res);

			$flujosiguiente = $row['Flufsc'];
			...
			if($flujosiguiente=='')
			{
				$select = "SELECT Flucod , Flufsc
						...
						WHERE  Flutem = '".$tipoRespAnterior."'
						  AND  Fluemp ='*'
						  AND  Flucod ='".$flujo."'"  ;
				$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 6:</b><br>".mysql_error());
				$flujosiguiente='';
				$row = mysql_fetch_array($res);
					...
					if($flujosiguiente=='')
					{
						$select = "SELECT Flucod , Flufsc
								...
								WHERE  Flutem = '*'
								  AND  Fluemp ='*'
								  AND  Flucod ='".$flujo."'"  ;
						$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 5:</b><br>".mysql_error());
						$flujosiguiente='';
						$row = mysql_fetch_array($res);
						...
						if($flujosiguiente=='')
						{
							$select = "SELECT Flucod , Flufsc
									...
									WHERE  Flutem = '*'
									  AND  Fluemp ='*'
									  AND  Flucod ='*'"  ;
							$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 4:</b><br>".mysql_error());
							$flujosiguiente='';
							$row = mysql_fetch_array($res);
			...
			//echo "Tipo de empresa paciente:".$tipoRespAnterior." Entidad paciente ".$entidad." Flujo ".$flujo." Flujo siguiente ".$flujosiguiente;

			if($flujosiguiente!='')
			{

				$select ="SELECT pemcod ,pempln
							FROM ".$wfachos."_000009
						   WHERE  pememp ='".$entidad."'
						     AND  pemcod = '".$empresaplan."'";
				$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 1:</b><br>".mysql_error());
				$row = mysql_fetch_array($res);
				$plan =$row['pempln'];
				$codigoempresa =$row['pemcod'];


				$select ="SELECT *
				            FROM ".$wfachos."_000019
						   WHERE  seremp ='".$entidad."'
						     AND  serpln = '".$plan."'
							 AND  serres = '01'
							 AND  serpro = '".$flujosiguiente."'";



				$res = mysql_query($select, $conex ) or die("<b>ERROR EN QUERY MATRIX 2:</b><br>".mysql_error());

				$delete = "DELETE  FROM  ".$wfachos."_000012  WHERE delhis = '".$historia."' AND  deling = '".$ingreso."'";
				$res2 = mysql_query($delete,$conex) or die ( "Error: ".mysql_errno()." - en el query: ".$delete." - ".mysql_error() );
				while($row = mysql_fetch_array($res))
				{
					...
					$query = "INSERT INTO {$wfachos}_000012 (Medico, Fecha_data, Hora_data, delhis, deling, delsop, delemp, delser, delfor, delest, delres, delobs, seguridad)
								VALUES('fachos', '{$hoy}', '{$hora}', '{$historia}', '{$ingreso}', '{$row['sersop']}', '{$codigoempresa}', 'n', '', 'na', '01', '', 'C-fachos' )";
				...
				$sqlflujo ="UPDATE ".$wfachos."_000011 SET lenpro = '".$flujosiguiente."' , lenrac ='01' , lenran='' WHERE lenhis ='".$historia."' AND  lening='".$ingreso."' ";
				$res      = mysql_query($sqlflujo, $conex ) or die("<b>ERROR EN QUERY MATRIX 3:</b><br>".mysql_error());
	...
	//$wbasedato = $wbasedatoMovhos;

	return;
}

function registroTrasladoUnix($historia, $ingreso, $ccoOrigen, $ccoDestino){
	global $wbasedato;
	global $wemp_pmla;
	global $conex;
	global $wuse;
	$conex_u              = odbc_connect('admisiones','','');
	$origenEsUrgencias    = es_urgencias( $ccoOrigen );
	$origenEsAmbulatorio  = es_ambulatorio( $ccoOrigen );
	$destinoHospitalario  = es_hospitalario( $ccoDestino );

	if( $destinoHospitalario ){

		$servicioUnix  = consultarServicioUnix( $ccoDestino );
		...
		$query = " UPDATE inpac
		            ...
		            WHERE pachis = '{$historia}'
		              AND pacnum = '{$ingreso}'";
		$rs    = odbc_do( $conex_u, $query );

		$query = " UPDATE inpacinf
		              SET pacinfate = 'H'
		            WHERE pacinfhis = '{$historia}'
		              AND pacinfnum = '{$ingreso}'";
		$rs    = odbc_do( $conex_u, $query );
	}
	odbc_close_all();
}

function consultarServicioUnix( $cco ){
	global $wbasedato;
	global $wemp_pmla;
	global $conex;
 ...
 ****************************************************************************************/
function consultarFraccion2( $conex, $wbasedato, $wcenmez, $articulo ){
	...
	if( $num > 0 ){
		...
		$val = consultarFraccionPorArticulo2( $conex, $wbasedato, $articulo, 1050 );
		...
		if( $num > 0 ){
			$val = consultarFraccionPorArticulo2( $conex, $wbasedato, $articulo, 1051 );
 ...
 ************************************************************************************************************/
function consultarFraccionPorArticulo2( $conex, $wbasedato, $articulo, $cco ){
...
else
{
	...
	$nuevaFacturacion = consultarAliasPorAplicacion($conex, $wemp_pmla, 'regrabarCargosAlTrasladarDeUrgencias');
				...
				// {
					...
					// {
						...
						if ( $wgrabar == 'ok')
						{
							...
							// if ($rowhab[0] > 0 or $cirugia_o_urgencia))		// 2011-11-08
							// {
								/****************************************************************************************
								 * Agosto 24 de 2012
								...
								if( $procesar ){
									...
									$articulos_recibir = unserialize(base64_decode($warr_art));
									$warr_art = $articulos_recibir['warr_art'];
									$wserori = $wcco;
									...
									if ($went_rec == "Ent") // ******* ENTREGA *******
									{
										...
										// {
											// $q = " UPDATE " . $wbasedato . "_000018 "
												...
												// ."        ubisac = '".$wccodes1."'," // Servicio Actual o Destino
										...
										// {
											$q = " UPDATE " . $wbasedato . "_000018 "
												...
												."        ubihan = '".$whab."',"        // Habitacion Anterior
												."        ubisac = '".$wccodes1."'," // Servicio Actual o Destino
												."        ubihac = '".$whabdes1."'," // Habitacion Actual o Destino
									...
									else // ******* RECIBO *******
									{
										...
										CambiodeFlujo($whis, $wing, trim($row_cco['Ubisan']), $row_cco['Ubisac']);
										...
										//Si el centro de costos origen es diferente al destino se hace el registro en las tablas de ingreso y egreso.
										if (trim($wserori) != trim($wccodes1))
										{
											// ======================================================================================================================================================================================================================
											...
											$q = " INSERT INTO ".$wbasedato."_000032 (   Medico       ,   Fecha_data,   Hora_data,   Historia_clinica ,   Num_ingreso,   Servicio       ,   Num_ing_Serv,   Fecha_ing ,   Hora_ing ,   Procedencia    , Seguridad     ) "
												."                            VALUES ('".$wbasedato."','".$wfecha."','".$whora."','".$whis."'         ,'".$wing."'   ,'".$wccodes1. "','" . $wingser . "' ,'" . $wfecha . "','" . $whora . "','".$wcco. "', 'C-" . $wuser . "')";
											$err = mysql_query($q, $conex) or die (mysql_errno() . $q . " - " . mysql_error());
											// ======================================================================================================================================================================================================================
											...
											$wcco_origen = es_urgencias($wserori);
											...
											// if($wcco_origen){
												...
												// $q =  " SELECT ROUND(TIMESTAMPDIFF(HOUR,CONCAT( Fecha_data, ' ', Hora_data ),now())/24,2) "
											...
											// else{
												...
												// // Aca calculo los días de estancia en el servicio  ************************
												// $q =  " SELECT ROUND(TIMESTAMPDIFF(HOUR,CONCAT( Fecha_ing, ' ', Hora_ing ),now())/24,2) "
													// . "   FROM ".$wbasedato."_000032 "
													// . "  WHERE Historia_clinica = '".$whis."'"
													// . "    AND Num_ingreso      = '".$wing."'"
													// . "    AND Servicio         = '".$wcco."'"
											...
											$wdiastan = calcularDiasEstancia( $conex, $wbasedato, $whis, $wing, $wcco );
											...
											$q =  " INSERT INTO ".$wbasedato."_000033(   Medico       ,   Fecha_data,   Hora_data,   Historia_clinica,   Num_ingreso,   Servicio ,  Num_ing_Serv,   Fecha_Egre_Serv ,   Hora_egr_Serv ,   Tipo_Egre_Serv ,  Dias_estan_Serv, Seguridad     ) "
												. "                            VALUES('".$wbasedato."','".$wfecha."','".$whora."','".$whis."'        ,'".$wing."'   ,'".$wcco."' ,".$wingser."  ,'".$wfecha."'      ,'".$whora."'     ,'".$wccodes1."',".$wdiastan."    , 'C-" . $wuser . "')";
											$err = mysql_query($q, $conex) or die (mysql_errno() . $q . " - " . mysql_error());
											// ======================================================================================================================================================================================================================
										}
										...
										if ($num_pedidos>0)
										{
											while($row_pedidos=mysql_fetch_array($res_pedidos))
											{
												...
												if($Pedido!='')
												{
													...
													if ($wccodes1!=$row_pedidos['movcco'])//Si es un traslado para otro cc (Otro servicio)
													{
														...
														if ($fecha_pedido != '0000-00-00')
														{
															...
															if($grabar_pedido == 'si')
															{
																...
																$q_nuevo = " INSERT INTO " . $wbasedato . "_000077 (   Medico       ,   	Fecha_data,    	Hora_data,     	Movfec,         		Movhis,                     Moving,                   Movhab,                Movser,    				  Movdie,		 			     Movind,				   Movobs,				      Movest,      		Movobc,                		Movval, 					  Movint,				      Movpco,						Movmpo,                      Movcan,                      Movaut,             Movcco,			Movpqu, 						Movods, 					Movdsn, 			Movnut,					Seguridad     ) "
																	."                                		  VALUES ('".$wbasedato ."','".$fecha_pedido."','".$whora."','".$fecha_pedido."','".$row_pedidos['movhis']."','".$row_pedidos['moving']."','".$whabdes1."','".$row_pedidos['movser']."','".$row_pedidos['movdie']."','".$row_pedidos['movind']."','".$row_pedidos['movobs']."', 'on','".$row_pedidos['movobc']."','".$row_pedidos['movval']."','".$row_pedidos['movint']."','".$row_pedidos['movpco']."','".$row_pedidos['movmpo']."','".$row_pedidos['movcan']."','".$row_pedidos['movaut']."','".$wccodes1."','".$row_pedidos['Movpqu']."','".$row_pedidos['Movods']."','".$row_pedidos['Movdsn']."','".$row_pedidos['Movnut']."',	'C-".$wuser."')";
																...
																for ($xx=0;$xx<$num_productos;$xx++)
																{
																	...
																	if ($fecha_pedido != '0000-00-00' && $grabar_pedido == 'si')
																	{
																		...
																		$q_insertar_pro = " INSERT INTO " . $wbasedato . "_000084 (		Medico, 		Fecha_data, 	 Hora_data, 		Detfec, 			Dethis, 						Deting, 				Detser, 						Detpat, 					Detpro, 						Detcos, 			 Detest, 		Detcan, 			   Detcco, 				Detcal, 					Detcla,     				Seguridad)
																															VALUES('".$wbasedato ."','".$fecha_pedido."','".$whora."','".$fecha_pedido."','".$row_pedidos['movhis']."','".$row_pedidos['moving']."','".$row_pedidos['movser']."','".$patron_con_productos."','".$row_productos['Detpro']."','".$row_productos['Detcos']."','on','".$row_productos['Detcan']."','".$wccodes1."','".$row_productos['Detcal']."','".$row_productos['Detcla']."','C-".$wuser."')
																						";
														...
														if ($fecha_pedido != '0000-00-00' && $grabar_pedido == 'si')
														{
															...
															$q_registro2 = " INSERT INTO " . $wbasedato . "_000078 (   Medico       ,   Fecha_data,    	Hora_data, 			      Audhis,   		         Auding,                     Audser,                      Audacc,     Auddie,	Audcco,		  Audusu,    Seguridad     ) "
																."                                		  VALUES ('".$wbasedato ."','".$fecha_pedido."','".$whora."','".$row_pedidos['movhis']."','".$row_pedidos['moving']."','".$row_pedidos['movser']."','PEDIDO POR TRASLADO','".$Pedido."','".$wccodes1."','".$wuser."','C-".$wuser."')";
										...
										{
											$wuse = $wuser;
											regrabarCargosAmbulatorios($whis, $wing, trim($row_cco['Ubisan']), $row_cco['Ubisac']);
										}
										...
										if(es_urgencias($row_cco['Ubisan']))
										{
											...
											if($rowTieneKar = mysql_fetch_array($resTieneKar))
											{
												...
												// --> Actualizar el parametro Kadare = 'off' del detalle del kardex.
												$sqlActPar2 = "
												UPDATE ".$wbasedato."_000060
												   SET Kadare = 'off'
												 WHERE Kadhis = '".$whis."'
												   AND Kading = '".$wing."'
												   AND Kadfec = '".$fechaKardex."'
												";
												mysql_query($sqlActPar2, $conex) or die("<b>ERROR EN QUERY MATRIX(sqlActPar):</b><br>".mysql_error());
											}
										}

										// actualizacion del listado de soportes cuando pasa un paciente de un centro de costos de urgencias o ambulatorio a piso debe
										// hospitalizacion

										CambiodeFlujo($whis, $wing, trim($row_cco['Ubisan']), $row_cco['Ubisac']);

										registroTrasladoUnix($whis, $wing, trim($row_cco['Ubisan']), $row_cco['Ubisac']);// en caso de que sea de urgencias a hospitalario.

										$datamensaje['mensaje'] = "recibido";
									...
									if (isset($wnum_art))
									{
										...
										for ($i = 1;$i <= $wcan_art;$i++)
										{
											...
											if ($went_rec == "Ent") // ******* ENTREGA *******
											{
												if($wdesapl=='off' and $woriapl=='on')
												{
													...
													$q = " SELECT spluen, splusa, splaen, splasa, splcco, Ccopap "
														...
														."    AND (splcco = '".$wcco."'"      //2 de Mayo de 2008
											...
											if ($went_rec == "Rec") // ******* RECIBO *******
											{
												if($wdesapl=='on' and $woriapl=='off')
												{
													...
													for ($j = 1;$j <= $num;$j++)
													{
														...
														if(($wuen - $wusa)>0)
														{
															...
															$infoFraccion = consultarFraccion2( $conex, $wbasedato, $wcenmez, $warr_art[$i][0] );
															/************************************************************************/
												...
												// 2012-03-15
												// =============================================================================
												recibir_medicamentos($whis,$wing,$wcco,$wccodes1,$warr_art[$i][0],$warr_art[$i][1],$woriapl);
									...
									if( $went_rec == "Ent" ){
										$datamensaje['mensaje'] = "Faltan articulos por aplicar en las ultimas dos rondas.";

									}
									else{
										$datamensaje['mensaje'] = "No de pudo realizar el recibo, faltan articulos por aplicar.";
									}
								}
