...
$(document).ready(function() 
{
	...
	if(selectorSede !== null)
	{
		selectorSede.addEventListener('change', () => {
			window.location.href = "Aplicacion_ipods.php?wemp_pmla="+$('#wemp_pmla').val()+"&selectsede="+$('#selectsede').val();
		});
	}	
});
...
//=============================================================================================================================================\\
// Enero 11 del 2022 Marlon Osorio	 -Se parametrizo el centro de costo de Dispensacion Servicio Farmaceutico
//=============================================================================================================================================\\
...
//									desde aplicacion_ipods.php y gestionEnfermieria.php														   \\
...
else
{
  ...
  $wusuario=trim($wuser1[1]);
     	                                         ...
     	                                         // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
  $wactualiz="05 de Abril de 2022";               	 // Aca se coloca la ultima fecha de actualizacion de este programa //
	                                             // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //

  encabezado("APLICACION DE MEDICAMENTOS",$wactualiz, "clinica", TRUE, FALSE);
  ...
  function consultaCentrosCostosNoDomiciliarios( $conex, $wbasedato, $esDomiciliario = false, $sCodigoSede = NULL ){
	...
	else
	{
		$sql = "SELECT Ccocod, UPPER( Cconom )
				  FROM ".$wbasedato."_000011
				 WHERE Ccoest  = 'on' 
				   AND ( Ccohos  = 'on' AND ccodom != 'on' OR Ccourg  = 'on')
				   {$sFiltroSede}
			  ORDER BY Ccoord, Ccocod; ";
	}
...
function consultarSiPuedeAnular( $historia, $ingreso, $medicamento, $ido )
{
  ...
  function elegir_centro_de_costo($selectsede = '')
     {
	  ...
	  if( !isset($consultaPorHistoria) ){
		  ...
		  //**************** llamada a la funcion consultaCentrosCostos y dibujarSelect************
						...
						$centrosCostos = consultaCentrosCostosNoDomiciliarios( $conex, $wbasedato, $esServicioDomiciliario, $selectsede );
  ...
  function elegir_historia($wzona, $selectsede = '')
     {
	  global $conex;
	  global $wcenmez;
	  global $wafinidad;
		  ...
		  for($i=1;$i<=$num;$i++)
			 {
			   ...
			   if( !esUsuarioLactario( $conex, $wbasedato, $wusuario ) ){
					...
					// $ccoQueryArts = '1120';
					$ccoQueryArts = consultarCcoLactarioUnificado( $conex, $wbasedato, $selectsede );
				}
			   ...
			   while ($j <= 6 and $i <= $num)
			      {
			       ...
			       echo "<td align=center><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla."&user=".$user."&whis=".$whis."&wing=".$wing."&wcco=".$wcco."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."' class=tipo3V>".$whab."</A></td>";
			       $row = mysql_fetch_array($res);
			       $j++;
			       $i++;
	              }
...
// Noviembre 17 de 2011
//=========================================================================================================
function validar_ipods($wcco, $selectsede = '')
  {
   global $conex;
   global $wbasedato;
   global $wemp_pmla;
		...
		if ($row[1] == "on" and !$wok)
		   {
			...
			echo " <script>
				   ...
				   window.location.href='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$sUrlCodigoSede.( isset($consultaPorHistoria) ? '&consultaPorHistoria=on' : '' )."'
				   </script> ";
 ...
 * Date: 2022-02-10
 * Descripcion : Se crea función para preguntar si un articulo es de lactario 
*/
function articuloLactario($CodigoArticulo)
{
	global $conex;
	global $wbasedato;
	global $wemp_pmla;
	...
	$QueryArticuloLactario = "SELECT  
				Arktip 
			FROM 
				".$wbasedato."_000068 
			WHERE 
				Arkcod = '".$CodigoArticulo."'
				AND Arktip = 'LC'";
	
	$resArticuloLactario = mysql_query($QueryArticuloLactario,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$QueryArticuloLactario." - ".mysql_error());
	$wLactarioArticulo = mysql_num_rows($resArticuloLactario);

	return $wLactarioArticulo;
 ...
 * By: Sebastian Alvarez Barona
 * Date: 2022-02-10
 * Descripcion: Se valida si la nutricion no ha sido entregada saldra un mensaje
 * que diga pendiente por entregar
 */
function esLactarioEntregado($CodigoArticulo, $whis, $wing)
{
	global $conex;
	global $wbasedato;
	global $wemp_pmla;
	global $whora_par_actual;
	global $whora_a_grabar;

	/** le pasamos la ronda actual a la variabe $whora_a_grabar para luego darle un formato especifico de la movhos_000298 16:00 - PM
	 * Se hizo para poder diferenciar el mensaje de pendiente de entrega cuando existe el mismo articulo a un mismo paciente.
	 */
	$whora_a_grabar = gmdate( "H:00 - A", $whora_par_actual*3600 );

	$QueryLactarioEntregado = "SELECT 
			Entest
		FROM
			".$wbasedato."_000298, ".$wbasedato."_000054 A, ".$wbasedato."_000068
		WHERE
		Enthis = '".$whis."' 
				AND	Arkcod = '".$CodigoArticulo."'
				AND A.Kadart = Arkcod
				AND Enting = '".$wing."'
				AND A.Kadart = Entart
				AND Entfec = '".date("y-m-d")."'
				AND Entido = A.Kadido
				AND Entron = '".$whora_a_grabar."'
				AND Entest = 'on'";

	$resLactario = mysql_query($QueryLactarioEntregado,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$QueryLactarioEntregado." - ".mysql_error());
	$wentrega = mysql_num_rows($resLactario);

	if( $wentrega >= 1 ){
		$rowsEnt = mysql_fetch_array( $resLactario);
		$valEnt = $rowsEnt['Entest'];
	}
...
//*******************************************************************************************************************************************
  ...
  echo "<form name='apl_ipods' action='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$sUrlCodigoSede."' method=post>";
			...
			if (isset($whis))
			   {
				...
				for ($i=1;$i<=$num;$i++)   //Recorre cada uno de los medicamentos
				{
					...
					{
						anularAplicacionMovhos($whis, $wing, $wart[$i], $wcco, $wdosis[$i], $wcanfra[$i], $whora_par_actual, $wfecha_actual, $wnoenviar[$i], $wStock[$i], $wido[$i]);
					}
											...
											if ( true || $wStock[$i]=="off" )     //Si entra aca es porque NO es del STOCK, puede que se envie o no pero si tiene saldo lo descuenta
											  {
												  ...
												  aplicarMedicamento($wbasedato,$whis,$wing,$row[1],$wcant_aplicar,$wdosis,$whora_a_grabar,$row[2],$wcco,$wcco1[0],$wusuario,$wfecha_actual,$wnoenviar[$i],$wufr,$row[22],$wcodvia);
								  ...
								  if ($wok == true and $waplicado=="off")
									 {
									  if ($wStock[$i]=="on")
										if( !$dosVar[ 'Defrci' ] ) //si NO pide cantidad al aplicar
											...
											{
												/** Llamamos a la función pasandole como parametro el codigo del articulo $row[1] */
												$articuloLactario = articuloLactario($row[1]);
												if($articuloLactario > 0){ /** Preguntamos si el articulo marcado como LC es mayor a 0 */
													$articuloNutricion = esLactarioEntregado($row[1], $whis, $wing); /** Llamamos a la función pasandole como parametro el codigo del articulo $row[1] */
													if($articuloNutricion == '' || $articuloNutricion !== 'on'){ /** Preguntamos si no esta vacio y es diferente de estado on el articulo en la movhos_000298 */
														echo "<td align=center rowspan=4 bgcolor='00FF00' colspan=2><font size=4 color='ffffff'><strong>Pendiente entrega</strong></font></td>"; /** Pintamos el mensaje */
													}else{
														if( !$dosVar[ 'Defrci' ] ) //si NO pide cantidad al aplicar
														echo "<td align=center rowspan=4 colspan=2><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on&wzona=".$wzona.$sUrlCodigoSede.( !empty( $pac_historia ) ? '&pac_historia='.$pac_historia : '' ).( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' )." ' class=tipo3V>Aplicar</A></td>";
														else
													...
													if( !$dosVar[ 'Defrci' ] ) //si NO pide cantidad al aplicar
													echo "<td align=center rowspan=4 colspan=2><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on&wzona=".$wzona.$sUrlCodigoSede.
													( !empty( $pac_historia ) ? '&pac_historia='.$pac_historia : '' ).( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' ).$sUrlCodigoSede." ' class=tipo3V>Aplicar</A></td>";
													else
													{
														echo "<td align=center rowspan=4 colspan=2>";
														echo "<select class=tipo3V name='dosisIpd[$i]' onChange='cambiarUrl( this, \"Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".( !empty( $pac_historia ) ? '&pac_historia='.$pac_historia : '' ).( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' ).$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on\", $i )'>";
				...
				echo "<table>";
				if( empty($pac_historia) )
					echo "<tr><td><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla."&wcco=".$wcco."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&wzona=".$wzona.$sUrlCodigoSede.( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' )."' class=tipo3V>Retornar</A></td></tr>";
				else
					echo "<tr><td><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$sUrlCodigoSede.( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' )."&consultaPorHistoria=on' class=tipo3V>Retornar</A></td></tr>";
				echo "</table>";
			   }
			  else
				 {
					  ...
					  if (!isset($wzona) or trim($wzona) == "")
					     {
					      ...
					      echo "<br><br><br><br>";
						   ...
						   elegir_historia($wzona, $selectsede);
					 }
					else                             //Termina modificacion de Mayo 26 de 2011
				       elegir_historia("", $selectsede);

				  echo "<br><br>";
				  echo "<table>";
				  if(empty($pac_historia))
					echo "<tr><td><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&wzona=".$wzona.$sUrlCodigoSede.( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' )."' class=tipo3V>Retornar</A></td></tr>";
				  else	
					echo "<tr><td><A HREF='Aplicacion_ipods.php?wemp_pmla=".$wemp_pmla.$sUrlCodigoSede.( !empty( $servicioDomiciliario ) ? '&servicioDomiciliario='.$servicioDomiciliario : '' )."&consultaPorHistoria=on' class=tipo3V>Retornar</A></td></tr>";
