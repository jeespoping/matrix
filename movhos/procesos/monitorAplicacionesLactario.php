...
class habitacion{
	...
	function __construct( $cco, $ccoNombre, $hab, $historia, $ingreso, $nombre, $enTraslado, $altaProceso, $fechaAltaProceso, $horaAltaProceso ){
...
if(!isset($_SESSION['user'])){
	...
	encabezado( "MONITOR DE APLICACIONES Y KARDEX DE LACTARIO", "Enero 29 de 2013" ,"clinica" );
					...
					for( $fechaInicial = $fechaFinal-$tiempoAMostrar*3600+1; $fechaInicial <= $fechaFinal; $fechaInicial += 2*3600 ){
						...
						if( $datosHabitaciones[ $i ]->tieneKardex[ $fecha ] != 'on' /*|| $datosHabitaciones[ $i ]->kardexConfirmado[ $fecha ] != 'on'*/ ){
							...
							if( !isset( $datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] ) ){
								$datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] = consultarKardexConfirmadoPorCco( $datosHabitaciones[ $i ]->historia, $datosHabitaciones[ $i ]->ingreso, date( "Y-m-d", $fechaInicial-24*3600 ), $ccoKardex );
							}
							...
							if( $datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] == 'on' ){	//Kardex del día anterior confirmado
								...
								query_articulos_kardex_no_confirmado( $rowsHabitaciones['Habhis'], $rowsHabitaciones['Habing'], date( "Y-m-d", $fechaInicial-24*3600 ), $resArticulos, $ccoKardex );
							...
							if( $datosHabitaciones[ $i ]->kardexConfirmado[ $fecha ] != 'on' ){		//Si el kardex esta sin confirmar
								...
								query_articulos_kardex_no_confirmado( $rowsHabitaciones['Habhis'], $rowsHabitaciones['Habing'], $fecha, $resArticulos, $ccoKardex );
