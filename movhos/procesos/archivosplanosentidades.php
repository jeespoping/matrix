...
$horasPacienteUrgArchivoPlano = consultarAliasPorAplicacion($conex, $wemp_pmla, 'horasPacienteUrgArchivoPlano');
...
//=======================================================================
	Este programa, genera un archivo plano en formato csv por cada responsable de los pacientes hospitalizados en la clínica y enviado por correo electrónico.
	
//=======================================================================
	Especificaciones del script
//=======================================================================
	
	Este script puede generar hasta dos archivos planos por responsable. EL primero de ellos es para pacientes INCAPACITADOS y el segundo para pacientes HOSPITALIZADOS

	1. PACIENTES INCAPACITADOS
	
		A cada responsable se le envía la siguiente información por paciente INCAPACITADO(Platip = I en movhos_000236) en la clínica:
		...
		Clasificacion;
		Diagnostico;
		Tipo identificación de la institucion;
		Numero de identificacion institucion;
		Nombre medico atendio;
		SOAT;
		Tipo de atencion
	
	2. PACIENTES HOSPITALIZADOS
	
		A cada responsable se le envía la siguiente información por paciente HOSPITALIZADO(Platip = H en movhos_000236):
		
		Historia de paciente
		Ingreso del paciente
		Nombre completo de paciente
		Fecha de ingreso del paciente
		Habitación en la que se encuentra el paciente
		Poliza
		...
		El correo electrónico al que se envía el archivo plano se encuentra configurado en la tabla ENTIDADES ENVIO HOSPITALIZADOS( movhos_000236 ) por codigo de responsable (Campo Planit)
		En el archivo plano se adiciona los pacientes que esten en urgencias por más de 24 horas si el campo Urgencias (Plaurg) está activo
		En el archivo plano se adiciona los diagnósticos del pacientes si el campo Con diagnóstico (Placdx) está activo
		En el archivo plano se adiciona el saldo de la cuenta del pacientes si el campo Con saldo (Plascu) está activo
	
	Por día, cada vez que se envía un correo electrónico a un responsable, el responsable queda registrado en la tabla LOG ENVIO ARCHIVOS PLANOS (movhos_000237). Una vez generado el registro
	para el resposable, el correo no vuelve a ser enviado durante el resto del día.
	
//=======================================================================
	Modificaciones
//=======================================================================
	Septiembre 17 de 2018 Edwin MG: 	- Al archivo plano que se genera para la empresa *(campo planit de movhos_000236) se le agrega el responsable del paciente
										- Se consulta en unix la cuenta del paciente solo si es necesario ya que siempre se está consultando
	Septiembre 11 de 2018 Edwin MG: 	- Al archivo plano que se genera para enviar por correo electrónico, se le agrega los diagnósticos del paciente siempre y cuando el campo CON DIAGNOSTICO(placdx) 
										  este activo en la tabla ENTIDADES ENVIO HOSPITALIZADOS(movhos_000026) para el correo electrónico al que se le enviará el archivo plano
										- Se parametriza el saldo de la cuenta del paciente. Para el archivo plano que se genera se muestra el saldo de la cuenta del paciente solo si el campo
										  SALDO DE CUENTA (Plascu) de la tabla ENTIDADES ENVIO HOSPITALIZADOS(movhos_000026) está activa
										- Se modifica el script para que tenga en cuenta el código empresa *, configurado en el campo CODIGO(planit) de la tabla ENTIDADES ENVIO HOSPITALIZADOS( movhos_000236 ).
										  Para la emprea * envía un correo con un archivo plano con todos los pacientes de la clinica sin importar el responsable.
										  
	Enero 29 de 2018 Jonatan: Se agregan los pacientes que llevan mas de 24 horas en urgencias controlados con el parametro Plaurg de la tabla movhos_000236
...
//----------Se organiza el correo para ser enviado a los interesados
	...
	//=================================================================================================================================
	// Pacientes hospitalizados y en urgencias con mas de 24 horas.
	...
	while($row_hos = mysql_fetch_array($res_hos)){
		...
		$array_ent_hos[$row_hos['Planit']]['cdx'] = $row_hos['Placdx'] == 'on' ? true : false;
		$array_ent_hos[$row_hos['Planit']]['scu'] = $row_hos['Plascu'] == 'on' ? true : false;
	}
	
	$consultarSaldos = false;
	if( isset( $array_ent_hos['*'] ) && $array_ent_hos['*']['scu'] ){
		$consultarSaldos = true;
	}
	...
	$arreglo = array();
	 ...
	 while ($row = mysql_fetch_array($res)){
		...
		$wtot = 0;
		if( $consultarSaldos || ( isset( $array_ent_hos[$empresa] ) && $array_ent_hos[$empresa]['scu'] ) ){
		...
		if($row['pachos'] == 'H'){
			...
			$arreglo[$empresa][$row['pachis'].$row['pacnum']]['empresa'] = $empresa;
			
			
			$arreglo['*'][$row['pachis'].$row['pacnum']] = $row ;
			$arreglo['*'][$row['pachis'].$row['pacnum']]['plan'] = $rowp['plades'];
			$arreglo['*'][$row['pachis'].$row['pacnum']]['cuenta_total'] = $wtot;
			$arreglo['*'][$row['pachis'].$row['pacnum']]['empresa'] = $empresa."-".trim($row['Ingres']);
		}
		...
		if($row['pachos'] == 'U'){
			...
			$arreglo_urg[$empresa][$row['pachis'].$row['pacnum']]['empresa'] = $empresa;
			
			$arreglo_urg['*'][$row['pachis'].$row['pacnum']] = $row ;
			$arreglo_urg['*'][$row['pachis'].$row['pacnum']]['plan'] = $rowp['plades'];
			$arreglo_urg['*'][$row['pachis'].$row['pacnum']]['cuenta_total'] = $wtot;
			$arreglo_urg['*'][$row['pachis'].$row['pacnum']]['empresa'] = $empresa."-".trim($row['Ingres']);
	...
	$responsable = array();
	$responsables_urg = array();
	
	foreach($array_ent_hos as $key => $value){
		
		$datos_hos = "";
		//Se valida si debe ir con dx o no
		$datos_hos .= "Historia;Ingreso;Nombre Paciente;Fecha Ingreso;Hora Ingreso;Habitacion;".( $key == '*' ? 'Aseguradora;': '' )."Poliza;Cedula;".( $value['scu'] ? "Total Cuenta;" : '' )."Hos;Plan".( $value['cdx'] ? ";Diagnostico" : '' ). PHP_EOL;
		...
		if($rowhos['cuantoshos'] == 0){			
			...
			if( $key == '*' )
				$nombre_adjunto_hos = "pacientes.csv";
			...
			if(!empty($responsable) or !empty($responsables_urg)){
				...
				foreach($responsable as $key1 => $value1){
					...
					$arDxsHis = consultarDiagnosticoPaciente($conex,$wbasedato,$value1['pachis'],trim($value1['pacnum']));
					$arDxs = "";
					
					if( count($arDxsHis) > 0 ){
						$ii = 0;
						foreach( $arDxsHis as $keyDx => $valueDx ){
							$arDxs = $keyDx."-".$valueDx;
							if( $ii == 0 )
								$datos_hos .= $value1['pachis'].";".trim($value1['pacnum']).";".trim($value1['pacap1'])." ".trim($value1['pacap2'])." ".trim($value1['pacnom']).";".trim($value1['pacfec']).";".trim($value1['pachor']).";".trim($value1['trahab']).";".( $key == '*' ? $value1['empresa'].';': '' ).trim($value1['pacpol']).";".trim($value1['pacced']).";".( $value['scu'] ? trim($value1['cuenta_total']).";" : '' ).trim($value1['pachos']).";".trim($value1['pacmrepla']).";\"".( $value['cdx'] ? $arDxs : '' )."\"". PHP_EOL;
							else
								$datos_hos .= ( $key == '*' ? ';' : '' ).( $value['scu'] ? ';' : '' ).";;;;;;;;;;\"".( $value['cdx'] ? $arDxs : '' )."\"". PHP_EOL;
							$ii++;
					...
					// $datos_hos .= $value1['pachis'].";".trim($value1['pacnum']).";".trim($value1['pacap1'])." ".trim($value1['pacap2'])." ".trim($value1['pacnom']).";".trim($value1['pacfec']).";".trim($value1['pachor']).";".trim($value1['trahab']).";".trim($value1['pacpol']).";".trim($value1['pacced']).";".trim($value1['cuenta_total']).";".trim($value1['pachos']).";".trim($value1['pacmrepla']).";\"".( $value['cdx'] ? implode( "\n", $arDxs ) : '' )."\"". PHP_EOL;
				...
				$datos_hos = $datos_hos."\n".($cuantos_total)." pacientes ".( $value['scu'] ? number_format($suma_total,2) : '' );
