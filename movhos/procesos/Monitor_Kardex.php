...
if (!isset($_SESSION['user']))
	...
	function consultarEdad($historia, $completa)
	{
		...
		if ($num > 0) {
			...
			//Edad
			$ann = (int)substr($fechaNacimiento, 0, 4) * 360 + (int)substr($fechaNacimiento, 5, 2) * 30 + (int)substr($fechaNacimiento, 8, 2);
			$aa = (int)date("Y") * 360 + (int)date("m") * 30 + (int)date("d");
			$ann1 = ($aa - $ann) / 360;
			$meses = (($aa - $ann) % 360) / 30;
			if ($ann1 < 1) {
				$dias1 = (($aa - $ann) % 360) % 30;
				$wedad = (string)(int)$meses . " mes(es) " . (string)$dias1 . " dia(s)";
				$anos = 0;
			} else {
				$dias1 = (($aa - $ann) % 360) % 30;
				$wedad = (string)(int)$ann1 . " a&ntilde;o(s) " . (string)(int)$meses . " mes(es) " . (string)$dias1 . " dia(s)";
				$anos = (int)$ann1;
			}
					...
					for ($i = 1; $i <= $j - 1; $i++) {
						...
						if ($j > 6)  //Numero de lineas que se muestran en pantalla por defecto si es mayor lo muestro en dos grupos de columnas
						{
							...
							if ($wopcion != "12") {
								...
								if (count($wsuspendidos) > 0) {                   //  Arreglo de suspen., Historia     
							...
							if ($wopcion != "12") {
								...
								//*** S U S P E N D I D O S ***
								if (count($wsuspendidos) > 0) {                   //  Arreglo de suspen., Hístoria     
									mostrar_suspendidos_CM($wsuspendidos, $wmat_estado[$i][1]);
								}
							}
							echo "</tr>";
			...
			if ($wopcion == 5) //se hace condición para que solo muestre la tabla de pacientes con la informacion de medicamento
			{
				...
				for ($i = 1; $i <= $num_pacientes; $i++) {
					...
					if ($westado == "on") {
						...
						// $q = "SELECT kadcpx, kadpro, kadart, CONCAT_WS( ' ', Percan, Peruni ) as Frecuencia, kadori, Kadfin, Kadhin, Viades, Kadobs, Kadfec, DK.Fecha_data"
						// ." FROM ".$wbasedato."_000054 DK, ".$wbasedato."_000040, ".$wbasedato."_000043"
						// ." WHERE Kadhis  = '".$whis."'"
						// ." AND   Kading  = '".$wing."'"
						// ." AND   Kadvia  =   Viacod "
						// ." AND   Kadfec  =   Kadfec "
