...
if(isset($accion))
{
	switch($accion)
	{
			...
			else
			{
				...
				if($rowObtConsec = mysql_fetch_array($resObtConsec))
				{
					...
					$sqlAsigTur = "INSERT INTO ".$wbasedato."_000178 (Medico, 			Fecha_data, 			Hora_data, 				Atutur, 			Atudoc, 				Atutdo, 				Atuest, Atusea, Atunom,					Atueda,					Atuted,				Atuten, 				Seguridad, 		id)
															  VALUES ('".$wbasedato."', '".date('Y-m-d')."', 	'".date('H:i:s')."',	'".$nuevoTurno."', 	'".$numDocumento."',	'".$tipDocumento."', 	'on', 	'*', 	'".$nombrePaciente."',	'".$edadPaciente."',	'".$tipoEdad."',	'".$categoriaEmp."',	'C-".$wuse."',	'')
					";
		...
		case 'calcularEdad':
		{
			...
			$edadAños 	= floor($fecha/31556926);
			
			if ($edadAños > 0)
			{
				$respuesta['cantidad'] 	= $edadAños;
				$respuesta['tipo'] 		= 'A';
				$respuesta['tipoDoc'] 	= (($edadAños >= 18) ? 'CC' : '');				
			}
			else
			{
...
//=====================================================================================================================================================================
				...
				case "Enter":
				{
					...
					if(arrayTextoLector.length == 13 && Number.isInteger(arrayTextoLector[0]*1))
					{
						...
						if(Number.isInteger(primerCaracterDelsegundoTab*1))
						{
							...
							año 			= arrayTextoLector[6].substr(1,arrayTextoLector[6].length)+arrayTextoLector[7].substr(0,1);
							...
							fechaNacimiento = $.trim(año+"-"+mes+"-"+dia);
						}
						// --> Sino, es una cedula
						else
						{						
	...
	function generarTurno(validarExisteTurno, turnoACancelar)
	{
		...
		{
			$("#divMensaje").html("Debe seleccionar si la edad corresponde a: años, meses o días.");
			return;
		}
		...
		{
			$("#divMensaje").html("Debe ingresar el tipo de entidad.");
			return;
		}
	...
	function imprimirTurno(fichoTurno)
	{
		...
		$("#fichoTurno").html(fichoTurno+"<br>").dialog({
			...
			modal: true,
			title: "<div align='center' style='font-size:1.2rem'>Por favor tome su tiquete.<br>En un momento será atendido.</div>",
			width: "23rem"
		});
					...
					<td align='center' colspan='2'>
						...
						foreach($arrTipDoc as $codTipDoc => $nomTipDoc)
						{
							...
							<td style='padding:2px'>
								...
								<label onClick='obtenerNombrePaciente()' style='border-radius:0.4em;width:2rem;height:1.8rem' for='radio".$codTipDoc."'>&nbsp;</label>&nbsp;&nbsp;".$nomTipDoc."
							</td>
							".(($x == 3) ? "</tr>" : "");
	...
	echo "		<tr>
				...
				<tr>
					...
					<td class='radio' style='color:#333333;font-size: 1rem;'>
						...
						&nbsp;
						<input type='radio' name='tipoEdad' value='A' id='radioA' />
						<label style='border-radius:0.4em;width:2rem;height:1.8rem' for='radioA'>&nbsp;</label>&nbsp;&nbsp;Años
						<input type='radio' name='tipoEdad' value='M' id='radioM' />
						<label style='border-radius:0.4em;width:2rem;height:1.8rem' for='radioM'>&nbsp;</label>&nbsp;&nbsp;Meses
						<input type='radio' name='tipoEdad' value='D' id='radioD' />
						<label style='border-radius:0.4em;width:2rem;height:1.8rem' for='radioD'>&nbsp;</label>&nbsp;&nbsp;Días
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan='2' align='center'>			
						...
						foreach($arrCategEmp as $codCatEmp => $nomCatEmp)
						{
							...
							<td style='padding:2px'>
								&nbsp;&nbsp;
								<input type='radio' name='categoriaEmp' value='".$codCatEmp."' id='radio".$codCatEmp."' />
								<label style='border-radius:0.4em;width:2rem;height:1.8rem' for='radio".$codCatEmp."'>&nbsp;</label>&nbsp;&nbsp;".$nomCatEmp."
							</td>
							".(($x == 4) ? "</tr>" : "");
			...
			<div id='tecladoFlotante' style='display:none;z-index:10000;position: absolute;' elementoEscribir=''>
				<table style='background-color:#FFFFFF;border:1px solid #AFAFAF;margin-top:1.3rem;border-radius:0.4em;border-collapse: separate;border-spacing: 0.4rem 0.4rem;'>
					...
					<tr>
						<td><input class='botonteclado' type='button' onclick='teclado(this)' value='-'></td>
						<td colspan='6' align='center'><input class='botonteclado' type='button' style='width:14.8rem;' onclick='teclado(this)' value=' '></td>
