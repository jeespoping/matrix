...
if(isset($operacion) && $operacion == 'marcarmuerte_hospitalizacion'){
	...
	if ($wcentral == FALSE)
	 ...
	 $q=" SELECT ROUND(TIMESTAMPDIFF(HOUR,Fecha_ing,now())/24,2), Num_ing_Serv "
	...
	$query = "	SELECT 	ROUND(TIMESTAMPDIFF(HOUR,(CONCAT ( Fecha_data, ' ', Hora_data )), now() )/24,2) AS diferencia  "
	...
	//$wcentral="CAMILLEROS";
		...
		if ($rowmue[0]!="on")  //No pide el camillero si el paciente Murio, porque se pidio cuando marco la muerte
			...
			$q = " INSERT INTO ".$wcencam."_000003 (   Medico     ,   Fecha_data,   Hora_data,   Origen     , Motivo           ,   Habitacion  , Observacion                                                                                                , Destino ,    Solicito    ,    Ccosto  , Camillero, Hora_respuesta, Hora_llegada, Hora_Cumplimiento, Anulada, Observ_central,    Central     , Seguridad        ) "
						."                  VALUES ('".$wcencam."','".$wfecha."','".$whora."','".$worigen."','PACIENTE DE ALTA','<b>".$whab_actual."</b><br>".$whabpac."' , 'Se dio alta definitiva desde gestion de enfermeria a la Historia: ".$whis."-".$wing." a las ".$whora."' , 'ALTA'  , '".$wusuario."', '".$wcco."', ''       , '00:00:00'    , '00:00:00'  , '00:00:00'       , 'No'   , ''            , '".$wcentral."', 'C-".$wusuario."')";  
			$err = mysql_query($q,$conex) or die (mysql_errno().$q." - ".mysql_error());
			//=======================================================================================================================================================
			} 
	}
...
if(isset($operacion) && $operacion == 'trasladoayuda'){
       ...
       $datamensaje = array('mensaje'=>'', 'notificacion'=>0 , 'formulario'=>'');
		...
		$select_tipo_cama .= "<SELECT id='cco_ayuda'>";
		$select_tipo_cama .= "<option></option>";
		
		while($row_cco = mysql_fetch_array($res_cco))
		   {			
			$select_tipo_cama .= "<option value='".$row_cco['ccocod']." - ".$row_cco['cconom']."'>".$row_cco['ccocod']." - ".$row_cco['cconom']."</option>";
		   }
		...
		$datamensaje['formulario'] .= "<table style='text-align: center; width: 100px;' border='0'>
										<tr class = fila1>
										<td colspan=2><b>Seleccionar centro de costos ayuda<b></td>
										</tr>
										<tr class = fila1>
...
function EntregaDesdeCirugiaAPiso(whis, wing, nombre, hab_destino, id_solicitud, med_pend){
	...
	function cambiarJustificacion( cmp, whis, wing, wfecha_actual, whora_par_actual, wpac, wido, idx ){
	...
	function aplicarArticulo( cmp, whis, wing, wfecha_actual, whora_par_actual, whab, wpac, wido, idx, art, cvi ){
	...
	function anularAplicacion( cmp, whis, wing, wfecha_actual, whora_par_actual, whab, wpac, wido, idx, art, wdosis, wcanfra, wnoenviar, wStock ){
	...
	window.onload = function(){
		...
		if( document.getElementById( "inRecargar" ).value == "on" ){
			...
			timer = setTimeout( "recargar();", 10000 );	//Recarga la pagina cada 5 minutos
		}
...
class habitacion{
	...
	function habitacion( $cco, $ccoNombre, $hab, $historia, $ingreso, $nombre, $enTraslado, $altaProceso, $fechaAltaProceso, $horaAltaProceso, $pacced, $pactid, $codigoEnti, $nombreEnti ){
		...
		for( $i = date( "G", $fechaFinal-$tiempoAMostrar*3600 ); $i < date( "G", $fechaFinal-$tiempoAMostrar*3600 )+$tiempoAMostrar; $i += 2 ){
			...
			$puedeAplicar = false;
			if( $pacConOrdenes && $ccoConOrdenes && esUrgencias( $conex, $cco ) ){
...
function consultarEstadosExamenesRol(){
	...
	$q = "SELECT Eexcod,Eexdes,Eexapa,Eexcan,Eexrea
...
function examenes_procedim_pend($whistoria, $wingreso){
	...
	$q = "  SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, Codcups as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle
			FROM
			...
			UNION
			SELECT
				Ordhis,Ording,Ordtor,Ordnro,Ordobs,Ordesp,Ordest,Ordfir,Dettor,Detnro,Detcod,Detesi,Detrdo,Detest,Detfec,Detjus,d.descripcion as Cconom,c.Descripcion,Protocolo, Detite, Tipoestudio,Ordusu,Detusu,Detalt,Detimp, Tiprju, c.Codigo as Codigo_cup, NoPos, {$whce}_000028.id as id_detalle
			FROM
		...
		$array_datos_procedimientos = array('array_procedimientos'=>$array_ordenes, 'cantidad_pendientes'=>$num_pye);
...
function pintarDatosFila( $datos ){
	...
	$q_cub =   " SELECT Habcod, Habcpa, Habzon  "
			 . "   FROM ".$wbasedato."_000020 "
			 . "  WHERE habcub = 'on'"
			 . "	AND habdis = 'on'"
			 . "	AND habhis = '' "
	...
	$array_cubiculos = array();
	while( $row_cubiculos = mysql_fetch_assoc($res_cub)) {
		$array_cubiculos[$row_cubiculos['Habzon']][$row_cubiculos['Habcod']] = $row_cubiculos;
	}
	...
	if( count($datos) > 0 ){
		...
		//Nomenclatura
		echo "<table align='center' style='border-radius: 35px; $ocultar' >";
		echo "<tr style='opacity:90'>";
		echo "<td class='fondoAlertaConfirmar' align='center' colspan='1' onMouseover='mostrarTooltip( this );'><p title='Mezclas sin confirmar'>Mezclas sin<br>confirmar</p></td>";
		echo "<td class='fila1' align='center' colspan='1' onMouseover='mostrarTooltip( this );'><p title='Sin Generar Kardex Hoy'><img src='/matrix/images/medical/movhos/NOTE16.ico'></p></td>";
		echo "<td class='fila1' align='center' colspan='1' onMouseover='mostrarTooltip( this );'><p title='Kardex Sin <br>Confirmar Hoy'><img src='/matrix/images/medical/movhos/Key04.ico'></p></td>";
		echo "<td class='fila1' align='center' colspan='2' onMouseover='mostrarTooltip( this );'><p title='Todos los medicamentos <br>obligatorios han sido aplicados'><img src='/matrix/images/medical/movhos/checkmrk.ico'><br>Aplicado</p></td>";
		echo "<td class='fondorojo' align='center' colspan='2' onMouseover='mostrarTooltip( this );'><p title='Al menos un medicamento <br>obligatorio no ha sido aplicado'><img src='/matrix/images/medical/root/borrar.png'><br>Sin Aplicar <br>o Sin Dieta</p></td>";
		echo "<td class='fondoamarillo' align='center' colspan='2' onMouseover='mostrarTooltip( this );'><p title='Los medicamentos no han sido aplicados <br>pero tienen justificaci&oacute;n o son a necesidad'><img src='/matrix/images/medical/root/borrarAmarillo.png'><br>Sin Aplicar</p></td>";
		echo "<td style='background-color:#3CB648;' class='fondoVerde' align='center' colspan='2' onMouseover='mostrarTooltip( this );'><p title='Órdenes Electrónicas'>Órdenes <br>Electrónicas</p></td>";
		echo "</tr>";		
		echo "</table>";
		...
		if($wsp == 'on' and $mostrar == 'on' and $slCcoDestino != ''){
			...
			if(!isset($registro_inicial)){
				...
				$registro_inicial = $registro_inicial+10;					
			}
			...
			$datos = array_slice($datos, $registro_inicial, 10);
			$paginas = floor($canti_pacientes / 10)+((($canti_pacientes % 10) > 0) ? 1 : 0);
			echo "<table width='100%'><tr><td width='30%'><font size=5>Pág. ".(($registro_inicial/10)+1)." de ".$paginas."</font></td>";			
			echo "<td width='40%'><table><tr><td><img src='../../images/medical/root/clinica.jpg' heigth='36' width='80'></td><td class=fila1><font size=5>".$datos_cco->codigo." - ".$datos_cco->nombre."</font></td><td><img src='../../images/medical/root/fmatrix.jpg' heigth='36' width='80'></td></tr></table></td>";
			echo "<td width='30%'></td>";
			echo "</tr></table>";
		...
		foreach( $datos as $keyDatos => $valueDatos ){
			...
			$wfecha_hora_llegada = $valueDatos->fecha_hora_llegada;
			$disabled_sala = "";
			$disabled_cubiculo = "";			
			$westado_checkbox = "";
			$hab_destino = "";
			...
			if( $i == 0 ){
				...
				echo "<td colspan=4 style='cursor: pointer' onclick='ejecutar(".chr(34).$path.chr(34).")'><font size=4>Contingencia</td>";
				/*if( $tiempoAMostrar == 24 )
				  {
				    $wcol=16-$wc;
				...
				if($wurgencias){
					...
					$ruta_dietas_aux = "Dietas.php?wemp_pmla=".$wemp_pmla."&wser=".$wcodser."&wcco=".$valueDatos->cco."&wfec=".$fecha;
					$path_dietas_seractual_aux = " style='cursor: pointer' onclick='ejecutar(".chr(34).$ruta_dietas_aux.chr(34).")";

					$ruta_dietas_aux_sgte = "Dietas.php?wemp_pmla=".$wemp_pmla."&wser=".$wcodser_Sig."&wcco=".$valueDatos->cco."&wfec=".$fecha;
				...
				if ($wnum > 0)
				   {
					...
					echo "<td colspan=6 id=td_cco><font size=4><b style='cursor: pointer' onclick='ejecutar(".chr(34).$path.chr(34).")'>".$datos_cco->codigo." - ".$datos_cco->nombre." ddsdsd ====>  </b></font></td>";
					
				    echo "<td colspan=$colspan><table>";
					for ($i=1;$i<=$wnum;$i++)
					   {
					    $row = mysql_fetch_array($wmsjCPA);
						$path = "Dietas.php?wemp_pmla=".$wemp_pmla."&wser=".$row[1]."&wcco=".$valueDatos->cco."&wfec=".$fecha;
						echo "<tr>";
					...
					echo "</table></td>";
				      ...
				      echo "<td colspan=$colspan id=td_cco><font size=4><b>".$datos_cco->codigo." - ".$datos_cco->nombre."</b></font></td>";
				   }
				echo "</font></td>";
				echo "</tr>";
				...
				*/
				...
				$wbasedatocliame = consultarAliasPorAplicacion($conex, $wemp_pmla, 'facturacion');
				$qccotrasautomaticos =   "	 SELECT Gescco   , Gesvis , Gesfhc , Gescam , Gesdhc ,Geshab
				...
				*/
				...
				if($wurgencias){
				echo "<td rowspan=2 width=200px>Tiempo atención
					<table>
					<tr>
					<td style='background-color:#FCEF69;font-size: 7pt;border-radius: 4px;border:1px solid #999999;padding:2px'>&nbsp;> $wtiempo_minimo horas&nbsp;</td>
					</tr>
					<tr>
					<td style='background-color:#FDCB7F;font-size: 7pt;border-radius: 4px;border:1px solid #999999;padding:2px'>&nbsp;> $wtiempo_maximo horas&nbsp;</td>
					</tr>				
					</table>
					</td>";
				}
				...
				if($waux == 'on'){				
					...
					if($ccoayuda == '' and !$wurgencias and !$wcirugia){
						echo "<td rowspan=2>Traslado ayuda</td>";
					}
				}
					...
					if($fecha == date( "Y-m-d" ))
					{
						...
						if($cco_activoDesdeHce =='on')
						{
							...
							else
							{
								   ...
								   if($row_sql = mysql_fetch_array($res_select))
								   {
										...
										else
									   {
											if($habitacionseleccion=='')
											{
												...
												$es_diferente = "UNO<div  class='cambio_habitacion_automatico' historia='".$valueDatos->historia."' ingreso='".$valueDatos->ingreso."' habitacion='".$valueDatos->codigo."'  cco='".$valueDatos->cco."' tipo_habitacion_cambio='".$habitacionseleccion."' >";
									   }
								   }
								...
								{
									$es_diferente = "DOS<div  class='cambio_habitacion_automatico' historia='".$valueDatos->historia."' ingreso='".$valueDatos->ingreso."' habitacion='".$valueDatos->codigo."'  cco='".$valueDatos->cco."' tipo_habitacion_cambio='".$vectorhabitacioneshce[$whcehab]."' >";
								}
							}
						}
			...
			//---------------
				...
				{	
					...
					$minutos = (strtotime(date("Y-m-d H:i:s")) - strtotime($wfecha_hora_llegada)) / 60;
					$minutos = abs($minutos); 
            ...
            if ($wdietas_Sig)
			   {
				...
				$path = "Dietas.php?wemp_pmla=".$wemp_pmla."&wser=".$wcodser_Sig."&wcco=".$valueDatos->cco."&wfec=".$fecha;
				if ($waux != 'on') //Si es diferente a Auxiliar de enfermeria
				   echo "<td align='center' style='cursor: pointer' onclick='ejecutar(".chr(34).$path.chr(34).")'>";
				  else
                   echo "<td align='center' '".$path_dietas_seractual_aux."'>";
			...
			//====================================================================================================================
				...
				if($wir_a_ordenes == 'on' and $paciente_ordenes == 'on'){
					...
					$origen = "Ordenes";
					$path_destino = "/matrix/hce/procesos/ordenes.php?wemp_pmla=".$wemp_pmla."&wcedula=".$valueDatos->documento."&wtipodoc=".$valueDatos->tid."&hce=on&programa=gestionEnfermeria&et=on&pgr_origen=gestionEnfermeria";
					$path_destino_consulta = "/matrix/hce/procesos/ordenes.php?wemp_pmla=".$wemp_pmla."&wcedula=".$valueDatos->documento."&wtipodoc=".$valueDatos->tid."&hce=on&programa=gestionEnfermeria&et=on&editable=off&pgr_origen=gestionEnfermeria";
			...
			if($wurgencias){
				...
				//Alta en proceso
				echo "<td align=center id='td_alta_proceso_$valueDatos->historia-$valueDatos->ingreso'><input type=checkbox $checked_proc_tras $checked_alta_proc class='msg_alta_proceso' title='Poner de Alta' id='alta_proceso_$valueDatos->historia-$valueDatos->ingreso' $westado_checkbox onclick='marcaraltaenproc_hospitalizacion(\"$valueDatos->historia\",\"$valueDatos->ingreso\", \"$wtraslado\", \"$westado_altaEnProceso\", \"$valueDatos->nombre\", \"$wbasedato\", \"$whce\", \"$wuser\", \"$valueDatos->id_tabla18\")'></td>";
				//Alta definitiva
				echo "<td align=center><input type=checkbox $checked_proc_tras $whabilitar_altadef class='msg_alta_proceso' title='Poner de Alta Definitiva' id='alta_definitiva_$valueDatos->historia-$valueDatos->ingreso' $westado_checkbox onclick='marcaraltadef_hospitalizacion(\"$valueDatos->historia\",\"$valueDatos->ingreso\", \"$wtraslado\", \"$westado_altaEnProceso\", \"$valueDatos->nombre\", \"$wbasedato\", \"$whce\", \"$wuser\", \"$valueDatos->id_tabla18\", \"$whab_actual\", \"\", \"$valueDatos->cco\")'></td>";
				//Muerte
				echo "<td align=center><input type=checkbox $checked_proc_tras $checked_muerte class='msg_muerte' title='Muerte' id='muerte_$valueDatos->historia-$valueDatos->ingreso' $westado_checkbox onclick='marcarmuerte_hospitalizacion(\"$valueDatos->historia\",\"$valueDatos->ingreso\", \"$wtraslado\", \"$westado_altaEnProceso\", \"$valueDatos->nombre\", \"$wbasedato\", \"$whce\", \"$wuser\", \"$valueDatos->id_tabla18\", \"$whab_actual\", \"\", \"$valueDatos->cco\")'></td>";
			...
			if($wurgencias){
				...
				echo "<div align=center>".$wconducta."<hr></div>";
			...
			if($wurgencias){
				...
				if($array_zonas[$array_his_cub[$valueDatos->historia.$valueDatos->ingreso]['habzon']]['Arecod'] != ""){
					 ...
					 $array_cubiculos_aux = $array_cubiculos[$array_zonas[$array_his_cub[$valueDatos->historia.$valueDatos->ingreso]['habzon']]['Arecod']];
					...
					//Si el paciente tiene sala asignada se muestra todo el listado de los cubiculos disponibles de esa sala.
						...
						if(is_array($array_cubiculos_aux)){
							...
							$array_cubiculos_aux = $array_cubiculos[$wconsala];
							echo "<option value=''></option>";
				...
				default:
						...
						if(is_array($wpacientes_por_entrega)){	
							...
							if(array_key_exists($his_ing, $wpacientes_por_entrega)){
								...
								$wser_destino = $todoslashab[$hab_asignada]['Habcco']; //Centor de costos de la habitacion asignada
								...
								if($hab_asignada != ''){
									...
									if(is_array($wpacientes_por_recibir)){
											...
											$no_recibir = "<div style='color:#000000;border-radius: 4px;border:1px solid #999999;padding:2px;background-color:#ffffff;font-size:10px;white-space:nowrap;cursor: pointer;' class='cancelar_recibo' title='Cancelar recibo' id='canc_traslado_".$valueDatos->historia."-".$valueDatos->ingreso."' onclick='cancelarReciboHospitalizacion( \"".$wemp_pmla."\", \"".$valueDatos->historia."\",\"".$valueDatos->ingreso."\",\"".$wser_anterior."\")'>Cancelar traslado</div>";
											...
											if($hab_asignada != ''){
													...
													if($datos_med_pac['wnum_art'] > 0){
														...
														echo $no_recibir;
														echo "<div align='center' style='display:none;cursor:default;background:none repeat scroll 0 0;position:relative;width:100 %;height:450px;overflow:auto;cursor: pointer' id='medicamentos_pendientes_$his_ing' title='Pendientes por aplicar'>";									
														echo $datos_med_pac['html'];
														echo "</div>";
														echo "</td>";
													}else{																																						
							...
							if($ubi_pac->servicioTemporal != ''){
								...
								if($ubi_pac->enProcesoTraslado == 'on'){
									...
									if($ubi_pac->altaEnProceso != 'off'){
										...
										echo "<td align=center class='mover_pac' id='cajon_trasl_ayuda_".$valueDatos->historia."_".$valueDatos->ingreso."' title='Trasladar ayuda diagnóstica' onclick='trasladoayuda(\"".$valueDatos->historia."\",\"".$valueDatos->ingreso."\",\"".$valueDatos->nombre."\", \"$ccoCodigo\", \"".$ubicacionPaciente->habitacionActual."\", \"".$wbasedato."\");' style='cursor:pointer;'><img id='pedir_cama_".$valueDatos->historia."_".$valueDatos->ingreso."'></td>";
									}
								}
							}
						}				
			...
			if($wir_a_ordenes == 'on'){
				...
				if($examenes_procedim_pend['cantidad_pendientes'] > 0){
					echo "<td colspan=2 align=center style='cursor:pointer;' onclick='fnMostrarProcedimientos(\"$valueDatos->historia\", \"$valueDatos->ingreso\");'>";
				}else{
				...
				echo "<INPUT TYPE='button' value='Cerrar' onClick='$.unblockUI();' style='width:100'><br><br>";
				...
				//print_r($examenes_procedim_pend['array_procedimientos']);
				foreach( $examenes_procedim_pend['array_procedimientos'] as $keyProcedimientos => $allProcedimientos ){
					...
					foreach($allProcedimientos as $key_p => $valueProcedimientos){
						...
						crearCampo("6","westadoexamen_$wordid_detalle",@$accionesPestana[$indicePestana.".9"],array("class"=>"campo2","medAsociado"=>$medAsociado,"onChange"=>"cambiar_estado_examen('$wemp_pmla','$wfecha1','$wexam','".$valueProcedimientos[ 'Ording' ]."','".$valueProcedimientos[ 'Ordhis' ]."', '$wordennro', '$wordite',this  , '$wordid_detalle', '$whce','$wnombre_examen','$westado_registro','$wbasedato','$wfechadataexamen','$anterior','$tieneMedicamentos');"),"$opcionesSeleccion");
			...
			// foreach( $valueDatos->horas as $keyHoras => $valueHoras ){
				...
				elseif( ( @$valueDatos->totalArticulos[ 'total' ] == 0 || @$valueDatos->totalArticulos[ 'total' ] == @$valueDatos->articulosNecesidadSinRonda['total'] ) ){
					...
					echo "<td $classCelda align='center' onClick='fnMostrar( this )' ronda='$keyHoras'>";
					...
					if( count($valueHoras->medicamentos) > 0 && !empty($valueHoras->color) ){
						...
						foreach( $valueHoras->medicamentos as $keyMedicamentos => $valueMedicamentos ){
							...
							if( $mostrarAplicaciones ){
								...
								echo "<select name='slJus' onchange='cambiarJustificacion( this, {$valueDatos->historia}, {$valueDatos->ingreso}, \"".date( "Y-m-d", $fechaFinal-$tiempoAMostrar*3600 + $l*3600 +1 )."\", \"".date( "H", $fechaFinal-$tiempoAMostrar*3600 + $l*3600 +1 )."\", \"pppp\", {$valueMedicamentos[ 'idOriginal' ]}, {$valueMedicamentos[ 'indexMed' ]} )'>";
...
if(!isset($_SESSION['user'])){
	...
	if( $mostrar == "off"  ){
		...
		echo "<tr><td class=fila1>Seleccionar Zona:</td><td class=fila2><span id='select_zonas'>sadsds</div></td></tr>";
		echo "<tr><td> &nbsp; </td></tr>";
		echo "</table>";
		...
		$q_pacientes_ordenes = " SELECT concat( ubihis,ubiing ) as hising
								   FROM root_000036, root_000037, ".$wbasedato."_000018, ".$wbasedato."_000016, ".$wbasedato."_000011, ".$wbasedato."_000053 f $tabla_habitaciones
								  WHERE ubihis = orihis
									AND ubiing = oriing
									AND oriori = '$wemp_pmla'
									AND oriced = pacced
		...
		switch(1){
			...
			default:
					...
					$sql = "SELECT
								...
								Cconom, Ubiptr, Ubialp, Ubifap, Ubihap, pacced, pactid, Ingres, Ingnre, {$wbasedato}_000018.id as id_tabla18, Ubisan
							FROM
								{$wbasedato}_000020,{$wbasedato}_000018,{$wbasedato}_000011,{$wbasedato}_000016,
								root_000036, root_000037 $tabla_habitaciones
							WHERE
								habcco = '$ccoCodigo'
								AND habest = 'on'
								AND ubihis = habhis
		...
		if( $numHabitaciones > 0 ){
			...
			for( $i = 0; $rowsHabitaciones = mysql_fetch_array( $resHabitaciones ); $i++ ){
				...
				$datosHabitaciones[ $i ]->fecha_hora_llegada = $rowsHabitaciones['18_fecha_data']." ".$rowsHabitaciones['18_hora_data']; //Fecha y hora de llegada
				...
				if( $pac_tieneKardex_hoy == 'on' or $pac_tieneKardex_ayer == 'on'){
					...
					for( $fechaInicial = $fechaFinal-$tiempoAMostrar*3600+1; $fechaInicial <= $fechaFinal; $fechaInicial += 2*3600 ){
						...
						$wfecha_a_buscar = date( "Y-m-d", $fechaInicial );	//Abril 24 de 2013
						...
						if( $datosHabitaciones[ $i ]->tieneKardex[ $fecha ] != 'on' /*|| $datosHabitaciones[ $i ]->kardexConfirmado[ $fecha ] != 'on'*/ ){
							...
							//Busco si el kardex del día anterior esta confirmado o no
							if( !isset( $datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] ) ){
								$datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] = consultarKardexConfirmado( $datosHabitaciones[ $i ]->historia, $datosHabitaciones[ $i ]->ingreso, date( "Y-m-d", $fechaInicial-24*3600 ) );
							}

							if( $datosHabitaciones[ $i ]->kardexConfirmado[ date( "Y-m-d", $fechaInicial-24*3600 ) ] == 'on' ){	//Kardex del día anterior confirmado
								query_articulos( $rowsHabitaciones['Habhis'], $rowsHabitaciones['Habing'], date( "Y-m-d", $fechaInicial-24*3600 ), &$resArticulos );
							}
							else{ //Kardex del día anterior sin confirmar
								query_articulos_kardex_no_confirmado( $rowsHabitaciones['Habhis'], $rowsHabitaciones['Habing'], date( "Y-m-d", $fechaInicial-24*3600 ), &$resArticulos );
								$datosHabitaciones[ $i ]->kardexConfirmado[ $fecha ] = 'off';
							}
							...
							if( $datosHabitaciones[ $i ]->kardexConfirmado[ $fecha ] != 'on' ){		//Si el kardex esta sin confirmar
								...
								query_articulos_kardex_no_confirmado( $rowsHabitaciones['Habhis'], $rowsHabitaciones['Habing'], $fecha, &$resArticulos );
								$fechaBaseKardex = $fecha;
								/******************************************************************************************/
							}
							else{		//Kardex confirmado
