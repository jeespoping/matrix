...
function generarQueryCombinado($variables, $tabla, $filtro, $filtro_aux)
{
	...
	if($num_rows == 0){
	 ...
	 die ("<br>\n<br>\n".
        " <H1>No puede ingresar al programa de Solicitud de Dietas desde un equipo externo a la Cl�nica Las Am�ricas <br><br> Volver al <a href='//mx.lasamericas.com.co/matrix/f1.php'> inicio </a>");
...
//2019-10-28 Camilo Zapata: Ahora el programa se refresca despues de darle click al bot�n grabar en la modal de patr�n SI(Servicio Individual), para que
                            habilite los campos de observaciones e intolerancias inmediatamente. buscar por fecha si es necesario.
//2019-10-23 Camilo Zapata: Modificaci�n en el programa para evitar caracteres erroneos en el nombre del patr�n DSN wpatron_dsn_asociado, de tal manera que
                            Como en la modificaci�n anterior ya se garantiza la configuraci�n ut8 en el momento de grabar el registro, no haga ninguna transformaci�n posterior, manteniendo los caracteres tal cual est�n grabados en la bd.
//2019-10-10 Camilo Zapata: Modificaci�n en la funci�n que hable la modal para DSN, para que calcule su respectivo tama�o sin asignar un tiempo de recarga.
                             y correcci�n en caracteres, en observaciones y nombres en el dns.(Buscar fecha de ser necesario.)
//2019-10-03 Camilo Zapata: Modificacion para la validaci�n del tama�o de texto permitido en observaciones e intolerancias. ya que son 350 caracteres
                            que se deben repartir entre los dos.
//2019-09-24 Camilo Zapata: Se verifica si un servicio(DSN) para una historia e ingreso ya existe programado para un dia espec�fico,
                            En caso de que ya exista se guardara en el log, la modificaci�n de la dieta, si no existe se almacenar� como nuevo pedido.
//2019-09-23 Camilo Zapata: Adici�n del nombre espec�fico de la zona y la habitaci�n para cada uno de los pacientes de urgencias.
//2019-09-19 Camilo Zapata: Se realiza las modificaciones necesarias para que en los servicios hospitalarios y urgencias se puedan visualizar
                            todos los pacientes que est�n en el centro de costos definido y zona( cuando existan ); En caso de ser necesario
                            buscar "wzona".
//Enero 24 de 2019 Edwin MG
//- No se permite modificar observaciones o intolerancias si no hay un patr�n seleccionado
//- Las observaciones se traen de acuerdo al �ltimo servicio
//=========================================================================================================================================\\
//Diciembre 21 de 2018 Edwin MG
//- SE modifica query que trae las observaciones del �ltimo servicio
//=========================================================================================================================================\\
//Diciembre 17 de 2018 Edwin MG
//Se realizan cambios varios:
//	- En la auditor�a (movhos 78) al hacer un cambio en observaciones queda la observaci�n anterior y la nueva
//	- Al empezar a escribir en el campo de observaciones el tiempo de refrescar la p�gina se detiene, una vez se deja de escribir pasados 5
//	  segundos las observaciones se graban y 45 segundos despues la p�gina se refresca
...
//Se corrige el redondeo en la edad para que tenga en cuenta los pacientes de menos de 1 a�oy asi se haga correctamente el cobro de la dieta.
//=========================================================================================================================================\\
...
//Se agrega en cada area de texto la validaci�n del m�ximo de caracteres, esto con el fin de que la informaci�n escrita no supere el tama�o de
//la tarjeta de dietas y asi no se pierda informaci�n en la impresi�n.
//=========================================================================================================================================\\
...
//Se crea un nuevo campo en movhos_000018 Ubidie, los registros que est�n en 'on' en este campo, esas hitorias se listan para la solicitud de dietas
//en urgencias.
//=========================================================================================================================================\\
//Agosto 14 de 2013
...
//=========================================================================================================================================\\
//Julio 17 de 2013
//Se valida que el servicio individual en el ultimo servicio no da�e la DSN del paciente, ademas que la DSN se marque en la primera carga del
//programa.
//=========================================================================================================================================\\
//Julio 12 de 2013
...
//ahora para obtener este dato lo har� en este mismo script.
//=============================================================================================================================================
//Noviembre 20 - J�natan Lopez
            ...
            if (trim(patron_asociado) != '')
                {
                    ...
                    //Valida que las cantidades si esten correctas
                    if(!esok) { alert("La cantidad o cantidades marcadas con rojo est�n erradas."); return; }
                  ...
                  {
                      alert('Debe asignar patr�n a la dieta.');
                      $("#ptr_dsn_text").focus();
                  }
...
� � �
    ...
    //Marca la historia de urgencias que tiene mas de 3 d�as para hacerle pedidos de alimentacion.
	...
	function grabar_observ_intoler(wemp_pmla, basedato, historia, ingreso, habitacion, servicio, i, j, usuario, obsint, centro_costos, fecha_interfaz)
		{
		  ...
		  try
		  {
			...
			if( ajax.responseText == 1 ){
				alert( "Ya existe un servicio posterior. Las observaciones no se reflejar�n en los servicios ya creados" );
			}
                   ...
                   // console.log(ajax.responseText);
                    if (ajax.readyState==4  && ajax.status==200)
                        ...
                        if (x == 1)
                            {
                             este.checked=false;
                             alert('El producto seleccionado no tiene costo en la tabla 000082, para el Patr�n: '+patron+' ** en la historia: ** '+historia+' **')
                             return;
                            }
	...
	function grabar_servindiv(wemp_pmla, basedato, codigo, fila, columna, patron, historia, ingreso, servicio, valor_neto, usuario, habitacion, centro_costos, estado, este, posicioninput, cancelo, campo_texto, fecha_interfaz, clasificacion, posicion)
        ...
        function confirmar_cancelacion(wemp_pmla, historia, ingreso, c, f, patron, centro_costos, servicio, fecha, habitacion, nom_pac, tipo_doc, doc_pac, proc_trasl, muerte, edad, alta_proc, tipo_empresa, dias_estancia, patron_combinable, usuario, modificar, chequeados_final, combinables, media_porcion, control_pos_quirur, wrol_usuario, wpatron_nutricion, wrolnutricion, windicador)
        {
                ...
                if (windicador != 'dsn')
                    {
                     confirmar=confirm("�Esta seguro de realizar esta operaci�n, cancelar� el servicio para el paciente y no podr� seleccionar otro patr�n o patrones?");
                    }
                    else
                        {
                        confirmar=confirm("                                                                               ! ALERTA � \n �Esta seguro de desea cancelar el patron "+patron+", se cancelar� la DSN para el paciente.?");
                        }
	...
	function grabar_datos(wemp_pmla, historia, ingreso, c, f, patron, centro_costos, servicio, fecha, habitacion, nom_pac, tipo_doc, doc_pac, proc_trasl, muerte, edad, alta_proc, tipo_empresa, dias_estancia, patron_combinable, usuario, modificar, chequeados_final, combinables, media_porcion, control_pos_quirur, wrol_usuario, wpatron_nutricion, wrolnutricion, codDSN, campInput)
		{
		  ...
		  //mostrar� un mensaje de confirmaci�n para la cancelaci�n del servicio.
          if(checkbox == true)
              {
               var estado = 'on';
              }
									...
									if (x == 1 || x == 8 || x == 11 || x == 114 )
										{
										 ...
										 alert('El Patr�n: **'+ patron +'** No tiene costo en la tabla 000079, para el tipo de empresa: ** '+ tipo_empresa +' ** en la historia: ** '+ historia+' **, o la edad del paciente es menor a 6 meses.');
										 }
									 ...
									 if (x == 2)
										{
                                         ...
                                         var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                         var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
										 alert('El Patr�n: **'+ patron +'** no se puede combinar con ning�n otro');
										 }
                                    ...
                                    if (x == 3 || x == 73)
										{
                                         ...
                                         var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                         var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
										 alert('El servicio de la historia seleccionada no puede ser adicionado o modificado porque esta fuera del horario. Favor revisar el inicio de horario de adici�n o el horario final del pedido en la parte superior de la pantalla.');
										 }
									 ...
									 if (x == 4 || x == 44 || x == 441 )
										{
										 cajon.style.backgroundColor="";
										 document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
										 alert('El Patr�n: **'+ patron +'** no se puede combinar con los patrones seleccionados.');
										 }
                                     ...
                                     if (x == 421 )
										{
										 cajon.style.backgroundColor="";
										 document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
										 alert('El patr�n principal de los patrones seleccionados no tiene costo en el servicio actual.');
										 }
                                    ...
                                    if (x == 5)
                                      ...
                                      if (x == 66)
                                       ...
                                       if (x == 76)
                                            ...
                                            var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                            var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
                                            alert('No es posible ingresar a **'+ patron +'** ya que no ha iniciado el horario de adici�n, favor revisar los horarios en la parte superior de la pantalla.');
                                            }
                                       ...
                                       if (x == 'cancelardsn')
                                        ...
                                        if (x == 91)
                                           ...
                                           // confirmar_cancelacion(wemp_pmla, historia, ingreso, c, f, patron, centro_costos, servicio, fecha, habitacion, nom_pac, tipo_doc, doc_pac, proc_trasl, muerte, edad, alta_proc, tipo_empresa, dias_estancia, patron_combinable);
                                            ...
                                            var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                            var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
                                            alert('No es posible modificar o solicitar alimentaci�n para el paciente ya que ha pasado el horario normal de pedido o ha cancelado la solicitud tres veces en el horario de adici�n.');
                                            return false;
                                            }
                                          ...
                                          if (x == 92)
                                           ...
                                           // confirmar_cancelacion(wemp_pmla, historia, ingreso, c, f, patron, centro_costos, servicio, fecha, habitacion, nom_pac, tipo_doc, doc_pac, proc_trasl, muerte, edad, alta_proc, tipo_empresa, dias_estancia, patron_combinable);
                                            ...
                                            var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                            var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
                                            alert('Cancel� el pedido en horario de adicion, solo podr� solicitar servicios individuales o un patr�n desde el siguiente servicio.');
                                            return false;
                                            }
    ...
    //										document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
                                        ...
                                        if (x == 109)// Este caso se da por la combinacion del resultado de las funciones traer_costo_del_patron_asociado y traer_costo_del_patron, ya que no hay costo para el servicio actual ni para el asociado.
                                            ...
                                            var dato_media_porcion = document.getElementById("dato_media_porcion").value;
                                            var media_porcion = document.getElementById("media_porcion"+f.toString()+"-"+dato_media_porcion).disabled=true;
                                            alert('El patr�n no tiene costo para el servicio actual, ni para el servicio asociado.');
                                            return false;
                                            }

                                          if (x == 30)// Este caso se da cuando el usuario intenta seleccionar otro patron que acompa�e a un postquirurgico, si no selecciona el cajon postquirurgico primero, se mostrara este mensaje.
                                            {
                                            ...
                                            alert('No es posible hacer la combinaci�n ya que no ha seleccionado el paciente como postquir�rgico, favor seleccione el caj�n de postquir�rgico y luego seleccione otro patr�n o patrones.');
                                            return false;
                                            }
                                           ...
                                           if (x == 31)// Este caso se da por la combinacion del resultado de las funciones traer_costo_del_patron_asociado y traer_costo_del_patron, ya que no hay costo para el servicio actual ni para el asociado.
                                            ...
                                            alert('No es posible deseleccionar el patr�n ya que no ha deseleccionado el paciente de postquir�rgico.');
                                            return false;
                                            }
                                            ...
                                            if (x == 'seleccionardsn')
                                                {
                                                    ...
                                                    document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
                                                    cajon.style.backgroundColor="";
                                                    alert('Su rol no esta autorizado para seleccionar patr�n.');
                                                    return false;
                                                }
                                            ...
                                            if (x == 'seleccionapatronnodsn_new')
                                                {
                                                    ...
                                                    document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
                                                    cajon.style.backgroundColor="";
                                                    alert('Su rol no esta autorizado para seleccionar este patr�n.');
                                                    return false;
                                                }
                                             ...
                                             if (servicio_sin_dsn == servicios_total)
                                                {
													...
													if(j[1] == 'nohaydsn'){
														cajon.style.backgroundColor="";
														document.getElementById("patron_grid"+f.toString()+"-"+c.toString()).checked=false;
														alert('El paciente no ha tenido pedidos de Dieta Seg�n Nutrici�n en d�as anteriores para este servicio, favor comunicarse con la nutricionista.');
														return false;
													}
													...
													if(j[1] == 'nohaydsn'){
														...
														alert('                                                                    **** ALERTA **** \n  Se reprogram� la �ltima DSN de este paciente, favor revisar el icono de informaci�n.');
                                              ...
                                              if (x == 'alerta_nutricionista')
                                                {
													var mensaje = "�Esta seguro que desea ingresar al patr�n "+patron+"?, cancelar� el patr�n actual del paciente si solicita productos para este servicio.";
											  ...
											  if (x == 'alerta_nutricionista_adicion')
                                                {
													var mensaje = "�Esta seguro que desea ingresar al patr�n "+patron+"?";
                                                    confirmar_solic_dsn(wemp_pmla, historia, ingreso, c, f, patron, centro_costos, servicio, fecha, habitacion, nom_pac, tipo_doc, doc_pac, proc_trasl, muerte, edad, alta_proc, tipo_empresa, dias_estancia, patron_combinable, usuario, modificar, chequeados_final, combinables, media_porcion, control_pos_quirur, wrol_usuario, wpatron_nutricion, wrolnutricion, mensaje)
                                                    return false;
                                                }
									 ...
									 * Si la fila tiene alg�n patr�n activo no se deje editar el campo observaciones
									 */
            ...
            if(valor == 'off')
                {
                    ...
                    if (hora_actual > hora_max_cancela)
                        {
                            alert("No es posible cancelar el servicio Postquir�rgico porque ha pasado la hora m�xima de cancelaci�n");
                            return false;
                        }
                    //Pregunta primero si desea quitar el posquirurgico.
                    confirmar=confirm("�Esta seguro de realizar esta operaci�n, cancelar� el patr�n que ha seleccionado despu�s del patron Postquir�rgico?");
	...
	$(document).ready(function(){
		...
		$( ".fila1, .fila2" ).each(function(){
			...
			//Si no hay patron seleccionado o no hay ning�npatron habilitado no se permite escribir en los campos de observaci�n e intolerancia
	...
	/**************************************************************************************************************
	 * Esta funci�n detiene la ejecuci�n de recargar la p�gina al momento de escribir en un textarea
	 **************************************************************************************************************/
...
function verificar_dia_sgte($whis, $wing){
    ...
    function traer_horario_servicio($wservicio)
    {
      ...
      return "<b>Horario <br>Normal</b>:".$row['serhin']."-".$row['serhfi']." <b>Adici�n:</b>".$row['serhia']."-".$row['serhad'];
 ...
 function traer_observaciones_dsn_enfer($whis, $wing, $wfecha_consulta, $wser)
 {
  ...
  function grabar_observ_intoler($wemp_pmla, $wbasedato, $whis, $wing, $whab, $wser, $wtexto, $wusuario, $wobsint, $wcco, $wfec)
	{
	  ...
	  //Si no hay un patr�n seleccionado no se deja modificar
	  ...
	  switch($wobsint)
		...
		//No afectar� a los servicios posteriores ya creados
  ...
  //==================================================================================================================
  //==================================================================================================================
  // Funcion que permite extraer la edad del paciente en a�os, meses y dias.
				                ...
				                $waccion = "MODIFICO ADICION";
				                 ...
				                 // $waccion="off";
								  if ($wmodificar != '2')
                                      ...
                                      echo "3";  //Mensaje javascript en el response text :  "El servicio de la historia seleccionada no puede ser adicionado o modificado porque esta fuera del horario. Favor revisar el inicio de horario de adici�n o el horario final del pedido en la parte superior de la pantalla"
  ...
  //==================================================================================================================
     ...
     //Consultar la composici�n actual de la dieta DSN en la fecha recibida desde la funci�n procesar_datos_dsn.
	...
	function procesar_datos_dsn($wemp_pmla, $wbasedato, $winf_prod, $wpatron, $whis, $wing, $wser, $wusuario, $whab, $wcco, $wfecha_interfaz, $wpatron_asociado, $wobservacion, $cant_product_servi)
     {
      ...
      foreach($array_primario as $valores_primarios)
      {
        ...
        foreach($arr_principal as $servicio => $array_productos )
        {
            ...
            //-->  Validar si ya existe servicio programado con las caracter�sticas indicadas, con el prop�sito de verificar si es una modificaci�n o un pedido nuevo. 2019-09-24
            ...
            if( $rex[0]*1 > 0 ){
                //--> Se hace una comparaci�n de los productos solicitados nuevos con los anteriores
			...
			//cancelar� los productos y pondr� los nuevos solicitados, con la fecha del dia siguiente.
			//===============================================================================================================
        ...
        foreach($arr_cantidad_productos as $serv_can => $cant_prod)
        {
            ...
            if($cant_prod > 0 && !array_key_exists($serv_can, $arr_principal))
            {
                ...
                //Se valida la hora final del servicio, si es mayor a la actual el registro sera cancelado para ma�ana
	  ...
	  for ($i=1;$i<=$num;$i++)
	     {
			...
			$wfecha_actual = $wmanana; //La fecha actual sera la fecha de ma�ana y la variable $wayer1 sera la fecha de hoy.
  ...
  function definir_div($wpatron, $fila, $whis, $wing, $wser, $i, $j, $whab, $wfec, $wcco, $wusuario, $wnombre_pac)
     {
      ...
      if ($wnovalidah == 'on')
		...
		//Validar si el paciente tiene DSN para el dia de hoy o ma�ana en cualquier servicio, si no es asi pinta el boton para recuperar la dieta.
		$q = " SELECT movdie
                 FROM ".$wbasedato."_000077
  ...
  function mostrar()
     {
	  ...
	  $wdisabledTextArea = $whabilitado;	//whabilidatos es ENABLED si est� en horario de pedido y DISABLED en caso contrario
      ...
      //si los patrones son combinables o no, porque solo se combinan cuando es adici�n.
      ...
      //Esta validacion se da cuando el usuario quiere ver registros de dietas de dias anteriores o posteriores
        if($wfecha != $wfec)
        {
			...
			switch(true){
				...
				default:
						echo '<br><br><font size="5"><span>Este centro de costos no puede realizar solicitud de alimentaci�n.</span></font>';
						return;
				break;
	  ...
	  if ($num_pac > 0)
	     {
		  ...
		  echo "<td>Media porci�n</td>";
          echo "<td align=center>Diagn�stico</td>";
	      echo "<td align=center>Observaciones <br> de la estancia <div style='display:block; font-size:10px; color:white;'>(M�ximo <span id='limite_obs'>".$wlimite_caracteres_observ."</span> car�cteres)</div></td>";
		  echo "<td align=center>Intolerancias <div style='display:block; font-size:10px; color:white;'>(M�ximo <span id='limite_into'>".$wlimite_caracteres_observ."</span> car�cteres)</div></td>";
		  ...
		  for($i=1;$i<=$num_pac;$i++)                             //For por habitacion o paciente
            ...
            if ($wptr[$i]=="on" and $wcir != 'on' and $wurg != 'on')
              ...
              //echo "<td $wbgcolor align=center id='tdcajonposquirur".$i."'><SPAN id='wcajonposquirurspan".$i."' class='tooltip { direction: n; width: auto; font-weight:bold; background: #222; color: #ddd; }' title='POSTQUIR�RGICO'><INPUT TYPE='checkbox' id='wcajonposquirur$i' NAME='wcelda[".$i."][".$j."]' $wchecked $whabilitaposqui onClick='grabar_posqx(\"".$wemp_pmla."\",\"".$wbasedato."\",\"".$whis[$i]."\",\"".$wing[$i]."\",\"".$whab[$i]."\",\"".$wser."\",\"wcajonposquirur$i\",this,\"tdcajonposquirur$i\",\"$wedad1[$i]\",\"$wtem[$i]\",\"$wusuario\",\"$wfec\",\"$wcco\",\"$whora_maxima_modificacion\",\"$whora_maxima_cancelacion\", \"$whora\");'><div id='patron_dato$i-$j'></div></SPAN></SPAN></td>";
                          ...
                          if (@$wpatrones[$row_die['diecod']])
							 {
								...
								//Si el patron no es combinable y es DSN, se marcar�.
					...
					//de forma automatica.
                     if($wprog_automatico == 'on')
                        ...
                        if($wpatronfinal[0] != 'servicio_individual')
							...
							//de ese patron en el dia anterior, por lo tanto debe hacer la solicitud de DSN y no del ultimo patr�n, el cual es diferente a DSN,
...
//          }
 ...
 function consultar_estado_posqui($whis, $wing, $wser)
  ...
  //==================================================================================================================
  //==================================================================================================================
  //Funcion que evalua si el paciente es de tipo POS, si es asi no guardar� merienda.
...
//Funcion que evalua si el paciente es de tipo POS, si es asi no guardar� merienda.
    function mensajedelservicio($wser)
     {
      ...
      if($whora >= $whorarioinicial)
      {
          ...
          {
          $wmensaje = "<span class='blink'>Se encuentra en horario de adici�n</span><br><font size=1> Solo podr� cancelar m�ximo 3 veces lo que pida en este horario <br> y no podr� pedir si cancela patrones pedidos en horario normal excepto NVO, LC y pacientes nuevos.</font>";
          }
          elseif($whora >= $whorariofinaladicion)
          {
	...
	function procesar_datos($wemp_pmla, $whis, $wing, $wpatron, $wcco, $wser, $wfec, $whab, $wpac, $wdpa, $wtid, $wptr, $wmue, $wedad, $walp, $wtem, $west, $wusuario, $wmodificar, $wchequeados, $wcombinables, $wpcomb, $wmedia_porcion, $wautomatico, $wservgrabado, $wcontrolposqui, $wrol_usuario, $wpatron_nutricion, $wrolnutricion, $wseleccionado, $wconfirmar_canceladsn, $codDSN)
    {
          ...
          $wpos_seractual = evaluar_emp_paciente($wtem, $wser); // Esta variable identifica si el paciente es POS, en caso de serlo no se guardar� el servicio de merienda(inicialmente).
          $wpos_serasociado = evaluar_emp_paciente($wtem, $wserv_asociados[0]); // Esta variable identifica si el paciente es POS y para el servicio asociado, en caso de serlo no se guardar� el servicio de merienda(inicialmente).
		...
		switch($wmodificar)
                                ...
                                if ($whora > $whora_ini_adicion )
                                {
                                    ...
                                    switch (trim($wdatoposquirur)) {
                                        ...
                                        case 'fueposqx':
                                                ...
                                                if($num_cancelapa >= 3)
                                                {
                                                    echo "91"; //Alerta javascript que muestra el mensaje "No es posible modificar o solicitar alimentaci�n para el paciente ya que a pasado la hora limite"
                                                    return;
                                                }
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                if($wvalidahorario == 'on')
                                                                {
																	if($num_cancelapa == 0)
																	{
																		...
																		//Si el paciente tiene cancelaciones el dia actual y el rol del que cancel� es diferente al de nutricionista.
																		if($num_aud > 0){
                            ...
                            elseif($whora > $whora_ini_adicion)
                            {
                                if ($row['hora_data'] < $whora_ini_adicion and $wpqx == '' or $wpqx == ' ')
                                {
									if($wrol_usuario != $wrol_nutricion){ //La cancelacion de la DSN no se valida con estaa alerta y no se le debe mostrar a las nutricionistas.
										...
										echo "9"; //Alerta javascript que muestra el mensaje "No es posible solicitar alimentaci�n para el paciente ya a pasado el limite de modificaci�n de pedido"
										return;
									}
                                }
            ...
            switch ($wpqx) {
                case '1':
                            ...
                            {
                              echo "30"; //Este dato genera un respuesta javascript con un mensaje diciendo "No es posible hacer la combinaci�n ya que no ha seleccionado el paciente como posquir�rgico"
                              return;
                            }
                ...
                case 'on':
                           ...
                           //En esta parte del codigo se elimina del grupo de patrones el que se cobra, siendo el que se cobra un patron posqx.
                            ...
                            {
                              echo "31"; //Este dato genera un respuesta javascript con un mensaje diciendo "No es posible deseleccionar el patron ya que no ha deseleccionado el paciente como posquir�rgico."
                              return;
                            }
            ...
            if ($wnocobrapatron == 'on')
            {
			  ...
			  if ($wbusca_costo=="on")
                 ...
                 //Esta validacion es importante ya que determina si el patron seleciconado tiene costo, en caso de estar conbinado o individual.
				  if ($wvalpat > 0)
					 {
                      //Esta validacion se da para determinar el costo del patron en el dia de hoy o de ma�ana, ya que puede cambiar para dias posteriores.
					  if ($wfecha >= $row_cos[1])
                      {
					...
					if ($wpatron2 == "") //Variable final al realizar este proceso = ($wdatopatron = str_replace(",","','",$wdatopatron_modifi); // se reeemplaza (,) por ',')
						...
						else
						{
                            if ($wmodificar != 1)
                            {
                                   ...
                                   echo "1"; //Este 1 devuelve un mensaje diciendo lo siguiente: "El Patr�n: **xx** No tiene costo en la tabla 000079, para el tipo de empresa: ** xx ** en la historia: ** xxxxxx **", en la funcion javascript grabar_datos.
                                   return;
                            }
                            else
                                {
                        ...
                        if ($wcontrolposqui == 'on' or $wcobraadicional == 'on')
                            {
                                ...
                                $wvalpat_asoc = $wdatos_historia1[1];  //Valor del patr�n asociado
          ...
          if ($num_cos > 0)
			 {
			  ...
			  $wcontrolregistro = 'on';                 //Esta variable se utiliza para controlar el registro automatico de las dietas desde un servicio asociado
                                                        //quiere decir desde la media ma�ana, algo o merienda, ya que algunos patrones no tienen costo en estos servicios.
                   ...
                   echo "<table style='text-align: left; width: 1000px; height: 200px;' border='1' cellpadding='1' cellspacing='1'>
                        <tbody>
                            ...
                            <table style='text-align: left; width: 200px; height: 194px;' border='1' cellpadding='2' cellspacing='2' class=fila2>
                                ...
                                <tr style='font-family: Arial;'>
                                    <td bgcolor=".$color_ant_sing.">PEDIDO PARA MA�ANA</td>
                                </tr>
                                <tr style='font-family: Arial;'>
                            ...
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 201px;'>Hora Distribuci�n</td>
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 188px;'>Hora M�xima Cancelaci�n</td>
                            </tr>
                            <tr>
                            ...
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 203px;'>Hora Inicio</td>
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 215px;'>Hora Final</td>
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 201px;'>Hora Inicio Distribuci�n</td>
                            <td style='font-family: Arial; font-weight: bold; text-align: center; width: 188px;'>Hora Final Distribuci�n</td>
                            </tr>
                            <tr>
