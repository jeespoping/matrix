...
function ArticulosXPaciente( $pac ){
	...
	$sql = "(SELECT 
				sum(kadcdi), sum(kaddis), kadart, artcom, kadsus
	        FROM 
...
elseif (!isset($pac['his']))
{
	...
	else
	{
		...
		ECHO "<BR/><B><A HREF='cargosprux.php?tipTrans=".$tipTrans."&emp=".$emp."&bd=".$bd."'>Retornar</a>";
		echo "</td></tr></table>";
	}
}else{
						...
						if( ($pac['con'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] || $tipTrans != "C" || $cargarmmq == "on" )
						{
							if( ($pac['kegra'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] || $tipTrans != "C"  || $cargarmmq == "on" )
							{
								if( ($pac['keact'] && $pac['ke'] ) || !$pac['ke'] || !$cco['tras'] || $tipTrans != "C"  || $cargarmmq == "on" )
								{
									...
									{
										$warning = "EL PACIENTE CON HISTORIA:".$pac['his']." FUE DADO DE ALTA DE LA INSTITUCIÓN!!! N22";
										$error['codInt']='0009';
										$error['codSis']=".";
										$error['descSis']=".";
										$error['clas']="#ff0000";
					...
					if( ($pac['con'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] || $tipTrans != "C" || $cargarmmq == "on" )
					{
						if( ($pac['keact'] && $pac['ke'] ) || !$pac['ke'] || !$cco['tras'] || $tipTrans != "C" || $cargarmmq == "on" )
						{
							if($pac['act'])
							{
								...
								{
									$warning = "EL PACIENTE CON HISTORIA:".$pac['his']." FUE DADO DE ALTA DE LA INSTITUCIÓN!!! N23";
									$error['codInt']='0009';
									$error['codSis']=".";
									$error['descSis']=".";
									$error['clas']="#ff0000";
...
//			echo '<textarea ALIGN="CENTER"  ROWS="3" name="artMostrar" cols="28"  color:"'.$color.'" style="font-family: Arial; font-size:14" readonly>'.$artMostrar.'</textarea></font>';
		...
		echo "<A HREF='cargosprux.php?usuario?=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar Usuario</a> </font>";
		echo "&nbsp; &nbsp;<A HREF='cargosprux.php?usuario=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."&ccoCod=UN.".$cco['cod']."'>Retornar Usuario+CC</a> </font>";

		echo "</td></tr>";
		echo"<tr><td class='titulo3'><input type='submit' value='ACEPTAR'></td></tr></form>";

		echo "<tr><td class='".$class."'>";
		echo "<A HREF='cargosprux.php?tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar</a>";
	...
	else
	{
		...
		echo "</td></tr>";
		echo "<tr><td class='".$class."'>";
		echo "<A HREF='cargosprux.php?usuario=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar con Usuario</a>&nbsp; &nbsp;";
		echo "<A HREF='cargosprux.php?tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar</a>";
		echo "</td></tr>";
