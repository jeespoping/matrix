...
//==================================================================================================================================
//PROGRAMA                   : Aplicacion_ipods_1.php
...
else
{
  ...
  $wusuario=trim($wuser1[1]);
     	                                         ...
     	                                         // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
  $wactualiz="Octubre 11 de 2012";               // Aca se coloca la ultima fecha de actualizacion de este programa //
	                                             // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
		  ...
		  for($i=1;$i<=$num;$i++)
			 {
			   ...
			   if ($num1 > 0)
                  {			   
				   ...
				   echo "<td align=center><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whis=".$whis."&wing=".$wing."&wcco=".$wcco."&whab=".$whab."&wpac=".$wpac."&walp=".$walp."&wzona=".$wzona."' class=tipo3V>Ver</A></td>";
			   ...
			   while ($j <= 6 and $i <= $num)
			      {
			       ...
			       echo "<td align=center><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla."&user=".$user."&whis=".$whis."&wing=".$wing."&wcco=".$wcco."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."' class=tipo3V>".$whab."</A></td>";
			       $row = mysql_fetch_array($res); 
			       $j++;
			       $i++;
	              }
...
function validar_ipods($wcco)
  {
   ...
   $row = mysql_fetch_array($res);
	...
	if( false && !esUsuarioLactario( $conex, $wbasedato, $wusuario ) ){
		...
		if ($row[1] == "on" and !$wok)
		   {
			...
			echo " <script>
				   ...
				   window.location.href='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla."'
				   </script> ";
...
//*******************************************************************************************************************************************
  ...
  //===========================================================================================================================================
  //===========================================================================================================================================
  echo "<form name='apl_ipods' action='Aplicacion_ipods_1.php' method=post>";
			   ...
			   { 
				...
				for ($i=1;$i<=$num;$i++)   //Recorre cada uno de los medicamentos
				{
					...
					if( trim( $row[17] ) != '' || trim( $row[18] ) != '' ){
						...
						if( trim( $row[17] ) != '' ){
							if( strtotime( $row[ 4 ]." ".valHoraStrToTime($row[ 5 ]).":00:00" ) + ( trim( $row[17] ) - 1 )*$row[6]*3600 < strtotime( $wfecha_actual." ".valHoraStrToTime($whora_par_actual).":00:00" ) ){
						...
						if( trim( $row[18] ) != '' ){
							if( strtotime( $row[ 4 ]." 00:00:00" ) + trim( $row[18] )*24*3600-1 < strtotime( $wfecha_actual." ".valHoraStrToTime($whora_par_actual).":00:00" ) ){
								$noMostarPorDMTTO = true;
							}
						}
					}
								...
								if (!isset($wanular) or $wanular != "on")       //Grabar o Aplicar
								   {
								  ...
								  else 
									{
									   if( !$dosVar[ 'Defrci' ] ) //si NO pide cantidad al aplicar
									     {
									       echo "<td align=center rowspan=4 colspan=2><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on ' class=tipo3V>Aplicar</A></td>";
										 }
									   else
										 {
										    echo "<td align=center rowspan=4 colspan=2>555";
											echo "<select class=tipo3V name='dosisIpd[$i]' onChange='cambiarUrl( this, \"Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on\", $i )'>";
											echo "<option></option>";
								  ...
								  if ($wok == true and $waplicado=="off")   
									 {
									  if ($wStock[$i]=="on")   
										if( !$dosVar[ 'Defrci' ] ) //si NO pide cantidad al aplicar
										    echo "<td align=center rowspan=4 colspan=2><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla.$waplicados."&wcco=".$wcco."&whis=".$whis."&wing=".$wing."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&whab=".$whab."&wpac=".$wpac."&wido[".$i."]=".$row[22]."&wapl[".$i."]=on ' class=tipo3V>Aplicar (Stock)</A></td>";
										 else
										    {
				...
				echo "<br><br>";
				echo "<table>";
				echo "<tr><td><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla."&wcco=".$wcco."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&wzona=".$wzona."' class=tipo3V>Retornar</A></td></tr>";
				echo "</table>";	  
			   }
			  else
				 {
				  ...
				  echo "<br><br>";
				  echo "<table>";
				  echo "<tr><td><A HREF='Aplicacion_ipods_1.php?wemp_pmla=".$wemp_pmla."&wfecha_actual=".$wfecha_actual."&whora_par_actual=".$whora_par_actual."&wzona=".$wzona."' class=tipo3V>Retornar</A></td></tr>";
				  echo "</table>";	  
				 }
		 }
		else
