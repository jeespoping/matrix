 ...
 * articulo no se encuentra en la taba 66
 */
function esMMQ( $art ){
...
function ArticulosXPaciente( $pac ){
	...
	//con origen en servicio farmaceutico
	$sql = "SELECT 
				sum(kadcdi), sum(kaddis), kadart, artcom
	        FROM 
				{$bd}_000054 a, {$bd}_000026 b
	        WHERE 
	        	kadhis='{$pac['his']}' AND 
	        	kading='{$pac['ing']}' AND
	        	kadsus!='on' AND
	        	kadori='SF' AND
	        	a.kadfec = '".date("Y-m-d")."' AND
	        	artcod = kadart
	        GROUP BY kadart
...
/**
 * Registra en Kardex el articulo dispensado 
 * 
...
function registrarArticuloKE( $art, $pac ){
...
include_once("movhos/validacion_histpru.php");
...
//Indica si el articulo debe ser mostrada en la pantalla
$mostraLista = true;
...
elseif (!isset($pac['his']))
{
	...
	else
	{
		...
		ECHO "<BR/><B><A HREF='cargospmmq.php?tipTrans=".$tipTrans."&emp=".$emp."&bd=".$bd."'>Retornar</a>";
		echo "</td></tr></table>";
	}
}else{
...
//				$cco['tras'] = esTraslado( $cco['cod'] );
				...
				if($pac['act'])
				{
					if(!$pac['mue'])
					{
						if( ($pac['con'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] )
						{
							if( ($pac['kegra'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] )
							{
								if( ($pac['keact'] && $pac['ke'] ) || !$pac['ke'] || !$cco['tras'] )
								{
									/**
									...
									{
										$warning = "EL PACIENTE CON HISTORIA:".$pac['his']." FUE DADO DE ALTA DE LA INSTITUCIÓN!!! N18";
										$error['codInt']='0009';
										$error['codSis']=".";
										$error['descSis']=".";
										$error['clas']="#ff0000";
		...
		if ($pac['act'])
		{
			...
			if(isset($art['cod']))
			{
				...
				if( !$val ){
					if( $ke && $artValido && $tipTrans == "C" && enLista( $listartpac, $art ) < 3
					&& enLista( $listartpac, $art ) > -1 && !$art['cva'] )
					{
					...
					if( ($pac['con'] && $pac['ke']) || !$pac['ke'] || !$cco['tras'] )
					{
						if( ($pac['keact'] && $pac['ke'] ) || !$pac['ke'] || !$cco['tras'] )
						{
							if($pac['act'])
							{
								...
								{
									$warning = "EL PACIENTE CON HISTORIA:".$pac['his']." FUE DADO DE ALTA DE LA INSTITUCIÓN!!! N19";
									$error['codInt']='0009';
									$error['codSis']=".";
									$error['descSis']=".";
									$error['clas']="#ff0000";
...
//			echo '<textarea ALIGN="CENTER"  ROWS="3" name="artMostrar" cols="28"  color:"'.$color.'" style="font-family: Arial; font-size:14" readonly>'.$artMostrar.'</textarea></font>';
		...
		echo "<A HREF='cargospmmq.php?usuario?=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar Usuario</a> </font>";
		echo "&nbsp; &nbsp;<A HREF='cargospmmq.php?usuario=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."&ccoCod=UN.".$cco['cod']."'>Retornar Usuario+CC</a> </font>";

		echo "</td></tr>";
		echo"<tr><td class='titulo3'><input type='submit' value='ACEPTAR'></td></tr></form>";

		echo "<tr><td class='".$class."'>";
		echo "<A HREF='cargospmmq.php?tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar</a>";
	...
	else
	{
		...
		echo "</td></tr>";
		echo "<tr><td class='".$class."'>";
		echo "<A HREF='cargospmmq.php?usuario=".$usuario."&amp;tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar con Usuario</a>&nbsp; &nbsp;";
		echo "<A HREF='cargospmmq.php?tipTrans=".$tipTrans."&amp;emp=".$emp."&bd=".$bd."'>Retornar</a>";
		echo "</td></tr>";
