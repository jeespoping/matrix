...
else
{
	...
	else
	{
		...
		." SELECT Devces,  Devjud, Devcfs, Devjus, Devnum, Devlin, Fenhis, Fening, Fenfue,Fencco, Fentip, 'artcod' as Artc, 'artnom' as Artn , 0 as Des "
		."      FROM ".$bd."_000028, ".$bd."_000002 "
		."     WHERE Devcon = ".$devCons." "
		."       AND Fennum = Devnum ";
		$err = mysql_query($q,$conex);
		echo "<BR><BR><I>".$q."</I><br>".mysql_error()."<br>";
		...
		$q = "INSERT INTO ".$table." "
		//."    SELECT Devces,  Devjud, Devcfs, Devjus, Devnum, Devlin, Fenhis, Fening, Fenfue, Fencco, Fentip, 'artcod' as Artc, 'artnom' as Artn , 0 as Des "
		."    SELECT      0,    NULL,      0,   NULL,    NULL,  NULL,   Aplhis, Apling, Aplfde, Aplcco,     '',           Aplart,            Apldes,  Aplaap  "
		."      FROM ".$apl." ";
		$err = mysql_query($q,$conex);
		echo "<BR><BR><I>".$q."</I><br>".mysql_error()."<br>";
		$q ="SELECT * FROM ".$table." ";
		...
		if($num >0)
		{
			for($i=0;$i<$num;$i++)
			{
				...
				if($i==0)
				{
					...
					echo "<td class='tituloSup' >Devolución Número ".$devCons."</td></tr>";
				}
				...
				if($cco[$ccoNum]['cod'] != $row['Fencco'])
				{
					...
					echo "<tr><td><td><tr>";
					echo "<tr><td class='acumulado1' colspan='7'>".$cco[$ccoNum]['cod']."-".$cco[$ccoNum]['nom']."<td><tr>";
