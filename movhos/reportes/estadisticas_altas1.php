...
{	            
  ...
  include_once("root/comun.php");
	                                         ...
	                                         // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
  $wactualiz="(2009-10-06)";                 // Aca se coloca la ultima fecha de actualizacion de este programa //
	                                         // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= //
  ...
  //FORMA ================================================================
  echo "<form name='altas' action='estadisticas_altas1.php' method=post>";
   ...
   if (!isset($wfec_i) or trim($wfec_i) == "" or !isset($wfec_f) or trim($wfec_f) == "" or !isset($wcco) or trim($wcco) == "" )
     {
	  ...
	  echo "<td colspan=2><font size=4>&nbspINTRODUZCA EL CENTRO DE COSTO&nbsp</font></td>";
	  echo "</tr>";
	  echo "</table>";
	  ...
	  echo "<tr class=seccion1><td align=center colspan=2><select name='wcco'>";
      ...
      { 
	   ...
	   $q = " SELECT MAX(TIMEDIFF(ubihad,ubihap)), ubisac, ubihis, ubiing "
	       ...
	       ."    AND ubisac  LIKE '".substr($wcco,0,strpos($wcco,'-')-1)."'"
	       ."    AND ubiald  = 'on' "
	       ."    AND ubifap  = ubifad "
	       ."    AND ubisac  = ccocod "
	       ."    AND ccohos  = 'on' "
       ...
       */
	   ...
	   if ($num > 0)
	      {
		   ...
		   for ($i=1;$i<=$num;$i++)
		      {   
		       ...
		       $warr[$j][0]=$row[4];                                                 //Cco
		       $warr[$j][7]=$row[3];                                                 //Cantidad de altas entre la devolucion y facturacion
	   ...
	   echo "<td align=center colspan=15 class=link><A href='estadisticas_altas1.php?wemp_pmla=".$wemp_pmla."&wfec_i=".$wfec_i."&wfec_f=".$wfec_f."'><b><font size=3 color=660099>Retornar</font></b></A></td>";
