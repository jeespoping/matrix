...
<script type="text/javascript">
	function inicio(){ 
		document.location.href='RepEntregaMedicamentosHorario1.php?wemp_pmla='+document.forms.forma.wemp_pmla.value; 
	}
	...
	******************************************************************************************************************************/
	 function consultar(){ 
		...
		document.location.href='RepEntregaMedicamentosHorario1.php?wemp_pmla='+formulario.wemp_pmla.value+'&waccion=a&wservicio='+formulario.wservicio.value
...
if (!$usuarioValidado){
	...
	//Forma
	echo "<form name='forma' action='RepEntregaMedicamentosHorario1.php' method='post'>";
...
//	$fecha = "2009-10-10";
	...
	switch ($waccion){
		case 'a':		//Consulta de los datos del reporte
			...
			+'&whorario='+'';
			 */
			echo "<meta http-equiv='refresh' content='120;url=RepEntregaMedicamentosHorario1.php?wemp_pmla=".$wemp_pmla."&wuser=".$wuser."&waccion=a&wservicio=".$wservicio."&whabitacion=".$whabitacion."&whoraini=".$whoraini."&whorafin=".$whorafin."&wdiasdif=".$wdiasdif."'>";
...
//				echo "La fecha final es: '$fechaFinal'";
				...
				$colMedicamentosPendientes = consultarMedicamentosPendientesHorario($wservicio, $fecha, $whabitacion, &$num);
				
				
				//Modificacion hecha por JuanC, si no encontro registros, busco en el kardex del dia anterior
				if ($num==0)
				   {
					$dia = time()-(1*24*60*60);   //Se resta un dia

			        $wayer = date('Y-m-d', $dia); //Formatea dia    
			        
				    $colMedicamentosPendientes = consultarMedicamentosPendientesHorario($wservicio, $wayer, $whabitacion, &$num);
				   } 
