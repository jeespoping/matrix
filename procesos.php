...
<BODY TEXT="#000066">
<center>
<A NAME="Arriba"><font size=5>Procesos Por Formulario</font></a>
</center>
<?php
...
if(!isset($_SESSION['user']))
	...
	if($Form != "0")
	{	
		$query = "select * from procesos where medico='".$owner."' and formulario='".substr($Form,0,$ini)."' order by codigo";
		$err = mysql_query($query,$conex);
		$num = mysql_num_rows($err);
		if($num>0)
		{
			...
			for ($i=0;$i<$num;$i++)
			{
				...
				echo "<td bgcolor=".$color." align=center><A HREF='/matrix/".$grupo."/procesos/".$row[4]."' target = '_blank'><font size=2>Ejecutar</font></a></td></tr>";
			}
			echo "</table>";
			echo "<input type='HIDDEN' name= 'owner' value='".$owner."'>";	
		}
		else
	  		echo "Sin Procesos Asociados";
	}
	if(isset($err))
	{
