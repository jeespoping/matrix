...
else
{
	...
	function grabar ($arriba,$abajo,$izquierda,$derecha,$tipo,$diente,$conex,$key,$exp,$medico)
	{
		...
		$fecha=date("Y-m-d");
		$hora=date("H:i:s");	
		$query= "UPDATE soe1_000005 SET Medico='soe1', Fecha_data='".$fecha."', Hora_data='".$hora."',  Fecha='".$fecha."',  Identificacion='".$exp[0]."', Odontologo='".$medico."',  Examen='".$tipo."', Num_diente='".$diente."', Arriba='".$arriba."', Abajo='".$abajo."', Izquierda='".$izquierda."', Derecha='".$derecha."', Modificacion='MODIFICADO (DATOS ANTERIORES)  Odontologo: ".$row["Odontologo"].", Fecha: ".$row["Fecha_data"].",  Hora: ".$row["Hora_data"].", Vestibular: ".$row["Arriba"].", Lingual: ".$row["Abajo"].", Distal: ".$row["Izquierda"].", Mesial: ".$row["Derecha"].", ANTERIOR MODIFICACION: ".$row["Modificacion"]."', Seguridad='A-".$key."'  WHERE id = '".$row["id"]."'";
		$err = mysql_query($query,$conex);
		//echo $query."<br>".mysql_errno().'='.mysql_error();
		...
		$fecha=date("Y-m-d");
		$hora=date("H:i:s");
		$query="INSERT INTO soe1_000005 (Medico, Fecha_data, Hora_data,  Fecha,  Identificacion, Odontologo,  Examen, Num_diente, Arriba, Abajo, Izquierda, Derecha, Modificacion, Seguridad) VALUES ('soe1','".$fecha."','".$hora."','".$fecha."' , '".$exp[0]."','".$medico."','".$tipo."','".$diente."','".$arriba."','".$abajo."' ,'".$izquierda."','".$derecha."','Grabado por Odontologo: ".$medico."','A-".$key."')";
		$err = mysql_query($query,$conex);
		//echo $query."<br>".mysql_errno().'='.mysql_error();
...
//		echo "<input type='hidden' name='medico' value='".$medico."'>";
	...
	{	$ini1=strpos($medico,"-");
		...
		echo "<tr><td bgcolor='#99FFFF' colspan= '2' ><font size=3  face='arial'><b>FECHA:</b> ".$fecha." </td>";
		...
		$query = "SELECT Subcodigo,Descripcion FROM `det_selecciones` WHERE medico = 'soe1' AND codigo = '014'  and subcodigo<'05' order by Subcodigo ";
			$err = mysql_query($query,$conex);
			$num = mysql_num_rows($err);
			if($num>0)
			{
		...
		echo "<td colspan= '2' bgcolor='#99CCFF' align=center><font size=3  face='arial'><b><table align=center  border='0'width=100 ><tr><td ></td><td><input type='text' name='arriba' size=4></td><td></td></tr><tr><td ><input type='text' name='izquierda' size=4></td><td></td><td><input type='text' name='derecha' size=4></td></tr><tr><td ></td><td><input type='text' name='abajo' size=4></td><td></td></tr></table></b></td></tr>";
		...
		{
			grabar ($arriba,$abajo,$izquierda,$derecha,$tipo,$diente,$conex,$key,$exp,$medico);
		}else{		
		}
		echo "<input type='hidden' name='medico' value='".$medico."'>";
		echo "<input type='hidden' name='pac' value='".$pac."'>";
		$hyper="<A HREF='/matrix/soe/procesos/regperiodontal_2.php?medico=".$medico."&amp;pac=".$pac."'>Movilidad y Bifurcacion</a>";
