...
/*******************************************************************************************************************************************
*                                           IMPRESION % HISTORICO FACTURAS X CENTRO DE COSTOS   PARA NIIF - UNIX                           *
********************************************************************************************************************************************/
//==========================================================================================================================================
//PROGRAMA				      :rep_porhisfacxcco.PHP                                                                                        |
//AUTOR				          :Ing. Gustavo Alberto Avendano Rivera.                                                                        |
//FECHA CREACION			  :Noviembre 10 DE 2014.                                                                                        |
//FECHA ULTIMA ACTUALIZACION  :10 de Noviembre de 2014.                                                                                     |
//DESCRIPCION			      :Este programa sirve para imprimir el % historico de facturas x centro de costos, después de generar en unix  |
//                             el programa de phisfxcon.4gl                                                                                 |
//                                                                                                                                          |
//TABLAS UTILIZADAS :                                                                                                                       |
//amepfxcco         : Tabla de Porcentajes de facturacion x centro costo.                                                                   | 
//==========================================================================================================================================
...
else
{
 ...
 $conexi=odbc_connect('facturacion','','')
	       ...
	       //Forma
 echo "<form name='forma' action='rep_porhisfacxcco.php' method='post'>";
 echo "<input type='HIDDEN' NAME= 'usuario' value='".$wuser."'/>";
 
if (!isset($carg) or $carg == '' or !isset($tip) or $tip == '' or !isset($pg) or $pg=='-' )
  {
  	echo "<form name='rep_porhisfacxcco' action='' method=post>";
	...
	echo "<tr><td align=center colspan=2>PORCENTAJE HISTORICO FACTURAS POR CENTRO DE COSTOS</td></tr>";
   ...
   $query = " SELECT ccoano,ccomes,ccomatr"
           ."   FROM amepfxcco"
           ."  GROUP BY 1,2,3"
           ."  ORDER BY 1,2,3";
  ...
  {	
   ...
   echo "<tr>";
   echo "<td align=center colspan='1' bgcolor=#FFFFFF><font size='3' text color=#003366><b>PORCENTAJE HISTORICO FACTURAS POR CENTRO DE COSTOS</b></font></td>";
   echo "</tr>";
   echo "<tr>";
  ...
  IF (($tip == 'N') and ($carg != '*'))
  {
   $query1="SELECT empcod,empnit"
	...
	$query2="SELECT ccoemp,ccocer,ccocco,ccovlrc,ccovlrt,ccopor"
	       ."  FROM amepfxcco,tmpempre"
		   ." WHERE ccoano='".$tpg[0]."'"
		   ."   AND ccomes='".$tpg[1]."'"
		   ."   AND ccomatr='".$tpg[2]."'"
		   ."   AND ccocer=empcod"
  ...
  while (odbc_fetch_row($err_o2))
   {
	...
	$vlrtcli1 = odbc_result($err_o2,4);//VLR TOTAL CLINICA
	$vlrtter1 = odbc_result($err_o2,5);//VLR TOTAL TERCERO
	$pporc1   = odbc_result($err_o2,6);//PORCENTAJE HISTORICO
    ...
    echo "<td bgcolor=#FFFFFF style='color:#003366; font-size:12px; text-align:right;'>".number_format($vlrtter1,0,'.',',')."</font></td>";
