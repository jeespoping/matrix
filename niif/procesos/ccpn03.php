...
<head>
    ...
    <script>
        ...
        function copiarValor2(nombre){
            opener.document.formNuevo.nombrelinea.value = nombre;
            window.close();
        }
    </script>
    <?php
...
if($accion == 1)
{
    $query_o2 = "SELECT connom FROM facon WHERE concod = '$codconcepto'";
...
if($accion == 2)
{
    $query_o3 = "SELECT nombre FROM amelinea WHERE codi = '$codlinea'";
    $err_o3 = odbc_do($conex_o, $query_o3);

    while($datoconcepto = odbc_fetch_row($err_o3))
    {
        ?>
        <form>
            <table class="table">
                <tr>
                    <?php $descripcionlinea=odbc_result($err_o3, 1); ?>
                    <td><label id="lbldescripcionconcepto"><a href="#" ><?php echo $descripcionlinea ?></a></label></td>
                    <script>copiarValor2('<?php echo $descripcionlinea ?>')</script>
                </tr>
            </table>
        </form>
        <?php
