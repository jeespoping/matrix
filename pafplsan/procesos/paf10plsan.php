...
echo "<form name='paf10plsan' action='paf10plsan.php' method=post>";  
  
     $query="SELECT * From pafplsan_000001 WHERE Id =".$wid; 
     ...
     if ($nroreg > 0)      //  Encontro 
     {
      ...
      $query = "SELECT ccocod,cconom FROM costosyp_000005 WHERE Ccoemp = '01' and ccocod='".$registro[12]."'";
      $resultado = mysql_query($query);
      $nroreg = mysql_num_rows($resultado);
      if ($nroreg > 0)      //  Encontro 
      {
      ...
      echo "<tr><td align=center colspan=6 bgcolor=#C0C0C0>";
   	  ...
   	  $f = date("Y-m-d");
   	  if ( $conf == "on")
   	  { $query="UPDATE pafplsan_000001 SET pafest='X',pafanu='".$user."-".$f."' WHERE Id =".$wid;
