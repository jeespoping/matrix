...
include_once("src/jpgraph_pie3d.php");    
      ...
      switch ($wind) 
      {
       case "1": 
        // tablas en Matrix
        $matrix="S";
        $query = "SELECT pafdia AS codigo,descripcion AS nombre,count(*) AS valor FROM pafplsan_000001,root_000011"
        ." WHERE pafdia=codigo AND paffec BETWEEN '".$wfec1."' AND '".$wfec2."' AND pafest='A' GROUP BY pafdia,descripcion ORDER BY count(*) DESC"; 
        $titulo= "PROGRAMA PLUS SANITAS - ORDENES POR DIAGNOSTICO - TOTAL ORDENES ";
        $matrix="S";
        break;
       case "2": 
        $matrix="S";
        $query = "SELECT pafexa AS codigo,nombre AS nombre,count(*) AS valor FROM pafplsan_000001,root_000012"
        ." WHERE pafexa=codigo AND paffec BETWEEN '".$wfec1."' AND '".$wfec2."' AND pafest='A' GROUP BY pafexa,nombre ORDER BY count(*) DESC";
        $titulo= "PROGRAMA PLUS SANITAS - ORDENES POR EXAMEN - TOTAL ORDENES ";
        $matrix="S";
        break;
       Case  "3":
        $matrix="S";
        $query = "SELECT pafcco AS codigo,cconom AS nombre,count(*) AS valor FROM pafplsan_000001,costosyp_000005"
        ." WHERE pafcco=ccocod AND paffec BETWEEN '".$wfec1."' AND '".$wfec2."' AND pafest='A' GROUP BY pafcco,cconom ORDER BY count(*) DESC";
        $titulo= "PROGRAMA PLUS SANITAS - ORDENES POR SERVICIO - TOTAL ORDENES ";
        break;
        Case  "4": 
        // tablas en UNIX 
        $matrix="N";
       ...
       // . " And cardetmes BETWEEN '".substr($wfec1,5,2)."' AND '".substr($wfec2,5,2)."'"
        ...
        . " GROUP BY 1,2,3"
        ."  ORDER BY 4 DESC";
        $titulo= "PROGRAMA PLUS SANITAS - TOTAL CARGOS HOSPITALIZACION Y AYUDAS ";
        break;        
      }
...
// Cuando el query  es sobre tablas de INFORMIX       
   ...
   $graph = new PieGraph(1200,700,"auto"); 
