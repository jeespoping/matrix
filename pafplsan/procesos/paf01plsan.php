<html> 
<head>
<title>Actualizacion Autorizaciones Programa PLUS SANITAS</title>
...
<script>
    function ira()
    {
	 document.paf01plsan.wced.focus();
	}
</script>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf01plsan.submit();   // Ojo para la funcion paf01plsan   (sencible a mayusculas)
	}

	function vaciarCampos()
	{document.forms.paf01plsan.wfec.value = 'yyyy-mm-dd';
	 document.forms.paf01plsan.wced.value = '';
     document.forms.paf01plsan.wap1.value = '';
	 document.forms.paf01plsan.wap2.value = '';
     document.forms.paf01plsan.wnom.value = '';
     document.forms.paf01plsan.wtel.value = '';
     document.forms.paf01plsan.wran.value = '';	
     document.forms.paf01plsan.wtip.value = '';	
     document.forms.paf01plsan.wdia.value = '';	
     document.forms.paf01plsan.wexa.value = '';	 
	 document.forms.paf01plsan.wrem.value = '';
	 document.forms.paf01plsan.wcco.value = '';
	 document.forms.paf01plsan.west.value = '';
	 document.forms.paf01plsan.word.value = '';
	 document.forms.paf01plsan.wser.value = '';
	 document.forms.paf01plsan.wran.value = '';	
     document.forms.paf01plsan.wcr1.value = '';    
     document.forms.paf01plsan.wcr2.value = '';     
    }
...
//==========================================================================================================================================
//PROGRAMA				      :Actualizacion Autorizaciones Programa plus sanitas                                                               
//AUTOR				          :Jair  Saldarriaga Orozco.                                                                        
//FECHA CREACION			  :Octubre 20 de 2015
//FECHA ULTIMA ACTUALIZACION  :Febrero 18 de 2020 Gabriel Agudelo.                                                                             

$wactualiz="PROGRAMA: paf01plsan.php Ver. 2020-02-18   ";
...
echo "<form name='paf01plsan' action='paf01plsan.php' method=post>";  
...
echo "<tr><td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FFFFFF><b>Actualizacion Autorizaciones Programa Plus Sanitas</b></font>";
...
{
  $query = "SELECT Max(id) FROM pafplsan_000001";
  $resultado = mysql_query($query);
  $registro = mysql_fetch_row($resultado);  	
  $nrorden=$registro[0];	
  echo "<td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FF0000><b>Nro Orden: ****** </b></font></tr>";  
 ...
 if ($wproceso == "Modificar" )  	
  {
   ...
   $query = "SELECT Paffec,Pafced,Pafape,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla"
           ." FROM pafplsan_000001 Where id = '".$wid."'";
  ...
  {   
    $query = "SELECT Afi_identific,Afi_apellido1,Afi_apellido2,Afi_nombres,Afi_telres, Afi_paf_codigo,Afi_tip_cotiza_ FROM pafplsan_000002 Where Afi_identific = '".$wid."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
    if ($nroreg > 0)      //  Encontro 
    {
...
/***********************************************************************************************************************************/	
	...
	echo "<td align=center bgcolor=#C0C0C0 colspan=1><font text color=#003366 size=3>Nro de orden NUEVA EPS: </font>";
    if (isset($word))
	...
	{	
	 echo "<input type='submit' value='Grabar'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
   	...
   	if ($wproceso == "Modificar" )  	
      echo "<A HREF='paf09plsan.php?wid=".$wid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
...
if ( $conf == "on" and isset($wape) and $wape<>'' and isset($wexa) and $wexa<>'' and isset($wcco) and $wcco<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   // Busco la fecha del ultimo examen igual al solicitado si es menor a 90 dias mensaje informativo
     $query = "SELECT paffec,Id FROM pafplsan_000001 Where pafced = '".$wced."' And pafexa='".$c4[0]."' Order by paffec DESC";
     ...
     $query = "SELECT * FROM pafplsan_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
       ...
       $query = "Update pafplsan_000001 SET pafape='".$wape."',pafnom='".$wnom."',paftel='".$wtel."',pafran='".$c1[0]."'"
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO pafplsan_000001 (Medico,Fecha_data,Hora_data,Paffec,Pafced,Pafape,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,"
	          ."Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla,Seguridad,id)"
	          ." VALUES ('pafplsan','".$fecha."','".$hora."','".$wfec."','".$wced."','".$wape."','".$wnom."','".$wtel."','".$c1[0]."','".$c2[0]
	  ...
	  if ($resultado)
	   {
			...
			$query = "SELECT Id FROM pafplsan_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
			$resultadoB = mysql_query($query,$conex);
			$nroreg = mysql_num_rows($resultadoB);
			if ( $nroreg > 0 )                   //Encontro
			{
			 ...
			 echo "<td align=center colspan=1 bgcolor=#99CCCC><A HREF='paf09plsan.php?wid=".$ultid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A></td></tr>";	
