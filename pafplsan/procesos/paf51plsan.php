...
<script>
    function ira()
    {
	 document.paf51plsan.word.focus();
	}
</script>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf51plsan.submit();   // Ojo para la funcion paf51plsan <> paf51plsan  (sencible a mayusculas)
	}
...
$wactualiz="PROGRAMA: paf51plsan.php Ver. 2020-02-25   JairS";
...
echo "<form name='paf51plsan' action='paf51plsan.php' method=post>";  	
...
if ($windicador == "PrimeraVez") 
{
   $query = "SELECT  atefec,atehor,ateced,atenom,atehis,atecar,ateord,atecit,atecon,ateex1,ateex2,atemed,atefpr,atehpr,ateobs "
          . " FROM pafplsan_000003 Where ateced = '".$wced."' And atefec = '".$wfec."' And atehor = '".$whor."'";   
   ...
   else                   // NO Encontro lleno los campos con los datos recibidos por la URL y busco el nro de ORDEN y ...
   {
    ...
    // Voy a la tabla de Ordenes PAF PLUS SANITAS busco que nro de ORDEN tenia para este dia
    $query = "SELECT Id FROM pafplsan_000001 Where pafced = '".$wced."' And paffec='".$wfec."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
...
/*****/	
   ...
   if ($wproceso == "Modificar" and $windicador=="PrimeraVez")  
    $query = "SELECT codigo,Descripcion FROM citplsan_000010 WHERE codigo='".$wmed."' Group By codigo,Descripcion"; 	         
   else   
   {   
    if ( isset($wcr3) and $wcr3!=""  )
     $query = "SELECT codigo,Descripcion FROM citplsan_000010 Where ( codigo like '%".$wcr3."%' or Descripcion like '%".$wcr3."%' ) Group By codigo,Descripcion ORDER BY Descripcion";  
    else
	{   
     if ( isset($wmed) and $wmed!="" )
	 {	 
	  $c3=explode('-',$wmed); 		
      $query = "SELECT codigo,Descripcion FROM citaplsan_000010 WHERE codigo='".$c3[0]."' Group By codigo,Descripcion"; 
	 } 
	 else   
      $query = "SELECT codigo,Descripcion FROM citplsan_000010 WHERE codigo='$&#$#' Group By codigo,Descripcion";  
    }
   } 
...
/****/    
   if (!isset($wfpr))   // Si no esta seteada entonces la inicializo con la fecha actual
	...
	if ($pos === false) 
	{ 
     $query = "SELECT Descripcion FROM citplsan_000010 WHERE codigo='".$wmed."'";
	 $resultadoB = mysql_query($query);            // Ejecuto el query 
	 $registroB = mysql_fetch_row($resultadoB);  
	 $wmed = $wmed."-".$registroB[0];
	}
	echo "<br><A HREF='http://mx.lasamericas.com.co/matrix/citas/procesos/000001_prx5.php?empresa=citplsan&wfec=".$wfpr."&wequ=".$wmed."' TARGET='_blank' >Agendar Cita</A>";
...
*/	 
    ...
    echo "<A HREF='paf52plsan.php?wced=".$wced."&wfec=".$wfec."&whor=".$whor."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
...
if ( $conf == "on" and isset($wfec) and $wfec<>'' and isset($whor) and $whor<>'' and isset($wced) and $wced<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   {	 
    ...
    if ($nroreg > 0)      //  Encontro 
    {	 	 
       $query = "Update pafplsan_000003 SET atenom='".$wnom."',atehis='".$whis."',atecar='".$wcar."',ateord='".$word. "',atecit='".$wcit
     ...
     else
     {
      ...
      $query = "INSERT INTO pafplsan_000003 VALUES ('pafplsan','".$fecha."','".$hora."','".$wfec."','".$whor."','".$wced."','".$wnom."','".$whis
