...
$query = "SELECT Afi_sexo,count(*)  FROM pafplsan_000002 GROUP BY Afi_sexo "; 
//$query = "SELECT Afi_tip_cotiza_ ,count(*)  FROM pafplsan_000002 GROUP BY Afi_tip_cotiza_  "; 
...
While ($i < $nroreg)
{
 ...
 $nombres[$i] = $registro[0]." ".$registro[1];
 $data[$i] = $registro[1];
 $wtot=$wtot+$data[$i];
 $i++;
}
...
$graph->title->Set("PROGRAMA PLUS SANITAS - USUARIOS POR SEXO - TOTAL USUARIOS ".$wtot ); 
