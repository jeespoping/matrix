...
<head>
    ...
    $accion = $_GET['accion'];
    $fechaActual = date('Y-m-d');       $horaActual = date('H:i:s');        $a�oActual = date('Y');                 $mesActual = date('m');
    ...
    case 'facturar':
        ...
        if($num > 0)
        {
            ...
            <section>
                ...
                $wresolucion= "Documento oficial de autorizaci�n de numeraci�n: ".$wnrores." del ".$wfecres.", "
                    ...
                    ."SS, 671 y SS 772, 773, 770 y SS del c�digo de comercio.<BR>"
            ...
            <div class="panel panel-info" align="center" style="width: 60%; border: none">
                ...
                <h4 style="color: #428BCA; font-size: large; font-weight: bold"><?php echo $codEmpRes.' '.$nomEmpresa ?></h4>
                <h3>Por valor de: </h3>
                <h3>$ <label><?php echo number_format($valTotCargos,0) ?></label></h3>
                <br><br>
                <form method="post">
            ...
            if(isset($_POST['btnProceed']))
            {
                ...
                mysql_queryV("insert into".' '."$wbasedato"."_000018(Medico,Fecha_data,Hora_data,fenano,fenmes,fenfec,fenffa,fenfac,fentip,fennit,fencod,
                 ...
                 VALUES('clisur','$fechaActual','$horaActual','$a�oActual','$mesActual','$fechaActual','$fueFact','$newCons','$tipoEmp','$nitEmpresa','$empCode',
                        ...
                        '$porDescTarifa','$porcReco','$valTope','VARIOS PACIENTES','NO APLICA','$estFactura','$wresolucion','$wcco','$valRecibo','off','NO APLICA','NO APLICA',
                        '0','','','$usuario')");
                ...
                for($i = 0; $i < $longitud; $i++)
                {
                    ...
                    $query1 = mysql_queryV("select id,Tcarvto,Tcarser,Tcarconcod,Tcartercod,Tcarterpor from".' '."$wbasedato"."_000106
                                           where Tcarfec BETWEEN '$fechaInicial' AND '$fechaFinal'
                    ...
                    //OBTENER LA SUMA DE LOS ABONOS(9001) POR HISTORIA E INGRESO POR FACTURA:
                    $queryAbonos = mysql_queryV("SELECT sum(Tcarvto) from".' '."$wbasedato"."_000106
                                                where Tcarfec BETWEEN '$fechaInicial' AND '$fechaFinal'
                                                and Tcarres LIKE '$codResponsable%'
                                                and Tcarhis = '$his'
                                                and Tcaring = '$ing'
                                            ...
                                            // AND Tcarconcod = '9001'");
                    $datoAbonos = mysql_fetch_array($queryAbonos);
                    $valorAbo = $datoAbonos[0];
                    $valorAbonos = $valorAbonos + $valorAbo;
                    ...
                    for($j = 0; $j < $conteo; $j++)
                    {
                        ...
                        //GRABAR EN ROOT_000122 PARA LA GENERACION DEL REPRESENTACION GRAFICA DE LA FACTURA
                ...
                //ACTUALIZAR TABLA_000018 -> GRABAR EL VALOR TOTAL DE LOS ABONOS POR FACTURA Y ACTUALIZAR EL VALOR TOTAL DE LA FACTURA(Fenval):
                $valorAbonos = abs($valorAbonos);
                $valTotCargos1 = $valTotCargos - $valorAbonos;
                mysql_queryV("update".' '."$wbasedato"."_000018 set Fenabo = '$valorAbonos', Fenval = '$valTotCargos1', Fensal = '$valTotCargos1' where Fenfac = '$newCons'");

                //ACTUALIZAR PATOL_000065 PARA AGRUPAR Y TOTALIZAR POR CONCEPTO:
                $queryConceptos = mysql_queryV("select sum(Fdevco) SUMA,Medico,Fecha_data,Hora_data,Fdefue,Fdedoc,Fdecco,Fdecon,Fdevco,Fdeter,
	                                           Fdepte,Fdevde,Fdesal,Fdeest,Fdeffa,Fdefac,fdeviv,Seguridad from".' '."$wbasedato"."_000065
                                               where Fdedoc = '$newCons'
                                               group by Fdecon");
                while($datConcep = mysql_fetch_array($queryConceptos))
                {
                    $Medico65 = $datConcep['Medico'];       $Fecha_data = $datConcep['Fecha_data']; $Hora_data = $datConcep['Hora_data'];   $Fdefue = $datConcep['Fdefue'];
                    $Fdedoc = $datConcep['Fdedoc'];         $Fdecco = $datConcep['Fdecco'];         $Fdecon = $datConcep['Fdecon'];         $Fdevco = $datConcep['Fdevco'];
                    $Fdeter = $datConcep['Fdeter'];         $Fdepte = $datConcep['Fdepte'];         $Fdevde = $datConcep['Fdevde'];         $Fdesal = $datConcep['Fdesal'];
                    $Fdeest = $datConcep['Fdeest'];         $Fdeffa = $datConcep['Fdeffa'];         $Fdefac = $datConcep['Fdefac'];         $fdeviv = $datConcep['fdeviv'];
                    $Seguridad = $datConcep['Seguridad'];   $suma = $datConcep['SUMA'];

                    mysql_queryV("delete from".' '."$wbasedato"."_000065 where Fdedoc = '$newCons' and Fdecon = '$Fdecon'");

                    mysql_queryV("insert into".' '."$wbasedato"."_000065(Medico,Fecha_data,Hora_data,Fdefue,Fdedoc,Fdecco,Fdecon,Fdevco,Fdeter,
                                            Fdepte,Fdevde,Fdesal,Fdeest,Fdeffa,Fdefac,fdeviv,Seguridad)
                                            values('$Medico65','$Fecha_data','$Hora_data','$Fdefue','$Fdedoc','$Fdecco','$Fdecon','$suma','$Fdeter',
                                            '$Fdepte','$Fdevde','$suma','$Fdeest','$Fdeffa','$Fdefac','$fdeviv','$Seguridad')");
                }
                ...
                <div align="center" style="width: 60%; border: none">
                    ...
                    <h3>El numero de la factura generada es :</h3><h2><?php echo $newCons ?></h2>
                    <br><br>
