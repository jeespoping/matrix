	     ...
	     if ($num > 0)
	        {
			 ...
			 for ($i=1;$i<=$num;$i++)
				...
				//////////////////CONSULTAR FACTURA POR CADA CARGO FACTURADO//////////////////////////
				$idCargo = $row[25];
				
				$qryFactura = " SELECT Rcffac "
						."   FROM ".$wbasedato."_000066 "
						."  WHERE Rcfreg = '".$idCargo."' ";
				 $commitFactura = mysql_query($qryFactura,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$qryFactura." - ".mysql_error());
				 $datoFactura = mysql_fetch_array($commitFactura);
				 $factura66 = $datoFactura['Rcffac'];		
				///////////////////////////////////////////
				
				
			     if ($row[27]==0 and $row[28]==0 and $wmueinv!="on" and $peranu=='on' and $numcca==0)
				 {
			        echo "<td align=center class='".$wcolor."'>
							<A href='cargos_ips.php?wid=".$row[25]."&wanular=S"."&wusuario=".$wusuario."&whistoria=".$whistoria."&wing=".$wing."&wfecing=".$wfecing."&wser=".$wser."&wcodemp=".$wcodemp."&wnomemp=".$wnomemp."&wno1=".$wno1."&wno2=".$wno2."&wap1=".$wap1."&wap2=".$wap2."&wdoc=".$wdoc."&wcco=".$wcco."&wccogra=".$wccogra."&wbasedato=".$wbasedato."&wnomcco=".$wnomcco."&wpaquete=".$wpaquete."&wcodpaq=".$wcodpaq."&wnompaq=".$wnompaq."&wemp_pmla=".$wemp_pmla."'>Anular</A>
						</td>";
				 }
			     else
				 {
					 ...
					 {
			            echo "<td align=center class='".$wcolor."'><b>Facturado - ".$factura66."</b></td>";
					 }
				 }	 
