	  ...
	  try
		{
			...
			*/
			...
			ajax.open("POST", "Ventas_nue_test.php",true);
	  ...
	  try
		{
			...
			{ 
				...
				if (ajax.readyState==4 && ajax.status==200)
				{
					...
					if (isset(document.getElementById("wload").value) && (document.getElementById("wload").value=="1"  || document.getElementById("wload").value=="2" || document.getElementById("wload").value=="3"))
					{
						submit_form('on');
					}
				}
...
{	   
  ...
  $wusuario = substr($user,$pos+1,strlen($user)); 
  																 ...
  																 // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= \\
  $wactualiz="Mayo 14 de 2012";                     			 // Aca se coloca la ultima fecha de actualizacion de este programa \\
	                                                             // =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*= \\
			          ...
			          if (!isset($fwventa) or $fwventa == "N" )  //Solo da la opcion de Grabar Venta mientras no se haya grabado la venta definitiva
			             {
						  ...
						  echo "<td align=center colspan=7 height=27 valign=middle> &nbsp; <input type='button' name='wventa_btn' id='wventa_btn' style='font-size:14px; font-weight:bold;' value='Grabar Venta' onclick='graba_venta()'> &nbsp; </td>";
                   ...
                   if ($whabilita_venta == "ENABLED")
		              {
	                   if (!isset($fwventa) or $fwventa == "N" )  //Solo da la opcion de Grabar Venta mientras no se haya grabado la venta definitiva
					   {
						  echo "<td align=center colspan=13 height=27 valign=middle> &nbsp; <input type='button' name='wventa_btn' id='wventa_btn' style='font-size:14px; font-weight:bold;' value='Grabar Venta' onclick='graba_venta()'> &nbsp; </td>";
						  echo "<input type='HIDDEN' name='wventa' id='wventa' value='N'>";      //Envio la venta como "N"
					   }
                      }    
	          echo "</tr>";
			...
			if (isset($wdocpac) && $wdocpac != "9999" and $wdocpac != "")
			{
				 if($conexUnix)
				 {
					...
					if (odbc_fetch_row($err_o))
					{
						...
						$edad = odbc_result($err_o,3);
						if(!empty($edad) && $edad!="")
							$wedad = calcula_edad($edad);
						$sexo = odbc_result($err_o,4);
					...
					else
					{
						...
						if (odbc_fetch_row($err_1))
						{
							...
							$edad = odbc_result($err_1,3);
							if(!empty($edad) && $edad!="")
								$wedad = calcula_edad($edad);
							$sexo = odbc_result($err_1,4);
							if(!empty($sexo) && $sexo!="" && $sexo!=" ")
								$wsexo = traer_sexo($sexo);
						}
						else
					...
					{
					  $select = " accdetpol, accdettip, accdetmun, '".$long."' as accdetffi, accdetzon, accdetnom, accdettel, accdetdir, accdetacc, accdethis ";
					  $from =	" inaccdet ";
					  ...
					  {
						  $wpoliza = odbc_result($err_acc,1);
						  $wtipdto = odbc_result($err_acc,2);
						  //$wmuni = odbc_result($err_acc,3);
						  $wfecven = odbc_result($err_acc,4);
						  $zona = odbc_result($err_acc,5);
						  $wzona = traer_zona($zona);

						  $wnompac=odbc_result($err_acc,6);
						  $wte1pac=odbc_result($err_acc,7);
						  $wdirpac=odbc_result($err_acc,8);
		  ...
		  if ($num > 0) //El articulo existe y tiene tarifa, entra por el then
		     {
			  ...
			  ///   {
				  ...
				  if ($wtipcli!="01-PARTICULAR" && isset($wlotven) && $wlotven=="on") 
                  {
					  ...
					  echo "<td class='fila1'>Fec. vence<br /><input type='text' name='wfve' id='wfve' size='10' size='10' value='' onkeypress='if (validar(event)) submit_form2(\"wdato\")'></td>";    //wfve
				  }
				  else
				  {
				...
				if ($num > 0) //El articulo existe y tiene tarifa, entra por el then
				   {
					...
					if ($wtipcli!="01-PARTICULAR" && isset($wlotven) && $wlotven=="on") 
					{
						...
						echo "<td class='fila1'>Fec. vence<br /><input type='text' name='wfve' id='wfve' size='10' value='' onkeypress='if (validar(event)) submit_form2(\"wdato\")'></td>";    //wfve
					}
					else
					{
