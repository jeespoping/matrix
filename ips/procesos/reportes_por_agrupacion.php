...
function cargar_hiddens_para_autocomplete_usuario()
{
	...
	while($row_usu = mysql_fetch_array($r_usu))
	{
		...
		$arr_usu[trim($row_usu['codigo'])] = trim($row_usu['nombre']);
	}
...
if ($inicial=='no' AND $operacion=='traeresultadov3')
{
	...
	if($tipodeformato == '01' or $tipodeformato=='04')
	{
		...
		//--- se añaden filtros si hay campos para hacer la consulta mas especifica
		...
		// ---- Se le agrega al where algo general  que es el periodo y el año  para buscar
	...
	if($tipodeformato == '03' )
	{
			...
			// -- las que se debe seleccionar un periodo y un año
			// -- y las que se buscan por una fecha inicial y otra final

				// busqueda con periodo y año
				if ($wfechainicial=='')
				{
					//--Consulta que trae  lo que se contesto a cada una de las preguntas , deacuerdo a la clasificacion de agrupaciones.
					//--Las tablas que tienen que ver en esta consulta son _000005 tabla de descriptores , _000007 tabla con el resultado de cada una de las preguntas
...
// y otros campos como el año y el periodo , para ser llenado por el usuario
					...
					//03
						...
						$rowtagrupacion1noaplica = mysql_fetch_array($restagrupacion1noaplica);
							...
							else
							{
								...
								$auxind = $auxind + 1;
								echo"<td align='center'  style='cursor: pointer;' bgcolor='".$color1."'><div  class='msg_tooltip'  title='Respuestas para cálculo ".$cuantost1." <br> Respuestas Totales ".$cuantostnoaplica1."' onclick='abrirporpreguntas(this,\"".$condicion."\",\"todos\",\"".$fechainicial2."\",\"".$fechafinal2."\",\"".$vectornomagrupaciones[$clasificacion]."\",\"".$vectorccocontestado[$j]."\",\"".$float_redondeado1."\",\"".$auxind."\")'><img style='float: right; display: inline-block;' id='img_mas-".$auxind."'' src='../../images/medical/hce/mas.PNG'>".$float_redondeado1."</div><div id='ddiv-".$auxind."'></div></td>";
								//echo"<td align='center'  style='cursor: pointer;' bgcolor='".$color."' ><div class='msg_tooltip'  title='Respuestas para cálculo ".$cuantost." <br> Respuestas totales ".$cuantostnoaplica."'    onclick='abrirporpreguntas(this,\"".$condicion."\",\"todos\",\"".$fechainicial1."\",\"".$fechafinal1."\",\"".$vectornomagrupaciones[$clasificacion]."\",\"".$vectorccocontestado[$j]."\",\"".$float_redondeado."\",\"".$auxind."\")' ><img style='float: right; display: inline-block;' id='img_mas-".$auxind."'' src='../../images/medical/hce/mas.PNG'>".$float_redondeado."</div><div id='ddiv-".$auxind."'></div></td>";
...
// Se añaden funcionalidades para poder ver resultados de encuestas relacionadas con encuestas magenta
// --------------------------------------------------------------------------------------------------------
// 2014-05-23 Felipe Alvarez
// Se añade funcionalidad para graficar los resultados de los informes de encuestas
// ---------------------------------------------------------------------------------------------------------
// 2015-11-11 Felipe Alvarez
// Se añaden funcionalidades para poder ver los resultados del tema invecla , pues se tiene que interpretar formula y
