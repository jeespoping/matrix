...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		 ...
		 var haymensaje = $("#dato_mensaje").length;
		// alertas individuales especiales
		// var validacionok1 =false;
		// var validacionok2 =false;

		//alert($("[name=wdci]").val());
		// si cilindro derecho vacio y eje derecho vacio  validacion pasa a true

		// var texto = $("#dato_mensaje").val();
		// if(texto =='')
		// {
		...
		// {
			...
			// mensaje ="Si  esta digitado el Cilidro debe esta digitada la Esfera";
			// if(haymensaje<= 0)
			// {
				// haymensaje = 1;
				// $("#dato_mensaje").val(mensaje);

			// }
			// $("[name=wdci]").addClass("campoObligatorio");
			// $("[name=wdej]").addClass("campoObligatorio");

		// }

		// if(!validacionok2)
		// {
			// mensaje ="Si  esta digitado el Cilidro debe estar digitada la Esfera";
			// if(haymensaje<= 0)
			// {
				// haymensaje = 1;
				// $("#dato_mensaje").val(mensaje);
			...
			// }
			// $("[name=wiej]").addClass("campoObligatorio");
			// $("[name=wici]").addClass("campoObligatorio");

		// }


		//---- si esta la esfera debe estar el cilindro
		...
		if(haymensaje > 0){
			...
			$( "input:text:enabled" ).each(function(){
				if($.trim($(this).val())== '')
				{
					if($(this).attr('name')=='wfac' ||  $(this).attr('name')=='wfre' || $(this).attr('name')=='wfen'  || $(this).attr('name')=='wde1' || $(this).attr('name')=='wde2' || $(this).attr('name')=='wde3' || $(this).attr('name')=='wde4' || $(this).attr('name')=='wffa'|| $(this).attr('name')=='wde5')
					{
			...
			$( "select:enabled" ).each(function(){
				...
				if($.trim($(this).val())== '')
				{
					if($(this).attr('name')=='wled' || $(this).attr('name')=='wlei')
					{
			...
			$( "input:radio" ).each(function(){
			    ...
			    var checados = $("input[type='radio'][name='"+name+"']:checked").length;
				if( checados == 0 ){
					if(!$("[name='"+name+"']").hasClass( "deshabilitado" ))
					{
						//console.log( name );

						$("input[type='radio'][name='"+name+"'][name='"+name+"']").parent().addClass("campoObligatorio");
					}
				}
			});
			}
			...
			if($("[name=wdci]").val() ==""  )
			{
				...
				if($("[name=wdej]").val()=="")
				{
					$("[name=wdci]").removeClass('campoObligatorio');
					$("[name=wdej]").removeClass('campoObligatorio');
			...
			if($("[name=wici]").val() !=""  )
			{
				if($("[name=wiej]").val()!="")
				{
						$("[name=wici]").removeClass('campoObligatorio');
						$("[name=wiej]").removeClass('campoObligatorio');
				}
			}
			...
			$("[name=wdad]").removeClass('campoObligatorio');
			$("[name=wiad]").removeClass('campoObligatorio');
...
else
{
	...
	// buscamos el numero de la venta correspondiente a la factura
	//echo "www".$wven."sssss";
	if($wven =='')
	{
		$query_venta = "	SELECT  Fdenve
							  FROM  uvglobal_000019
							 WHERE  Fdefac = '".$wfac."'";

		$res_venta = mysql_query($query_venta);
		if($row_venta = mysql_fetch_array($res_venta))
		{
			$wven = $row_venta['Fdenve'];
			//echo "cambio la ventaaaaaaaa";
		}
	}


	// si la venta es vacia puede que se haya hecho una venta y aun no la factura , se debe buscar en la tabla
	// de ventas con el documento del usuario
	/*if($wven =='')
	{
	...
	if (isset($wdoc))
    {
        //echo " -------------- aca tambien Entro 22222";
         echo "<INPUT TYPE='text' NAME='wdoc' size=10 VALUE='".$wdoc."' $wreadonly></INPUT><INPUT TYPE='hidden' NAME='wdesdeventas' size=10 VALUE='".$wdesdeventas."' $wreadonly></INPUT></td>";
		...
		if($modificarselect=='')
		 {
			...
			// {
				$query_venta = " SELECT Vennum
								   FROM uvglobal_000016 LEFT JOIN  uvglobal_000133 ON Vennum = ordven , uvglobal_000001, uvglobal_000017
								  WHERE Vennit = '".$wdoc."'
									AND Vennum = Vdenum
									AND Vdeart = Artcod
									AND SUBSTRING_INDEX( Artgru, '-', 1 ) IN ('LO','LE','MT','LC')
									AND ordven IS NULL
									AND Venest != 'off'
									GROUP BY Vennum ";
			...
			// }else
			// {
					// echo "<option value='".$wven."' selected >".$wven."</option>";
		...
		else
		{
		 ...
		 echo "</td>";
		 echo "<td align=center bgcolor=#DDDDDD colspan=2><b><font text color=990000 size=4> ".$wnombre."</font></b></td>";
         echo "<td align=center bgcolor=#DDDDDD colspan=2><b><font text color=990000 size=3> Tel: ".$wtelefono."</font></b></td>";
    }
    else
    {
        echo "<INPUT TYPE='text' NAME='wdoc' size=10 onchange='enter()'></INPUT><INPUT TYPE='hidden' value='no' NAME='wdesdeventas' size=10 onchange='enter()'></INPUT></td>";
        echo "<td align=center bgcolor=#DDDDDD ><font text color=#003366 size=2>Nro Venta</font><INPUT TYPE='text' NAME='wven' id='wven' size=10 color=#003366 VALUE='".$wven."' readonly></INPUT></td>";
...
*/
		...
		*/
        ...
        if (isset($wled) && $wled != "" && isset($wdoc) && isset($wven))
        {
           ...
           // echo "<INPUT TYPE='text' NAME='wled' size=10 VALUE='".$wled."' onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
        ...
        {
           // echo "<INPUT TYPE='text' NAME='wled' size=10 onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
            echo "<select NAME='wled' onkeyup='this.value=this.value.toUpperCase();' onchange='enter();' ><option value=''>...seleccione</option>";
        ...
        //if (isset($wlei) && $wlei != "" && isset($wffa) && isset($wfac) && $impr_otro_lente==true) // 2012-05-08
        if (isset($wlei) && $wlei != "" && isset($wven) && $impr_otro_lente==true)
		{
            //echo "<INPUT TYPE='text' NAME='wlei' size=10 VALUE='".$wlei."' onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
        ...
        else
        {
           ...
           // echo "<INPUT TYPE='text' NAME='wlei' size=10 onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
			 echo "<select NAME='wlei' onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'><option value=''>...seleccione</option>";
    ...
    else
    {
     ...
     // if (isset($wref) and $wref <> "" and isset($wffa) and isset($wfac) )
        if (isset($wref) and $wref <> "" and isset($wven) )
   	    {
          //echo "<INPUT TYPE='text' NAME='wref' size=10 VALUE='".$wref."' onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
		/*
          $query=	"SELECT Artnom,Vdeart FROM uvglobal_000016, uvglobal_000017, uvglobal_000001
					  WHERE Venffa = '".$wffa."' And Vennfa = '".$wfac."' And Vdenum = Vennum And Vdeart = Artcod
					    AND Artcod = '".$wref."' And mid(Artgru,1,2) = 'MT'";
		*/
		 echo "<select NAME='wref'  onkeyup='this.value=this.value.toUpperCase();' onchange='enter();' ><option value=''>...Seleccione</option>";
        ...
        {
          //echo "<INPUT TYPE='text' NAME='wref' size=10 onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'></INPUT>";
			echo "<select NAME='wref'  onkeyup='this.value=this.value.toUpperCase();' onchange='enter();'><option value=''>...Seleccione</option>";
    ...
    else
    {
		...
		//--Agrego campos de venta , debido a que ya se puede hacer una venta y dejar para despues la factura y se tendra
		//--que seguir haciendo la orden de laboratorio
		// $wven='';
		// buscamos el numero de la venta correspondiente a la factura
		// $query_venta = "	SELECT  Fdenve
							  // FROM  uvglobal_000019
        ...
        // si existe factura
		 ...
		 //echo "<INPUT TYPE='text' NAME='wven' id='wven' size=10 color=#003366 VALUE='".$wven."' readonly></INPUT></td><td align=center bgcolor=#DDDDDD colspan=4></td></tr>";
        ...
        if($wven!='')
		{
			...
			echo "9<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$wvenfac."' readonly></INPUT></td>";
		...
		else
		{
			if (isset($wfac) )
			{
			   ...
			   if( $wproceso != "Nuevo" ){
					echo "8<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$wfac."' readonly ></INPUT></td>";
			   }
			   else{
					echo "7<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$wfac."' readonly></INPUT></td>";
			   }
			   ...
			   if( !tieneNCFacturacion( $wfac, $sede ) )
			   {
				   ...
				   {
					$registro = mysql_fetch_row($resultado);
					echo "<td align=center bgcolor=#DDDDDD colspan=4 rowspan='1'><b><font text color=990000 size=2> VALOR: ".$registro[0]."<b>&nbsp;&nbsp;&nbsp;</b> USUARIO: ".$registro[1]."</font></b></td>";
				   }
				   else
				   {
			...
			else
			{
			 if (isset($wdoc) )
			 {
			   ...
			   if ($nroreg > 0)   //Encontro
			   {
				...
				if( !tieneNCFacturacion( $registro[2], $sede ) ){
				   ...
				   if( ordenLaboratorio( $registro[2] ) ){
						...
						echo "".$registro[2] ."<INPUT TYPE='text' NAME='wffa' size=5 VALUE='".$registro[3]."' readonly></INPUT>";

						if( $wproceso != "Nuevo" ){
							echo "6<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly></INPUT></td>";
						}
						else{
							echo "5<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly></INPUT></td>";
						}
						...
						if( $wproceso != "Nuevo" ){
							echo "4<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly></INPUT></td>";
						}
						else{
							echo "3<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly></INPUT></td>";
						}

						$wnumfactura = $registro[2];
					}
				}
				else
				{
					...
					if( $wproceso != "Nuevo" ){
						echo "1<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly></INPUT></td>";
					}
					else
					{
						echo "2<INPUT TYPE='text' NAME='wfac' size=10 VALUE='".$registro[2]."' readonly ></INPUT></td>";
					}
			   ...
			   else
			   {
				...
				{
					echo "<td align=center bgcolor=#DDDDDD colspan=4>";
					echo "<font size=3 text color=#33FFFF></font>";
					echo "</b></td>";
    ...
    if ($wproceso != "Consultar" && $es_modificable )  //PARA QUE NO MUESTRE EL BOTON <GRABAR> NI EL "Checkbox" SI ESTOY CONSULTANDO
    {
      // Boton grabar y variable 'wdat' en un checkbox para indicar que los datos ya estan completos para validar
        ...
        echo "<input type='submit' value='Grabar' onclick='iragrabar()' ><input type=hidden id='grabarsi' name='grabarsi' value='no'>";

        if (isset($wdat))
		{
            echo "<div id='divcheckbox' style='display:none'><INPUT TYPE = 'Checkbox' NAME='wdat' id='wdat' VALUE='".$wdat."'></INPUT></div></td>";
			$wdat=1;
		}
		else
		{
            echo "<div id='divcheckbox' style='display:none'><INPUT TYPE='Checkbox' NAME='wdat' id='wdat' size=10></INPUT></div></td>";
			$wdat=1;
		}
	}
...
///////////        CUANDO YA HAY DATOS DIGITADOS       ///////////////
{
	...
	//echo "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".$todok."bbbbbb".$wproceso."ssssssssss";
    if ($todok)
    {
     if ($wproceso == "Nuevo")
     {
        ...
        if( !ordenLaboratorio( $wfac ) )
		{
              ...
              ."ordvem,ordcco,seguridad,ordven) "
              ...
              ."'".$wobs."','".$wcaj."','".$wffa."','".$wfac."','".$wfec."','".$wfre."','".$wfen."','".$wvel."','".$wvem."','".$sede."','C-uvglobal','".$wven."')";
//				echo "...........<b>";
...
//			   	   echo "...........<b>";
                   ...
                   if($wdesdeventas =='si')
				   {
					...
					echo "alert('Grabacion Exitosa');window.close();";
					echo "</script>";
				   }
				   else
				   {
...
//     		construirMensajes( $lenteIzquierdo );
      ...
      echo "<center><table border=0 style='display:none' >";
      echo "<tr><td align=center colspan=100 bgcolor=#006699>";
      echo "<font size=3 text color=#33FFFF><input type='hidden' id='dato_mensaje' value='$mensaje'></font>";
      echo "</td></tr></table></center>";
     }
    }
