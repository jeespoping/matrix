...
//-------------------I------------------------I------------------------------------------------------------------------
//  2013-08-05       I Juan C. Hdez           I Se adiciona la variable seguridad=NO, para que el conex no valide este programa
//-------------------I------------------------I------------------------------------------------------------------------
...
function calcularTotal($pagos, $fuente, $valor, &$total, &$totalF, &$radio)
{
	$totalF=0;
	for ($i=0;$i<count($pagos['codigo']);$i++)
	{
...
function pintarEncabezado($wactualiz)
{
	...
	echo"<form action='translado.php' method='post' name='forma' >";
...
function pintarAlert4($cajCod, $consulta)
{
	global $wbasedato;
	echo "</table>";
	echo"<form action='translado.php' method='post' name='forma1' ><CENTER><fieldset style='border:solid;border-color:#000080; width=330' ; color=#000080>";
	echo "<table align='center' border=0 bordercolor=#000080 width=700 style='border:solid;'>";
...
function pintarFormulario($fuente, $numero, $fecha, $valor, $documento, $banco, $radio, $pagos, $total, $totalF, $cajCod, $cajDes, $consecutivoT, $clase, $id, $fuenteT)
{
	...
	echo "<form name='forma1' action='translado.php' method='post'>";
	...
	for($i=0; $i < count($pagos['codigo']); $i++)
	{
		if (isset ($fuente[$i][0]))
		{
			...
			for($j=0; $j < count($fuente[$i]); $j++)
			{
				...
				echo "<td class='texto'>$ ".number_format((float)$valor[$i][$j],0,"",".")."</td>";
				echo "</tr>";
...
{
	echo "<table align='center'>";
	echo "<tr><td class='titulo4' >VALOR A TRASLADAR: $ ".number_format((float)$totalF,0,"",".")."</td></tr>";
	...
	echo "</select></td>";
	echo "<tr><td class='titulo4' >&nbsp;</td></tr>";
	echo "<tr><td class='titulo4' ><input type='checkbox' name='transladar'>Trasladar	&nbsp;<input type='button' value='ACEPTAR'  onclick='javascript:enter()'></td></tr>";
	echo "</table></br>";
	echo "</form>";
}
...
function pintarBoton2($totalF, $consulta)
{
	...
	echo "<table align='center'>";
	echo "<tr><td class='titulo4' >VALOR TOTAL TRASLADADO: $ ".number_format((float)$totalF,0,"",".")."</td></tr>";
	echo "<tr><td class='titulo4' >&nbsp;</td></tr>";
			...
			if (!isset ($bandera1) or $bandera1!=3) //generacion de informacion para el transalado
			{
				...
				if(!isset($transladar)) //proceso previo antes de guardar el translado
				{
					...
					if ($contador>0)
					{
						...
						calcularTotal($pagos, $fuente, $valor, $total, $totalF, $radio);
