...
//==========================================================================================================================================
// Actualizacion	:	2011-12-05																											|
// Por				:	Edwin Molina Grisales																								|
// Cambios			:	En la impresion, se quita el tipo para el lente																		|
//																																			|
// Actualizacion	:	2011-09-06																											|
// Por				:	Gustavo Avendaño																									|
// Cambios			:	En la impresion, codigo invima																						|
//																																			|
// Actualizacion	:	2009-11-23																											|
// Por				:	Edwin Molina Grisales																								|
// Cambios			:	En la impresion, para los valores de los cilindros iguales a 0 no se muestra nada									|
//==========================================================================================================================================
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////ACA COMIENZA LA IMPRESION
 ...
 $qhayfac= "SELECT  ordfac
			 FROM uvglobal_000133
		    WHERE ordnro = '".$wnro."'";



$reshayfac = mysql_query($qhayfac, $conex) or die("ERROR EN QUERY ".$qhayfac."");
$tienefac ='';
if($rowhayfac = mysql_fetch_array($reshayfac))
{
	$tienefac =$rowhayfac['ordfac'];
}

if ( $tienefac == '' || $tienefac == 'NO APLICA' ||  TRIM($tienefac)=='')
{
	/* ordfec,ordnro,orddoc,uvglobal_000041.clinom,uvglobal_000041.clite1,ordran,ordtus,orddsi,orddes,orddci,orddej,orddad,orddte,"
	...
	$q = " CREATE TEMPORARY TABLE if not exists tempo2 as "
		...
		."        ordisi,ordies,ordici,ordiej,ordiad,ordite,ordled,artnom as nomled,ordlei,ordedp,ordtra,ordbif,ordmon,ordref,ordmet,ordcom,ordcol,ordpin,"
		."        ordde1,ordbra,ordde2,ordter,ordde3,ordpla,ordde4,ordotr,ordde5,ordobs,ordcaj,ordfac,ordvel,ordvem ,Venvto,'' as saldo,Vennum,artima as imad "
		."   FROM uvglobal_000041,uvglobal_000016,uvglobal_000133 left join uvglobal_000001"
		."     ON ordled = artcod "
		."  WHERE ordnro = '".$wnro."'"
		."    AND orddoc = clidoc"
		."    AND ordven = Vennum";

	$res = mysql_query($q, $conex) or die("ERROR EN QUERY 1	 ".$q."");
	...
	$q1 =" CREATE TEMPORARY TABLE if not exists tempo3 as "
		...
		."        ordde1,ordbra,ordde2,ordter,ordde3,ordpla,ordde4,ordotr,ordde5,ordobs,ordcaj,ordfac,ordvel,ordvem,Venvto,'' as saldo,Vennum,imad,artima as imai"
	...
	$query2 =" SELECT ordfec,ordnro,orddoc,clinom,clite1,ordran,ordtus,orddsi,orddes,orddci,orddej,orddad,orddte,ordisi,ordies,ordici,ordiej,ordiad,ordite,ordled,"
			...
			."        ordde1,ordbra,ordde2,ordter,ordde3,ordpla,ordde4,ordotr,ordde5,ordobs,ordcaj,ordfac,ordvel,ordvem,Venvto,'' as saldo,Vennum,imad,imai"
...
echo "<td bgcolor='#FFFFFF'align=center width=138><font text color=#000000 size=1>ESFERA</td>";
echo "<td bgcolor='#FFFFFF'align=center width=138><font text color=#000000 size=1>CILINDRO</td>";
echo "<td bgcolor='#FFFFFF'align=center width=138><font text color=#000000 size=1>EJE</td>";
echo "<td bgcolor='#FFFFFF'align=center width=138><font text color=#000000 size=1>ADD</td>";
// echo "<td bgcolor='#FFFFFF'align=center><font text color=#000000 size=1>TIPO</td>";
echo "</tr>";
...
echo "<td  bgcolor=#FFFFFF ><font size=1 color='#000000'><b>Cod. Invima: </b></font></td>";
echo "<td  bgcolor=#FFFFFF ><font size=1><b>&nbsp;$row2[50]</b></font></td>";
echo "</tr>";
...
echo "<td  bgcolor=#FFFFFF ><font size=1 color='#000000'><b>Cod. Invima: </b></font></td>";
echo "<td  bgcolor=#FFFFFF ><font size=1><b>&nbsp;$row2[51]</b></font></td>";
echo "</tr>";
echo "</table>";
...
$query_saldo = "SELECT  Rdevta,Rdevca
				  FROM  uvglobal_000021
				 WHERE  Rdevta='".$row2[49]."' AND Rdeest='on'";
...
$wsaldo = ($row2[47]*1) - ($vrecibido*1);

$res2 = mysql_query($query_saldo,$conex);

echo "<table border=0 >";
echo "<Tr >";
echo "<td  bgcolor=#FFFFFF width=14%><font size=1><b>Nro.Venta:</b></font></td>";
echo "<td  bgcolor=#FFFFFF width=12%><font size=1>$row2[49]</font></td>";
...
echo "<td  bgcolor=#FFFFFF width=12%><font size=1>$row2[44]</font></td>";
 ...
 $querynuevo = " 		SELECT Venffa,Vennum,Empcod ,uvglobal_000016.Fecha_data,Empnit,Empnom, Clinom ,Venvto, Venviv,Vendes, Vencop , Venest ,Venffa , Vennfa  "
				 ."   	  FROM uvglobal_000016 LEFT JOIN uvglobal_000041 ON Clidoc = Vennit , uvglobal_000024" //, ".$wbasedato."_000100 "
				 ."  	 WHERE Vennum like '".$row2[49]."'"
				 ."    	   AND Vencod = empcod";
 
 $resnuevo = mysql_query($querynuevo,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$querynuevo." - ".mysql_error());
 $numnuevo = mysql_num_rows($resnuevo);
 $rownuevo = mysql_fetch_array($resnuevo);

	   
 $wval       = $rownuevo['Venvto'];
	...
	if( $rownuevo['Vennfa']!='' )
	{
		 ...
		 */
		  $qfactura = " SELECT  fenvnc , fenvnd"
					 ."   FROM uvglobal_000018, uvglobal_000024 " //, ".$wbasedato."_000100 "
					 ."  WHERE fenffa like '".$rownuevo['Venffa']."'"
					 ."    AND fenfac like '".$rownuevo['Vennfa']."'"
					 ."    AND fencod = empcod ";
...
echo "<td  bgcolor=#FFFFFF width=12%><font size=1><b>Vlr.Venta:</b></font></td>";

/* echo "<td  bgcolor=#FFFFFF width=15%><font size=1>".number_format($row2[47],0,'.',',')."</font></td>";*/

echo "<td  bgcolor=#FFFFFF width=15%><font size=1>".number_format($wval,0,'.',',')."</font></td>";
echo "<td  bgcolor=#FFFFFF width=12%><font size=1><b>Vlr.Saldo:</b></font></td>";
echo "<td  bgcolor=#FFFFFF width=15%><font size=1>".number_format($wsal,0,'.',',')."</font></td>";
echo "<Tr >";
echo "</table>";
...
else
{
	...
	$res = mysql_query($q, $conex) or die("ERROR EN QUERY este");

	$q1 =" CREATE TEMPORARY TABLE if not exists tempo3 as "
		." SELECT ordfec,ordnro,orddoc,clinom,clite1,ordran,ordtus,orddsi,orddes,orddci,orddej,orddad,orddte,ordisi,ordies,ordici,ordiej,ordiad,ordite,ordled,"
		."        nomled,ordlei,artnom as nomlei,ordedp,ordtra,ordbif,ordmon,ordref,ordmet,ordcom,ordcol,ordpin,"
		."        ordde1,ordbra,ordde2,ordter,ordde3,ordpla,ordde4,ordotr,ordde5,ordobs,ordcaj,ordfac,ordvel,ordvem,fenval,fensal,fdenve,imad,artima as imai"
		."   FROM tempo2 LEFT JOIN uvglobal_000001"
		."     ON ordlei = artcod";
	...
	$query2 =" SELECT ordfec,ordnro,orddoc,clinom,clite1,ordran,ordtus,orddsi,orddes,orddci,orddej,orddad,orddte,ordisi,ordies,ordici,ordiej,ordiad,ordite,ordled,"
			."        nomled,ordlei,nomlei,ordedp,ordtra,ordbif,ordmon,ordref,artnom as nomref,ordmet,ordcom,ordcol,ordpin,"
			."        ordde1,ordbra,ordde2,ordter,ordde3,ordpla,ordde4,ordotr,ordde5,ordobs,ordcaj,ordfac,ordvel,ordvem,fenval,fensal,fdenve,imad,imai"
			."   FROM tempo3 LEFT JOIN uvglobal_000001"
			."     ON ordref = artcod";
