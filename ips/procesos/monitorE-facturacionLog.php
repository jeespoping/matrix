...
else
{
	...
	if(isset($_GET['automatizacion_pdfs'])){
		txtLog ("include ips/funcionesE-facToyota.php");
		include_once("ips/funcionesE-facToyota.php");
	}
	else{
		txtLog ("include ips/funcionesE-fac.php");
...
//=====================================================================================================================================================================
    ...
    function verLog()
    {
        ...
        try {
			$log = file_get_contents('log_la.txt');
			echo $log;
        } catch (Exception $e) {
        }
    }
...
if(isset($accion)) 
{
	...
	switch($accion)
	{
		...
		{
			txtLog("descargarRepGrafica wsDownload");
			$respuesta 	= wsDownload($wemp_pmla, $fuente, $documento);
			txtLog("respuesta: $respuesta");
			...
			txtLog("descargarRepGrafica end", false);
			break;
		}
		case 'actualizarMonitor':
		{
			txtLog("actualizarMonitor... pag $pagina");
			...
			txtLog("Obtener maestro de estados...");
			$arrEstados = array();
			$sqlEstados = "
			...
			txtLog("Obtener los documentos para paginar...");
			$arrayDoc = array();
			$sqlDocP  = "
			SELECT id
			  FROM root_000122
			...
			foreach($arrayFac as $idx => $infoFac){
				txtLog("--------------------------");
				txtLog("fuente " . $infoFac['Faefue'] . " doc " . $infoFac['Faedoc']);				
				...
				// --> Monitor para un usuario que solo va a imprimir facturas
					...
					txtLog("Listar sin llamar wsDocumentStatus");	
					$respuesta["html"].= '
					<tr class="'.$fila.'">
						<td>'.$infoFac['Fecha_data'].' '.$infoFac['Hora_data'].'</td>
						<td>'.$infoFac['Faefue'].'</td>
			...
			txtLog("totalReg " . $respuesta["totalReg"]);
			txtLog("totalPag " . $respuesta["totalPag"]);
...
//=====================================================================================================================================================================
	...
	function actualizarMonitor(idPagina){
		...
		$.post("monitorE-facturacionLog.php",
		{	
			...
			alert('Error actualizando monitor: ' + response.responseText);
			$("#modalCosultando").modal('hide');
		});
	}
	//--------------------------------------------------------
...
=====================================================================================================================================================================-->	
	...
	txtLog("Consultar cco del usuario");
	...
	txtLog("Consultar estado del cron");
		...
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				<div class="modal-body" align="center">
					<p id="lblConsultando">Consultando...<span class="glyphicon glyphicon-refresh" aria-hidden="true"></span></p>
				</div>
			</div>
		</div>
	</div>
