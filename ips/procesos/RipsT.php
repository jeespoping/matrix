...
<!--
	...
	function ajaxquery(fila,empresa, wemp_pmla)
	{
		...
		ajax=nuevoAjax();
		st="ripsT.php?empresa="+empresa+"&wemp_pmla="+wemp_pmla+"&wfec1="+x2+"&wfec2="+x3+"&wenv="+x1+"&ti="+x4+"&wemp="+x5+"&wffa="+x6+"&wfac="+x7;
...
/**********************************************************************************************************************
	   PROGRAMA : RipsT.php
	   Fecha de Liberacion : 2019-09-17
	   ...
	   .2019-09-17: Camilo Zapata
        ...
        .2012-01-19
              Se modifico el programa para utilizar tablas temporales con le proposito de disminuir el tiempo de ejecucion de los
              querys para generar los diferentes archivos que componen el RIPS.
...
else
{
	...
	if(!isset($ti) or !isset($wenv) or ($wenv == 0 and $ti == 0 and $wfac == "" ))
	{
		...
		echo "<tr><td align=center colspan=2>PROGRAMA DE GENERACION DE RIPS Ver. 2012-01-19</td></tr> ";
	...
	else
	{
		...
		switch ($ti)
		{
			case 0:
				...
				if( isset($wfac) and trim($wfac) !="" )
					echo "<tr><td align=center bgcolor=#999999 colspan=2><font size=2><b>Factura Fuente. : ".$wffa." - factura: ".$wfac."</b></font>";
			break;
			case 1:
        ...
        . " (UNIQUE INDEX idx1(Fenfac),INDEX idx2(Fenhis,Fening)) "
        . " select Fenfac, Fenhis, Fening, Fencod, Fenfec, Fencop, Fenval from clisur_000018 where Fenfac in ".$win." ";
        ...
        . " (INDEX idx1(Fenhis,Fening),INDEX idx2(Tcarprocod(20)),INDEX idx3(Tcarconcod(20))) "
        . " select Fenfac, Fenhis, Fening, Pachis, Pactdo, Pacdoc, Inghis, Ingnin, Ingord, Ingfei, Inghin, Tcarhis, Tcaring, Tcarprocod, Tcarpronom, Tcarcan, Tcarvun, Tcarvto, Tcarconcod, Tcarfec, clisur_000106.hora_data as hora_data  "
        . " from temp18, clisur_000100, clisur_000101, clisur_000106, clisur_000066 "
        . " where Fenhis = Pachis "
        . "   and Fenhis = Inghis "
        . "   and Fening = Ingnin "
        . "   and Fenhis = Tcarhis "
		...
		if($num > 0)
		{
			...
			while($row = mysql_fetch_row($err))
			{
				...
				if($pos != -1)
				{
					for ($j=3;$j<=$fac[$pos][1][0]+2;$j++)
					{
						//echo " <br>edb AC-Fac : ".$fac[$pos][0][0]." pos : ".$pos." j : ".$j." ".$fac[$pos][$j][0]." : ".$fac[$pos][$j][1]."<br>";
