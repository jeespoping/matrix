...
else
{
	...
	if(!isset($wcco))//centro de costos una vez ya se escogio la devolucion
	{
		...
		if(!isset($cco)) //nombre del select de centros de costo, se muestra primer pantalla de seleccion de los datos para la consulta
		{
			...
			echo "<input type='text' readonly='readonly' name='fecha1' value='".$fecha1."' class=tipo3 ><input type='button' name='envio3' value='...' onclick=".$cal."' size=10 maxlength=10></td></tr>";
			?>
	<script type="text/javascript">//<![CDATA[
	Zapatec.Calendar.setup({weekNumbers:false,showsTime:false,timeFormat:'12',electric:false,inputField:'fecha1',button:'envio3',ifFormat:'%Y-%m-%d',daFormat:'%Y/%m/%d'});
	//]]></script>
	<?php

			echo "<tr><td align=left bgcolor=".$AzulClar." ><font size=3 color='#FFFFFF'><b>FECHA FINAL: </b></font>";
			echo "&nbsp;&nbsp;&nbsp;<input type='text' readonly='readonly' name='fecha2' value='".$fecha2."' class=tipo3 ><input type='button' name='envio4' value='...' onclick=".$cal."' size=10 maxlength=10></td></tr>";
			?>
	<script type="text/javascript">//<![CDATA[
	Zapatec.Calendar.setup({weekNumbers:false,showsTime:false,timeFormat:'12',electric:false,inputField:'fecha2',button:'envio4',ifFormat:'%Y-%m-%d',daFormat:'%Y/%m/%d'});
	//]]></script>
	<?php
