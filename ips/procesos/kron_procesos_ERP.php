 ...
 * MODIFICACIONES
 *************************************************************************************************
 * 2018-08-02  Jerson trujillo:
	Se apaga la funcion recuperarTurnosDeUrgencias(), porque ya no tiene uso y ademas se demora mucho tiempo ejecutandose
 * 2017-06-30  Edwar Jaramillo:
    Modificaciones realizadas por Felipe que no habían sido versionadas, para guardar en cliame_106 los número de documento unix y linea por cada insumo.
...
function actualizarMedicamentos($wemp_pmla)
{
    ...
    /*$sqlppal = "SELECT Tcardoi,Tcarlin,Tcarfac , ".$wbasedato."_000106.id, Tcarprocod ,Fdeubi,Logdoc , Logpro , Fenfue
	 ...
	 $sqlppal = "SELECT Tcardoi,Tcarlin,Tcarfac , {$wbasedato}_000106.id, Tcarprocod ,Fdeubi,Logdoc , Logpro , Fenfue, Tcarhis,Tcarvex,Tcarvre
                  FROM {$wbasedato}_000106
                  LEFT JOIN  {$wbasedato_mov}_000158 ON ( Tcardoi = Logdoc AND Tcarlin = Loglin) ,
                             {$wbasedato_mov}_000003 , {$wbasedato_mov}_000002
                 WHERE  Tcardoi !=''
    ...
    if($clave !=0)
    {
        foreach($rows as $row)
        {
			...
			/*
			los valores que puede tomar 000003.Fdeubi son:
			* M: el regitro esta en Matrix, no ha pasado a itdro.
			* UI: esta en Unix, especificamente en itdro y esta en estado Inconsistente.
			* US: esta en Unix, especificamente en itdro y esta en estado Sin procesar.
			* UP: esta en Unix, especificamente en itdro y esta en estado Procesado.*
			*/
			
            // el proceso solo sigue si se encuentra en estado UP = procesado a unix , US = Unix sin procesar (esto es porque quedan muchos
            if ($estado =='UP' || $estado =='US' || $estado =='M')
            {
                    ...
                    if( $documentoppal !='' )
                    {
                            ...
                            if($validacion!='si')
                            {
                                ...
                                if($nodiferenteprocedimiento)
                                {
									...
									// actualizo tambien el valor exedente y el valor reconocido si es necesario
                                    
									
									if( $row['Tcarvex']>0 )
									{
										//echo "<br>historia : ".$row['Tcarhis'] ." Valor exedente". $row['Tcarvex']. " Valor reconocido". $row['Tcarvre'];
										$sqlupdate = " UPDATE FACARDET
                                                      SET cardetfac = '".$row['Tcarfac']."' ,
													      cardetvre =  '".$row['Tcarvre']."',
														  cardetvex =  '".$row['Tcarvex']."'
                            ...
                            else
                            {
                                ...
                                while($rowval = mysql_fetch_array($resval))
                                {
                                    ...
                                    if($diferenteprocedimiento)
                                    {
										...
										// if($row['Tcarhis']=='660429')
										// {
											//echo "<br>historia : ".$row['Tcarhis'];
											// echo "entro1";
...
function actualizarCargosIngresosInactivosUnix($conex, $conexUnix, $wemp_pmla, $hay_unix)
{
    ...
    $explode_campo_rango       = explode(":", $campo_rango_consulta_ActMed_ERP);
    $campo_fecha_consulta      = 'Tcarfec';
    // $rango_dias_modificar_ing_unix = 20;
    if(count($explode_campo_rango) == 2)
    {
        $campo_fecha_consulta      = trim($explode_campo_rango[0]);
        $rango_dias_modificar_ing_unix = trim($explode_campo_rango[1])*1;
    }
    ...
    if($permitir_historia_ingreso_inactivo == 'on')
    {
        ...
        if($validacionUnixCorrecto)
        {
            ...
            $sql = "SELECT  c106.Tcarhis, c106.Tcaring, c106.Tcariux AS wingreso_unx, c106.Tcardoi, c106.Tcarlin AS linea_insumo, c106.Tcarfac AS facturable_mx, c106.id AS id_106, c106.Tcarprocod,
                    ...
                    WHERE   c106.{$campo_fecha_consulta} > '{$fechaant}'
                            AND c106.Tcardoi <> ''
                            AND c106.Tcardev <> 'on'
                            AND c106.Tcarmiu = 'on'
