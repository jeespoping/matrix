<?php
	...
	************************************************/
	...
	else
	{
		...
		//Se consulta la sede o entidad Mavila :)
		...
		//$wactualiz="(Versión Abril 15 de 2006)";                     
		// Aca se coloca la ultima fecha de actualizacion de este programa \\
	 	// =*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=
	 	// *=*=*=*=*=*=*=*= \\
		...
		//INICIO DEL FORMUARIO DE LA PAGINA MAVILA :)
	...
	//Funcionalidad para Calculo del año bisiesto :)
	function bisiesto($year)
	{
		return(($year % 4 == 0 and $year % 100 != 0) or $year % 400 == 0);
	}

	//Funcionalidad para validar fecha :)
	...
	function consultarAliasAplicacion($conexion, $codigoInstitucion, $nombreAplicacion){
		$q = "   SELECT Detval FROM root_000051 WHERE Detemp = '".$codigoInstitucion."' AND Detapl = '".$nombreAplicacion."'";
		//echo 'Consulta de validacion: '.$q;
	...
	function trasladoautomatico(&$wexiste, $wdevol, $wbodega, $wconceptraslado, $wccogra, $wvaltar, $wno2, $waprovecha, $wconinv, $wpaquete, $wprocod, $wfecha, $hora, $wconmvto, $wcantidad, $wusuario, $whistoria, $wing )
	{
		...
		//Se adiciona variable global para almacenar número de autorización Mavila :)
		global $w_autorizacion;
	...
	function formasdepago($fk)
	{
		...
		echo "<table border='0'>";
			echo "<tr>";
				echo "<td align=center colspan=15 class=encabezadoTabla>";
				echo "<b>D E T A L L E &nbsp;&nbsp; D E L&nbsp;&nbsp; A B O N O</b>";
				echo "</td>";
		echo "</tr>";
		//Se recorre el resultado de la consulta Mavila :)
		for ($j=1;$j<=$fk;$j++)
		{
			...
			echo "</select></td>";
		      ...
		      echo "</select></td>";
		      //Inicio de la constrcción de los campos  a mostrar :)
		      echo "</tr>";
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		function validar_datos_a_grabar()
		{
			...
			global $wcodpaq;
			global $wpaquete;
			//Se adiciona variable global de la información de la empresa Mavila :)
			global $wemp_pmla;
			
			//Se adiciona variable global para el número de autorizaciones Mavila :)
			global $w_autorizacion;
			$wok="on";
		...
		//Verifico que el concepto si coincida con la descripcion en la interfaz
			if ($wok=="on")
			{
				...
				else
				{
					...
					?>
					<script>
						alert ("LA DESCRIPCION DEL CONCEPTO NO COINCIDE CON EL CODIGO DEL CONCEPTO, AL SELECCIONAR ACEPTAR SE MOSTRARA CORRECTAMENTE LA INFORMACION.");
					</script>
					<?php
				}
				...
				if($wconinv !="on")
				{
					...
					if(isset($wprocod)){
						...
						else
						{
							...
							?>
							<script>
								alert ("EL CODIGO DEL PROCEDIMIENTO NO CORRESPONDE CON LA DESCRIPCION, AL SELECCIONAR ACEPTAR SE MOSTRARA CORRECTAMENTE LA INFORMACION.");
							</script>
							<?php
						}
				...
				//la cual contiene la relacion codigo, nombre de articulo, ademas solo hace esta validacion si en el formulario se envia el codigo del articulo. Jonatan Lopez
					if(isset($wprocod)){
						...
						else
						{
							...
							?>
							<script>
								alert ("EL CODIGO DEL INSUMO NO CORRESPONDE CON LA DESCRIPCION; AL SELECCIONAR ACEPTAR SE MOSTRARA EL NOMBRE CORRECTO.");
							</script>
							<?php
						}
					}
	  ...
	  if($wok=="on")
	  {
	  	...
	  	//Se adiciona validación del campo de número de autorización Mavila :)
	  	$w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');
	  	$w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');
	  	//echo 'Se muestra varieble: '.$w_solicitar_autorizacion;
	  	//var_dump($conex);
	  	//Se realiza validación de la funcionalidad para solicitar el número de autorización Mavila :)
		if ($w_solicitar_autorizacion == 'on') {
			if($wok=="on"){
				if($w_autorizacion ==""){
					...
					?>
					<script>
					    alert ("DEBE INGRESAR NUMERO DE AUTORIZACION");
					</script>
					<?php
			    }
			}
			//}else{
			//	echo 'No realizao validación: '.$w_solicitar_autorizacion;
		}
	  //Se adiciona validación del campo de número de autorización Mavila :)
	...
	//FUNCION PARA MOSTRAR LOS ARTICULOS SELECCIONADOS PARA LA VENTA
	     function mostrar()
	     {
			...
			global $w_autorizacion;
	     	...
	     	{
	     			//Se modifica la consulta de los cargos para el paciente Mavila :)
			     	$q  = " SELECT tcarusu, tcarhis, tcaring, tcarfec, tcarsin, tcarres, tcarno1, ";
			     	$q .= " tcarno2, tcarap1, tcarap1,tcardoc, tcarser, ";
			     	$q .= " tcarconcod, tcarconnom, tcarprocod, tcarpronom, tcartercod, tcarternom, ";
			     	$q .= " tcarterpor, tcarcan, tcarvun, tcarvto, ";
			     	$q .= " tcarrec, tcarfac, tcarfec, id, tcartfa, tcarfex, tcarfre, tcarapr, tcardev ";
			     	//Se adiciona campo para consultar la autorzacion del cargo :)
			     	$q .= " ,tcaraut ";
			     	$q .= " FROM ".$wbasedato."_000106 "; //."  WHERE tcarusu = '".$wusuario."'"
			     	$q .= " WHERE tcarhis = '".$whistoria."'";
			     	$q .= " AND tcaring = '".$wing."'";  //."    AND tcarser = '".$wcco."'"
			     	$q .= " AND tcarest = 'on' ";
			     	$q .= " ORDER BY id desc ";
	     	}
	       	elseif($wcajadm=="on")   //Si el usuario es Administrativo o Supervisor muestro todos los cargos de la historia sin importar quien los grabo
	       	{
	       			...
	       			$q = " SELECT tcarusu, tcarhis, tcaring, tcarfec, tcarsin, tcarres, tcarno1, ";
			     	$q .= " tcarno2, tcarap1, tcarap1,tcardoc, tcarser, ";
			     	$q .= " tcarconcod, tcarconnom, tcarprocod, tcarpronom, tcartercod, ";
			     	$q .= " tcarternom, tcarterpor, tcarcan, tcarvun, tcarvto, ";
			     	$q .= " tcarrec, tcarfac, tcarfec, id, tcartfa, tcarfex, tcarfre, tcarapr, tcardev ";
			     	//Se adiciona campo para consultar la autorzacion del cargo :)
			     	$q .= " ,tcaraut ";
			     	$q .= " FROM ".$wbasedato."_000106 ";
			     	$q .= "  WHERE tcarhis = '".$whistoria."'";
			     	$q .= " AND tcaring = '".$wing."'";
			     	$q .= " AND tcarest = 'on' ";
			     	$q .= " ORDER BY id desc";
	       	...
	       	//echo $q;
		    //Se realiza consulta Mavila :)
	       	$res = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
	       	$num = mysql_num_rows($res);
	       	//Se verifica el resultado de la consulta Mavila :)
	       	if ($num > 0)
	       	{
	       	$wtotcta=0;
	       	$wtotpac=0;
	       	$wtotres=0;
	       	//Inicio de nueva tabla :)
	       	echo "</table>";
	       	echo "<center><table border='0'>";

	       	echo "<br>";
	        //echo "<tr><td colspan=14>&nbsp</td></tr>";
	        //Se consulta la validación de la solicitud del número de autorización  Mavila :)
	        $w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');

	        //echo "<tr><td align=center colspan=14 class=".$wcf2."><b>DETALLE DE LA CUENTA</b></td></tr>";
	        echo "<tr>";
	        	//Se adiciona validación ddel tamañano de la activiacion de la solicitud de el número de autorización Mavila :)
	        	if ($w_solicitar_autorizacion == 'on') {
					echo "<td align=center colspan=19 class=encabezadoTabla>";
				}else{
					echo "<td align=center colspan=18 class=encabezadoTabla>";
				}
	       		echo "<b>D E T A L L E &nbsp;&nbsp; D E &nbsp;&nbsp; L A &nbsp;&nbsp; C U E N T A</b></td>";
				echo "</tr>";
	       	...
	       	echo "<tr>";
		       	...
		       	//Se muestra el campo de la tabla siempre que se encuentre activa la funcionalidad para soliciar el numero de autorización  Mavila :)
		       	if ($w_solicitar_autorizacion == 'on') {
		       		echo "<th class=encabezadoTabla>Aut.</th>";
				}
		       	echo "<th class=encabezadoTabla>&nbsp;</th>";
	       	echo "</tr>";

	       	//Se recorre la consulta para mostrar los campos :)
	       	...
	       	for ($i=1;$i<=$num;$i++)
	       	{
				...
				// Consulto la tabla de permisos para definir que acciones puede ejecutar el usuario actual
				 ...
				 // Consulto si el recibo tiene cuadre de caja activo
	       		...
	       		//Se construyen los campos de la tabla de cargos :)
	       		echo "<tr>";
			    	...
			    	//Se adiciona campo para mostrar el número de autorización :)
			    	if ($w_solicitar_autorizacion == 'on') {
			    		echo "<td align=CENTER class='".$wcolor."'>".$row['tcaraut']."</td>"; 
			    	}
			     	$peranu = 'on';
			    ...
			    }//Fin del recorrido de la tabla  Mavila:)
			    ...
			    echo "<tr>";
			     	...
			     	//Se adiciiona campo del número de autorización Mavila :)
			     	if ($w_solicitar_autorizacion == 'on') {
				     	echo "<td align=RIGHT class=colorAzul5 colspan=1>";
			     			echo "<span class=textoMedio><b>&nbsp;</span></b>";
			     		echo "</td>";
				    }
			    echo "</tr>";
			   	...
			   	echo "<tr>";
			     	...
			     	echo "<td align=CENTER class=colorAzul5 colspan=1><span class=textoMedio><b>S</span></b></td>";
			     	echo "<td align=RIGHT class=colorAzul5 colspan=1><span class=textoMedio><b>&nbsp;</span></b></td>";
			     	//Se adiciona campo para cuando se muestra el numero de autorización Mavila :)
			     	if ($w_solicitar_autorizacion == 'on') {
				     	echo "<td align=RIGHT class=colorAzul5 colspan=1>";
			     		echo "<span class=textoMedio><b>&nbsp;</span></b>";
			     		echo "</td>";
	...
	********************************************************************************/
	   ...
	   //===========================================================================
	   {
		...
		// JULIO 13 DE 2016: Si existe la variable $wsucursal en la url, utilizara ese valor, sino utilizara el valor por defecto de la root_000051 para TrasladoAutomaticoenCargo.
		  ...
		  //==================================================================================================
	   	...
	   	//INICIO CONDICION PARA LOS MOVIMIENTOS DE BODEGA :)
	   	if( !$esTrasladoAutomatico or ( $esTrasladoAutomatico && $trasladoRealizado ) or (isset($wdevol) and $wdevol=="on") or (isset($waprovecha) or $waprovecha=="on") ){/*2020-01-09 esta condicion busca garantizar que no se hagan movimientos de venta para bodegas que requieren traslado automático si no hay saldo suficiente en estas y que el traslado automático realmente se haya realizado, por lo tanto para cualquier caso que no implique este comportamiento (aprovechamientos, devoluciones o ventas directas) se permite continuar normalmente.*/
			...
			//INICIO CONDICIONAL PARA GRABAR EN EL KARDEX  MAVILA :)
	        if((isset($wexiste) and ($wexiste >= $wcantidad) and ($wconser == "A" or $wconser == "H")) or ($wconinv!="on" and $wconser=="H")) //tipo de servicio es (P)os o (A)mbos y valido que la cantidad a grabar sea menor a la que existe en el kardex
	          {
			      ...
			      if(isset($wconinv) and $wconinv=="on" and (!isset($waprovecha) or $waprovecha=="off"))  //Si mueve inventarios y no es un aprovechamiento
			      {
					...
					if(($wcantidad*$wvaltar) > 0 or $wdevol=="on")
					{
			          ...
			          //SEPTIEMBRE 5  DE 2007:
						...
						//Se valida funcionalidad para cada entidad :))
						$w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');


						//Se modidifa consulta de para insertar cargo Mavila :)
						$q  =" INSERT INTO ".$wbasedato."_000106 ";
						$q .=" (Medico, Fecha_data , Hora_data, ";
						$q .=" tcarusu, tcarhis, tcaring, ";
						$q .=" tcarfec, tcarsin, tcarres, ";
						$q .=" tcarno1 ,tcarno2, tcarap1, tcarap2, ";
						$q .=" tcardoc, tcarser, ";
						$q .=" tcarconcod, tcarconnom, ";
						...
						if ($w_solicitar_autorizacion == 'on') {
							$q .=" tcardev, tcaraut, Seguridad) ";
						}else{
							$q .=" tcardev, Seguridad) ";
						}		
						$q .=" VALUES ";
						$q .=" ('".$wbasedato."','".$wfecha."' ,'".$hora."' ,";
						$q .=" '".$wusuario."','".$whistoria."','".$wing."', ";
						$q .=" '".$wfeccar."', '".$wser."','".$wcodemp."-".$wnomemp."', ";
						$q .=" '".$wno1."','".$wno2."' ,'".$wap1."','".$wap2."', ";
						$q .=" '".$wdoc."','".$wccogra."', ";
						$q .=" '".$wcodcon."','".$wnomcon."', ";
						$q .=" '".$wprocod."','".$wpronom."', ";
						$q .=" '".$wcodter."','".$wnomter."', '".$wporter."', ";
						$q .=" '".$wcantidad."','".$wvaltar."', ";
						$q .=" '".round($wcantidad*$wvaltar)."', ";
						$q .=" '".strtoupper($wrecexc)."', ";
						$q .=" '".strtoupper($wfacturable)."','".$wtipfac."', ";
						$q .=" 'on','".$wnromvto."','".$wconmvto."', '".$waprovecha."', ";
						if ($w_solicitar_autorizacion == 'on') {
							$q .=" '".$wdevol."', '".$w_autorizacion."', 'C-".$wusuario."')";
						}else{
							$q .=" '".$wdevol."', 'C-".$wusuario."')";
						}
						$res2 = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
					}
					//echo "Consulta 1".$q; //Mavila :))
				...
				$res2 = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
					...
					$wrecexc    ="";
					$wfacturable="";
					//Se limpia la variable adicionada Mavila :) 
					$w_autorizacion="";
		...
		//Inicio de la construcción de la tabla :)
		echo "<tr>";
	...
	if(isset($whistoria)) //Si ya fue digitado el documento del cliente
	{
	  ...
	  if($whistoria != "" and strpos($whistoria,"-")==0)  //Por si digitan la historia junto con el número de ingreso, no deje grabar
	  {
	  	...
	  	if(isset($wwing) and $wwing != "")
	  	{
	  		...
	  		$res1 = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
	  		$num1 = mysql_num_rows($res1);
	  		//Inicio el resultado de la consulta Mavila :)
	  		if($num1 > 0)
	  		{
	  			$row1 = mysql_fetch_array($res1);
	  			$whis=$row1[0];
	              ...
	              <?php
	        }//fin el resultado de la consulta :)
	      }
	      else
	      {
	      	$whis="";
	...
	if(isset($wing))
	{
		...
		$res = mysql_query($q,$conex) or die ("Error: ".mysql_errno()." - en el query: ".$q." - ".mysql_error());
		$row = mysql_fetch_array($res);
		//inicia del resultado de la consulta :)
		if($row[0] == 0)
		{
			?>
			<script>
	...
	//SERVICIO ACTUAL:
		...
		echo "</tr>";
		//Fin de la construccion de la tabla Mavila :)
	...
	// si existe parameto en root_51, y si es ultimo o penultimo ingreso
		...
		echo "<div ".$css.">";
		echo "<center>";
		//Inicio de la tabla para realizar el respectivo registro :)
		echo "<table border='0'>";
		echo "<tr>";
		echo "<td align=center colspan=15 class=encabezadoTabla>";
		echo "<b>I N G R E S O &nbsp&nbsp&nbsp D E &nbsp&nbsp&nbsp C A R G O S</b>";
		echo "</td>";
		echo "</tr>";
		...
		echo "<tr>";
			...
			$w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');
			if ($w_solicitar_autorizacion == 'on') {
				echo "<th colspan=1 class=encabezadoTabla>Autorizaci&oacuten:</th>";
			}
		echo "</tr>";
		...
		switch ($warctar)
		{
		 ...
		 case "000104":  //Tarifas de procedimientos y examenes
		 {
	        ...
	        if ($num == 0)
	        {
	        	?>
	        	<script>
	        		alert ("EL PROCEDIMIENTO O INSUMO NO EXISTE O NO TIENE ESTA TARIFA DEFINIDA o NO ESTA DEFINIDO PARA EL CONCEPTO SELECCIONADO o NO TIENE EXISTENCIA PARA EL CENTRO DE COSTOS SELECCIONADO o LA HOMOLAGACION ESTA MAL HECHA (Codigo definido como del proveedor)");
	        		function ira(){document.cargos.wprocod.focus();}
	        	</script>
	        	<?php
	        }
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
	       		...
	       		//Se adiciona funcionadlidad para adicionar el campo del número de autorización Mavila :) 
	       		$w_solicitar_autorizacion = consultarAliasAplicacion($conex, $wemp_pmla, 'solicitarAutorizacionRegistroCargos');
						if ($w_solicitar_autorizacion == 'on') {
							if (!isset($w_autorizacion) or $w_autorizacion=="" ){
				       			echo "<td align=center class=".$wcf.">";
				       				echo "<INPUT TYPE='text' NAME='w_autorizacion' id='w_autorizacion' ";
				       				echo " VALUE='' placeholder=''>";
				       			echo "</td>";
				       		}else{
				       			echo "<td align=center class=".$wcf.">";
				       				echo "<INPUT TYPE='text' NAME='w_autorizacion' id='w_autorizacion' ";
				       				echo " VALUE='".($w_autorizacion)."' placeholder=''>";
				       			echo "</td>";
				       		}
				       	}
	       		//Fin de funcionalidad de campo de número de autorización :)
	       		echo "</tr>";
				...
				echo "<tr>";
				echo "<td class=".$wcf2." colspan=15 align=center>";
					echo "<b>Grabar</b>";
					echo "<input type='radio' name='wgrabar' SIZE=2>";
				echo "</td>";
				echo "</tr>";           //wcons
				...
				echo "</td>";
				echo "</tr>";
				//FIN DE LA TABLA PARA EL REGISTRO DE DATOS :)
			echo "</table>";
			echo "</div>";
	//------------------> div nuevos cargos
	  ...
	  if ($wconmca=="on")   //Indica que es un abono y que mueve caja
	  {
		  ...
		  if ($wtiene_consecutivo == "on")
		  {
		      ...
		      //ACA SE VALIDA QUE SE HALLAN DIGITADO TODOS LOS DATOS NECESARIOS PARA LA FORMA DE PAGO SELECCIONADA
		         ...
		         { ////
		           ...
		           //Se valida funcionalidad para cada entidad :))
						...
						//Se adiciona validacion del nombre del campo de tcarca a tcarcan Mavila :)   
		           		...
		           		$q .=" VALUES ('".$wbasedato."','".$wfecha."' ,'".$hora."' ";
	           			$q .=" ,'".$wusuario."' ,'".$whistoria."' ,'".$wing."' ";
	           			$q .=" ,'".$wfeccar."','".$wser."','".$wcodemp."-".$wnomemp."' ";
	           			$q .=" ,'".$wno1."','".$wno2."' ,'".$wap1."','".$wap2."' ";
	           			$q .=" ,'".$wdoc."','".$wccogra."','".$wcodcon."','".$wnomcon."' ";
	           			$q .=" ,'".$wprocod."','".$wpronom."' ";
	           			$q .=" ,'".$wcodter."','".$wnomter."','".$wporter."' ";
	           			$q .=" ,'".$wcantidad."','".$wvaltar."' ";
	           			$q .=" ,'".round($wcantidad*$wvaltar)."' ";
	           			$q .=" ,'".$wrecexcfpa."','".$wfacturable."','".$wtipfac."' ";
	           			$q .=" ,'on' ,'".$wnromvto."' ,'".$wconmvto."' ,'".$waprovecha."' ";
	           			//Se adiciona validacion de la consulta del numero de autorizacion :)
	           			if ($w_solicitar_autorizacion == 'on') { 
	           				$q .=" ,'".$w_autorizacion."' ,'C-".$wusuario."')";
	           			}else{
	           				$q .=" ,'C-".$wusuario."')";
	           			}
	           			//echo "Consulta 2".$q;
			    ...
			    $wtotfpa    =0;
			    $wconabo    ="off";
			    //Se adiciona campo de numero de autorizacion :)
			    $w_autorizacion="";
			    echo "<input type='HIDDEN' name='wcodcon' value='".$wcodcon."'>";
			    echo "<input type='HIDDEN' name='wnomcon' value='".$wnomcon."'>";
			    echo "<input type='HIDDEN' name='wccogra' value='".$wccogra."'>";
			    ...
			    unset($wfacturable);
			    unset($wgrabar);
			    //Se adiciona variable para el campo de numero de autorizacion :)
			    unset($w_autorizacion);
			    ?>
			    <script>
			    	enter();
		      ...
		      //ACA SE VALIDA QUE SE HALLAN DIGITADO TODOS LOS DATOS NECESARIOS PARA LA FORMA DE PAGO SELECCIONADA
				...
				if ($obliga[1]=="on" and $wtotfpa > 0)
	              ...
	              */
			      ...
			      $wtotfpa    =0;
			      $wconabo    ="off";
			      //Se adiciona campo de número de autorización :)
			      $w_autorizacion="";
			      echo "<input type='HIDDEN' name='wcodcon' value='".$wcodcon."'>";
			      echo "<input type='HIDDEN' name='wnomcon' value='".$wnomcon."'>";
			      echo "<input type='HIDDEN' name='wccogra' value='".$wccogra."'>";
