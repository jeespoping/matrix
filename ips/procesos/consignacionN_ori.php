...
function grabarMovimientos($fuenteT, $consecutivoT, $numero, $fuente, $cantidad, $destino, $id, $valor, $caja, $wusuario, $pago)
{
	...
	else if ($valor>$cantidad)
	{
		...
		$q= " INSERT INTO ".$wbasedato."_000022 (   Medico       ,   Fecha_data,           Hora_data,                    rfpfue              ,   rfpnum    ,                rfpfpa              ,  rfpvfp        ,   rfpdan         ,   rfpobs     , rfpest,   rfpcco  ,    rfppla,           rfpaut,       rfpecu,   rfpcaf,            rfpban,      Seguridad        ) "
		."           Consignar                  VALUES ('".$wbasedato."','".date('Y-m-d')."', '".(string)date("H:i:s")."' ,'".$fuenteT."',          ".$consecutivoT.",        '".$pago."'         ,       ".$valor.",     '".$id."',      '',  'on'  ,  '".$wcco."', '', '',      'N',   '".$caja."',    '', 'C-".$wusuario."') ";
...
function consultarRegistros ($cco, $fuenteT, $consulta, &$fuente, &$numero, &$fecha, &$valor, &$documento, &$banco, &$pagos, &$total, &$id, &$cantidad)
{
	...
	for ($i=0;$i<$num;$i++)
	{
		...
		$err4 = mysql_query($q,$conex) or die( mysql_errno()." - Error en el query $q - ".mysql_error() );
		...
		$err3 = mysql_query($q,$conex) or die( mysql_errno()." - Error en el query $q - ".mysql_error() );
...
function incrementarConsecutivo($fuenteT)
{
	...
	$errlock = mysql_query($q,$conex) or die( mysql_errno()." - Error en el query $q - ".mysql_error() );
...
function calcularTotal($pagos, $fuente, $radio, $valor, $radio2 , &$total, &$totalF, &$radio, &$cantidad)
{
	$totalF=0;
...
function pintarAlert5Consignar($consecutivoT)
{
	global $wbasedato;
	echo "</table>";
	echo"<form action='consignacionN.php' method='post' name='forma1' ><CENTER><fieldset style='border:solid;border-color:#000080; width=330' ; color=#000080>";
...
function pintarFormulario($fuente, $numero, $fecha, $valor, $documento, $banco, $radio, $pagos, $total, $totalF, $cajCod, $cajDes, $consecutivoT, $clase, $id, $fuenteT, $radio2, $cantidad, $con)
{
	...
	for($i=0; $i < count($pagos['codigo']); $i++)
	{
		...
		if (isset ($fuente[$i][0]))
		{
			...
			{
				if ($pagos['tipo'][$i]=='on')
				echo "<tr><td class='titulo2' colspan='8'><input type='checkbox' name='radio2[".$i."]' ".$radio2[$i]." onclick='enter6( this )'>&nbsp;&nbsp;FORMA DE PAGO: ".$pagos['codigo'][$i]."-".$pagos['nombre'][$i]."</td></tr>";
				else
				echo "<tr><td class='titulo2' colspan='6'><input type='checkbox' name='radio2[".$i."]' ".$radio2[$i]." onclick='enter6( this )'>&nbsp;&nbsp;FORMA DE PAGO: ".$pagos['codigo'][$i]."-".$pagos['nombre'][$i]."</td></tr>";
			}
			else
			{
				if ($pagos['tipo'][$i]=='on')
			...
			for($j=0; $j < count($fuente[$i]); $j++)
			{
				...
				echo "<td ".$clase1.">$ ".number_format((float)$valor[$i][$j],0,"",",")."</td>";
				...
				{
					echo "<td ".$clase1.">$".number_format($cantidad[$i][$j],",","",".")."</td>";
					echo "<input type='HIDDEN' name='cr".$i."][".$j."]' value='".$cantidad[$i][$j]."'>";
				}
				echo "</tr>";
			...
			echo "<td class='acumulado1' >$ ".number_format((float)$total[$i],0,"",",")."</td>";
			echo "</table><br>";
		}
...
function pintarBoton1($totalF, $bancos, $wbanF, $auto)
{
	...
	echo "<table align='center'>";
	echo "<tr><td class='titulo4' >VALOR A CONSIGNAR: $ ".number_format((float)$totalF,0,"",",")."</td></tr>";
	...
	if($cajCod != '')
	{
		...
		if (isset($consulta) and $consulta!='')// consulta de una cosignacion
		{
			...
			if ($wbanF!='')
			{
				...
				pintarFormulario($fuente, $numero, $fecha, $valor, $documento, $banco, '', $pagos, $total, $totalF, $cajCod, $cajDes, $consulta,2, $id, $fuenteT, $radio2, $cantidad, false );
			...
			if (!isset ($bandera1) or $bandera1!=3) //generacion de informacion para la consignacion
			{
				...
				if(!isset($consignar)) //proceso previo antes de guardar el translado
				{
					...
					if ($contador>0)
					{
						...
						calcularTotal($pagos, $fuente, $radio, $valor, $radio2, &$total, &$totalF, &$radio, &$cantidad);
