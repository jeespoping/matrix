...
//=====================================================================================================================================================================
	...
	function cargar_datos_caja()
	{
		...
		$data = array(	'wcco'=>'', 'wnomcco'=>'', 'wbod'=>'', 'wcaja'=>'', 'wnomcaj'=>'', 'wcajadm'=>'',
...
//=====================================================================================================================================================================
			...
			else
			{
				...
				else
				{
					...
					if(data.ingresoActivoUnix)
						$("#ingresoActivoUnix").css({"color":"green"}).html("<b>Activo</b>").attr("estado", "on");
					else	
						$("#ingresoActivoUnix").css({"color":"red"}).html("<b>Inactivo</b>").attr("estado", "off");
	...
	function tipo_de_cargo(tipo)
	{
		...
		switch(tipo)
		{
			...
			case 'trauma':
			{
				$("#contenedorTrauma").css({"border":"1px solid #000000", "padding":"2px", "background-color":"#FFFFCC"});
				$("#pedirVias").show(400);
				$("#tdVias1").show(0);
				pintarSelectVias();
		...
		$.post("<?=$URL_AUTOLLAMADO?>?"+url_add_params,
			{
				...
				{
				$("#div_abono").html(data);
				$.blockUI({ message: $('#div_abono' ),
							css: { left:( $(window).width() - 1500 )/2 +'px',
									top: '350px',
									width: '1500px'
								 }
						 });
	...
	function datos_desde_conceptoxcco()
	{
		...
		$.post("<?=$URL_AUTOLLAMADO?>?"+url_add_params,
		{
			...
			if(data.tieneInsumosPorAplicar == "on")
			{
				...
				$("#botonGrabar").tooltip({track: true, delay: 0, showURL: false, showBody: ' - ', opacity: 0.95, left: -50 });
			}
			else
			{
	...
	function abrirHce()
	{
		...
		{
			var url 	= "/matrix/HCE/procesos/HCE_Impresion.php?empresa=hce&wemp_pmla="+$("#wemp_pmla").val()+"&wcedula="+$("#wdoc").val()+"&wtipodoc="+$("#wtip_doc").val()+"&wdbmhos="+$("#wbasedato_movhos").val()+"&whis="+$("#whistoria").val()+"&wing="+$("#wing").val()+"&wservicio=*&protocolos=0&CLASE=I&BC=1";
	...
	function abrirBitacora()
	{
		...
		var url 	= "/matrix/movhos/procesos/rBitacora.php?ok=0&empresa=movhos&wemp_pmla="+$("#wemp_pmla").val()+"&whis="+whis+"&wnin="+wing;
		alto		= screen.availHeight;
		ventana 	= window.open('','','fullscreen=1,status=0,menubar=0,toolbar=0,location=0,directories=0,resizable=0,scrollbars=1,titlebar=0');
		ventana.document.open();
