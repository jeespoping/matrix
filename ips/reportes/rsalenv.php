 ...
 * 2019-11-01 Andres Alvarez: Se adicion drop tablespaces tmp
...
function consultarFacturasOptima($wfecini, $wfecfin, $wempCod, $wesf)
{
    ...
    if ($num>0)
    {
        ...
        // $q = "Drop tablespace tmp ENGINE = InnoDB";
        //     $rq=mysql_query($q, $conex) or die(mysql_error()."-".mysql_errno());
       ...
       $qtmp = "CREATE TEMPORARY TABLE IF NOT EXISTS tmp
                (INDEX idx(Rdefac, Carncr), INDEX idx2_tmp(Renfue, Rennum))
        ...
        */
        ...
        $qcaus="CREATE TABLE IF NOT EXISTS tmpcato
                ...
                FROM    ".$wbasedato."_000071 AS c71
                        INNER JOIN
                        tmp ON (tmp.Rennum = c71.Docnum AND tmp.Renfue = c71.Docfue)
                        INNER JOIN
        ...
        // Crear un array de la tabla temporal tmp
...
function consultarFacturas($wfecini, $wfecfin, $wempCod, $wesf)
{
    ...
    if ($num>0)
    {
        ...
        //fuentes para notas credito y notas debito
        $qtmp = "CREATE  TABLE IF NOT EXISTS tmp" //temporal que almacena los datos de los documentos que pertencen a las facturas que fueron filtradas por fecha
...
else
{
    ...
    echo "<form action='rsalenv.php' method=post name='forma'>";
    ...
    else
    {
        ...
        echo "<table  align=center width='60%'>";
        if( $plano != "on" )echo "<tr><td align=center><A href='rsalenv.php?wemp_pmla=".$wemp_pmla."&amp;wfecini=".$wfecini."&amp;wfecfin=".$wfecfin."&amp;wemp=".$wemp."&amp;bandera='1'><span style='font-size:12px;'>VOLVER</span></A></td></tr><tr align='center'><td><input type=button value='Cerrar' style='width:100' onclick='javascript:window.close();'></td></tr>";
        ...
        echo "<br><center><A href='rsalenv.php?wemp_pmla=".$wemp_pmla."&amp;wfecini=".$wfecini."&amp;wfecfin=".$wfecfin."&amp;wemp=".$wemp."&amp;bandera='1'><span style='font-size:12px;'>VOLVER</span></A></center>";
