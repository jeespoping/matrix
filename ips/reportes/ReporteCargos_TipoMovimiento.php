...
include_once("conex.php");
 /**********************************************************************************************************
 *
 * Programa				    :	ReporteCargos_TipoMovimiento.php
 * Fecha de Creación 	:	2017-03-07
 * Autor				      :	Arleyda Insignares Ceballos
 * Descripcion		    :	Reporte de Cargos x Tipo de Movimiento: Es un listado de todos los movimientos de
 *                 	    grabado, regrabado y anulación.                      
 *                      
 ***************************************   Modificaciones  *************************************************
 * 2017-05-02  - Arleyda Insignares C.  
 *               Se cambio campo centro de costos diseñado como autocompletar por un multiselect y de esta
 *               forma poder seleccionar varios centros de costos.
 * 2017-03-15  - Arleyda Insignares C.  
 *               1. Se adiciona campo 'Entidad'  
 *               2. Se centra la columna 'cantidad. 
 *               3. Se adiciona tipo de documento en la columna 'paciente' 
 *               4. Se adiciona Tipo de movimiento seleccionado en la primera columna del detalle.
 **********************************************************************************************************/               
 
 $wactualiz = "2017-05-02";
 ...
 header('Content-type: text/html;charset=ISO-8859-1');
  ...
  // *************************************    FUNCIONES AJAX  Y PHP  **********************************************
      ...
      if (isset($_POST["accion"]) && $_POST["accion"] == "ConsultarDetalle"){
        ...
        // Filtrar la tabla movhos_000107 con una tabla temporal y obtener mayor rendimiento en la consulta
          ...
          $qcon =  "  SELECT A.Fecha_data, A.Hora_data, A.Audhis, A.Auding, A.Audreg, A.Audusu, A.Audcau, A.Audjus,
                             B.Tcarcan, B.Tcarvun, B.Tcarvto, C.Caudes, U.Descripcion as Nomusuario,D.Pacdoc,D.Pactdo,
                             concat (D.Pacno1,' ',D.Pacno2,' ', D.Pacap1, ' ',D.Pacap2) as Nompac, B.Tcarres, E.Empnom
                      FROM T1 A
          ...
          if  ($numcon > 0)
          {
              ...
              { 
                 ...
                 $detalle .= "<tr class='".$fondo."' onclick='ilumina(this,\"".$fondo."\")'>
                             ...
                             <td >".utf8_encode($row["Audcau"])."</td>
                             <td >".utf8_encode($row["Caudes"])."</td>
                             <td >".utf8_encode($row["Audjus"])."</td>
                             </tr>";
  ...
  <html>
  <head>
    <title>Reporte de Cargos por Tipo de Movimiento</title>
    <meta charset="utf-8">
    ...
    <script type="text/javascript">
      ...
      function Consultar(){
          ...
          // Limpiar campos centro de costos y usuario
          ...
          $.post("ReporteCargos_TipoMovimiento.php",
               {
                consultaAjax:  true,
       ...
       function Exportar(){
            ...
            tmpElemento.download = 'reportecargos_tipomovimiento.xls';
    ...
    <body >
      <?php
        ...
        $wtitulo  = "REPORTE DE CARGOS POR TIPO DE MOVIMIENTO";
      ...
      <br>&nbsp;
      <center>
      <table width='1550px' id='tbldetalle' name='tbldetalle' class='tbldetalle' style='border: 1px solid blue;display:none;' >
      <thead>
        <tr class='encabezadotabla'>
          ...
          <th align='center' colspan='2'>CAUSA</th>
      ...
      <table width='1550px' id='tblexcel' name='tblexcel' class='tblexcel' style='border: 1px solid blue;display:none;' >
      <thead>
      <tr class='encabezadotabla'>
