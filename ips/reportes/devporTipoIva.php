...
else
{
	...
	echo "<form action='devporTipoIva.php' method=post>";
	...
	{
		/**********************************************Segunda página**************************/
		/***********************************REALIZO LOS AJUSTES PRELIMINARES********************/
		...
		echo "<tr><td align=center colspan=2 bgcolor=".$wcf2."><font size=6 text color=#FFFFFF><b>DEVOLUCIONES POR TIPO DE IVA</b></font></td></tr>";
		echo "</tr>";
		echo "</center></table >";
		...
		for ($k=0;$k<count($wccoe);$k++)
		{
			...
			if ($rango1==1)
			{
				...
				$q = " SELECT menfac, menfec, venffa, arttiv, artiva, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan), SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(Mdecan) "
				."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
				...
				."     AND Mdeart = artcod "
				."     AND Mencco = '".$wccoe[$k]."'"
				."   GROUP BY venffa, vennum, Menfec, arttiv, artiva "
			...
			if ($rango2==1)
			{
				...
				$q = " SELECT rdenum, menfac, menfec, venffa, arttiv, artiva, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan), SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(Mdecan) "
				."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
			...
			if ($contador>0)
			{
				...
				echo "<tr><th align=CENTER bgcolor=DDDDDD colspan='9'><font size=2>".$wccoe[$k]."-".$wccodes[$k]."</font></th></tr>";
				...
				for ($i=0;$i<$contador;$i++)
				{
					...
					if ($cantidad>=1)
					{
						...
						$wtotveniva = $wtotveniva + $vector[$i][7];
						$wtotventot = $wtotventot + $vector[$i][8];
					}
				}
		...
		echo "<tr>";
		echo "<td colspan=6 bgcolor=DDDDDD>TOTALES</td>";
		echo "<td align=right bgcolor=DDDDDD><font size=2>".number_format($wtotccobru,0,'.',',')."</font></td>";
		echo "<td align=right bgcolor=DDDDDD><font size=2>".number_format($wtotccoiva,0,'.',',')."</font></td>";
		echo "<td align=right bgcolor=DDDDDD><font size=2>".number_format($wtotccotot,0,'.',',')."</font></td>";
		echo "<tr>";
		echo "</table>";
		...
		for ($k=0;$k<count($wccoe);$k++)
		{
			...
			if ($rango1==1)
			{
				...
				$q = " SELECT tranum, Menfac, Menfec, venffa, arttiv, artiva, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan), SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)),vennfa"
				."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
				."   WHERE Menfec between '".$wfecini."'"
				."     AND '".$wfecfin."'"
				."     AND Menfac = vennum "
			...
			if ($rango2==1)
			{
				...
				$q = " SELECT rdenum, Menfac, Menfec, venffa, arttiv, artiva, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan), SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), vennfa"
				."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
				."   WHERE Menfec between '".$wfecini2."'"
				."     AND '".$wfecfin2."'"
				."     AND Menfac = vennum "
			...
			if ($contador>0)
			{
				...
				for ($i=0;$i<$contador;$i++)
				{
					...
					echo "<td><font size=2>".$vector[$i][3]."</font></td>";
					echo "<td><font size=2>".$vector[$i][9]."</font></td>";
					echo "<td><font size=2>".$vector[$i][2]."</font></td>";
					echo "<td><font size=2>".$vector[$i][4]."</font></td>";
					echo "<td><font size=2>".$vector[$i][5]."</font></td>";
					echo "<td align=right><font size=2>".number_format($vector[$i][6],0,'.',',')."</font></td>";
		...
		echo "<td align=right bgcolor=DDDDDD><font size=2>".number_format($wtotccoiva1,0,'.',',')."</font></td>";
		echo "<td align=right bgcolor=DDDDDD><font size=2>".number_format($wtotccotot1,0,'.',',')."</font></td>";
		echo "<tr>";
