<html>
<head>
<title>Reporte facturas por Entidad x Codigo
</title>
</head>
...
// ===========================================================================================================================================
// PROGRAMA				      :Reporte de Facturas por Entidad x Codigo                                              			 			 |
// AUTOR				      :Ing. Gabriel Agudelo           						                                                       	 |
// FECHA ACTUALIZACION        :Octubre 28 - 2019 				                                                                   			 |
// DESCRIPCION			      :Reporte que muestra las facturas realizadas por paciente y agrupadas por Codigo responsable
// TABLAS UTILIZADAS :                                                                                                                       |
// VARIABLE EMPRESA 000018	  :Tabla que contiene los campos de facturacion solicitados como fecha, fuente, numero de factura
//							   y datos del paciente
// VARIABLE EMPRESA 000024	  :Tabla que contiene el nit y el nombre del responsable
//                                                                                         													 |
// ==========================================================================================================================================
*/
$wactualiz = "2019-10-28";
...
echo "<form action='rep_SaldoFacturasxEntxCod.php?wemp_pmla=$wemp_pmla' method='post'>";
...
if( $mostrar == 'off' )				//si no hay rango de fechas entonces  pedirlos al usuario
{
	...
	$sql= " SELECT  Empcod,empnom "				//De la tabla del maestro de empresas traemos el nit y el nombre
	...
	else
	{
		...
		//Esta consulta me trae las facturas por paciente ordenadas por el nit del responsable, se realizan varias uniones para permitir que se listen
                //todas las facturas por fennit en la primera, fendpa en la segunda y fenres en la tercera.
			 	...
			 	$q = " SELECT * FROM (
                	SELECT    fenffa,fenfac,fenfec,fendpa,fennpa,fenval as valorfact,fencod,empnit,empnom,
                       ...
                       WHERE    A.fenfec BETWEEN '$fechaini'
                         AND    '$fechafin'
                         AND    A.fencod LIKE '".$wemp."'
                         AND    A.fencod = B.empcod
                         AND    B.empcod = B.empres
                         AND    (fenval + fenabo + fencmo + fencop + fendes) > 0
                         AND    A.fenest = 'on'
                   UNION ALL
                      SELECT    fenffa,fenfac,fenfec,fendpa,fennpa,fenval as valorfact,fencod,fendpa,empnom,
                                fenvnc,fendes,(fenval-fenvnc+fenvnd-fenrbo) as neto, fenvnd, fenrbo
                        FROM    ".$wbasedato."_000018 A, ".$wbasedato."_000024 B
                       WHERE    A.fenfec BETWEEN '$fechaini'
                         AND    '$fechafin'
                   ...
                   UNION ALL
                      SELECT    fenffa,fenfac,fenfec,fendpa,fennpa,fenval as valorfact,fencod,fendpa,empnom,
                       ...
                       WHERE    A.fenfec BETWEEN '$fechaini'
                         ...
                         AND    (fenval + fenabo + fencmo + fencop + fendes) > 0
                         AND    A.fenest = 'on' ) a
                    GROUP BY    fenffa,fenfac,fenfec,fendpa,fennpa,valorfact,fencod,fendpa,empnom,
		...
		for($i=0;$i<$num; )
		{
			...
			echo "<tr><td align='left' colspan='11' class='titulo'>" .$wfencod." - " .$wempnom. "</td></tr>";
			echo "<br>";
			echo "<tr class='encabezadoTabla'>";
			...
			while($wfencod==$row['fencod']) // mientras sea el mismo codigo, que me traiga la informacion de los pacientes
                        {								// que posee ese codigo
