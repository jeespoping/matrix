	...
	else
	{
		...
		for ($k=0;$k<count($wccoe);$k++)
		{
			...
			if ($rango2==1)
			{
				...
				$q = " SELECT rdenum, menfac, menfec, venffa, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), SUM(mdevto) "
			...
			if ($contador>0)
			{
				...
				echo "<tr><th align=CENTER bgcolor=DDDDDD colspan='6'><font size=2>".$wccoe[$k]."-".$wccodes[$k]."</font></th></tr>";
				...
				for ($i=0;$i<$contador;$i++)
				{
					...
					if ($cantidad>=1)
					{
						...
						$wtotvenbru = $wtotvenbru + $vector[$i][4];
						$wtotveniva = $wtotveniva + $vector[$i][5];
					}
				}
		...
		for ($k=0;$k<count($wccoe);$k++)
		{
			...
			if ($rango1==1)
			{
				...
				$q = " SELECT tranum, menfac, menfec, venffa,  SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((vdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), sum(mdevto), vennfa "
				."    FROM $table, ".$wbasedato."_000016, ".$wbasedato."_000017 "
				."   WHERE Menfec between '".$wfecini."'"
				."     AND '".$wfecfin."'"
				."     AND Menfac = vennum "
			...
			if ($rango2==1)
			{
				...
				$q = " SELECT rdenum, Menfac, Menfec, venffa, SUM(ROUND((Mdecan*vdevun/(1+(vdepiv/100))),0)-vdedes*Mdecan/vdecan) + SUM(ROUND((((Mdecan*vdevun/(1+(vdepiv/100)))-vdedes*Mdecan/vdecan)*(vdepiv/100)),0)), sum(mdevto), vennfa"
				."    FROM $table, ".$wbasedato."_000001, ".$wbasedato."_000016, ".$wbasedato."_000017 "
				."   WHERE Menfec between '".$wfecini2."'"
				."     AND '".$wfecfin2."'"
				."     AND Menfac = vennum "
