...
*/ $actualiz = "Noviembre 20 de 2017"; /*
 ACTUALIZACIONES:
 *
 =================================================================================
 20 noviembre 2017 Jonatan: Se agrega el campo Logarc para controlar si el incremento es por archivo plano, ademas se reorganizan los campos en la exportacion
...
if(isset($operacion) && $operacion == 'ver_detalle'){
	...
	$datamensaje['html_titulo_detalle'] .= "<div align=left>".$num_art." Articulos</div>";
	...
	$datamensaje['html_titulo_detalle_excel'] .= "<thead><tr><th>Codigo</th><th>Tarifa</th><th>Nombre_generico</th><th>Nombre_comercial</th><th>UN</th><th>Grupo</th><th>POS</th><th>CUM</th><th>Registro_Invima</th><th>Precio_regulado</th><th>Tarifa_con_incremento</th><th>Clasificacion</th><th>Fecha_codificacion</th></tr></thead><tbody>";
	$j = 0;
	while($row = mysql_fetch_array($res)){
		...
		$datamensaje['html_detalle_datos_excel'] .= "<tr><td>".$row['Logcodart']."</td><td>".$row['Logtarifa']."</td><td>".utf8_encode($array_articulos[$row['Logcodart']]['Artgen'])."</td><td>".utf8_encode($array_articulos[$row['Logcodart']]['Artcom'])."</td><td>".$array_articulos[$row['Logcodart']]['Artuni']."</td><td>".$grupo."</td><td>".$artpos."</td><td>".$array_articulos[$row['Logcodart']]['Artcum']."</td><td>".$array_articulos[$row['Logcodart']]['Artreg']."</td><td>".$regulado."</td><td align=right>".number_format($row['Lognewval'], 2)."</td><td>".$clase_articulo."</td><td>".consultar_fecha_codif($row['Logcodart'])."</td></tr>";
		$j++;
...
function traer_listado_log(){
	...
	$respuesta['html'] .=  "<tr class=encabezadoTabla><td colspan='24' align=center>LISTA DE INCREMENTOS REALIZADOS</td></tr>";
	$respuesta['html'] .=  "<tr class=encabezadoTabla><td>Fecha y Hora</td><td>Nit Entidad</td><td>Fecha inicio incremento</td><td>Grupo Articulos Inicial</td><td>Grupo Articulos Final</td><td>Grupos Excluidos</td><td>Grupos Incluidos</td><td>Código Inicial</td><td>Código final</td><td>Codigos Excluidos</td><td>Codigos Incluidos</td><td>Regulados</td><td>Pareto</td><td>POS</td><td>Tipo</td><td>Articulo IDC</td><td>Tarifa</td><td>Redondear</td><td>% Incremento</td><td>Valor(Tarifa fijada)</td><td>Valor adicional</td><td>Basado en costo</td><th>Archivo Plano</th><td>Observaciones</td></tr>";
            ...
            var dataSource = shield.DataSource.create({
                ...
                schema: {
                    ...
                    fields: {
                        ...
                        Tarifa_con_incremento: { type: String },                                              
                            ...
                            rows: [
                                ...
                                return {
                                    cells: [
                                        ...
                                        { type: String, value: item.Tarifa_con_incremento },
...
echo "
	<div class='ui-tabs ui-widget ui-widget-content ui-corner-all' style='width:1650px;'>
