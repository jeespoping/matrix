...
//==========================================================================================================================================\\
//==========================================================================================================================================\\
//     Programa que permite descargar el archivo .txt que contiene la información requerida para el SISMED
...
//2019-10-25 camilo zapata: Debido a cambios en la configuración de los datos en unix( codigo cum ), estaba llegando incompleto en términos del tamaño,
//							por lo tanto un codigo cum de unix no coincidia con los codigos disponibles en matrix(cliame_000244),
//							generando omisión de los coodigos a la hora de reportarse, por lo tanto se crea una función(completarCodigo()) que completa el tamaño del codigo
...
//                          alguno no cumpla con dicha condición se notifica al usuario en pantalla el/los códigos a corregir.
//2016-01-08 Juan C Hdez: Se modifica el query que hace relación de los (000106) con las facturas (000066) para que solo traiga los registros
...
//2013-01-22 camilo Zapata: se modifico el script para que genere el reporte con los cums sin ceros a la izquierda de tal manera que se mejore
//							la consolidación y se disminuyan los errores de registros repetidos causados por esto: 000198765-02 y 00198765-02
...
//2012-10-19 camilo Zapata: se modificó el script para que consulte en root_000051 los conceptos correspondientes a los distintos movimientos
...
//2013-01-04 camilo Zapata: se agregó al script la posibilidad de consultar en la tabla de cargos para las ventas de clisur.
//DESCRIPCION                                                                                                          						\\
//=========================================================================================================================================\\
//=========================================================================================================================================\\
 if(isset($ajaxdes))
 ...
 }else
 {
	...
	function imprimirDatos($datos, $tipo, $ano, $mesI, $mesF, $origen, $archivoContenedor=""){//funcion que agrega cada registro al archivo.
		...
		if( $archivoContenedor == "" ){
			$nombre_archivo = "MED".$tipo."_".$ano."_".$mesI."_".$mesF.".txt"; //Sm(SisMed)(c o v compras o ventas) año y mes.
			switch($tipo)
			{
				case 'v':
...
/***************************************************************INICIO FUNCIONES PARA LAS COMPRAS************************************************************************/
	 ...
	 function generarInformeCompras($concepto, $consultarUnix, $consultarCargos, $origenReporte ) //Esta funcion consulta las compras en matrix y en caso de ser necesario invoca la función que realiza lo mismo en servinte
	 {
		global $wbasedatos;
		global $conex;
		global $wemp_pmla;
		...
		//acá se consultan los conceptos de las compras
		$query = "SELECT Detval
					FROM root_000051
	...
	/*	$query = "SELECT Detval
		...
		//creamos la tabla temporal para las tablas de encabezado y detalle de inventario filtrada por año, fecha, concepto y estado.
		...
		while($reg = mysql_fetch_array($rs))
		{
			...
			//el siguiente if pregunta si los valores máximos y mínimos son iguales para evitar hacer una consulta innecesaria
	 ...
	 function comprasServinte($ano, $wmesI, $wmesF) //esta funcion consulta las compras en unix y las retorna a la funcion generarInformeCompras para que consolide
	 {
		...
		$id=date('s');
		$comprasUnix=array();
		//acá se consultan las conceptos de las compras
		$query = "SELECT Detval
					FROM root_000051
				   WHERE Detemp = '{$wemp_pmla}'
			...
			while($row=odbc_fetch_row($resFac))
			{
				...
				if( Is_Numeric(  $cum[0] ) ){//2019-10-23
					...
					if( !in_array( "'".$cum[0]."'", $articulosSinCum )   ){
						$error   = 1;
						$mensaje .= "<br><span  class='subtituloPagina2'>El artículo: ".odbc_result($resFac,'art')." NO tiene codigo cum asociado en servinte en Compras</span>";
						continue;
					}
				}
				...
				if(array_key_exists($cum1, $cums))
				{
					if(array_key_exists($articulo, $cums[$cum1]))
					{
						...
						if(!isset($comprasUnix[$mes][$cum]['unidades']) or ($comprasUnix[$mes][$cum]['unidades']=='') )//si está vacio se asigna
						{
							$comprasUnix[$mes][$cum]['unidades'] = ($canArticulos*($cums[$cum1][$articulo]['factor'])*1);
							$comprasUnix[$mes][$cum]['total'] = $valorTotal*1;
						}else
							{//si no está vacio se acumula.
								$comprasUnix[$mes][$cum]['total'] += $valorTotal*1;
								$comprasUnix[$mes][$cum]['unidades'] += ($canArticulos*($cums[$cum1][$articulo]['factor'])*1);
							}

						//MANEJO DE MÍNIMOS Y MÁXIMOS,
	 ...
	 function consolidarCompras($comprasServinte, $comprasMatrix, $ano, $wmesI, $wmesF, $origenReporte )//funcion que consolida los informes de compras de matrix y unix en el archivo
	 {
		...
		//ACÁ SE GUARDAN EN EL PARCIAL LAS COMPRAS TRAIDAS DE UNIX.
		...
		foreach($parcial as $keyMes=>$comprasCums)
		{
			foreach($comprasCums as $keyCum=>$datos)
			{
				if($datos['total']>0 and $datos['unidades']>0)
				{
					...
					//acá se va a verificar que las cantidades sean positivas en caso de que esto no se dé, no se muestra en el reporte.
...
/*********************************************************************************************************************************************************************/
	 ...
	 function generarInformeVentas($concepto, $consultarUnix, $consultarCargos, $origenReporte )
	 {
				...
				$qtemp = "CREATE TEMPORARY TABLE IF NOT  EXISTS $tmventas "
						...
						."SELECT Tcarprocod Vdeart,  SUBSTRING( tcarfec,6,2 ) Venmes, Tcarcan Vdecan, ABS(Tcarvun) Vdevun, Rcfffa Venffa, Rcffac Vennfa, SUBSTRING( tcarfec,1,4 ) Venano, 1 fac
						   ...
						   WHERE Tcarfec BETWEEN '".$fechaInicial."' AND '".$fechaFinal."'
							 ...
							 AND Rcfreg = a.id
							 AND Rcfest = 'on' ";         //Modificación 2016-01-08
						/*." UNION ALL "
						."SELECT Tcarprocod Vdeart,  SUBSTRING( tcarfec,6,2 ) Venmes, Tcarcan Vdecan, ABS(Tcarvun) Vdevun, Rcfffa Venffa, Rcffac Vennfa, SUBSTRING( tcarfec,1,4 ) Venano, -1 fac
		...
		//cremos una tabla temporal para buscar las facturas, que evidencien el máximo y el mínimo.
		$tmMinMax = "tmpMinMax";
		$qaux = "DROP TABLE IF EXISTS $tmMinMax";
		$resdr = mysql_query($qaux,$conex) or die (mysql_errno().":".mysql_error());
		$qtemp = "CREATE TEMPORARY TABLE IF NOT EXISTS $tmMinMax"
	 ...
	 //función que consulta las ventas en servinte.
	 function ventasServinte($ano, $wmesI, $wmesF )
	 {
		...
		$id=date('s');
		$ventasUnix=array();
		//acá se consultan los conceptos de las ventas  para unix
		$query = "SELECT Detval
					FROM root_000051
				   WHERE Detemp = '{$wemp_pmla}'
		...
		while($row=odbc_fetch_row($resFac))
		{
			...
			if( Is_Numeric(  $cum[0] ) ){//2019-10-23
				...
				if( !in_array( "'".$cum[0]."'", $articulosSinCum )   ){
					$error    = 1;
					$mensaje .= " <br><span  class='subtituloPagina2'>El artículo: ".odbc_result($resFac,'art')." NO tiene codigo cum asociado en servinte en ventas</span>";
					continue;
				}
			}
			//$cum1 = completarCodigo($cum[0])."-".completarSufijo($cum[1]);
			...
			if(array_key_exists($cum1, $cums))
			{
				...
				//SI ES LA PRIMER QUE ENTRA A ESTE CUM Y NO HAY VALORES MÍNIMOS
				...
				}else//SI YA HAY ALGO EN LOS PRECIOS MÍNIMOS SE DEBE COMPARAR
					{
						//-----echo "<br>entró: ".$cum."-codigo-".$articulo." antes: ".$anterior."-".$canant." despues:".$nuevo."-".$cannue;
				...
				//SI ES LA PRIMER QUE ENTRA A ESTE CUM Y NO HAY VALORES MÁXIMOS
				...
				}else//SI YA HAY ALGO EN LOS PRECIOS MÁXIMOS SE DEBE COMPARAR
					{
		...
		/*foreach( $articulosProbarCum as $key=>$codigoCum ){
			...
			echo "<pre>".print_r( $ventasUnix[1][$codigoCum], true )."</pre>";
		}*/
...
/*********************************************************INICIO FUNCIONES REPORTE CONSOLIDADO (NUEVA NORMATIVIDAD)******************************************************/
	...
	function crearArrayCums(){
		...
		global $cums;
		global $articulosSinCum;
		$q64 = "SELECT Cumcod, Cumint, Cumequ FROM {$wbasedatos}_000244";
...
//********************************************************************FIN FUNCIONES***********************************************************************************/
			...
			if(wfecI>wfecF)
			{
			...
			}else
			 {
				...
				try
				{
					...
					var ajax = nuevoAjax();
					ajax.open("POST", "rep_Sismed.php",true);
					...
					ajax.onreadystatechange=function()
					{
						if (ajax.readyState==4)
						{
							...
							if( error*1 == 0 ){
								...
								var href="rep_Sismed.php?ajaxdes=Sismed&wdesc="+nombreArchivo+"&wemp_pmla="+wemp_pmla;
	...
	<?php
		...
		//**************************************************************FUNCIONES QUE SE EJECUTAN PARA ARMAR EL MENÚ PRINCIPAL***************************************************************************************//
		 ...
		 function rangoFechas()
		 {
				...
				echo "AÑO: <select id='wano' name='wano'>";
				for($i=$año; $i>1999; $i--)
				{
		 ...
		 else
		 {
			 if(!isset($warchi))
			 {
				...
				encabezado("GENERACIÓN DE NUEVO INFORME SISMED",$wactualiz, "clinica");
				$institucion = consultarInstitucionPorCodigo($conex, $wemp_pmla);
				$wbasedatos = $institucion->baseDeDatos;

				 echo "<form name='rep_Sismed' action='rep_Sismed.php?wemp_pmla=".$wemp_pmla."' method=post>";
