...
//Funcion que consulta la fecha y hora de la factura asociada al accidente, esa fecha y hora es el igreso y 2 horas despues es el egreso.
function consultar_datos_ing_egr($num_factura){

	global $conex;
	global $wbasedato_farm;

	$datos_array = array('fecha_ing'=>'','hora_ing'=>'','fecha_egr'=>'', 'hora_egr'=>'');

	$q = "SELECT b.Fecha_data as fecha_ing, b.Hora_data as hora_ing
		    FROM ".$wbasedato_farm."_000016 as a, ".$wbasedato_farm."_000019 as b
		   WHERE Vennum = Fdenve
		     AND Fdefac = '".$num_factura."';";
	$res = mysql_query($q, $conex) or die ("Error: " . mysql_errno() . " - en el query: " . $q . " - " . mysql_error());
	$row = mysql_fetch_array($res);

	$fecha_ing = $row['fecha_ing'];
	$hora_ing = $row['hora_ing'];

	$fecha_aux = $fecha_ing." ".$hora_ing;

	$fecha_egreso = strtotime ( '+2 hour' , strtotime ( $fecha_aux) ) ;
	$fecha_egreso = date ( 'Y-m-d H:i:s' , $fecha_egreso );

	$datos_fecha_egr = explode(" ", $fecha_egreso); //Separo la fecha de la hora.

	$datos_array = array('fecha_ing'=>$fecha_ing,'hora_ing'=>$hora_ing, 'fecha_egr'=>$datos_fecha_egr[0], 'hora_egr'=>$datos_fecha_egr[1]);
...
{
	$lafecha = "";
	//Valida la variable $fecha para que no muestre // si esta vacio. //Jonatan Lopez 24 enero 2017
	if(trim($fecha) != ''){
...
function condicion_victima($condicion)
{
	$valor = '0';
	if(trim($condicion)=='C') $valor = '1';
	if(trim($condicion)=='P') $valor = '2';
	if(trim($condicion)=='O') $valor = '3';
	if(trim($condicion)=='I') $valor = '4';
	return $valor;
}
...
{
	$valor = '';
	if(ereg('PARTICULAR',$tipo)) $valor = '3';
	if(ereg('PUBLICO',$tipo)) $valor = '4';
	if(ereg('OFICIAL',$tipo)) $valor = '5';
	if(ereg('EMERGENCIA',$tipo)) $valor = '6';
	if(ereg('DIPLOMATICO',$tipo)) $valor = '7';
	if(ereg('TRANSPORTE',$tipo)) $valor = '8';
	if(ereg('ESCOLAR',$tipo)) $valor = '9';
	return $valor;
}
function cambio_boleano($dato)
{
	$valor = '';
	if($dato=='S') $valor = '1';
	if($dato=='N') $valor = '0';
...
function formato_hora($hora)
{
	...
	{
		$datos = explode(":",$hora);
		$hora = num2dec($datos[0]);
		$minuto = num2dec($datos[1]);
		$horadec = $hora.":".$minuto;
	}
	else
	{
...
<?php
   ...
   ******************************************************************/
	 ...
	 * Modificado:
	 * 2017-01-24 - Se repara la generacion del archivo plano en cuento a: Condiciones del accidentado, estado asegurado, intervencion de la autoridad,
	 * fecha de ingreso y egreso, vigencia desde-hasta, cambios solicitados por cristian. Jonatan Lopez
	 ...
	 * 2012-10-31 - Se adicionó la tabla $wbasedato_farm_000020 en la Consulta de la lista de facturas del documento de modo que
	 * que se valide por responsable las facturas consultadas y no se muestren dobles las facturas que estan repetidas en la tabla
	 * $wbasedato_farm_000018 - Mario Cadavid
	 ...
	 * 2012-01-24 - Se cambió la carpeta de generación de planos, de ../../planos/FURIPS/farpmla/ a ../../IPS/FURIPS/farpmla/
	 ...
	 * 2012-01-03 - Se permite modificar el campo de fecha de ingreso del paciente en la cuenta de cobro
	 * petición realizada por Sandra de farmastore - Mario Cadavid
	 ...
	 * 2011-11-24 - El nombre de la aseguradora estatal estaba quemado por lo que
...
else	// Si el usuario está registrado inicia el programa
{
  ...
  $wactualiz = " Septiembre 24 de 2013";
...
// Se consulta el código de la fuente y si es glosa o no
$qfue ="   SELECT Carfue, Carglo "
	  ."     FROM ".$wbasedato_farm."_000040 "
	  ."  	WHERE Carfue = '".$fuente."' "
	  ."	  AND Carest = 'on' ";
$resfue = mysql_query($qfue,$conex) or die (mysql_errno()." - ".mysql_error());
$rowfue = mysql_fetch_array($resfue);

// Consulto la lista de facturas del documento
$q =   "   SELECT Fendpa, Fenfac, Fennac, Fenfec, b.id idfactura "
	  ."     FROM ".$wbasedato_farm."_000021, ".$wbasedato_farm."_000020, ".$wbasedato_farm."_000018 b "
	  ."  	WHERE Renfue = '".$fuente."' "
	  ."	  AND Rennum = '".$dcto."' "
	  ."	  AND Renfue = Rdefue "
	  ."	  AND Rennum = Rdenum "
  ...
  $total_folios = mysql_num_rows($res);
	...
	if($tbpac!="")
	{
		...
		if(!$catastrofico)
		{
			...
			if (odbc_fetch_row($err_o))
			{
				...
				$nitAsegura=str_replace("-","",$array['accdetres']);
		...
		*/
		...
		else
		{
			...
			if ($err_o)
			{
				...
				$datos_ingreso_egreso = consultar_datos_ing_egr($row_fact['Fenfac']);

				$fechaing = $datos_ingreso_egreso['fecha_ing'];
				$fechaegr = $datos_ingreso_egreso['fecha_egr'];
				$horaing = $datos_ingreso_egreso['hora_ing'];
				$horaegr = $datos_ingreso_egreso['hora_egr'];
				$diaing = $array['egrdia'];
				$diadef = $array['egrdin'];
				...
				$ingFec=$array['egring'];
				$egrFec=$array['egregr'];
				//Se comentan estos campos ya que estaban repetidos Jonatan 24 enero 2017
				// $horaing=$array['egrhoi'];
				// $horaegr=$array['egrhoe'];
				$diasEst=$ingFec-$egrFec;
				//$diasEst = dias_transcurridos($fechaing, $fechaegr);
				$fecha=date("d/m/Y");
		...
		*/
	    ...
	    // Obtengo los topes actuales para aseguradora del paciente y aseguradora estatal
...
radicaci&oacute;n
		...
		/*
		*********** Se comenta porque la respuesta a glosa se marca en facturación no desde el cco ************
		if (isset($rowfue['Carglo']) && $rowfue['Carglo']=="on")
			$rg = '<span class="campoCod"> &nbsp; X &nbsp; </span>';
		else
			$rg = ' ___ ';
		*/
		$rg = ' ___ ';
 ...
 <table style="width: 100%;" border="0" cellpadding="2" cellspacing="2">
  ...
  <tr>
	...
	if(isset($nombres[1]) and $nombres[1] != ''){
		$nombre2 = $nombres[1];
	}
	else{
		//Se corrige esta validacion cuando los nombres estan separados por dos espacios // Jonatan Lopez Enero 23 de 2017
		$nombres2 = explode("  ", trim($nombre));
		if($nombres2[1] != ''){
			$nombre2 = $nombres2[1];
		}else{
			$nombre2="";
      ...
      <table style="width: 100%;" border="0" cellpadding="2" cellspacing="2">
       ...
       echo '</tbody>
		...
		<tr>
		  ...
		  <td style="width: 273px;"><div class="campoFecha">'.formato_hora(str_replace(".",":",$hora)).' hrs</div><br>
...
<table style="width: 640px;" cellpadding="2" cellspacing="2">
  ...
  <tbody>
	...
	<td class="total">Total Folios &nbsp; <span class="campoCod"> &nbsp; '.$total_folios.' &nbsp; </span></td>
      ...
      <table style="width: 100%;" border="0" cellpadding="2" cellspacing="2">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="width: 113px;">a las &nbsp; &nbsp;<span class="campoFecha">&nbsp;'.formato_hora(str_replace(".",":",$horaing)).' hrs&nbsp;</span></td>
            ...
            <td style="width: 113px;">a las &nbsp; &nbsp;<span class="campoFecha">&nbsp;'.formato_hora(str_replace(".",":",$horaegr)).' hrs&nbsp;</span></td>
...
<table style="border: 2px solid rgb(51, 51, 51); width: 570px;" cellpadding="2" cellspacing="2">
  <tbody>
	    ...
	    <tbody>
		  <tr>
			...
			<tr>
			  ...
			  <td style="width: 10%;"><div align="left"><input type="text" value="'.$diasEst.'" name="fecent" id="fecent" size="1" style="border-top: 0px;border-right: 0px;border-left: 0px;border-bottom: 1px #333333 solid; text-align:center; font-size: 5pt !important; font-size: 4.5pt;" /></div></td>
			</tr>
		</tbody>
      </table>
	  </td>
	...
	/* Ya no se maneja consecutivo por txt, sino que se toma el número del documento
	  if($cont==1)
		$consecutivo = obtener_consecutivo("../../IPS/FURIPS/farpmla/consecutivo.txt");
	*/
	  ...
	  {
		  // Consulto detalle de la venta para listar los productos
		  $qart =    "SELECT Artnom, Artcna, Artcod "
		...
		$codigo_cums = str_replace("0000","",$row_art['Artcna']);

		if($codigo_cums == 'NO APLICA'){
			$codigo_cums = $row_art['Artcod'];
		}

		$furips2 .= $num_factura.','.$dcto.',1,'.$codigo_cums.','.substr($row_art['Artnom'],0,40).','.$row_vde['Vdecan'].','.$row_vde['Vdevun'].','.$totalvde.','.$totalvde.'
';
...
';
		...
		$furips = ',,'.trim($num_factura).','.trim($dcto).','.trim($cod_habilitacion).','.trim($apellido1).','.trim($apellido2).','.trim($nombre1).','.trim($nombre2).','.trim($tipo).','.trim($doc).','.db2fecha($fechanac).','.trim($sexo).','.trim(str_replace("-"," ",$direccion)).','.trim($geoVictima['Dcod']).','.trim($geoVictima['Mcod']).','.trim($telefono).','.condicion_victima(trim($ocupante)).',01,,'.trim($Alug).','.db2fecha($fechaAccidente).','.formato_hora(trim(str_replace(".",":",$hora))).','.trim($geoSitio['Dcod']).','.trim($geoSitio['Mcod']).','.trim($Azon).','.estado_asegurado(trim($Aase)).','.trim($marca).','.trim($placa).','.tipo_vehiculo($Atipo).','.trim($aseguradora_cod).','.trim($poliza_num).','.db2fecha($Pinicio).','.db2fecha($Pven).','.cambio_boleano(trim($Ainteraut)).','.cambio_boleano($cobro_excedente_valor).',,,,,,,'.trim($Ptipoide).','.trim($Pidentificacion).','.trim($Papellido1).','.trim($Papellido2).','.trim($Pnombre1).','.trim($Pnombre2).','.trim(str_replace("-"," ",$Pdireccion)).','.trim($Ptelefono).','.trim($geoPropietario['Dcod']).','.trim($geoPropietario['Mcod']).','.trim($Capellido1).','.trim($Capellido2).','.trim($Cnombre1).','.trim($Cnombre2).',,'.trim($Cdoc).','.trim(ereg_replace("[^A-Za-z0-9]", "",$Cdir)).','.trim($geoConductor['Dcod']).','.trim($geoConductor['Mcod']).','.trim($Ctel).',,,,,,,,,,,,,,,,,'.db2fecha($fechaing).','.formato_hora(trim(str_replace(".",":",$horaing))).','.db2fecha($fechaegr).','.formato_hora(trim(str_replace(".",":",$horaegr))).','.trim($diaing).',,,'.trim($diadef).',,,'.trim($Medapellido1).','.trim($Medapellido2).','.trim($Mednombre1).','.trim($Mednombre2).','.trim($Medtipoide).','.trim($Medidentificacion).','.quitar_guion(trim($Medregistro)).','.trim($total_facturado).','.trim($total_ase_est).',0,0,'.trim($total_folios).'
';

		//$furips2 = $ctaNum.','.$soatNum.','.$nitAsegura.','.$nitEmp.','.$facturadoTex.','.$facturado.','.$paciente.','.$tipPac.','.$idePac.','.$ingFec.','.$egrFec.','.$fecha.'';

			$nombre_archivo = obtener_nombre_archivo($cod_habilitacion,$dcto);
			crear_archivo("../../ips/FuRips/farpmla/".$nombre_archivo,$furips,$cont);

			$nombre_archivo = obtener_nombre_archivo2($cod_habilitacion,$dcto);
			crear_archivo("../../ips/FuRips/farpmla/".$nombre_archivo,$furips2,$cont);
