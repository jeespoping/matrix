...
<head>
	<META name="DownloadOptions" content="nosave">
  	<title>REGISTROS MEDICOS Y ESTADISTICA</title>
...
else
{
	...
	echo "<form name='ac' action='ac_regmed.php' method=post>";
	...
	if(!isset($wano))
	{
		...
		{
			case "regmed":
				echo "<tr><td align=center colspan=2><b>ESTADISTICAS<b></td></tr>";
			break;
			
		}
		echo "<tr><td align=center colspan=2>REGISTROS MEDICOS Y ESTADISTICA</td></tr>";
		echo "<tr><td bgcolor=#cccccc align=center>Año</td><td bgcolor=#cccccc align=center>";
		switch($grupo)
		{
			case "regmed":
				$query = "SELECT Actano from regmed_000001 GROUP BY Actano";
			break;
		}
	...
	else
	{
		...
		{
			case "regmed":
				$query = "select Actano, Actcod, Actfec, Actnom   from  regmed_000001 ";
