...
<script>
    function ira()
    {
	 document.cotizaciones21.wmar.focus();
	}
</script>
...
<script type="text/javascript">
	function enter()
	{
		document.forms.cotizaciones21.submit();   // Ojo para la funcion cotizaciones21 <> Cotizaciones21  (sencible a mayusculas)
	}

	function vaciarCampos()
	{
	 document.forms.cotizaciones21.wmar.value = '';
	 document.forms.cotizaciones21.wpre.value = '';
     document.forms.cotizaciones21.wbar.value = '';
     document.forms.cotizaciones21.wreg.value = '';
     document.forms.cotizaciones21.wvec.value = 'dd-mm-aaaa';
     document.forms.cotizaciones21.wcum.value = '';
     document.forms.cotizaciones21.wact.value = '';     
     document.forms.cotizaciones21.wcot.value = '';
     document.forms.cotizaciones21.wiva.value = '';
     document.forms.cotizaciones21.wsug.value = '';          
	 document.forms.cotizaciones21.wobs.value = '';
    }
...
$wactualiz="PROGRAMA: cotizaciones21.php Ver. 2017-09-06   JairS - AngelaO";
...
{	    
   ...
   {
      $todok = false;  
      $msgerr=$msgerr." Debe indicar si posee Codigo de Identificación. <br>";
   }
   ...
   {
     $todok = false;     
     $msgerr=$msgerr." Fecha del Vencimiento del Registro debe ser aaaa-mm-dd.<br> ";
   }
   ...
   {
      $todok = false;
      $msgerr=$msgerr." Debe existir Clasificacion de acuerdo al riesgo.<br> ";
   }
   ...
   { 
	if ($act <> 0 )
	{
     ...
     {
      $todok = false;
      $msgerr=$msgerr." Nuevo precio cotizado debe ser verificado.<br> ";
     } 
    } 
   }
   ...
   {
      $todok = false;
      $msgerr=$msgerr." Precio sugerido al publico debe ser numerico.<br> ";
   }   
...
echo "<form name='cotizaciones21' action='cotizaciones21.php' method=post>";  
		
echo "<center><table border=1>";
echo "<tr><td align=center colspan=6 bgcolor=#005588><font size=3 text color=#FFFFFF><b>ARTICULOS A COTIZAR POR PROVEEDOR</b></font></tr>";//#006699 - #FFFFFF
echo "<tr><td align=center colspan=6 bgcolor=#005588><font size=2 text color=#FFFFFF><b>".$wactualiz."</b></font></td></tr>";//#006699 -  #FFFFFF


echo "<tr><td align=center colspan=6 bgcolor=#C3D5F7><font size=4 text color=#000000><b>DATOS DEL ANTISEPTICO O DESINFECTANTE A COTIZAR:</b></font></tr>";//#FFCC66- #FFFFFF
   $query = "SELECT concod,connom,conuni,conmes,conano FROM cotizaci_000008 Where concod = '".$wcod."' Order by connom";
   $resultado = mysql_query($query);
   $nroreg = mysql_num_rows($resultado);
   $numcam = mysql_num_fields($resultado);   
   $registro = mysql_fetch_row($resultado);  	   

   echo "<tr><td colspan=2 align=left bgcolor=#C3D5F7><b>Codigo: ".$registro[0]."<b></td>";//#FFCC33
   echo "<td colspan=4 align=left bgcolor=#C3D5F7><b>Descripcion: ".$registro[1]."<b></td>";//#FFCC33
   echo "<tr><td colspan=2 align=left bgcolor=#C3D5F7><font size=5><b>Unidad: ".$registro[2]."<b></td>";//#FFCC33
   echo "<td colspan=2 align=left bgcolor=#C3D5F7><b>Consumo Mes: ".$registro[3]."<b></td>";//#FFCC33
   echo "<td colspan=2 align=left bgcolor=#C3D5F7><b>Consumo Año: ".$registro[4]."<b></td>";//#FFCC33
   echo "<tr>";	

if ($windicador == "PrimeraVez") 
{
		
   $query = "SELECT cotmar,cotpre,cotbar,cotreg,cotvec,cotcla,cotact,cotcot,cotiva,cotsug,cotobs FROM cotizaci_000007"
...
echo "<tr><td align=center colspan=6 bgcolor=#C3D5F7><font size=4 text color=#000000><b>PARAMETROS TECNICOS LEGALES DEL ANTISEPTICO O DESINFECTANTE</b></font></tr>";//#FFCC66-#FFFFFF
echo "<tr><td align=center colspan=6 bgcolor=#C3D5F7><font size=3 text color=#000000>Recuerde que la presentación comercial se indica en unidad mínima a cotizar, unidad de empaque.</b></font></tr>";//#FFCC66-#FFFFFF
echo "<tr><td align=center colspan=6 bgcolor=#C3D5F7><font size=3 text color=#000000>Tenga en cuenta que el valor a cotizar es de acuerdo a la unidad mínima de medida expuesta en la plataforma.</b></font></tr>";//#FFCC66-#FFFFFF
    ...
    echo "<td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Presentacion Comercial:</font></b><br>";
    if (isset($wpre))
    ...
    echo "<td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Posee Código Identificación?, cuál?:</font></b><br>";
    if (isset($wbar)) {
   ...
   // echo "<INPUT TYPE='text' NAME='wbar' size=40 maxlength=30 ></INPUT></td>";
    ...
    echo "<tr><td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Registro INVIMA:</font></b><br>";//#DDDDDD
    if (isset($wreg))
    ...
    echo "<td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Vencimiento del registro (aaaa-mm-dd):</font></b><br>";//#DDDDDD
    if (isset($wvec))
    ...
    echo "<td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Clasificacion de riesgo:</font></b><br>";//#DDDDDD
    if (isset($wcum))
    ...
    echo "<tr><td align=center colspan=6 bgcolor=#C3D5F7><font size=4 text color=#000000><b>PARAMETROS COMERCIALES</b></font></tr>";    //#FFCC66-#FFFFFF
         
    echo "<tr><td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Precio anterior cotizado.</font></b><br>";//#DDDDDD
    if (isset($wact))
    ...
    echo "<td align=center bgcolor=#8991AF colspan=2><b><font text color=#003366 size=3>Nuevo precio a cotizar.</font></b><br>";//#DDDDDD
    if (isset($wcot))
    ...
    echo "<td align=center bgcolor=#8991AF colspan=1><b><font text color=#003366 size=3>Iva %</font></b><br>";//#DDDDDD
    if (isset($wiva))
   ...
   */
    ...
    // $wproceso y wnit son variables escondidas que enviaremos cada vez a travez del formulario	   	   	     
   	...
   	echo "<tr><td align=center colspan=10 bgcolor=#005588>";//#CCCCCC
   	echo "<input type='submit' value='Grabar'>";          
   	echo "<input type='button' name='BReset' onclick ='vaciarCampos()' value='Inicializar' id='BReset'>";
   	echo "<tr><td align=center colspan=10 bgcolor=#8991AF>";//#CCCCCC
   	echo "<li><A HREF='cotizaciones20.php' style='color: aliceblue'>Regresar</A>";
   	echo "</td></tr>";	
...
//--if ( isset($wmar) and $wmar<>'' and isset($wpre) and $wpre<>'')
{
  ...
  // invoco la funcion que valida los campos
    $wsug=0;  // en Agosto de 2017 Servicio Farmacéutico dijo que esta variable no era necesaria.
  ...
  { 
   ...
   {	     
     ...
     $query = "SELECT cotano,cotnit,cotcod,cotcot "
             ." FROM cotizaci_000007 Where cotano = '".$wano."' And cotnit = '".$wnit."' And cotcod = '".$wcod."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
	  ...
	  $query = "Update cotizaci_000007 SET cotmar='".$wmar."',cotpre='".$wpre."',cotbar='".$wbar."',cotreg='".$wreg."'"
               .",cotvec='".$wvec."',cotcla='".$wcum."',cotact=".$wact.",cotcot=".$wcot.",cotiva=".$wiva.",cotsug=".$wsug.",cotobs='".$wobs
               ."' Where cotano = '".$wano."' And cotnit = '".$wnit."' And cotcod = '".$wcod."'";
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO cotizaci_000007 VALUES ('cotizaci','".$fecha."','".$hora."','".$wano."','".$wnit."','".$wcod."','".$wmar
