...
echo "<form name='cotizaciones00' action='cotizaciones00.php' method=post>"; 
...
$wano=2021;                 //COMO PARA EL 2021 COTIZARAN TODO ENERO TOCO ASI.
...
{        	
   ...
   /*	    echo "<font size=3 text color=#33FFFF>";
   /*	    echo "<MARQUEE BEHAVIOR=SCROLL BGCOLOR=#006699 LOOP=-1>";
   /*	    echo "Ayer 19 de Enero se presento un problema en la grabación de medicamentos, por lo que solicitamos revisar lo grabado entre las 8am y 1pm, pedimos nos disculpen por las molestias ocasionadas.</MARQUEE></font>";
   /*	    echo "</td></tr>";
   /****************************************************************/
   ...
   echo "<tr><td align=center bgcolor=#DDDDDD colspan=1><b><font text color=#003366 size=2> <i>PROGRAMA: cotizaciones00.php Ver. 2009/10/06 JairS</font></b><br>";
   echo "<tr><td align=center bgcolor=#DDDDDD colspan=1><A HREF='cotizaciones00.php?wano=".$wano."&wnit=".$wnit."&windicador=PrimeraVez&wproceso=Nuevo'>Adicionar nuevo articulo</A></td>";
   
 if (isset($wproceso))
 {
  if ($wproceso == "Eliminar")
  {
   $query = "Delete From cotizaci_000003 WHERE cotano='".$wano."' and cotnit='".$wnit."' and cotcod='".$wcod."'";
   ...
   if (isset($wproceso) and $wproceso=="Nuevo")   //muestro todos los consumos del ultimo año
    $query = "SELECT concod,connom,conuni,conmes,conano FROM cotizaci_000001 Order by connom";
   else
   {  	   
   ...
   //echo "<td colspan=2 align=center bgcolor=#DDDDDD><b>CODIGO<br>CUM</td>";
   ...
   echo "</tr>"; 
		...
		{		
         ...
         $query = "SELECT cotmar,cotpre,cotbar,cotreg,cotvec,cotcum,cotact,cotcot,cotiva,cotsug,id FROM cotizaci_000003";
        ...
        else
        {
	     ...
	     echo "<A HREF='cotizaciones01.php?wano=".$wano."&wnit=".$wnit."&wcod=".$registro[0]."&windicador=PrimeraVez&wproceso=Actualizar&wid=".$wid."'>Actualizar</A></td>";	     

	     echo "<td colspan=2 align=center color=#FFFFFF bgcolor=".$wcf.">";
         echo "<A HREF='cotizaciones00.php?wano=".$wano."&wnit=".$wnit."&wcod=".$registro[0]."&windicador=PrimeraVez&wproceso=Eliminar&wid=".$wid."'>Eliminar</A></td>";	     
