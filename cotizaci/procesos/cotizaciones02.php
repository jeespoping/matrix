...
echo "<form name='cotizaciones02' action='cotizaciones02.php' method=post>"; 
...
{        	            	
   ...
   echo "<tr><td align=center bgcolor=#DDDDDD colspan=1><b><font text color=#003366 size=2> <i>PROGRAMA: cotizaciones02.php Ver. 2009/10/06 JairS</font></b><br>";
   echo "<tr><td align=center bgcolor=#DDDDDD colspan=1><A HREF='cotizaciones02.php?wano=".$wano."&wnit=".$wnit."&windicador=PrimeraVez&wproceso=Nuevo'>Adicionar nuevo articulo</A></td>";
   
 if (isset($wproceso))
 {
  if ($wproceso == "Eliminar")
  {
   $query = "Delete From cotizaci_000004 WHERE cotano='".$wano."' and cotnit='".$wnit."' and cotcod='".$wcod."'";
   ...
   if (isset($wproceso) and $wproceso=="Nuevo")   //muestro todos los consumos del ultimo año
    $query = "SELECT concod,connom,conuni,conmes,conano FROM cotizaci_000002 Order by connom";
   else
   {  	   
	// Muestro lo cotizado por el proveedor el año anterior y los consumos del ultimo año segun el query en 
	// UNIX "consumGral.sql" con el que poblamos la tabla cotizaci_000002 en Matrix   
	
    $wano2= (integer) $wano - 1;    
            
    $query = "SELECT concod,connom,conuni,conmes,conano FROM cotizaci_000004,cotizaci_000002 "
   ...
   echo "</tr>"; 
		...
		{		
         ...
         $query = "SELECT cotmar,cotpre,cotbar,cotreg,cotvec,cotcla,cotact,cotcot,cotiva,cotsug,id FROM cotizaci_000004";
        ...
        else
        {
	     ...
	     echo "<A HREF='cotizaciones03.php?wano=".$wano."&wnit=".$wnit."&wcod=".$registro[0]."&windicador=PrimeraVez&wproceso=Actualizar&wid=".$wid."'>Actualizar</A></td>";	     
 
	     echo "<td colspan=2 align=center color=#FFFFFF bgcolor=".$wcf.">";
         echo "<A HREF='cotizaciones02.php?wano=".$wano."&wnit=".$wnit."&wcod=".$registro[0]."&windicador=PrimeraVez&wproceso=Eliminar&wid=".$wid."'>Eliminar</A></td>";	     
