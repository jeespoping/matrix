<html> 
<head>
<title>Actualizacion Autorizaciones Programa PAF FALLA CARDIACA</title>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf01faca.submit();   // Ojo para la funcion paf01faca <> paf01faca  (sencible a mayusculas)
	}

	function vaciarCampos()
	{document.forms.paf01faca.wfau.value = 'yyyy-mm-dd';
	 document.forms.paf01faca.wced.value = '';
     document.forms.paf01faca.wap1.value = '';
	 document.forms.paf01faca.wap2.value = '';
     document.forms.paf01faca.wnom.value = '';
     document.forms.paf01faca.wtel.value = '';
     document.forms.paf01faca.wran.value = '';	
     document.forms.paf01faca.wtip.value = '';	
     document.forms.paf01faca.wdia.value = '';	
     document.forms.paf01faca.wexa.value = '';	 
	 document.forms.paf01faca.wcco.value = '';
	 document.forms.paf01faca.west.value = '';
	 document.forms.paf01faca.word.value = '';
	 document.forms.paf01faca.wser.value = '';
	 document.forms.paf01faca.wran.value = '';	
     document.forms.paf01faca.wcr1.value = '';    
     document.forms.paf01faca.wcr2.value = '';     
    }
...
//==========================================================================================================================================
//PROGRAMA				      :Actualizacion Autorizaciones Programa PAF FALLA CARDIACA                                                                   
//AUTOR				          :Jair  Saldarriaga Orozco.                                                                        
//FECHA CREACION			  :Agosto 2016-06-09
//FECHA ULTIMA ACTUALIZACION  :Junio 22 de 2018.                                                                             

$wactualiz="PROGRAMA: paf01faca.php Ver. 2018-08-23   JairS";
...
echo "<form name='paf01faca' action='paf01faca.php' method=post>";  
...
echo "<tr><td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FFFFFF><b>Actualizacion Autorizaciones Programa PAF FALLA CARDIACA</b></font>";
...
{
  $query = "SELECT Max(id) FROM paffaca_000001";
  $resultado = mysql_query($query);
  $registro = mysql_fetch_row($resultado);  	
  $nrorden=$registro[0];	
 ...
 if ($wproceso == "Modificar" )  	
  {
   ...
   $query = "SELECT Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla"
           ." FROM paffaca_000001 Where id = '".$wid."'";
  ...
  { 
    $query = "SELECT Docaf,Apeaf1,Apeaf2,Nomaf,Sexo,Fec_nacimiento,Tip_afiliado,Telaf,Codips FROM paffaca_000002 Where  Docaf = '".$wid."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
    if ($nroreg > 0)      //  Encontro 
    {
...
/***********************************************************************************************************************************/	
	...
	{	
   	...
   	if ($wproceso == "Modificar" )  	
      echo "<A HREF='paf09faca.php?wid=".$wid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
...
if ( $conf == "on" and isset($wap1) and $wap1<>'' and isset($wexa) and $wexa<>'' and isset($wcco) and $wcco<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   // Busco la fecha del ultimo examen igual al solicitado si es menor a 90 dias mensaje informativo
     $query = "SELECT paffau,Id FROM paffaca_000001 Where pafced = '".$wced."' And pafexa='".$c4[0]."' Order by paffau DESC";
     ...
     $query = "SELECT * FROM paffaca_000001 Where paffau = '".$wfau."' And pafced = '".$wced."' And pafexa='".$c4[0]."' And paford = '".$word."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
       ...
       $query = "Update paffaca_000001 SET pafap1='".$wap1."',pafap2='".$wap2."',pafnom='".$wnom."',paftel='".$wtel."',pafran='".$c1[0]."'"
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO paffaca_000001 (Medico,Fecha_data,Hora_data,Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,"
	  ...
	  if ($resultado)
		{
			...
			$query = "SELECT Id FROM paffaca_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
			$resultadoB = mysql_query($query,$conex);
			$nroreg = mysql_num_rows($resultadoB);
			if ( $nroreg > 0 )                   //Encontro
			{
			 $registroB = mysql_fetch_row($resultadoB);
			 $ultid=$registroB[0];
			 echo "<A HREF='paf09faca.php?wid=".$ultid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A></td></tr>";		
			} 
