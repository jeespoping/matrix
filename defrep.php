...
<BODY TEXT="#000066">
<center>
<A NAME="Arriba"><h1>Definicion de Reportes</h1></a>
</center>
...
else
{
	...
	if (isset($wpar))
	{
		...
		else
		{
			...
			{
				case 1:
				$query = "update reportes set descripcion='".$Descripcion."',nombre='".$Nombre."', nivel=".$Nivel." where medico='".$Medico."' and formulario='".$Formulario."' and codigo='".$Codigo."'";
				$err = mysql_query($query,$conex);
				break;
				case 2:
				$query = "insert reportes values ('".$Medico."','".$Formulario."','".$Codigo."','".$Descripcion."','".$Nombre."',".$Nivel.")";
				$err = mysql_query($query,$conex);
				if ($err != 1)
				{
					echo "<center><table border=0 aling=center>";
	...
	echo "<tr><td align=center><A HREF='det_defrep.php?pos1=".$key."&amp;pos2=0&amp;pos3=0'>Nuevo</td></tr></table><BR>\n";
	$query = "select * from reportes where medico='".$key."'";
	$err = mysql_query($query,$conex);
	...
	if ($num > 0)
	{
		...
		for ($i=0;$i<$num;$i++)
		{
			...
			echo "<td bgcolor=".$color." align=center><font size=2><A HREF='det_defrep.php?pos1=".$key."&amp;pos2=".$row[1]."&amp;pos3=".$row[2]."'>Editar</font></td>";
			echo "</tr>";
		}
		echo "</tabla>";
		echo "<table border=0 align=center>";
