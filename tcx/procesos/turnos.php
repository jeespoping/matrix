...
if(isset($accionAjax)) 
{
	...
	if(!isset($_SESSION['user'])){		
		...
		$respuesta['msj'] 	= utf8_encode('Primero recargue la p�gina principal de Matrix � inicie sesi�n nuevamente, para poder relizar esta acci�n.');
	}
	else{
...
<!--
	...
	function vistaOperativa(){
		...
		$("#tablaColores").after("<div id='divPedirCausa' style='display:none;font-family: verdana;font-size: 9pt;' align='center'>"
								+"La cx ha <span id='textMsj'></span> m�s tarde de lo programado.<br><br>"
	...
	function iniciarCx(turno, primeraVez){
		...
		if(primeraVez){
			if(!confirm("Est� seguro en INICIAR la cirug�a?"))
				return;
		}
	...
	function terminarCx(turno, primeraVez){
		...
		if(primeraVez){
			if(!confirm("Est� seguro en TERMINAR la cirug�a?"))
				return;
		}
...
[DOC]
	   ...
	   .2009-10-08
			** Se modifico en la estructura de la base de datos las tablas 7 (Movimiento de cancelaciones) y 11 (movimiento de turnos)
			...
			Se modifico igualmente el programa para que diera la posibilidad a este manejo, igualmente la capa de presentaci�n se
	   ...
	   .2008-02-15
	   		...
	   		** Se agrego la funcion Bisiesto (Evaluacion de a�os Bisiestos) que no se encontraba en el codigo.
...
function valcam($ok,$conex,$wnci,$wqui,$whin,$whfi,$wfec,$wquix,$whinx,$whfix,$wfecx,$dataC,$NC,$dataM,$NM,&$dataE,&$NE,&$werr,&$e, $causaModificacion)
{
	...
	{
		$e=$e+1;
		$werr[$e]="ERROR, DEBE SELECCIONAR UNA CAUSA DE MODIFICACI�N";
	}
...
else
{
	...
	if ($num > 0)
	{
		...
		if($ok == 99)
		{
			...
			echo "<td rowspan=1 bgcolor='#cccccc' align=center valign=center>A�o - Mes - Dia<br><input type='TEXT' name='wfecha' size=10 maxlength=10 id='wfecha' readonly='readonly' value=".$wfecha." class=tipo6></td><td bgcolor='#cccccc' align=center valign=center><IMG SRC='/matrix/images/medical/TCX/calendario.jpg' id='trigger1'></td>";
			?>
			...
			if ($num>0)
			{
				for ($i=0;$i<$num;$i++)
				{
					...
					if($row['Turcdr'] != "")
						$msjCausas.= "<tr><td class='subtitle'>Reprogramaci�n:</td><td>".$arrCausas[$row['Turcdr']]."</td></tr>";					
	...
	else
	{
		...
		echo "<td bgcolor='#cccccc' align=center valign=center>A�o - Mes - Dia<br><input type='TEXT' name='wfecha' size=10 maxlength=10 id='wfecha' readonly='readonly' value=".$wfecha." class=tipo6></td>";
