<html>
<head>
  	<title>MATRIX Gestion de Materiales e Insumos en Turnos de Cirugia ListaM.php</title>
...
/**********************************************************************************************************************
	   ...
	   .2011-01-20
	   		Se modifico en el programa el mecanismo de actualizacion del estado de los materiales para hacerlo de forma
	   		exclusiva en el turno que se quiere modificar y no sobre todos turnos mostrados en pantalla.
...
else
{
	...
	echo "<form name='ListaM' action='ListaM.php' method=post>";
	...
	else
	{
		...
		$query  = "SELECT '1', Turqui, Turhin, Turhfi, Turfec, Turndt, Turdoc, Turhis, Turnin, Turnom, Turfna, Tursex, Turins, Turtcx, Turtip, Turtan, Tureps, Turuci, Turbio, Turinf, Turmat, Turmok, Turban, Turtel, Turord, Turcom, Turcir, Turmed, Turequ, Turusg, Turusm, Turest, Turmok, Turtur FROM ".$empresa."_000011 ";
		$query .= " where turfec between '".$wfecha1."' and '".$wfecha2."'";
		$query .= "   and turmat = 'on' ";
		$query .= "   and Turcom not like '%Turno Cambiado%' ";
		//$query .= "   and Turmok = 'off' ";
        $query .= "   UNION ";
        $query .= "SELECT '2', Turqui, Turhin, Turhfi, Turfec, Turndt, Turdoc, Turhis, Turnin, Turnom, Turfna, Tursex, Turins, Turtcx, Turtip, Turtan, Tureps, Turuci, Turbio, Turinf, Turmat, Turmok, Turban, Turtel, Turord, Turcom, Turcir, Turmed, Turequ, Turusg, Turusm, Turest, Turmok, Turtur FROM ".$empresa."_000011 ";
		$query .= " where turfec between '".$wfecha1."' and '".$wfecha2."'";
		$query .= "   and turmat = 'on' ";
		$query .= "   and Turcom like '%Turno Cambiado%' ";
		$query .= "   UNION ";
		$query .= "SELECT '3', Mcaqui, Mcahin, Mcahfi, Mcafec, Mcandt, Mcadoc, Mcahis, Mcanin, Mcanom, Mcafna, Mcasex, Mcains, Mcatcx, Mcatip, Mcatan, Mcaeps, Mcauci, Mcabio, Mcainf, Mcamat, Mcamok, Mcaban, Mcatel, Mcaord, Mcacom, Mcacir, Mcamed, Mcaequ, Mcausg, Mcausm, Mcaest, Mcamok, Mcatur FROM ".$empresa."_000007 ";
		$query .= " where Mcafec between '".$wfecha1."' and '".$wfecha2."'";
		$query .= "   and Mcamat = 'on' ";
        $err = mysql_query($query,$conex) or die(mysql_errno().":".mysql_error());
        $num = mysql_num_rows($err);
        for ($i=0;$i<$num;$i++)
		...
		for ($i=0;$i<$num;$i++)
		{
			...
			if($wimpre[$i][21] == "on")
			{
				...
				{
					echo "<tr><td id=".$tipo."CK>";
					echo "<input type='RADIO' name=est[".$i."] value=0 onclick='return false'>PENDIENTE  <BR>";
					echo "<input type='RADIO' name=est[".$i."] value=1 checked onclick='return false'>PROCESADO  <BR>";
					echo "&nbsp;<input type='checkbox' name='CHECK[".$i."]' onclick='return false'  class=".$class."> ok</td>";
				}
			}
			else
			{
				...
				{
					echo "<tr><td id=".$tipo."CK>";
					echo "<input type='RADIO' name=est[".$i."] value=0 checked onclick='return false'>PENDIENTE  <BR>";
					echo "<input type='RADIO' name=est[".$i."] value=1 onclick='return false'>PROCESADO  <BR>";
					echo "<input type='checkbox' name='CHECK[".$i."]' onclick='return false' class=".$class."> ok</td>";
				}
			}
			if ($wimpre[$i][25] < 3)
