<html>
<head>
  	<title>MATRIX Camas Proyectadas Para Pacientes Quirurgicos con UCI CamasUCI.php</title>
...
<!--
	function enter()
	{
		document.forms.CamasCXF.submit();
	}
	function teclado()  
	{ 
...
/**********************************************************************************************************************  
	   ...
	   Version Actual : 2015-03-10
	   
	   OBJETIVO GENERAL :Este programa ofrece al usuario una interface grafica que permite las camas que se necesitaran 
	   para los pacientes que seran intervenidos quirurgicamente.
...
else
{
	...
	echo "<tr><td align=center colspan=4 id=tipo19> Ver. 2015-03-10</td></tr>";
	echo "<tr><td align=center colspan=4 id=tipo14>CAMAS PROYECTADAS PARA PACIENTES QUIRURGICOS CON UCI</td></tr>";
	if (!isset($wfecha))
		$wfecha=date("Y-m-d");
	...
	echo "<td bgcolor='#cccccc' align=center valign=center>A&ntilde;o - Mes - Dia<br><input type='TEXT' name='wfecha' size=10 maxlength=10 id='wfecha' readonly='readonly' value=".$wfecha." class=tipo6></td><td bgcolor='#cccccc' align=center valign=center><IMG SRC='/matrix/images/medical/TCX/calendario.jpg' id='trigger1'></td></tr>";
	?>
	...
	$wimpre=array();
	echo "<table border=0 align=center>";
	echo "<td id=tipoT01>TURNO NRo.</td><td id=tipoT02>DOCUMENTO</td><td id=tipoT00>HISTORIA</td><td id=tipoT02>INGRESO</td><td id=tipoT04>PACIENTE</td><td id=tipoT05>RESPONSABLE</td><td id=tipoT06>CIRUGIA</td></tr>";
	//                  0       1       2      3       4        5      6       7
	$query = "SELECT Turtur, Turdoc, Turhis, Turnin, Turnom, Tureps, Turcir, Entdes FROM ".$empresa."_000011,".$empresa."_000003 ";
	...
	for ($i=0;$i<$num;$i++)
	{
		...
		echo "<tr><td id=".$tipo."01>".$wimpre[$i][0]."</td><td id=".$tipo."02>".$wimpre[$i][1]."</td><td id=".$tipo."00>".$wimpre[$i][2]."</td><td id=".$tipo."05>".$wimpre[$i][3]."</td><td id=".$tipo."17>".$wimpre[$i][4]."</td><td id=".$tipo."03>".$wimpre[$i][5]."</td><td id=".$tipo."04>".$wimpre[$i][6]."</td></tr>";
	}
	echo "<tr><td align=left colspan=7 bgcolor=#999999>TOTAL CAMAS : ".$num."</td></tr>";
	echo "</table>";
}
?>
</body>
