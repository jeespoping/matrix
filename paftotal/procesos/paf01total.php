<html> 
<head>
<title>Actualizacion Autorizaciones Programa PAF SALUD TOTAL</title>
...
<script>
    function ira()
    {
	 document.paf01total.wced.focus();
	}
</script>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf01total.submit();   // Ojo para la funcion paf01total <> paf01total  (sencible a mayusculas)
	}

	function vaciarCampos()
	{document.forms.paf01total.wfau.value = 'yyyy-mm-dd';
	 document.forms.paf01total.wced.value = '';
     document.forms.paf01total.wap1.value = '';
	 document.forms.paf01total.wap2.value = '';
     document.forms.paf01total.wnom.value = '';
     document.forms.paf01total.wtel.value = '';
     document.forms.paf01total.wran.value = '';	
     document.forms.paf01total.wtip.value = '';	
     document.forms.paf01total.wdia.value = '';	
     document.forms.paf01total.wexa.value = '';	 
	 document.forms.paf01total.wcco.value = '';
	 document.forms.paf01total.west.value = '';
	 document.forms.paf01total.word.value = '';
	 document.forms.paf01total.wser.value = '';
	 document.forms.paf01total.wran.value = '';	
     document.forms.paf01total.wcr1.value = '';    
     document.forms.paf01total.wcr2.value = '';     
    }
...
//==========================================================================================================================================
//PROGRAMA				      :Actualizacion Autorizaciones Programa PAF SALUD TOTAL                                                                   
//AUTOR				          :Jair  Saldarriaga Orozco.                                                                        
//FECHA CREACION			  :Junio 9 de 2016
//FECHA ULTIMA ACTUALIZACION  :Junio 9 de 2016.                                                                             

$wactualiz="PROGRAMA: paf01total.php Ver. 2016-06-09   JairS";
...
Function validar_datos($fec,$ced,$ap1,$nom,$cco,$dia,$exa,$rem,$fre,$ord,$fem,$fci,$hor,$fca,$cau ) 
{	    
   global $todok;   
   ...
   */
   ...
   {
      $todok = false; 
      $msgerr=$msgerr." El Campo 1er Apellido no puede ser nulo.";   
   }   
   ...
   if (empty($cco))
   {
    ...
    // Chequeo el nro de la Orden de la EPS  
 	if (empty($ord))
    {
      $todok = false; 
      $msgerr=$msgerr." El Nro de la Orden de la EPS no puede ser nula.";   
    }	

	
    // Chequeo la fecha de emision de la orden  en la EPS
	if (empty($fem) or ($fem == '0000-00-00') )
    {
      $todok = false; 
      $msgerr=$msgerr." La Fecha de emision de la orden de la EPS no puede ser nula.";   
    }
    else	
     if ($fem > $fec) 	
     {
      $todok = false;     
      $msgerr=$msgerr." Fecha de emision de Orden en la EPS no puede ser mayor a la fecha de Vigencia. ";   
     }	
...
echo "<form name='paf01total' action='paf01total.php' method=post>";  
...
echo "<tr><td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FFFFFF><b>Actualizacion Autorizaciones Programa PAF SALUD TOTAL</b></font>";
if ($wproceso == "Modificar" )
 echo "<td align=center colspan=1 bgcolor=#99CCCC><font size=3 text color=#FF0000><b>Nro Orden: ".$wid."</b></font></tr>";
else
{
  $query = "SELECT Max(id) FROM paftotal_000001";
  $resultado = mysql_query($query);
  $registro = mysql_fetch_row($resultado);  	
  $nrorden=$registro[0];	
   //echo "<td align=center colspan=1 bgcolor=#99CCCC><font size=3 text color=#FF0000><b>Nro Orden: ****** </b></font></tr>";  
  echo "<td align=center colspan=1 bgcolor=#99CCCC><font size=3 text color=#FF0000><b>Nro Orden: ".(($nrorden) + 1) ."</b></font></tr>";  
 ...
 if ($wproceso == "Modificar" )  	
  {
   ...
   $query = "SELECT Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla"
           ." FROM paftotal_000001 Where id = '".$wid."'";
  ...
  { 
    $query = "SELECT Docaf,Apeaf1,Apeaf2,Nomaf,Sexo,Fec_nacimiento,Tip_afiliado,Telaf,Codips FROM paftotal_000002 Where  Docaf = '".$wid."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
    if ($nroreg > 0)      //  Encontro 
    {
  ...
  if (!isset($wfau))    // Si no esta seteada entonces la inicializo con la fecha actual, OJO si va a copiar este campo cambie el trigger1 por triggerx
    $wfau=date("Y-m-d");
    
    echo "<tr><td bgcolor=#C0C0C0 align=center colspan=1><font text color=#003366 size=3>Fecha de la autorizacion:<br>";
   	$cal="calendario('wfau','1')";
	echo "<input type='TEXT' name='wfau' size=10 maxlength=10  id='wfau' Readonly value=".$wfau." class=tipo3><button id='trigger1' ".$w." onclick=".$cal.">...</button></td>";
	?>
	  <script type="text/javascript">//<![CDATA[
	   Zapatec.Calendar.setup({weekNumbers:false,showsTime:true,timeFormat:'12',electric:false,inputField:'wfau',button:'trigger1',ifFormat:'%Y-%m-%d',daFormat:'%Y/%m/%d'});	
	   //]]></script>
	<?php
    ...
    if (isset($wap1))
     echo "<INPUT TYPE='text' NAME='wap1' size=30 maxlength=20 ".$wmodifi." VALUE='".$wap1."' onKeyUp='form.wap1.value=form.wap1.value.toUpperCase()'></INPUT></td>"; 
    else
     echo "<INPUT TYPE='text' NAME='wap1' size=30 maxlength=20 ".$wmodifi." onKeyUp='form.wap1.value=form.wap1.value.toUpperCase()'></INPUT></td>"; 
 
    echo "<td align=center bgcolor=#C0C0C0 colspan=1><b><font text color=#003366 size=2>2do Apellido:</font></b><br>";
    if (isset($wap2))
     echo "<INPUT TYPE='text' NAME='wap2' size=30 maxlength=20 ".$wmodifi." VALUE='".$wap2."' onKeyUp='form.wap2.value=form.wap2.value.toUpperCase()'></INPUT></td>"; 
    else
     echo "<INPUT TYPE='text' NAME='wap2' size=30 maxlength=20 ".$wmodifi." onKeyUp='form.wap2.value=form.wap2.value.toUpperCase()'></INPUT></td>"; 

    echo "<td align=center bgcolor=#C0C0C0 colspan=1><b><font text color=#003366 size=2>Nombres:</font></b><br>";
    if (isset($wnom))
     echo "<INPUT TYPE='text' NAME='wnom' size=30 maxlength=25 ".$wmodifi." VALUE='".$wnom."' onKeyUp='form.wnom.value=form.wnom.value.toUpperCase()'></INPUT></td>"; 
    else
     echo "<INPUT TYPE='text' NAME='wnom' size=30 maxlength=25 ".$wmodifi." onKeyUp='form.wnom.value=form.wnom.value.toUpperCase()'></INPUT></td>"; 
...
/***********************************************************************************************************************************/	
	...
	echo "<td align=center bgcolor=#C0C0C0 colspan=1><font text color=#003366 size=3>Nro de orden : </font>";
    if (isset($word))
	...
	{	
   	...
   	if ($wproceso == "Modificar" )  	
      echo "<A HREF='paf09total.php?wid=".$wid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
  
   	echo "</td>";	


if ( $conf == "on" and isset($wap1) and $wap1<>'' and isset($wexa) and $wexa<>'' and isset($wcco) and $wcco<>'' and $windicador <> "PrimeraVez" )   
{
  ///////////              Cuando ya hay datos capturados todos los datos      //////////////////

  // invoco la funcion que valida los campos 
  validar_datos($wfau,$wced,$wap1,$wnom,$wcco,$wdia,$wexa,$wrem,$wfre,$word,$wfem,$wfci,$whor,$wfca,$wcau); 
  ...
  { 
   ...
   // Busco la fecha del ultimo examen igual al solicitado si es menor a 90 dias mensaje informativo
     $query = "SELECT paffau,Id FROM paftotal_000001 Where pafced = '".$wced."' And pafexa='".$c4[0]."' Order by paffau DESC";
     ...
     $query = "SELECT * FROM paftotal_000001 Where paffau = '".$wfau."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
       ...
       $query = "Update paftotal_000001 SET pafap1='".$wap1."',pafap2='".$wap2."',pafnom='".$wnom."',paftel='".$wtel."',pafran='".$c1[0]."'"
			   ...
			   .",pafcau='".$wcau."',paflla='".$wlla."',Fecha_data='".$fecha."',Hora_data='".$hora."',paford='".$word."' "
               ." Where paffau='".$wfau."' And pafced='".$wced."' And pafexa='".$c4[0]."' ";
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO paftotal_000001 (Medico,Fecha_data,Hora_data,Paffau,Pafced,Pafap1,Pafap2,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,"
	          ."Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla,Seguridad,id)"
	          ." VALUES ('paf','".$fecha."','".$hora."','".$wfau."','".$wced."','".$wap1."','".$wap2."','".$wnom."','".$wtel
              ."','".$c1[0]."','".$c2[0]."','".$c3[0]."','".$c4[0]."','".$c7[0]."','".$c5[0]."','A','".$wobs."','".$word."','".$c6[0]."','".$wfre
              ."','".$wfem."','".$wfci."','".$whor."','".$wfca."','".$wcau."','".$wlla."','".$user."','')";			  
      
              $resultado = mysql_query($query,$conex);  
	  if ($resultado)
		{
			echo "<br><tr><td bgcolor=#33FFFF colspan=1 align=center>Registro Adicionado</td>";
		   
			//Despues de adicionado permito imprimir
			$query = "SELECT Id FROM paftotal_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
			// $resultadoB = mysql_query($query,$conex);
			// var_dump($resultadoB);
			$nroreg = mysql_num_rows($resultadoB);
			if ( $nroreg > 0 )                   //Encontro
			{
			 ...
			 echo "<td align=center colspan=1 bgcolor=#99CCCC><A HREF='paf09.php?wid=".$ultid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A></td></tr>";		
			} 
