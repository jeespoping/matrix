<HTML>
<HEAD>
<TITLE>Orden de servicio PAF TOTAL</TITLE>
</HEAD>
<BODY>
<?php
...
echo "<tr><td align=center bgcolor=#DDDDDD colspan=2><b><font text color=#003366 size=3><i>PAF SALUD TOTAL - CLINICA LAS AMERICAS</font></b><br>";

echo "<form name='paf10total' action='paf10total.php' method=post>";  
  
     $query="SELECT paffau,pafnom,pafap1,pafap2,pafced,paftel,pafdia,pafexa,pafcco,Seguridad,pafobs From paftotal_000001 WHERE Id =".$wid; 
     ...
     if ($nroreg > 0)      //  Encontro 
     {
      ...
      echo "<tr><td align=center colspan=6 bgcolor=#C0C0C0>";
   	  ...
   	  $f = date("Y-m-d");
   	  if ( $conf == "on")
   	  { $query="UPDATE paftotal_000001 SET pafest='X',pafanu='".$user."-".$f."' WHERE Id =".$wid;
