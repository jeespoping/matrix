<html> 
<head>
<title>Actualizacion Autorizaciones Programa PAF NUEVA EPS</title>
...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf01.submit();   // Ojo para la funcion paf01 <> paf01  (sencible a mayusculas)
	}

	function vaciarCampos()
	{document.forms.paf01.wfec.value = 'yyyy-mm-dd';
	 document.forms.paf01.wced.value = '';
     document.forms.paf01.wap1.value = '';
	 document.forms.paf01.wap2.value = '';
     document.forms.paf01.wnom.value = '';
     document.forms.paf01.wtel.value = '';
     document.forms.paf01.wran.value = '';	
     document.forms.paf01.wtip.value = '';	
     document.forms.paf01.wdia.value = '';	
     document.forms.paf01.wexa.value = '';	 
	 document.forms.paf01.wrem.value = '';
	 document.forms.paf01.wcco.value = '';
	 document.forms.paf01.west.value = '';
	 document.forms.paf01.word.value = '';
	 document.forms.paf01.wser.value = '';
	 document.forms.paf01.wran.value = '';	
     document.forms.paf01.wcr1.value = '';    
     document.forms.paf01.wcr2.value = '';     
    }
...
//==========================================================================================================================================
//PROGRAMA				      :Actualizacion Autorizaciones Programa PAF NUEVA EPS                                                                 
//AUTOR				          :Jair  Saldarriaga Orozco.                                                                        
//FECHA CREACION			  :Octubre 20 de 2015
//FECHA ULTIMA ACTUALIZACION  :Octubre 20 de 2015.                                                                             

$wactualiz="PROGRAMA: paf01.php Ver. 2015-20-15   JairS";
...
echo "<form name='paf01' action='paf01.php' method=post>";  
...
echo "<tr><td align=center colspan=2 bgcolor=#99CCCC><font size=3 text color=#FFFFFF><b>Actualizacion Autorizaciones Programa Cardiovascular NUEVA EPS</b></font>";
...
{
  $query = "SELECT Max(id) FROM paf_000001";
  $resultado = mysql_query($query);
  $registro = mysql_fetch_row($resultado);  	
  $nrorden=$registro[0];	
...
if ($windicador == "PrimeraVez") 
{
 if ($wproceso == "Modificar" )  	
  ...
  {   
    $query = "SELECT Afi_identific,Afi_apellido1,Afi_apellido2,Afi_nombres,Afi_telres, Afi_paf_codigo,Afi_tip_cotiza_ FROM paf_000002 Where Afi_identific = '".$wid."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
    if ($nroreg > 0)      //  Encontro 
    {
...
/***********************************************************************************************************************************/	
	...
	{	
	 echo "<input type='submit' value='Grabar'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
   	...
   	if ($wproceso == "Modificar" )  	
      echo "<A HREF='paf09.php?wid=".$wid."' TARGET='_blank' >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;imprimir</A>";	     
...
if ( $conf == "on" and isset($wape) and $wape<>'' and isset($wexa) and $wexa<>'' and isset($wcco) and $wcco<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   // Busco la fecha del ultimo examen igual al solicitado si es menor a 90 dias mensaje informativo
     $query = "SELECT paffec,Id FROM paf_000001 Where pafced = '".$wced."' And pafexa='".$c4[0]."' Order by paffec DESC";
     ...
     $query = "SELECT * FROM paf_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
     ...
     if ( $nroreg > 0 )                   //Encontro
     {
       ...
       $query = "Update paf_000001 SET pafape='".$wape."',pafnom='".$wnom."',paftel='".$wtel."',pafran='".$c1[0]."'"
     ...
     else
     {
      ...
      $fecha = date("Y-m-d");
	  $hora = (string)date("H:i:s");		      
      $query = "INSERT INTO paf_000001 (Medico,Fecha_data,Hora_data,Paffec,Pafced,Pafape,Pafnom,Paftel,Pafran,Paftip,Pafdia,Pafexa,pafrem,"
	          ."Pafcco,Pafest,Pafobs,paford,pafser,paffre,paffem,paffci,pafhor,paffca,pafcau,paflla,Seguridad,id)"
	          ." VALUES ('paf','".$fecha."','".$hora."','".$wfec."','".$wced."','".$wape."','".$wnom."','".$wtel."','".$c1[0]."','".$c2[0]
	  ...
	  if ($resultado)
	   {
			...
			$query = "SELECT Id FROM paf_000001 Where paffec = '".$wfec."' And pafced = '".$wced."' And pafexa='".$c4[0]."'";     // And paford = '".$word."'";
			$resultadoB = mysql_query($query,$conex);
			$nroreg = mysql_num_rows($resultadoB);
			if ( $nroreg > 0 )                   //Encontro
			{
