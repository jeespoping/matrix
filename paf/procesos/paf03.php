...
//Forma
echo "<form name='paf03' action='paf03.php' method=post>";  
 ...
 else      // Cuando ya estan todos los datos escogidos
 {
	...
	$archivo = fopen("paf03.txt","w"); 
	
	$query="SELECT id,paffec,pafced,pafape,pafnom,pafexa FROM  paf_000001"
    ...
    { 
      ...
      */
	  ...
	  { // Busco las citas con codigo que empiece por "EC"
        $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
              ." And Fecha >= '".$registro[1]."' And Cod_exa like 'EC%' "; 
      }
	  else
	  {	  
		...
		{ // Busco las citas con codigo que empiece por  "EL" o "EC"
		  $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
                 ." And Fecha >= '".$registro[1]."' And ( Cod_exa like 'EL%' or Cod_exa like 'EC%' ) ";  	
	    } 		 
		else
		{	
		 ...
		 { // Busco las citas con codigo que empiece por  "HO"
		   $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
                  ." And Fecha >= '".$registro[1]."' And Cod_exa like 'HO%' "; 
		 }		 
		 else
		 {	 
		  ...
		  { // Busco las citas con codigo que empiece por  "MA"
      		   $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
                      ." And Fecha >= '".$registro[1]."' And Cod_exa like 'MA%' ";
		  }
		  else
		  {  
		   ...
		   { // Busco las citas con codigo que empiece por  "PE"
      		    $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
                       ." And Fecha >= '".$registro[1]."' And Cod_exa like 'PE%' ";  
		   }
		   else
		   {	   
			...
			{ // Busco las citas con codigo que empiece por  "EK"
     		  $query ="SELECT cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol from citasca_000001 where Activo='A' And cedula='".$registro[2]."'"
                     ." And Fecha >= '".$registro[1]."' And Cod_exa like 'EK%' ";  
			}
		   }	
		  }	   
	     }
	   ...
	   if ($query !== "")
	   {
		...
		//cod_med,cod_exa,fecha,Hi,Hf,Edad,fecsol
	...
	echo "<li><A href='paf03.txt' >Presione Clic Derecho Para Bajar El Archivo ...</A>";
    echo "<br>";
