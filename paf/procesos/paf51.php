...
<script type="text/javascript">
	...
	function enter()
	{
		document.forms.paf51.submit();   // Ojo para la funcion paf51 <> paf51  (sencible a mayusculas)
	}
...
$wactualiz="PROGRAMA: paf51.php Ver. 2012-10-22   JairS";
...
echo "<form name='paf51' action='paf51.php' method=post>";  	
...
if ($windicador == "PrimeraVez") 
{
   $query = "SELECT  atefec,atehor,ateced,atenom,atehis,atecar,ateord,atecit,atecon,ateex1,ateex2,atemed,atefpr,atehpr,ateobs "
          . " FROM paf_000003 Where ateced = '".$wced."' And atefec = '".$wfec."' And atehor = '".$whor."'";   
   ...
   else                   // NO Encontro lleno los campos con los datos recibidos por la URL y busco el nro de ORDEN y ...
   {
    ...
    $query = "SELECT Id FROM paf_000001 Where pafced = '".$wced."' And paffec='".$wfec."'";
    $resultado = mysql_query($query);
    $nroreg = mysql_num_rows($resultado);
...
/*****/	
   ...
   if ($wproceso == "Modificar" and $windicador=="PrimeraVez")  
    $query = "SELECT codigo,Descripcion FROM citaspaf_000010 WHERE codigo='".$wmed."' Group By codigo,Descripcion"; 	         
   else   
   {   
    if ( isset($wcr3) and $wcr3!=""  )
     $query = "SELECT codigo,Descripcion FROM citaspaf_000010 Where ( codigo like '%".$wcr3."%' or Descripcion like '%".$wcr3."%' ) Group By codigo,Descripcion ORDER BY Descripcion";  
    else
	{   
     if ( isset($wmed) and $wmed!="" )
	 {	 
	  $c3=explode('-',$wmed); 		
      $query = "SELECT codigo,Descripcion FROM citaspaf_000010 WHERE codigo='".$c3[0]."' Group By codigo,Descripcion"; 
	 } 
	 else   
      $query = "SELECT codigo,Descripcion FROM citaspaf_000010 WHERE codigo='$&#$#' Group By codigo,Descripcion";  
    }
   } 
...
/****/    
   if (!isset($wfpr))   // Si no esta seteada entonces la inicializo con la fecha actual
	...
	if ($pos === false) 
	{ 
     $query = "SELECT Descripcion FROM citaspaf_000010 WHERE codigo='".$wmed."'";
	 $resultadoB = mysql_query($query);            // Ejecuto el query 
	 $registroB = mysql_fetch_row($resultadoB);  
	 $wmed = $wmed."-".$registroB[0];
	}
	echo "<br><A HREF='http://mx.lasamericas.com.co/matrix/citas/procesos/000001_prx5.php?empresa=citaspaf&wfec=".$wfpr."&wequ=".$wmed."' TARGET='_blank' >Agendar Cita</A>";
...
if ( $conf == "on" and isset($wfec) and $wfec<>'' and isset($whor) and $whor<>'' and isset($wced) and $wced<>'' and $windicador <> "PrimeraVez" )   
{
  ...
  { 
   ...
   {	 
    ...
    if ($nroreg > 0)      //  Encontro 
    {	 	 
       $query = "Update paf_000003 SET atenom='".$wnom."',atehis='".$whis."',atecar='".$wcar."',ateord='".$word. "',atecit='".$wcit
     ...
     else
     {
      ...
      $query = "INSERT INTO paf_000003 VALUES ('paf','".$fecha."','".$hora."','".$wfec."','".$whor."','".$wced."','".$wnom."','".$whis
